...
import os
import re
from parse import LibrarySource, Constant, Parameter, Subroutine, Type, DoxygenGrouping
...
def write_c_header(library,output):
    ...
    output.write('/*\n * opencmiss.h. This file is automatically generated from opencmiss.f90.\n' + \
...
def write_c_f90(library,output):
    ...
    output.write('!\n! opencmiss_c.f90. This file is automatically generated from opencmiss.f90.\n' + \
        '! Do not edit this file directly, instead edit opencmiss.f90 or the generate_bindings script\n!\n' + \
        'MODULE OPENCMISS_C\n\n' + \
        '  USE ISO_C_BINDING\n' + \
    ...
    output.write('\n'.join(('  PUBLIC %s' % subroutine_c_name(subroutine)[1] for subroutine in library.public_subroutines)))
    output.write('\nCONTAINS\n\n')
...
def constant_to_c_header(constant):
    ...
    if constant.resolved:
        if constant.doxygen_comment != '':
            return 'const int %s = %d; /*<%s */\n' % (constant.name,constant.value,constant.doxygen_comment)
        else:
...
def subroutine_c_name(subroutine):
...
def subroutine_to_c_header(subroutine):
    ...
    output += ' */\n'
    output += 'CMISSError %s(' % subroutine_c_name(subroutine)[0]

    c_parameters = _chain_iterable([parameter_to_c(p) for p in subroutine.parameters])
    comments = _chain_iterable([parameter_doxygen_comments(p) for p in subroutine.parameters])
...
def subroutine_to_c_f90(subroutine):
    ...
    (c_name, c_f90_name) = subroutine_c_name(subroutine)

    output =  '  FUNCTION %s(%s) &\n    & BIND(C, NAME="%s")\n\n' %(c_f90_name,','.join(c_f90_params),c_name)
    ...
    pre_post_lines = [parameter_conversion(parameter) for parameter in subroutine.parameters]
    ...
    post_lines = list(_chain_iterable(reversed([p[2] for p in pre_post_lines])))
    ...
    content.append('CALL %s(%s)' % (function_call,','.join([parameter_call_name(p) for p in subroutine.parameters]+[c_f90_name])))
...
def parameter_conversion(parameter):
    ...
    (routine_c_name,routine_c_f90_name) = subroutine_c_name(parameter.routine)
    ...
    if parameter.var_type == Parameter.CUSTOM_TYPE:
        if parameter.array_dims > 0:
            local_variables.append('TYPE(%s), TARGET :: %s(%s)' % (parameter.type_name,parameter.name,','.join(size_list)))
        else:
            local_variables.append('TYPE(%s), POINTER :: %s' % (parameter.type_name,parameter.name))
        ...
        if parameter.routine.name.endswith('TypeInitialise'):
            ...
            post_call.extend(('%sPtr=C_LOC(%s)' % (parameter.name,parameter.name),
                'ENDIF',
                'ENDIF'))
        elif parameter.routine.name.endswith('TypeFinalise'):
            pre_call.extend(('IF(C_ASSOCIATED(%sPtr)) THEN' % parameter.name,
                'CALL C_F_POINTER(%sPtr,%s)' % (parameter.name,parameter.name),
            ...
            if parameter.array_dims > 0:
                pre_call.append('IF(C_ASSOCIATED(%sPtr)) THEN' % parameter.name)
                if parameter.intent == 'IN':
                    # Passing an array of CMISS Types to Fortran
                    pre_call.append('CALL %ssCopy(%s,%sSize,%sPtr,%s)' % (parameter.type_name,parameter.name,parameter.name,parameter.name,routine_c_f90_name))
                else:
                    # Getting an array of CMISS Types from Fortran and setting an array of C pointers
                    local_variables.append('INTEGER(C_INT) :: %sIndex' % parameter.name)
                    local_variables.append('TYPE(C_PTR), POINTER :: %sCPtrs(:)' % parameter.name)
                    post_call.extend(('CALL C_F_POINTER(%sPtr,%sCPtrs,[%s])' % (parameter.name,parameter.name,','.join(size_list)),
                        'DO %sIndex=1,%sSize' % (parameter.name,parameter.name),
                        '%sCPtrs(%sIndex) = C_LOC(%s(%sIndex))' % ((parameter.name,) * 4),
                        'ENDDO'))
    ...
    #Character arrays
        ...
        if parameter.array_dims > 1:
            ...
            char_sizes = ''
        local_variables.append('CHARACTER(LEN=%s-1) :: Fortran%s%s' % (size_list[-1],parameter.name,char_sizes))
        local_variables.append('%s, POINTER :: %sCChars(%s)' % (PARAMETER_F90TYPES[parameter.var_type],parameter.name,','.join(':'*parameter.array_dims)))
        if parameter.intent == 'IN':
            #reverse to account for difference in storage order
            pre_call.append('CALL C_F_POINTER(%s,%sCChars,[%s])' % (parameter.name,parameter.name,','.join(reversed(size_list))))
            if parameter.array_dims > 1:
                char_sizes = '(%s)' % ','.join(size_list[:-1])
                pre_call.append('CALL CMISSC2FStrings(%sCChars,Fortran%s)' % (parameter.name,parameter.name))
            else:
                char_sizes = ''
                pre_call.append('CALL CMISSC2FString(%sCChars,Fortran%s)' % (parameter.name,parameter.name))
        else:
            if parameter.array_dims > 1:
                raise ValueError, "output of strings >1D not implemented"
            post_call.append('CALL C_F_POINTER(%s,%sCChars,[%s])' % (parameter.name,parameter.name,size_list[0]))
            post_call.append('CALL CMISSF2CString(Fortran%s,%sCChars)' % (parameter.name,parameter.name))

    #Arrays of floats, integers or logicals
    elif parameter.array_dims > 0:
        if parameter.var_type == Parameter.CHARACTER:
            local_variables.append('CHARACTER, POINTER :: %s(%s)' % (parameter.name,','.join([':']*parameter.array_dims)))
        else:
            local_variables.append('%s, POINTER :: %s(%s)' % (PARAMETER_F90TYPES[parameter.var_type],parameter.name,','.join([':']*parameter.array_dims)))
        if parameter.pointer == True and parameter.cintent == 'OUT':
            ...
            post_call.extend(('%sPtr = C_LOC(%s(1))' % (parameter.name,parameter.name),
            ...
            pre_call.extend(('IF(C_ASSOCIATED(%s)) THEN' % c_f90_name,
                'CALL C_F_POINTER(%s,%s,[%s])' % (c_f90_name,parameter.name,','.join(size_list)),
                'IF(ASSOCIATED(%s)) THEN' % parameter.name))
            post_call.extend(('ELSE',
                '%s = CMISSErrorConvertingPointer' % routine_c_f90_name,
                'ENDIF',
...
def parameter_c_f90_name(parameter):
    ...
    if parameter.var_type == Parameter.CUSTOM_TYPE or \
            (parameter.array_dims > 0 and parameter.var_type != Parameter.CHARACTER):
        c_f90_name += 'Ptr'
    return c_f90_name
...
def parameter_c_f90_names(parameter):
    ...
    return ','.join([s for s in parameter_size_list(parameter)[1]]+[parameter_c_f90_name(parameter)])
...
def parameter_c_f90_declaration(parameter):
    ...
    if parameter.array_dims > 0:
        output += 'TYPE(C_PTR), %sINTENT(%s) :: %s' % (value,parameter.cintent,c_f90_name)
    else:
        output += '%s, %sINTENT(%s) :: %s' % (PARAMETER_F90TYPES[parameter.var_type],value,parameter.cintent,c_f90_name)
    return output
...
def parameter_to_c(parameter):
    ...
    return tuple([size_type+size_name for size_name in required_size_list]+[param])
    ...
    for dim in parameter.array_spec:
        if dim == ':':
            if parameter.required_sizes == 1:
                ...
                try:
                    size_list.append(['%sNumStrings' % parameter.name, '%sStringLength' % parameter.name][i])
                    size_doxygen.append(['Number of strings in %s' % parameter.name, 'Length of strings in %s' % parameter.name][i])
                except IndexError:
                    raise ValueError, ">2D arrays of strings not supported"
            else:
                size_list.append('%sSize%d' % (parameter.name,i+1))
                size_doxygen.append('Size of dimension %d of %s' % (i+1,parameter.name))
            i += 1
