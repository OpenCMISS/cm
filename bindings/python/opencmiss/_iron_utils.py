...
def wrap_cmiss_routine(routine, args=None):
    ...
    if isinstance(r, list):
        ...
        status = r
        return_val = None
    if status != _iron_swig.cvar.CMISS_NO_ERROR:
        if status == _iron_swig.cvar.CMISS_POINTER_IS_NULL:
            raise CMISSError("CMISS type pointer is null")
        elif status == _iron_swig.cvar.CMISS_POINTER_NOT_NULL:
            raise CMISSError("CMISS type pointer is not null")
        elif status == _iron_swig.cvar.CMISS_COULD_NOT_ALLOCATE_POINTER:
            raise CMISSError("Could not allocate pointer")
        elif status == _iron_swig.cvar.CMISS_ERROR_CONVERTING_POINTER:
            raise CMISSError("Error converting pointer")
        else:
            raise CMISSError(_iron_swig.CMISSExtractErrorMessage()[1])
    return return_val
