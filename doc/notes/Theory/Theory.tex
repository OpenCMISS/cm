...
Now, if we have a contravariant vector, $\vect{v}$ we can write
\begin{equation}
  \vect{v}=v^{i}\vect{g}_{i}
\end{equation}
where $v^{i}$ are the components of the contravariant vector, and $\vect{g}_{i}$ are the covariant base vectors.

Similarly, if we have a contravariant vector, $\vect{v}$ we can write
\begin{equation}
  \vect{v}=v_{i}\vect{g}^{i}
\end{equation}

This time  $v_{i}$ are the components of the covariant vector, and $\vect{g}^{i}$ are the contravariant base vectors. We now note that
\begin{equation}
...
Note that the base vectors are all mutually orthogonal and constant then
...
The covariant derivative of a rank 1 contravariant tensor $v^{k}$ is
\begin{equation}
  \covarderiv{v^{k}}{i} =\delby{v^{k}}{\xi^{i}}+\christoffel{j}{i}{k}v^{j}
\end{equation}
and the covariant derivative of a rank 1 covariant tensor $v_{k}$ is
\begin{equation}
...
\begin{equation}
  \vect{g}_{i}=\begin{bmatrix} 
    r\sin\theta\cos\phi\vect{i}_{1} + r\sin\theta\sin\phi\vect{i}_{2}  - r\cos\theta\vect{i}_{3} \\ 
