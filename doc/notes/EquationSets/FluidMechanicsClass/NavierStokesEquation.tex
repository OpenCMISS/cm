...
\eqref{NSENeumannBC} and shown in \eqref{FEMNSE2}. For our current treatment, we will assume the values of viscosity $\mu$ and density $\rho$ are constant. These can be interpolated:
...
\end{equation}
 ...
 Using the same basis functions for the respective velocity and pressure spaces for a Galerkin finite element formulation, our spatial weights become for the velocity space:
\begin{equation}
  \fnof{w_{i}}{\vect{\xi}}=\idxgbfn{i}{m}{\alpha}{\vect{\xi}}\idxgsf{i}{m}{\alpha}
\end{equation}
and pressure:
\begin{equation}
  \fnof{w_{i}}{\vect{\xi}}=\phi_{im}^{\alpha}({\vect{\xi}})\idxgsf{i}{m}{\alpha}
\end{equation}
...
%pressure
  ...
  \fnof{\idxnodedof{p}{}{n}{\beta}}{t}\idxgsf{i}{n}{\beta}\\
  \pbrac{\delby{\phi_{im}^{\alpha}({\vect{\xi}})\idxgsf{i}{m}{\alpha}}{x^{k}}-
  \christoffel{i}{k}{h}\phi_{im}^{\alpha}({\vect{\xi}})\idxgsf{h}{m}{\alpha}
    }\abs{\fnof{\matr{J}}{\vect{\xi}}}d\vect{\xi}\\
%viscous stress
...
%pressure
  ...
  \fnof{\idxnodedof{p}{}{n}{\beta}}{t}\idxgsf{i}{n}{\beta}
  \pbrac{\delby{\phi_{im}^{\alpha}({\vect{\xi}})\idxgsf{i}{m}{\alpha}}{x^{k}}}\abs{\fnof{\matr{J}}{\vect{\xi}}}d\vect{\xi}\\
%viscous stress
...
%time dependence
 ...
 %pressure
    ...
    \pbrac{\delby{\phi_{im}^{\alpha}({\vect{\xi}})}{x^{k}}}\abs{\fnof{\matr{J}}{\vect{\xi}}}d\vect{\xi}\\
 % %viscous stress
...
where $w_i$ will be the classic Galerkin weight and $\Psi_i$ will be the Petrov-Galerkin for the Navier-Stokes equations:
...
\end{equation}
where $\bar{U}$ is the characteristic velocity and $\nu=\frac{\mu}{\rho}$ is the kinematic viscosity. The effective Peclet number $\gamma$ will be:

\begin{equation}
  \gamma= Re\abs{u_j}L = \frac{u_j^2L^2}{\nu}
\end{equation}

Equation \eqref{NSEPGFEM1} then becomes:
