...
CONTAINS
  ...
  SUBROUTINE COMPUTATIONAL_NODE_NUMBERS_SET(MY_NODE_NUMBER,NUMBER_OF_NODES,ERR,ERROR,*)
    ...
    CALL ENTERS("COMPUTATIONAL_NODE_NUMBERS_SET",ERR,ERROR,*999)
    ...
    CALL EXITS("COMPUTATIONAL_NODE_NUMBERS_SET")
    RETURN 
999 CALL ERRORS("COMPUTATIONAL_NODE_NUMBERS_SET",ERR,ERROR)
    CALL EXITS("COMPUTATIONAL_NODE_NUMBERS_SET")
    RETURN 1
  ...
  SUBROUTINE ENTERS(NAME,ERR,ERROR,*)
    ...
    IF(DIAG_OR_TIMING) THEN
      ...
      !CALL SYSTEM('echo enters ' // NAME //  ' >> callstack.txt')
      RETURN
...
999 RETURN 1
  ...
  SUBROUTINE EXITS(NAME)
    ...
    IF(DIAG_OR_TIMING) THEN
      ...
      !CALL SYSTEM('echo exits ' // NAME //  ' >> callstack.txt')
      RETURN
      ROUTINE_PTR=>ROUTINE_STACK%STACK_POINTER
...
999 CALL ERRORS("TIMING_SUMMARY_OUTPUT",ERR,ERROR)
 ...
 !
  ...
  SUBROUTINE WRITE_ERROR(ERR,ERROR,*)
    ...
    POSITION=INDEX(ERROR,ERROR_SEPARATOR_CONSTANT)
    IF(NUMBER_OF_COMPUTATIONAL_NODES>1) THEN
      WRITE(OP_STRING,'(">>ERROR (",I0,"): ",I5,": ",A)') MY_COMPUTATIONAL_NODE_NUMBER,ERR,CHAR(EXTRACT(ERROR,1,POSITION-1))
    ELSE
