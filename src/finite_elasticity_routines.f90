...
MODULE FINITE_ELASTICITY_ROUTINES
  ...
  PUBLIC FINITE_ELASTICITY_FINITE_ELEMENT_DEFORMATION_CALCULATE
  PUBLIC FINITE_ELASTICITY_FINITE_ELEMENT_JACOBIAN_EVALUATE
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_JACOBIAN_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: PRESSURE_COMPONENT,MESH_COMPONENT1,MESH_COMPONENT2
    ...
    INTEGER(INTG) :: MESH_COMPONENTS(4),NUMBER_OF_ELEMENT_PARAMETERS(4)
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        ...
        IF(EVALUATE_JACOBIAN) THEN
          ...
          DO nh=1,FIELD_VARIABLE%NUMBER_OF_COMPONENTS
            MESH_COMPONENTS(nh)=FIELD_VARIABLE%COMPONENTS(nh)%MESH_COMPONENT_NUMBER
            DEPENDENT_BASIS2=>DEPENDENT_FIELD%DECOMPOSITION%DOMAIN(MESH_COMPONENTS(nh))%PTR% &
              & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS
            QUADRATURE_SCHEMES(nh)%PTR=>DEPENDENT_BASIS2%QUADRATURE%QUADRATURE_SCHEME_MAP(BASIS_DEFAULT_QUADRATURE_SCHEME)%PTR
            IF(FIELD_VARIABLE%COMPONENTS(nh)%INTERPOLATION_TYPE==FIELD_NODE_BASED_INTERPOLATION) THEN
              NUMBER_OF_ELEMENT_PARAMETERS(nh)=DEPENDENT_BASIS2%NUMBER_OF_ELEMENT_PARAMETERS
...
!                          SUM2=SUM2+DPHIDZ(mi,ms,mh)*CAUCHY_TENSOR(mi,ni)*DPHIDZ(ni,ns,nh)
!                        ENDDO !mi
!                      ENDDO !ni
!                    ENDIF
!                    DO ni=1,DEPENDENT_BASIS2%NUMBER_OF_XI
!                      DO mi=1,DEPENDENT_BASIS1%NUMBER_OF_XI
!                        SUM1=SUM1+DPHIDZ(mi,ms,mh)*ELASTICITY_TENSOR(IDX(mi,mh),IDX(ni,nh))*DPHIDZ(ni,ns,nh)
!                      ENDDO !mi
!                    ENDDO !ni
!                    SUM1=SUM1+SUM2
                ...
                DO ns=1,NUMBER_OF_ELEMENT_PARAMETERS(nh)
                 ...
                 !Loop over element rows belonging to hydrostatic pressure
                  ...
                  DO ms=1,NUMBER_OF_ELEMENT_PARAMETERS(PRESSURE_COMPONENT)
                    !WRITE(*,*) "mh",PRESSURE_COMPONENT,"ms",ms,"mhs",mhs,"nh",nh,"ns",ns,"nhs",nhs
                    mhs=mhs+1
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN 
        ...
        SELECT CASE(EQUATIONS_SET%SUBTYPE)
        ! ---------------------------------------------------------------
        CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE) ! 4 dependent components, works with 2PK, not Cauchy
        ...
        CASE(EQUATIONS_SET_NO_SUBTYPE,EQUATIONS_SET_MEMBRANE_SUBTYPE, EQUATIONS_SET_MOONEY_RIVLIN_SUBTYPE, &
          ...
          & EQUATIONS_SET_TRANSVERSE_ISOTROPIC_GUCCIONE_SUBTYPE,EQUATIONS_SET_TRANSVERSE_ISOTROPIC_HUMPHREY_YIN_SUBTYPE, &
          ...
          & EQUATIONS_SET_GUCCIONE_ACTIVECONTRACTION_SUBTYPE) ! 4 dependent components
...
999 CALL ERRORS("FINITE_ELASTICITY_FINITE_ELEMENT_POST_RESIDUAL_EVALUATE",ERR,ERROR)
  ...
  !>Calculate the deformation tensor for a finite elasticity finite element equations set.
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_DEFORMATION_CALCULATE(EQUATIONS_SET,USER_ELEMENT_NUMBER,XI,C,INV_C,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_SET_TYPE), POINTER :: EQUATIONS_SET !<A pointer to the equations set
    INTEGER(INTG), INTENT(IN) :: USER_ELEMENT_NUMBER !<The element number to calculate the deformation at
    ...
    INTEGER(INTG) :: var1 ! Variable number corresponding to 'U' in single physics case
    REAL(DP) :: DZDNU(3,3),AZL(3,3),AZU(3,3),DZDNUT(3,3)
    REAL(DP) :: Jxxi,I3
    ...
    CALL ENTERS("FINITE_ELASTICITY_FINITE_ELEMENT_DEFORMATION_CALCULATE",ERR,ERROR,*999)
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN 
        ...
        IF(ASSOCIATED(DECOMPOSITION)) THEN
          ...
          CALL DECOMPOSITION_TOPOLOGY_ELEMENT_CHECK_EXISTS(DECOMPOSITION_TOPOLOGY,USER_ELEMENT_NUMBER, &
              & USER_ELEMENT_EXISTS,ELEMENT_NUMBER,GHOST_ELEMENT,ERR,ERROR,*999)
          IF(USER_ELEMENT_EXISTS) THEN
            IF(GHOST_ELEMENT) THEN
              LOCAL_ERROR="Cannot calculate deformation tensor for user element "// &
            ...
            ELSE
              ...
              NUMBER_OF_XI=DECOMPOSITION%DOMAIN(MESH_COMPONENT_NUMBER)%PTR%TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)% &
                & BASIS%NUMBER_OF_XI
              ...
              CALL FIELD_INTERPOLATE_XI(FIRST_PART_DERIV,XI,DEPENDENT_INTERPOLATED_POINT,ERR,ERROR,*999)
              CALL FIELD_INTERPOLATED_POINT_METRICS_CALCULATE(DEPENDENT_BASIS%NUMBER_OF_XI,DEPENDENT_INTERPOLATED_POINT_METRICS, &
                & ERR,ERROR,*999)
              CALL FIELD_INTERPOLATE_XI(FIRST_PART_DERIV,XI,GEOMETRIC_INTERPOLATED_POINT,ERR,ERROR,*999)
              CALL FIELD_INTERPOLATED_POINT_METRICS_CALCULATE(DEPENDENT_BASIS%NUMBER_OF_XI,GEOMETRIC_INTERPOLATED_POINT_METRICS, &
                & ERR,ERROR,*999)
              CALL FIELD_INTERPOLATE_XI(FIRST_PART_DERIV,XI,FIBRE_INTERPOLATED_POINT,ERR,ERROR,*999)
              ...
              IF(EQUATIONS%OUTPUT_TYPE>=EQUATIONS_TIMING_OUTPUT) THEN
                ...
                USER_ELAPSED=USER_TIME3(1)-USER_TIME2(1)
                  ...
                  CALL WRITE_STRING_VALUE(GENERAL_OUTPUT_TYPE,"User time for deformation field calculation = ", &
                    & USER_ELAPSED,ERR,ERROR,*999)
                  CALL WRITE_STRING_VALUE(GENERAL_OUTPUT_TYPE,"System time for deformation field calculation = ", &
          ...
          ELSE
            LOCAL_ERROR="The specified user element number of "// &
              ...
              & " does not exist in the decomposition of field number "// &
    ...
    CALL EXITS("FINITE_ELASTICITY_FINITE_ELEMENT_DEFORMATION_CALCULATE")
    RETURN

999 CALL ERRORS("FINITE_ELASTICITY_FINITE_ELEMENT_DEFORMATION_CALCULATE",ERR,ERROR)
    CALL EXITS("FINITE_ELASTICITY_FINITE_ELEMENT_DEFORMATION_CALCULATE")
    RETURN 1
  END SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_DEFORMATION_CALCULATE
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_STRAIN_CALCULATE(EQUATIONS_SET,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: FIELD_VAR_TYPE
    INTEGER(INTG) :: DEPENDENT_NUMBER_OF_COMPONENTS
    INTEGER(INTG) :: NUMBER_OF_DIMENSIONS,NUMBER_OF_XI,PRESSURE_COMPONENT
    INTEGER(INTG) :: DEPENDENT_NUMBER_OF_GAUSS_POINTS
    INTEGER(INTG) :: MESH_COMPONENT_NUMBER
    INTEGER(INTG) :: var1 ! Variable number corresponding to 'U' in single physics case
    INTEGER(INTG) :: var2 ! Variable number corresponding to 'DELUDLEN' in single physics case
    REAL(DP) :: DZDNU(3,3),P,E(3,3),AZL(3,3),AZU(3,3),DZDNUT(3,3)
...
999 CALL ERRORS("FINITE_ELASTICITY_FINITE_ELEMENT_STRAIN_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_SURFACE_PRESSURE_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,var1,var2,ERR,ERROR,*)
    ...
    TYPE(FIELD_VARIABLE_COMPONENT_TYPE), POINTER :: PRESSURE_COMPONENT
    TYPE(FIELD_VARIABLE_TYPE), POINTER :: FIELD_VARIABLE
    ...
    LOGICAL :: ELEMENT_NONZERO_PRESSURE,FACE_NONZERO_PRESSURE

    CALL ENTERS("FINITE_ELASTICITY_SURFACE_PRESSURE_RESIDUAL_EVALUATE",ERR,ERROR,*999)

    NULLIFY(EQUATIONS,DEPENDENT_FIELD,FIELD_VARIABLE)
    ...
    ! Maybe pass this information through function arguments?
    ...
    DEPENDENT_INTERPOLATED_POINT_METRICS=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT_METRICS(FIELD_VAR_U_TYPE)%PTR

    FACE_PRESSURE_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_PARAMETERS(FIELD_VAR_DUDN_TYPE)%PTR
    !ELEMENT_NONZERO_PRESSURE=.FALSE.
...
!         FACE_COMPONENTS=OTHER_XI_DIRECTIONS3(normal_component_idx,2:3,1)  !Two xi directions for the current face
        ...
        IF(ANY(ABS(FACE_PRESSURE_INTERPOLATION_PARAMETERS%PARAMETERS(:,normal_component_idx))>ZERO_TOLERANCE)) THEN
          ...
          !ELEMENT_NONZERO_PRESSURE=.TRUE.
        ENDIF

        !Nonzero surface pressure found?
        IF(NONZERO_PRESSURE) THEN
          !Grab some other pointers
          PRESSURE_COMPONENT=>DEPENDENT_FIELD%VARIABLES(FIELD_VAR_DUDN_TYPE)%COMPONENTS(normal_component_idx)
...
! can't guarantee it and checking every single thing will be a fair bit of overhead
          DO gauss_idx=1,FACE_NUMBER_OF_GAUSS_POINTS 
            ...
            PRESSURE_GAUSS=FACE_INTERPOLATED_POINT%VALUES(normal_component_idx,NO_PART_DERIV)    !Surface pressure at this gauss point
            ...
            JGW_PRESSURE=DEPENDENT_INTERPOLATED_POINT_METRICS%JACOBIAN* &
            ...
            DO component_idx=1,NUMBER_OF_DIMENSIONS
              ...
              JGW_PRESSURE_NORMAL_PROJECTION=DOT_PRODUCT(DEPENDENT_INTERPOLATED_POINT_METRICS%GU(normal_component_idx,:), &
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR",ERR,ERROR)
  ...
  !>Evaluates the 2nd Piola-Kirchoff stress tensor at a given Gauss point
  SUBROUTINE FINITE_ELASTICITY_GAUSS_PIOLA_TENSOR(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
      & MATERIALS_INTERPOLATED_POINT,PIOLA_TENSOR,AZL,AZU,Jznu,ELEMENT_NUMBER,GAUSS_POINT_NUMBER,ERR,ERROR,*)
    ...
    REAL(DP) :: I1,I2,I3            !Invariants, if needed
    REAL(DP) :: ACTIVE_STRESS_11,ACTIVE_STRESS_22,ACTIVE_STRESS_33 !Active stress to be copied in from independent field.
    REAL(DP) :: TEMP(3,3),TEMPTERM1,TEMPTERM2  !Temporary variables
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    TYPE(FIELD_VARIABLE_TYPE), POINTER :: FIELD_VARIABLE
    REAL(DP), DIMENSION (:), POINTER :: C !Parameters for constitutive laws

    CALL ENTERS("FINITE_ELASTICITY_GAUSS_PIOLA_TENSOR",ERR,ERROR,*999)
    ...
    CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE)
      ...
      DO component_idx=1,FIELD_VARIABLE%NUMBER_OF_COMPONENTS
        ...
        PIOLA_TENSOR(component_idx,component_idx)=PIOLA_TENSOR(component_idx,component_idx)+TEMPTERM1
      ENDDO
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_PIOLA_TENSOR",ERR,ERROR)
    CALL EXITS("FINITE_ELASTICITY_GAUSS_PIOLA_TENSOR")
    RETURN 1
  END SUBROUTINE FINITE_ELASTICITY_GAUSS_PIOLA_TENSOR
