...
999 CALL ERRORS("FINITE_ELASTICITY_FINITE_ELEMENT_JACOBIAN_EVALUATE",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
	  ...
	  IF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_NODE_BASED_INTERPOLATION) THEN
	    DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(COMPONENT_IDX)%PARAM_TO_DOF_MAP% &
	      ...
	      IF (NONLINEAR_MATRICES%UPDATE_RESIDUAL) THEN !1st itteration - add displacement bcs to undeformed geomteric field
	        DEPENDENT_FIELD%VARIABLES(1)%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)= &
	          & DEPENDENT_FIELD%VARIABLES(1)%PARAMETER_SETS%PARAMETER_SETS(2)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)+ &
	          & GEOMETRIC_FIELD%VARIABLES(1)%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)
	...
	ENDDO !component_idx	
	  ...
	  DEPENDENT_COMPONENT_INTERPOLATION_TYPE=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(component_idx)%INTERPOLATION_TYPE	  
	  IF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_NODE_BASED_INTERPOLATION) THEN
     	    DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(COMPONENT_IDX)% &
	  ...
	  ELSEIF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_ELEMENT_BASED_INTERPOLATION) THEN
	    ...
	    IF (NONLINEAR_MATRICES%UPDATE_RESIDUAL) THEN !1st iteration - set initial guess
	      DEPENDENT_FIELD%VARIABLES(1)%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)=-5.0_DP
	    ELSE ! subsequent iterations - copy parameters in dependent filed 2 to dpendent field 1
	      DEPENDENT_FIELD%VARIABLES(1)%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)= &
		& DEPENDENT_FIELD%VARIABLES(1)%PARAMETER_SETS%PARAMETER_SETS(2)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)	
	      NONLINEAR_MATRICES%ELEMENT_RESIDUAL%VECTOR(element_dof_idx)=0.0_DP !Also set residual values to zero as this is reused for Jacobian calculations		  			      
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_DFDZ(INTERPOLATED_POINT,DEPENDENT_NUMBER_OF_ELEMENT_PARAMETERS,ELEMENT_NUMBER, &
    ...
    DO parameter_idx=1,DEPENDENT_NUMBER_OF_ELEMENT_PARAMETERS
      DO component_idx=1,3 !FIELD%NUMBER_OF_COMPONENTS-1
        DFDZ(parameter_idx,component_idx)=0.0_DP
	DO xi_idx=1,3
    DFDZ(parameter_idx,component_idx)=DFDZ(parameter_idx,component_idx)+DFDXI(parameter_idx,xi_idx)* &
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_DFDZ",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_EQUATIONS_SET_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,FIELD_USER_NUMBER,FIELD,ERR,ERROR,*)
        ...
        CASE(EQUATIONS_SET_SETUP_DEPENDENT_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            ELSE
              ...
              CALL FIELD_DIMENSION_CHECK(FIELD,FIELD_DELUDELN_VARIABLE_TYPE,FIELD_VECTOR_DIMENSION_TYPE,ERR,ERROR,*999)
              ...
              CALL FIELD_NUMBER_OF_COMPONENTS_GET(EQUATIONS_SET%GEOMETRY%GEOMETRIC_FIELD,FIELD_U_VARIABLE_TYPE, &
                & NUMBER_OF_DIMENSIONS,ERR,ERROR,*999)
              NUMBER_OF_COMPONENTS=NUMBER_OF_DIMENSIONS+1 !Include hydrostatic pressure component
              CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(FIELD,FIELD_U_VARIABLE_TYPE,NUMBER_OF_COMPONENTS,ERR,ERROR,*999)
              CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(FIELD,FIELD_DELUDELN_VARIABLE_TYPE,NUMBER_OF_COMPONENTS,ERR,ERROR,*999)
              SELECT CASE(EQUATIONS_SET%SOLUTION_METHOD)
              CASE(EQUATIONS_SET_FEM_SOLUTION_METHOD)
                DO component_idx=1,NUMBER_OF_DIMENSIONS
                  CALL FIELD_COMPONENT_INTERPOLATION_CHECK(FIELD,FIELD_U_VARIABLE_TYPE,component_idx, &
                    & FIELD_NODE_BASED_INTERPOLATION,ERR,ERROR,*999)
                  CALL FIELD_COMPONENT_INTERPOLATION_CHECK(FIELD,FIELD_DELUDELN_VARIABLE_TYPE,component_idx, &
                    & FIELD_NODE_BASED_INTERPOLATION,ERR,ERROR,*999)
                ENDDO !component_idx
                CALL FIELD_COMPONENT_INTERPOLATION_CHECK(FIELD,FIELD_U_VARIABLE_TYPE,4, &
                  & FIELD_ELEMENT_BASED_INTERPOLATION,ERR,ERROR,*999)
                CALL FIELD_COMPONENT_INTERPOLATION_CHECK(FIELD,FIELD_DELUDELN_VARIABLE_TYPE,4, &
                  & FIELD_ELEMENT_BASED_INTERPOLATION,ERR,ERROR,*999)
              CASE(EQUATIONS_SET_BEM_SOLUTION_METHOD)
                CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
              CASE(EQUATIONS_SET_FD_SOLUTION_METHOD)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
              ...
              ELSE
                ...
                CALL FIELD_DIMENSION_CHECK(FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VECTOR_DIMENSION_TYPE,ERR,ERROR,*999)
                CALL FIELD_DATA_TYPE_CHECK(FIELD,FIELD_U_VARIABLE_TYPE,FIELD_DP_TYPE,ERR,ERROR,*999)
                CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(FIELD,FIELD_U_VARIABLE_TYPE,2,ERR,ERROR,*999)
              ENDIF
            ELSE
