      ...
      IF(FACE%BOUNDARY_FACE) THEN 
        ...
        IF(NONZERO_PRESSURE) THEN
          ...
          DO ng=1,DEPENDENT_QUADRATURE_SCHEME%NUMBER_OF_GAUSS
            ...
            !IF(DECOMP_FACE%XI_DIRECTION<0_INTG) PRESSURE_GAUSS=-PRESSURE_GAUSS            !Crucial detail here
              ...
              DO ns=1,NUMBER_OF_FACE_PARAMETERS(nh)
                ...
                DO ms=1,NUMBER_OF_FACE_PARAMETERS(mh)
                  ...
                  JACOBIAN_MATRIX%ELEMENT_JACOBIAN%MATRIX(mhs,nhs)=JACOBIAN_MATRIX%ELEMENT_JACOBIAN%MATRIX(mhs,nhs)+ &
                    ...
                    & EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_PARAMETERS(FIELD_VAR_U_TYPE)%PTR%SCALE_FACTORS( &
                    & BASES(mh)%PTR%ELEMENT_PARAMETERS_IN_LOCAL_FACE(ms,naf),mh)* &
                    & EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_PARAMETERS(FIELD_VAR_U_TYPE)%PTR%SCALE_FACTORS( &
                    & BASES(mh)%PTR%ELEMENT_PARAMETERS_IN_LOCAL_FACE(ns,naf),nh)
                ENDDO !ms    
              ENDDO !ns
            ENDDO !oh
            DO oh=1,OFF_DIAG_COMP(NUMBER_OF_DIMENSIONS)
...
999 CALL ERRORS("FINITE_ELASTICITY_SURFACE_PRESSURE_JACOBIAN_EVALUATE",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_SURFACE_PRESSURE_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,var1,var2,ERR,ERROR,*)
    ...
    TYPE(BASIS_TYPE), POINTER :: DEPENDENT_FACE_BASIS,COMPONENT_FACE_BASIS,DEPENDENT_BASIS,COMPONENT_BASIS
    ...
    NULLIFY(DECOMP_FACE)
    NULLIFY(DEPENDENT_FACE_BASIS,COMPONENT_FACE_BASIS,DEPENDENT_BASIS,COMPONENT_BASIS)
