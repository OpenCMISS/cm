...
999 CALL ERRORS("FINITE_ELASTICITY_FINITE_ELEMENT_JACOBIAN_EVALUATE",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    TYPE(BASIS_TYPE), POINTER :: DEPENDENT_BASIS,FIBRE_BASIS,GEOMETRIC_BASIS,MATERIAL_BASIS
    TYPE(EQUATIONS_TYPE), POINTER :: EQUATIONS
    ...
    TYPE(FIELD_TYPE), POINTER :: DEPENDENT_FIELD,FIBRE_FIELD,GEOMETRIC_FIELD,MATERIAL_FIELD
    ...
    TYPE(FIELD_INTERPOLATION_PARAMETERS_TYPE), POINTER :: GEOMETRIC_INTERPOLATION_PARAMETERS, &
      & FIBRE_INTERPOLATION_PARAMETERS,MATERIAL_INTERPOLATION_PARAMETERS,DEPENDENT_INTERPOLATION_PARAMETERS
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: GEOMETRIC_INTERPOLATED_POINT,FIBRE_INTERPOLATED_POINT, &
      & MATERIAL_INTERPOLATED_POINT,DEPENDENT_INTERPOLATED_POINT
	...
	MATERIAL_FIELD=>EQUATIONS%INTERPOLATION%MATERIAL_FIELD
	...
	GEOMETRIC_BASIS=>GEOMETRIC_FIELD%DECOMPOSITION%DOMAIN(GEOMETRIC_FIELD%DECOMPOSITION%MESH_COMPONENT_NUMBER)%PTR% &
          & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS
        MATERIAL_BASIS=>MATERIAL_FIELD%DECOMPOSITION%DOMAIN(MATERIAL_FIELD%DECOMPOSITION%MESH_COMPONENT_NUMBER)%PTR% &
          & TOPOLOGY%ELEMENTS%ELEMENTS(ELEMENT_NUMBER)%BASIS
	...
	element_dof_idx=0
	  ...
	  IF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_NODE_BASED_INTERPOLATION) THEN
	    DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(COMPONENT_IDX)%PARAM_TO_DOF_MAP%NUMBER_OF_NODE_PARAMETERS
	      ...
	      ELSE ! copy parameters in dependent filed 2 to dpendent field 1
	        DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)= &
		  & DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(2)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)
	      ENDIF	  	      	      
            ENDDO !parameter_idx
	...
	ENDDO !component_idx	
        ...
        !Update hydrostatic presssure component, i.e. last component of depedent field) 	
	...
	DO parameter_idx=1,GEOMETRIC_BASIS%NUMBER_OF_ELEMENT_PARAMETERS
	  ...
	  ENDDO
        ...
        MATERIAL_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION%MATERIAL_INTERP_PARAMETERS
        ...
        MATERIAL_INTERPOLATED_POINT=>EQUATIONS%INTERPOLATION%MATERIAL_INTERP_POINT	
        ...
        CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,MATERIAL_INTERPOLATION_PARAMETERS,ERR,ERROR,*999)
	  ...
	  CALL FIELD_INTERPOLATE_XI(FIRST_PART_DERIV,DEPENDENT_QUADRATURE_SCHEME%GAUSS_POSITIONS(:,gauss_idx), &
	    & MATERIAL_INTERPOLATED_POINT,ERR,ERROR,*999)
	    ...
	    & FIBRE_INTERPOLATED_POINT,DZDNU,Jxxi,Jznu,ERR,ERROR,*999)    
          ...
          CALL FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR(DEPENDENT_INTERPOLATED_POINT,MATERIAL_INTERPOLATED_POINT, &
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_DXDNU",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR(DEPENDENT_INTERPOLATED_POINT,MATERIAL_INTERPOLATED_POINT, &
    & CAUCHY_TENSOR,DZDNU,ERR,ERROR,*)

    !Argument variables    
    TYPE(FIELD_TYPE), POINTER :: FIELD    
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: DEPENDENT_INTERPOLATED_POINT,MATERIAL_INTERPOLATED_POINT
    ...
    C(1,0)=MATERIAL_INTERPOLATED_POINT%VALUES(1,1)
    C(0,1)=MATERIAL_INTERPOLATED_POINT%VALUES(2,1)
