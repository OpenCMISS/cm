  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_JACOBIAN_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
	...
	element_dof_idx=0
        DO component_idx=1,DEPENDENT_NUMBER_OF_COMPONENTS
          ...
          IF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_NODE_BASED_INTERPOLATION) THEN	    DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(component_idx)%PARAM_TO_DOF_MAP% &          & NUMBER_OF_NODE_PARAMETERS            DO parameter_idx=1,DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS              element_dof_idx=element_dof_idx+1	      DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(2)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)= &	        & DISTRIBUTED_VECTOR_CMISS1%DATA_DP(element_dof_idx)		            ENDDO !parameter_idx	  ELSEIF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_ELEMENT_BASED_INTERPOLATION) THEN            element_dof_idx=element_dof_idx+1	  	  	    DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(2)%PTR%PARAMETERS%CMISS%DATA_DP(element_dof_idx)= &	      & DISTRIBUTED_VECTOR_CMISS1%DATA_DP(element_dof_idx)	  	  ENDIF        ENDDO	DEPENDENT_TOTAL_ELEMENT_PARAMETERS=element_dof_idx		n=0        DO component_idx=1,DEPENDENT_NUMBER_OF_COMPONENTS          DEPENDENT_COMPONENT_INTERPOLATION_TYPE=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(component_idx)%INTERPOLATION_TYPE          IF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_NODE_BASED_INTERPOLATION) THEN	    DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(component_idx)%PARAM_TO_DOF_MAP% &          & NUMBER_OF_NODE_PARAMETERS            DO parameter_idx=1,DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS              n=n+1	      	      DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(2)%PTR%PARAMETERS%CMISS%DATA_DP(n)= &	        & DISTRIBUTED_VECTOR_CMISS1%DATA_DP(n)+DELTA			      CALL FINITE_ELASTICITY_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*999)	      DEPENDENT_FIELD=>EQUATIONS%INTERPOLATION%DEPENDENT_FIELD                      EQUATIONS_MATRICES=>EQUATIONS%EQUATIONS_MATRICES
              NONLINEAR_MATRICES=>EQUATIONS_MATRICES%NONLINEAR_MATRICES		
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
	...
	element_dof_idx=0
	  ...
	  IF (DEPENDENT_COMPONENT_INTERPOLATION_TYPE==FIELD_NODE_BASED_INTERPOLATION) THEN
	    DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(COMPONENT_IDX)%PARAM_TO_DOF_MAP% &          & NUMBER_OF_NODE_PARAMETERS            DO parameter_idx=1,DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS
	...
	DO parameter_idx=1,GEOMETRIC_BASIS%NUMBER_OF_ELEMENT_PARAMETERS
	  ...
	  ENDDO
        ...
        CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,GEOMETRIC_INTERPOLATION_PARAMETERS, &             & ERR,ERROR,*999)        CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,FIBRE_INTERPOLATION_PARAMETERS, &             & ERR,ERROR,*999)        CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,MATERIALS_INTERPOLATION_PARAMETERS, &             & ERR,ERROR,*999)        CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,DEPENDENT_INTERPOLATION_PARAMETERS, &             & ERR,ERROR,*999)
...
999 CALL ERRORS("FINITE_ELASTICITY_FINITE_ELEMENT_RESIDUAL_EVALUATE",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_DEFORMATION_GRADEINT_TENSOR(DEPENDENT_INTERPOLATED_POINT,GEOMETRIC_INTERPOLATED_POINT, &
    ...
    !Argument variables
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: DEPENDENT_INTERPOLATED_POINT,GEOMETRIC_INTERPOLATED_POINT, &         & FIBRE_INTERPOLATED_POINT        !INTEGER(INTG) :: GAUSS_POINT_NUMBER     
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_DFDZ(INTERPOLATED_POINT,DEPENDENT_NUMBER_OF_ELEMENT_PARAMETERS,ELEMENT_NUMBER, &
    ...
    DO parameter_idx=1,DEPENDENT_NUMBER_OF_ELEMENT_PARAMETERS
      DO component_idx=1,3 !FIELD%NUMBER_OF_COMPONENTS-1
        DFDZ(parameter_idx,component_idx)=0.0_DP
	DO xi_idx=1,3
    DFDZ(parameter_idx,component_idx)=DFDZ(parameter_idx,component_idx)+DFDXI(parameter_idx,xi_idx)* &        & DXIDZ(xi_idx,component_idx)	ENDDO  
      ENDDO    
    ENDDO
