    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN 
        ...
        CASE(EQUATIONS_SET_NO_SUBTYPE,EQUATIONS_SET_MEMBRANE_SUBTYPE, EQUATIONS_SET_MOONEY_RIVLIN_SUBTYPE, &
          & EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE, EQUATIONS_SET_ISOTROPIC_EXPONENTIAL_SUBTYPE, &
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_DEFORMATION_GRADIENT_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
    ...
    REAL(DP) :: ACTIVE_STRESS !Active stress to be copied in from independent field.
    ...
    CASE(EQUATIONS_SET_MOONEY_RIVLIN_SUBTYPE,EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE,EQUATIONS_SET_MEMBRANE_SUBTYPE, &
      ...
      IF (EQUATIONS_SET_SUBTYPE /= EQUATIONS_SET_MEMBRANE_SUBTYPE) THEN
          ...
          !QUICK HACK TO CHECK THAT CONTRACTIONS WOULD WORK
          !PIOLA_TENSOR(3,3)=2.0_DP*(C(1)+C(2)*(AZL(1,1)+AZL(2,2))+P*AZU(3,3))
      ELSE
        ! Membrane Equations
      ...
      !QUICK HACK TO CHECK ISOTROPIC CONTRACTION WOULD WORK
      !PIOLA_TENSOR(1,1)=2.0_DP*(C(1)+C(2)*(AZL(2,2)+AZL(3,3))+P*AZU(1,1))+20.0
      !PIOLA_TENSOR(1,2)=2.0_DP*(     C(2)*(-AZL(2,1))        +P*AZU(1,2))
      !PIOLA_TENSOR(2,1)=PIOLA_TENSOR(1,2)
      !PIOLA_TENSOR(2,2)=2.0_DP*(C(1)+C(2)*(AZL(3,3)+AZL(1,1))+P*AZU(2,2))
      ...
      !1 isotropic value assumed here.
        CALL FIELD_PARAMETER_SET_GET_GAUSS_POINT(EQUATIONS_SET%INDEPENDENT%INDEPENDENT_FIELD, & 
          ...
          &  FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,GAUSS_POINT_NUMBER,1,ACTIVE_STRESS,ERR,ERROR,*999) ! get the independent field stress value

        PIOLA_TENSOR(1,1)=PIOLA_TENSOR(1,1)+ACTIVE_STRESS
        PIOLA_TENSOR(2,2)=PIOLA_TENSOR(2,2)+ACTIVE_STRESS
        PIOLA_TENSOR(3,3)=PIOLA_TENSOR(3,3)+ACTIVE_STRESS
      ENDIF 
...
!                   ENDIF
        ...
        CASE(EQUATIONS_SET_SETUP_INDEPENDENT_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
           ...
           !Mooney Rivlin active contraction subtype
           CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE)
               NUMBER_OF_COMPONENTS = 1 !one contractile stress value that will be added to all three directions - could generalize to 3 components or more if needed.
             IF(EQUATIONS_SET%SOLUTION_METHOD /= EQUATIONS_SET_FEM_SOLUTION_METHOD .OR. &
               &.NOT. EQUATIONS_SET%INDEPENDENT%INDEPENDENT_FIELD_AUTO_CREATED) THEN
               CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
             END IF
