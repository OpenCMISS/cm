...
! JJOR:  this is in the wrong place as NUMBER_OF_DIMENSIONS is not yet defined, hence returns a random large number, shifted further down
...
!kmith
              ...
              ELSE
                ...
                !\todo check whether this can be deleted
                CALL FIELD_NUMBER_OF_COMPONENTS_GET(EQUATIONS_SET%GEOMETRY%GEOMETRIC_FIELD,FIELD_U_VARIABLE_TYPE, &
                  & NUMBER_OF_DIMENSIONS,ERR,ERROR,*999)

                ! JJOR shifted from above since NUMBER_OF_DIMENSIONS is now defined.
                IF(IS_HYDROSTATIC_PRESSURE_DEPENDENT_FIELD) THEN
                  NUMBER_OF_COMPONENTS = NUMBER_OF_DIMENSIONS + 1
                ELSE
                  NUMBER_OF_COMPONENTS = NUMBER_OF_DIMENSIONS
