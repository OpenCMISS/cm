        ...
        DO element_idx=ELEMENTS_MAPPING%INTERNAL_START,ELEMENTS_MAPPING%INTERNAL_FINISH
          ...
          DO gauss_idx=1,DEPENDENT_NUMBER_OF_GAUSS_POINTS
            ...
            IF(NUMBER_OF_DIMENSIONS==3) THEN
              ...
              CALL FIELD_PARAMETER_SET_UPDATE_GAUSS_POINT(DEPENDENT_FIELD,FIELD_U1_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
...
999 CALL Errors("FiniteElasticityGaussDeformationGradientTensor",err,error)
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
    ...
    REAL(DP) :: WV_PRIME    
    REAL(DP) :: ISOMETRIC_FORCE_AT_FULL_ACT,LENGTH_HALF_SARCO,ACTIVATION
...
!        endif
      ...
      CASE(EQUATIONS_SET_MONODOMAIN_ELASTICITY_W_TITIN_SUBTYPE)
        ...
        PIOLA_TENSOR(1,1)=PIOLA_TENSOR(1,1)+VALUE
          ...
          LENGTH_HALF_SARCO=SQRT(AZL(1,1))
          ! isometric force-length-relation at full activation -> from Gordon 1966
          IF(LENGTH_HALF_SARCO.LE.0.635) THEN
            ISOMETRIC_FORCE_AT_FULL_ACT=0.0_DP
          ELSE IF(LENGTH_HALF_SARCO.LE.0.835) THEN 
            ISOMETRIC_FORCE_AT_FULL_ACT=4.2_DP*(LENGTH_HALF_SARCO-0.635_DP)
          ELSE IF(LENGTH_HALF_SARCO.LE.1.0) THEN
            ISOMETRIC_FORCE_AT_FULL_ACT=0.84_DP+0.9697_DP*(LENGTH_HALF_SARCO-0.835_DP)
          ELSE IF(LENGTH_HALF_SARCO.LE.1.125) THEN
            ISOMETRIC_FORCE_AT_FULL_ACT=1.0_DP
          ELSE IF(LENGTH_HALF_SARCO.LE.1.825) THEN
            ISOMETRIC_FORCE_AT_FULL_ACT=1.0_DP-1.4286_DP*(LENGTH_HALF_SARCO-1.125_DP)
          ELSE
            ISOMETRIC_FORCE_AT_FULL_ACT=0.0_DP
          ENDIF

!          ! isometric force-length-relation at full activation -> from Rassier 1999
!          IF(LENGTH_HALF_SARCO.LE.0.635) THEN
!            ISOMETRIC_FORCE_AT_FULL_ACT=0.0_DP
!          ELSE IF(LENGTH_HALF_SARCO.LE.0.85) THEN 
!            ISOMETRIC_FORCE_AT_FULL_ACT=3.2558*(LENGTH_HALF_SARCO-0.635_DP)
!          ELSE IF(LENGTH_HALF_SARCO.LE.1.2) THEN
!            ISOMETRIC_FORCE_AT_FULL_ACT=0.7_DP+0.8571_DP*(LENGTH_HALF_SARCO-0.85_DP)
!          ELSE IF(LENGTH_HALF_SARCO.LE.1.25) THEN
!            ISOMETRIC_FORCE_AT_FULL_ACT=1.0_DP
!          ELSE IF(LENGTH_HALF_SARCO.LE.1.95) THEN
!            ISOMETRIC_FORCE_AT_FULL_ACT=1.0_DP-1.4286_DP*(LENGTH_HALF_SARCO-1.25_DP)
!          ELSE
!            ISOMETRIC_FORCE_AT_FULL_ACT=0.0_DP
!          ENDIF 	    
   
          IF(ISOMETRIC_FORCE_AT_FULL_ACT<1E-9_DP) THEN ! Do not devide by zero
            ISOMETRIC_FORCE_AT_FULL_ACT=1E-9_DP
          ENDIF

          ! normalized active force -> Cell-Model
          dof_idx=FIELD_VARIABLE%COMPONENTS(1)%PARAM_TO_DOF_MAP%GAUSS_POINT_PARAM2DOF_MAP%GAUSS_POINTS(GAUSS_POINT_NUMBER, &
            & ELEMENT_NUMBER)
          CALL FIELD_PARAMETER_SET_GET_LOCAL_DOF(EQUATIONS_SET%INDEPENDENT%INDEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE, &
            & FIELD_VALUES_SET_TYPE,dof_idx,VALUE,ERR,ERROR,*999)
          ...
          ! Activation factor (gamma), decribes the propability of titin to be bound to the thin filament
          ! -> divide normalized active force (VALUE) by the isometric force at full activation 
          ACTIVATION=VALUE/ISOMETRIC_FORCE_AT_FULL_ACT
        ...
        CASE(EQUATIONS_SET_SETUP_GEOMETRY_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            CASE(EQUATIONS_SET_ORTHOTROPIC_MATERIAL_COSTA_SUBTYPE, &
                  ...
                  & EQUATIONS_SET_MONODOMAIN_ELASTICITY_VELOCITY_SUBTYPE,EQUATIONS_SET_TRANSVERSE_ISOTROPIC_HUMPHREY_YIN_SUBTYPE)
        ...
        CASE(EQUATIONS_SET_SETUP_MATERIALS_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
              ...
              CASE(EQUATIONS_SET_NEARLY_INCOMPRESSIBLE_MOONEY_RIVLIN_SUBTYPE)
                NUMBER_OF_COMPONENTS = 3;                  
              CASE(EQUATIONS_SET_TRANSVERSE_ISOTROPIC_GUCCIONE_SUBTYPE,EQUATIONS_SET_TRANSVERSE_ISOTROPIC_HUMPHREY_YIN_SUBTYPE)
                NUMBER_OF_COMPONENTS = 4;
              CASE(EQUATIONS_SET_ELASTICITY_FLUID_PRESSURE_STATIC_INRIA_SUBTYPE)
                NUMBER_OF_COMPONENTS = 6;
