...
MODULE FINITE_ELASTICITY_ROUTINES
  ...
  PUBLIC FINITE_ELASTICITY_INITIAL_GUESS  !temporary
CONTAINS
...
!    CALL WRITE_STRING(GENERAL_OUTPUT_TYPE," *** Calculating Residulas - Finite Elasticity ***",ERR,ERROR,*999)
	...
	DO parameter_idx=1,GEOMETRIC_BASIS%NUMBER_OF_ELEMENT_PARAMETERS
	  ...
	  element_dof_idx=0
		...
		DO parameter_idx=1,DEPENDENT_BASIS%NUMBER_OF_ELEMENT_PARAMETERS  !DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS
	          element_dof_idx=element_dof_idx+1		    
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_DFDZ",ERR,ERROR)
  ...
  !>Initial guess for dependent variable - undeformed dof value + displacement
  SUBROUTINE FINITE_ELASTICITY_INITIAL_GUESS(EQUATIONS_SET,ERR,ERROR,*)
...
!    TYPE(EQUATIONS_TYPE), POINTER :: EQUATIONS
    ...
    INTEGER(INTG) :: NUMBER_OF_GLOBAL_DEPENDENT_DOFS,NUMBER_OF_GLOBAL_GEOMETRIC_DOFS,dof_idx,element_dof_idx,component_idx
    INTEGER(INTG) :: COMPONENT_NUMBER,DEPENDENT_NUMBER_OF_COMPONENTS,DEPENDENT_NUMBER_OF_ELEMENT_PARAMETERS
    INTEGER(INTG) :: DEPENDENT_COMPONENT_INTERPOLATION_TYPE,DEPENDENT_COMPONENT_NUMBER_OF_PARAMETERS


    CALL ENTERS("FINITE_ELASTICITY_INITIAL_GUESS",ERR,ERROR,*999)
    ...
    DO dof_idx=1,NUMBER_OF_GLOBAL_DEPENDENT_DOFS    
      IF (dof_idx<=NUMBER_OF_GLOBAL_GEOMETRIC_DOFS) THEN !geometric vars, so add displacement BC to undeformed geomtery
        DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(dof_idx)= &
          & GEOMETRIC_FIELD%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(dof_idx)+ &		
	  & DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(dof_idx)				  		
      ELSEIF (dof_idx.GT.NUMBER_OF_GLOBAL_GEOMETRIC_DOFS) THEN !
        DEPENDENT_FIELD%PARAMETER_SETS%PARAMETER_SETS(1)%PTR%PARAMETERS%CMISS%DATA_DP(dof_idx)=-5.0
      ENDIF
    ENDDO	

  
    CALL EXITS("FINITE_ELASTICITY_INITIAL_GUESS")
    RETURN
999 CALL ERRORS("FINITE_ELASTICITY_INITIAL_GUESS",ERR,ERROR)
    CALL EXITS("FINITE_ELASTICITY_INITIAL_GUESS")
    RETURN 1
  END SUBROUTINE FINITE_ELASTICITY_INITIAL_GUESS
