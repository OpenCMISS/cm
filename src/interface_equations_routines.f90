...
CONTAINS
  ...
  SUBROUTINE INTERFACE_EQUATIONS_CREATE_FINISH(INTERFACE_EQUATIONS,ERR,ERROR,*)
    ...
    TYPE(FIELD_TYPE), POINTER :: DEPENDENT_FIELD,GEOMETRIC_FIELD,PENALTY_FIELD
              ...
              IF(ASSOCIATED(INTERFACE_DEPENDENT)) THEN
                IF(ASSOCIATED(INTERFACE_EQUATIONS%INTERPOLATION)) THEN
                  ...
                  CALL INTERFACE_EQUATIONS_DOMAIN_INTERPOLATION_SETUP(INTERFACE_EQUATIONS%INTERPOLATION% &
                    & INTERFACE_INTERPOLATION,GEOMETRIC_FIELD,DEPENDENT_FIELD,PENALTY_FIELD,ERR,ERROR,*999)
                  !\todo change INTERFACE_EQUATIONS_DOMAIN_INTERPOLATION_SETUP input arguments to remove the need to input GEOMETRIC_FIELD & DEPENDENT FIELD. 
                  !Currently geometric and dependent fields are pased into the subroutine (and now also the PENALTY FIELD) so the subroutine can be reused to 
                  !set both the interface_variable interpolation (eg Lagrange, Penalty and interface geometry field) or a coupled mesh variable interpolation 
                  !(eg mesh dependent/geometric fields). An alternative would be to send through which interpolation is required and then loop through to create the interpolation sets. 
                  !Remove NULLIFY(PENALTY_FIELD) below once this has been implemented.
                  ...
                  DO variable_idx=1,INTERFACE_DEPENDENT%NUMBER_OF_DEPENDENT_VARIABLES
                    ...
                    IF(ASSOCIATED(DEPENDENT_VARIABLE)) THEN
                      ...
                      IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
                        ...
                        CALL INTERFACE_EQUATIONS_DOMAIN_INTERPOLATION_SETUP(INTERFACE_EQUATIONS%INTERPOLATION% &
                          & VARIABLE_INTERPOLATION(variable_idx),GEOMETRIC_FIELD,DEPENDENT_FIELD,PENALTY_FIELD,ERR,ERROR,*999)
                      ELSE
...
999 CALL ERRORS("INTERFACE_EQUATIONS_DOMAIN_INTERPOLATION_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE INTERFACE_EQUATIONS_DOMAIN_INTERPOLATION_SETUP(DOMAIN_INTERPOLATION,GEOMETRIC_FIELD,DEPENDENT_FIELD,PENALTY_FIELD, &
    &  ERR,ERROR,*)
    ...
    IF(ASSOCIATED(GEOMETRIC_FIELD)) THEN
      IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
        ...
        DO interpolation_set_idx=1,DOMAIN_INTERPOLATION%NUMBER_OF_DEPENDENT_INTERPOLATION_SETS
          ...
          ENDIF
        ENDDO !interpolation_set_idx
        IF(ASSOCIATED(DOMAIN_INTERPOLATION%INTERPOLATION%INTERFACE_EQUATIONS%INTERFACE_CONDITION%PENALTY)) THEN
