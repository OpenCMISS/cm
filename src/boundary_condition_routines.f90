                            ...
                            CASE(FIELD_NODE_BASED_INTERPOLATION)                      
                              IF(DEPENDENT_FIELD%DECOMPOSITION% &
                                ...
                                IF(ASSOCIATED(DOMAIN_FACES)) THEN
                                  ...
                                  IF(NUMBER_OF_SET_DOF_INTEGRATED>0.AND.NUMBER_OF_SET_DOF_POINT==0) THEN
...
999 CALL ERRORS("BOUNDARY_CONDITIONS_INTEGRATED_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE BOUNDARY_CONDITIONS_FACE_BASIS_CALCULATE(BOUNDARY_CONDITIONS,VARIABLE_TYPE,component_idx,FACE_NUMBER, &
    ...
    !out, supposedly, goes NUMBER_DOFS_IN_FACE (Why ?) and certain entries in what BOUNDARY_CONDITIONS points to:
    !BOUNDARY_CONDITIONS%BOUNDARY_CONDITIONS_VARIABLE_TYPE_MAP(VARIABLE_TYPE)%PTR%NEUMANN_BOUNDARY_CONDITIONS%FACE_INTEGRATION_MATRIX
    ...
    IF(ASSOCIATED(BOUNDARY_CONDITIONS)) THEN
      EQUATIONS_SET=>BOUNDARY_CONDITIONS%EQUATIONS_SET  !Why from BOUNDARY_CONDITIONS to EQUATIONS_SET, not the other way round ?
      IF(ASSOCIATED(EQUATIONS_SET)) THEN
        EQUATIONS=>EQUATIONS_SET%EQUATIONS
        IF(ASSOCIATED(EQUATIONS)) THEN
          BOUNDARY_CONDITIONS_VARIABLE=>BOUNDARY_CONDITIONS% &
              ...
              IF(ASSOCIATED(TOPOLOGY)) THEN
                ...
                IF(ASSOCIATED(DOMAIN_FACES)) THEN
                  ...
                  IF(ASSOCIATED(DOMAIN_FACE)) THEN
                    ...
                    ELEMENT_NUMBER=DOMAIN_FACE%ELEMENT_NUMBER  !not used
