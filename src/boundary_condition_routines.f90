...
MODULE BOUNDARY_CONDITIONS_ROUTINES
  ...
  INTEGER(INTG), PARAMETER :: BOUNDARY_CONDITION_INCREMENTED_TYPE=13 !<The dof is a fixed boundary condition, to be used with load increment loop. \see BOUNDARY_CONDITIONS_ROUTINES_BoundaryConditions,BOUNDARY_CONDITIONS_ROUTINES
  !>@}
  ...
  PUBLIC BOUNDARY_CONDITION_NOT_FIXED,BOUNDARY_CONDITION_FIXED,BOUNDARY_CONDITION_MIXED,BOUNDARY_CONDITION_FIXED_INLET, & 
    ...
    & BOUNDARY_CONDITION_ROBIN, BOUNDARY_CONDITION_INCREMENTED_TYPE
...
!!the data from each rank into all ranks. We will see how this goes for now.
                  ...
                  DO dof_idx=1,FIELD_VARIABLE%NUMBER_OF_DOFS
                    IF(BOUNDARY_CONDITION_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(dof_idx)==BOUNDARY_CONDITION_FIXED.OR. &
                      ...
                      & BOUNDARY_CONDITION_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(dof_idx)==BOUNDARY_CONDITION_INCREMENTED_TYPE) THEN
                    ...
                    IF(ASSOCIATED(BOUNDARY_CONDITIONS_DIRICHLET)) THEN
                      ...
                      DO dof_idx=1,FIELD_VARIABLE%NUMBER_OF_DOFS
                        IF(BOUNDARY_CONDITION_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(dof_idx)==BOUNDARY_CONDITION_FIXED.OR. &
                          ...
                          & BOUNDARY_CONDITION_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(dof_idx)==BOUNDARY_CONDITION_FIXED_WALL.OR. &
                          & BOUNDARY_CONDITION_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(dof_idx)==BOUNDARY_CONDITION_MOVED_WALL.OR. &
                          & BOUNDARY_CONDITION_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(dof_idx)==BOUNDARY_CONDITION_INCREMENTED_TYPE) &
                          & THEN
...
999 CALL ERRORS("BOUNDARY_CONDITIONS_BOUNDARY_ADD_DOFS",ERR,ERROR)
  ...
  SUBROUTINE BOUNDARY_CONDITIONS_INTEGRATED_CALCULATE(BOUNDARY_CONDITIONS,VARIABLE_TYPE,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG), POINTER :: TEMP_SET_NODES(:),TEMP_FACES(:),TEMP_DOFS(:) !Temporary lists of faces, nodes and dofs on domain of interest
    TYPE(BOUNDARY_CONDITIONS_NEUMANN_VALUES_TYPE), POINTER :: NEUMANN_VALUES
    INTEGER(INTG) :: TEMP_NUMBER_OF_SET_NODES,TEMP_NUMBER_OF_FACES,NUMBER_OF_SET_DOF
    INTEGER(INTG) :: local_ny,component_idx,nd,nn,nf,nf1,M,N,i,j,ms,ns,derivative
    INTEGER(INTG) :: id,NUMBER_DOFS_IN_FACE,TOTAL_NUMBER_OF_FACE_DOF,global_ny
    INTEGER(INTG) :: MAX_NUMBER_DOFS_IN_FACE,FACE_NUMBER,NODE_NUMBER,x_pos,NODE
    INTEGER(INTG) :: face_local_node_index,face_local_deriv_index
    LOGICAL :: DOF_LOCATED
    TYPE(LIST_TYPE), POINTER :: TEMP_SET_NODES_LIST,TEMP_FACES_LIST,TEMP_DOFS_LIST
    TYPE(VARYING_STRING) :: LOCAL_ERROR
                                        ...
                                        DO nf1=1,DOMAIN_FACES%NUMBER_OF_FACES
                                          ...
                                          IF(DOMAIN_FACE%NUMBER==DOMAIN_NODE%NODE_FACES(face_local_node_index) &
                                            ...
                                            DO ms=1,DOMAIN_FACE%BASIS%NUMBER_OF_ELEMENT_PARAMETERS
                                              ...
                                              IF(local_ny==NEUMANN_VALUES%SET_DOF(nd)) ns = ms
                                            ENDDO !ms
          ...
          IF(EQUATIONS_SET%DEPENDENT%DEPENDENT_FINISHED) THEN
            ...
            IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
              ...
              IF(ASSOCIATED(BOUNDARY_CONDITIONS_VARIABLE)) THEN
                ...
                CASE(BOUNDARY_CONDITION_FREE_WALL)
                  BOUNDARY_CONDITIONS_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(global_ny)=BOUNDARY_CONDITION_FREE_WALL
                CASE(BOUNDARY_CONDITION_INCREMENTED_TYPE) !For load increment loops
                  BOUNDARY_CONDITIONS_VARIABLE%GLOBAL_BOUNDARY_CONDITIONS(global_ny)=BOUNDARY_CONDITION_INCREMENTED_TYPE
