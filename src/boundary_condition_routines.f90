  ...
  SUBROUTINE BOUNDARY_CONDITIONS_VARIABLE_FINALISE(BOUNDARY_CONDITIONS_VARIABLE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(BOUNDARY_CONDITIONS_VARIABLE)) THEN
      ...
      IF(ASSOCIATED(BOUNDARY_CONDITIONS_VARIABLE%DIRICHLET_BOUNDARY_CONDITIONS)) THEN
        ...
        IF (ALLOCATED(BOUNDARY_CONDITIONS_DIRICHLET%LINEAR_SPARSITY_INDICES)) THEN
          DO equ_set_idx=1,SIZE(BOUNDARY_CONDITIONS_DIRICHLET%LINEAR_SPARSITY_INDICES,1)
            DO equ_matrix_idx=1,SIZE(BOUNDARY_CONDITIONS_DIRICHLET%LINEAR_SPARSITY_INDICES,2)
              IF(ASSOCIATED(BOUNDARY_CONDITIONS_DIRICHLET%LINEAR_SPARSITY_INDICES(equ_set_idx,equ_matrix_idx)%PTR)) THEN
                DEALLOCATE(BOUNDARY_CONDITIONS_DIRICHLET%LINEAR_SPARSITY_INDICES(equ_set_idx,equ_matrix_idx)%PTR)
              ENDIF
            ENDDO
          ENDDO
          DEALLOCATE(BOUNDARY_CONDITIONS_DIRICHLET%LINEAR_SPARSITY_INDICES)
        ENDIF
