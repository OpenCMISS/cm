...
MODULE SOLUTION_MAPPING_ROUTINES
  ...
  PUBLIC SOLUTION_MAPPING_CREATE_FINISH,SOLUTION_MAPPING_CREATE_START,SOLUTION_MAPPING_DESTROY, &
    & SOLUTION_MAPPING_EQUATIONS_SET_ADD,SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_VARIABLES_SET, &
...
!!TODO: see how slow this is. At the moment we go through number of ranks*number of global rows. We could presort the global rows into a list for each rank. This would take additional memory.
                        DO rank=0,COMPUTATIONAL_ENVIRONMENT%NUMBER_COMPUTATIONAL_NODES
                          DO global_row=1,EQUATIONS_MAPPING%TOTAL_NUMBER_OF_ROWS
                            RANK_DOF=.FALSE.
                            local_row=0
                            ...
                            IF(RANK_DOF) THEN
                              ...
                              DO equations_matrix_idx=1,EQUATIONS_MAPPING%NUMBER_OF_MATRIX_VARIABLES
                                ...
                                DEPENDENT_VARIABLE=>EQUATIONS_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(equations_matrix_idx)% &
                                  & VARIABLE
          ...
          SOLUTION_MAPPING%TOTAL_NUMBER_OF_ROWS=NUMBER_OF_GLOBAL_SOLVER_ROWS
          ...
          DO rank=0,COMPUTATIONAL_ENVIRONMENT%NUMBER_COMPUTATIONAL_NODES
            ...
            DO equations_set_idx=1,SOLUTION_MAPPING%NUMBER_OF_EQUATIONS_SETS
              ...
              IF(rank==COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
                ...
                CALL SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_INITALISE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP( &
                ...
                ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM( &
                  & EQUATIONS_MAPPING%NUMBER_OF_EQUATIONS_MATRICES),STAT=ERR)
                IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations set to solver map equations to solver matrix maps em.", &
                  & ERR,ERROR,*999)
                ...
                ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_ROW_TO_SOLVER_ROWS_MAPS( &
                  & EQUATIONS_MAPPING%NUMBER_OF_ROWS),STAT=ERR)
...
!!TODO: see how slow this is. At the moment we go through number of ranks*number of global rows. We could presort the global rows into a list for each rank. This would take additional memory.
              DO global_row=1,ROW_DOFS_MAPPING%NUMBER_OF_GLOBAL
                ...
                DO rank_idx=1,ROW_DOFS_MAPPING%GLOBAL_TO_LOCAL_MAP(global_row)%NUMBER_OF_DOMAINS
                  IF(ROW_DOFS_MAPPING%GLOBAL_TO_LOCAL_MAP(global_row)%DOMAIN_NUMBER(rank_idx)==rank &
                    & .AND.ROW_DOFS_MAPPING%GLOBAL_TO_LOCAL_MAP(global_row)%LOCAL_TYPE(rank_idx)/=DOMAIN_LOCAL_GHOST) THEN
                    RANK_DOF=.TRUE.
                    local_row=ROW_DOFS_MAPPING%GLOBAL_TO_LOCAL_MAP(global_row)%LOCAL_NUMBER(rank_idx)
                    EXIT
                  ENDIF
                ...
                IF(RANK_DOF) THEN
                  INCLUDE_ROW=.FALSE.
                  DO equations_matrix_idx=1,EQUATIONS_MAPPING%NUMBER_OF_MATRIX_VARIABLES
                    local_dof=EQUATIONS_MAPPING%EQUATIONS_ROW_TO_VARIABLES_MAPS(local_row)%ROW_TO_DOFS_MAP(equations_matrix_idx)
                    variable_type=EQUATIONS_MAPPING%MATRIX_VARIABLE_TYPES(equations_matrix_idx)
                  ...
                  IF(INCLUDE_ROW) THEN
                    ...
                    IF(rank==COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
                      ...
                      IF(ERR/=0) CALL FLAG_ERROR("Could not allocate row to equations set map equations row number.",ERR,ERROR,*999)
                      ALLOCATE(SOLUTION_MAPPING%SOLVER_ROW_TO_EQUATIONS_SET_MAPS(NUMBER_OF_LOCAL_SOLVER_ROWS)% &
                      ...
                      !Initialise
                      CALL SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_INITALISE(SOLUTION_MAPPING% &
            ...
            DO rank=0,COMPUTATIONAL_ENVIRONMENT%NUMBER_COMPUTATIONAL_NODES              
              ...
              DO equations_set_idx=1,SOLUTION_MAPPING%NUMBER_OF_EQUATIONS_SETS
                ...
                IF(rank==COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
                  ...
                  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations to solver matrix maps sm variable types.",ERR,ERROR,*999)
                  ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
...
!!TODO: see how slow this is. At the moment we go through number of ranks*number of globals. We could presort the global nys into a list for each rank. This would take additional memory.
                DO variable_idx=1,NUMBER_OF_VARIABLES
                  ...
                  IF(ASSOCIATED(COL_DOFS_MAPPING)) THEN
                    IF(rank==COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
                      ...
                      ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                        & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_idx)%COLUMN_NUMBERS(DEPENDENT_VARIABLE% &
                        & NUMBER_OF_DOFS),STAT=ERR)
                      IF(ERR/=0) CALL FLAG_ERROR("Could not allocate variables to solver column maps column numbers.", &
                        & ERR,ERROR,*999)
                      ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                        & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_idx)%COUPLING_COEFFICIENTS(DEPENDENT_VARIABLE% &
                        & NUMBER_OF_DOFS),STAT=ERR)
                      IF(ERR/=0) CALL FLAG_ERROR("Could not allocate variables to solver column maps coupling coefficients.", &
                        & ERR,ERROR,*999)
                      ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                        & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_idx)%ADDITIVE_CONSTANTS(DEPENDENT_VARIABLE% &
                        & NUMBER_OF_DOFS),STAT=ERR)
                      IF(ERR/=0) CALL FLAG_ERROR("Could not allocate variables to solver column maps additive constants.", &
                        & ERR,ERROR,*999)
                      !Setup
                      NUMBER_OF_EQUATIONS_MATRICES=NUMBER_OF_EQUATIONS_MATRICES+EQUATIONS_MAPPING% &
                        & VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%NUMBER_OF_EQUATIONS_MATRICES
                    ENDIF !rank==my rank
            ...
            ALLOCATE(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)%SOLVER_COL_TO_VARIABLE_MAPS( &
            ...
            DO rank=0,COMPUTATIONAL_ENVIRONMENT%NUMBER_COMPUTATIONAL_NODES
              ...
              DO equations_set_idx=1,SOLUTION_MAPPING%NUMBER_OF_EQUATIONS_SETS
                ...
                IF(rank==COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
                  ...
                  ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                    & solver_matrix_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS(NUMBER_OF_EQUATIONS_MATRICES),STAT=ERR)
                  IF(ERR/=0) &
                    & CALL FLAG_ERROR("Could not allocate equations to solver matrix maps sm equations to solver matrix maps.", &
                    & ERR,ERROR,*999)
                  !Set up arrays
                  SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                    & solver_matrix_idx)%NUMBER_OF_EQUATIONS_MATRICES=NUMBER_OF_EQUATIONS_MATRICES
                      ...
                      IF(FIXED_CONDITIONS%GLOBAL_BOUNDARY_CONDITIONS(field_dof)==EQUATIONS_SET_NOT_FIXED) THEN
                        ...
                        IF(rank==COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
                          ...
                          ALLOCATE(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                            ...
                            & NUMBER_OF_GLOBAL_SOLVER_COLS)%EQUATIONS_MATRIX_NUMBERS(NUMBER_OF_EQUATIONS_MATRICES),STAT=ERR)
                          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations matrix numbers.",ERR,ERROR,*999)
                          ALLOCATE(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                            & SOLVER_COL_TO_EQUATIONS_SET_MAPS(equations_set_idx)%SOLVER_COL_TO_EQUATIONS_MAPS( &
                            & NUMBER_OF_GLOBAL_SOLVER_COLS)%EQUATIONS_COL_NUMBERS(NUMBER_OF_EQUATIONS_MATRICES),STAT=ERR)
                          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations column numbers.",ERR,ERROR,*999)
                          ALLOCATE(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                            & SOLVER_COL_TO_EQUATIONS_SET_MAPS(equations_set_idx)%SOLVER_COL_TO_EQUATIONS_MAPS( &
                            & NUMBER_OF_GLOBAL_SOLVER_COLS)%COUPLING_COEFFICIENTS(NUMBER_OF_EQUATIONS_MATRICES),STAT=ERR)
                          ...
                          SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                            & SOLVER_COL_TO_EQUATIONS_SET_MAPS(equations_set_idx)%SOLVER_COL_TO_EQUATIONS_MAPS( &
                            & NUMBER_OF_GLOBAL_SOLVER_COLS)%NUMBER_OF_EQUATIONS_MATRICES=NUMBER_OF_EQUATIONS_MATRICES
                          !Loop over the global matrices associated with the variable and set the column maps
                          DO equations_matrix_idx=1,NUMBER_OF_EQUATIONS_MATRICES
                            !Set the column map
                            MATRIX_NUMBER=EQUATIONS_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                              & EQUATIONS_MATRIX_NUMBERS(equations_matrix_idx)
                            equations_column=EQUATIONS_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                            ...
                            SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                              ...
                              & EQUATIONS_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(MATRIX_NUMBER)%MATRIX_COEFFICIENT
                          ENDDO !equations_matrix_idx
                          !Set up the solver column/dofs -> variable dofs map
                          !Initialise
                          CALL SOLUTION_MAPPING_SOLVER_COL_TO_VARIABLE_MAP_INITIALISE(SOLUTION_MAPPING% &
                            & SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)%SOLVER_COL_TO_VARIABLE_MAPS( &
                            & NUMBER_OF_GLOBAL_SOLVER_COLS),ERR,ERROR,*999)
                          ...
                          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate solver column to variable maps variable type.", &
                            & ERR,ERROR,*999)
                          ALLOCATE(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                            & SOLVER_COL_TO_VARIABLE_MAPS(NUMBER_OF_GLOBAL_SOLVER_COLS)%VARIABLE_DOF(1),STAT=ERR)
                          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate solver column to variable maps variable dof.", &
                            & ERR,ERROR,*999)
                          ALLOCATE(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                            & SOLVER_COL_TO_VARIABLE_MAPS(NUMBER_OF_GLOBAL_SOLVER_COLS)%VARIABLE_COEFFICIENT(1),STAT=ERR)
                          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate solver column to variable maps variable coefficient.", &
                            & ERR,ERROR,*999)
                          ALLOCATE(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)% &
                            & SOLVER_COL_TO_VARIABLE_MAPS(NUMBER_OF_GLOBAL_SOLVER_COLS)%ADDITIVE_CONSTANT(1),STAT=ERR)
                          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate solver column to variable maps additive constant.", &
                            & ERR,ERROR,*999)
                          !Setup
                          ...
                          SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)%SOLVER_COL_TO_VARIABLE_MAPS( &
                            & NUMBER_OF_GLOBAL_SOLVER_COLS)%VARIABLE_COEFFICIENT(1)=1.0_DP
                          SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(solver_matrix_idx)%SOLVER_COL_TO_VARIABLE_MAPS( &
                            & NUMBER_OF_GLOBAL_SOLVER_COLS)%ADDITIVE_CONSTANT(1)=0.0_DP
                          !Set up the equations variables -> solver columns mapping
                          !No coupling yet so the mapping is 1-1
                          SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                            & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_type)%COLUMN_NUMBERS(local_dof)= &
                            & NUMBER_OF_GLOBAL_SOLVER_COLS
                          SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                            & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_type)%COUPLING_COEFFICIENTS(1)=1.0_DP
                          SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                            & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_type)%ADDITIVE_CONSTANTS(1)=0.0_DP
                          ...
                          DO equations_matrix_idx=1,NUMBER_OF_EQUATIONS_MATRICES
                            ...
                            ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)% &
                              ...
                              & EQUATIONS_MATRIX_OFFSET+equations_matrix_idx)%PTR%EQUATIONS_COL_SOLVER_COLS_MAP(equations_column)% &
                              & SOLVER_COLS(1),STAT=ERR)
                            IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations column to solver columns map solver colums.", &
                              & ERR,ERROR,*999)
                            ALLOCATE(SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)% &
                              & EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM(solver_matrix_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS( &
                              & EQUATIONS_MATRIX_OFFSET+equations_matrix_idx)%PTR%EQUATIONS_COL_SOLVER_COLS_MAP(equations_column)% &
                              & COUPLING_COEFFICIENTS(1),STAT=ERR)
                            IF(ERR/=0)  &
                              & CALL FLAG_ERROR("Could not allocate equations column to solver columns map coupling coefficients.",&
                              & ERR,ERROR,*999)
                            SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                              & solver_matrix_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS(EQUATIONS_MATRIX_OFFSET+equations_matrix_idx)% &
                              & PTR%EQUATIONS_COL_SOLVER_COLS_MAP(equations_column)%NUMBER_OF_SOLVER_COLS=1
                            SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                              & solver_matrix_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS(EQUATIONS_MATRIX_OFFSET+equations_matrix_idx)% &
                              & PTR%EQUATIONS_COL_SOLVER_COLS_MAP(equations_column)%SOLVER_COLS(1)=NUMBER_OF_GLOBAL_SOLVER_COLS
                            SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                              & solver_matrix_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS(EQUATIONS_MATRIX_OFFSET+equations_matrix_idx)% &
                              & PTR%EQUATIONS_COL_SOLVER_COLS_MAP(equations_column)%COUPLING_COEFFICIENTS(1)=EQUATIONS_MAPPING% &
                              & EQUATIONS_MATRIX_TO_VARIABLE_MAPS(MATRIX_NUMBER)%MATRIX_COEFFICIENT
                          ENDDO !equations_matrix_idx
                        ENDIF !rank==my rank
                      ELSE
                        IF(rank==COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
                          ...
                          !No coupling yet so the mapping is 1-1
                          SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                            & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_type)%COLUMN_NUMBERS(local_dof)=0
                          SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                            & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_type)%COUPLING_COEFFICIENTS(local_dof)=0.0_DP
                          SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                            & solver_matrix_idx)%VARIABLE_TO_SOLVER_COL_MAPS(variable_type)%ADDITIVE_CONSTANTS(local_dof)=0.0_DP
                          !Set up the equations columns -> solver columns mapping
                          DO equations_matrix_idx=1,NUMBER_OF_EQUATIONS_MATRICES
                            MATRIX_NUMBER=EQUATIONS_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                              & EQUATIONS_MATRIX_NUMBERS(equations_matrix_idx)
                            ...
                            SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM( &
                              & solver_matrix_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS(EQUATIONS_MATRIX_OFFSET+equations_matrix_idx)% &
                              & PTR%EQUATIONS_COL_SOLVER_COLS_MAP(equations_column)%NUMBER_OF_SOLVER_COLS=0
                          ENDDO !equations_matrix_idx
                        ENDIF !rank==my rank
                      ENDIF !field dof is fixed
                    ENDIF !rank dof
                  ENDDO !global_dof
                  EQUATIONS_MATRIX_OFFSET=EQUATIONS_MATRIX_OFFSET+NUMBER_OF_EQUATIONS_MATRICES
                ENDDO !variable_idx
              ENDDO !equations_set_idx
            ENDDO !rank
    ...
    IF(DIAGNOSTICS1) THEN
      ...
      DO row_idx=1,SOLUTION_MAPPING%NUMBER_OF_ROWS
        ...
        CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,SOLUTION_MAPPING%SOLVER_ROW_TO_EQUATIONS_SET_MAPS(row_idx)% &
          ...
          & '("      Equations sets indices :",5(X,I13))','(30X,5(X,I13))',ERR,ERROR,*999) 
        CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,SOLUTION_MAPPING%SOLVER_ROW_TO_EQUATIONS_SET_MAPS(row_idx)% &
      ...
      DO equations_set_idx=1,SOLUTION_MAPPING%NUMBER_OF_EQUATIONS_SETS
        ...
        EQUATIONS_MAPPING=>EQUATIONS%EQUATIONS_MAPPING
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"    Equations set index : ",equations_set_idx,ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Number of equations set rows = ",EQUATIONS_MAPPING%NUMBER_OF_ROWS, &
          & ERR,ERROR,*999)
      ...
      DO solver_matrix_idx=1,SOLUTION_MAPPING%NUMBER_OF_SOLVER_MATRICES
        ...
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"  Number of columns = ",SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP( &
...
!!TODO: write out the variables somehow.
      ...
      DO equations_set_idx=1,SOLUTION_MAPPING%NUMBER_OF_EQUATIONS_SETS
        ...
        DO solver_matrix_idx=1,SOLUTION_MAPPING%NUMBER_OF_SOLVER_MATRICES
          CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"    Solver matrix : ",solver_matrix_idx,ERR,ERROR,*999)
          CALL WRITE_STRING(DIAGNOSTIC_OUTPUT_TYPE,      "      Equations matrix columns to solver matrix columns:",ERR,ERROR,*999)
          CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Number of equations matrices = ",SOLUTION_MAPPING% &
            & EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)%EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM(solver_matrix_idx)% &
            & NUMBER_OF_EQUATIONS_MATRICES,ERR,ERROR,*999)
          DO equations_matrix_idx=1,SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)% &
            ...
            DO column_idx=1,EQUATIONS_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(equations_matrix)%NUMBER_OF_COLUMNS
          ...
          DO variable_idx=1,SOLUTION_MAPPING%EQUATIONS_SET_TO_SOLVER_MAP(equations_set_idx)% &
            ...
            DO local_dof=1,DEPENDENT_VARIABLE%NUMBER_OF_DOFS
          ...
          ENDDO !variable_idx
        ENDDO !solver_matrix_idx
        CALL WRITE_STRING(DIAGNOSTIC_OUTPUT_TYPE,"    Equations matrix indexing:",ERR,ERROR,*999)
        DO equations_matrix_idx=1,EQUATIONS_MAPPING%NUMBER_OF_EQUATIONS_MATRICES
...
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_COL_TO_SOLVER_COLS_MAP_FINALISE",ERR,ERROR)    
  ...
  SUBROUTINE SOLUTION_MAPPING_EQUATIONS_COL_TO_SOLVER_COLS_MAP_INITALISE(EQUATIONS_COL_SOLVER_COLS_MAP,ERR,ERROR,*)
    ...
    CALL ENTERS("SOLUTION_MAPPING_EQUATIONS_COL_TO_SOLVER_COLS_MAP_INITALISE",ERR,ERROR,*999)

    EQUATIONS_COL_SOLVER_COLS_MAP%NUMBER_OF_SOLVER_COLS=0
    
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_COL_TO_SOLVER_COLS_MAP_INITALISE")
    RETURN
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_COL_TO_SOLVER_COLS_MAP_INITALISE",ERR,ERROR)    
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_COL_TO_SOLVER_COLS_MAP_INITALISE")
    RETURN 1
   
  END SUBROUTINE SOLUTION_MAPPING_EQUATIONS_COL_TO_SOLVER_COLS_MAP_INITALISE
...
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_FINALISE",ERR,ERROR)    
  ...
  SUBROUTINE SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_INITALISE(EQUATIONS_ROW_SOLVER_ROWS_MAP,ERR,ERROR,*)
    ...
    CALL ENTERS("SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_INITALISE",ERR,ERROR,*999)

    EQUATIONS_ROW_SOLVER_ROWS_MAP%NUMBER_OF_SOLVER_ROWS=0
    
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_INITALISE")
    RETURN
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_INITALISE",ERR,ERROR)    
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_INITALISE")
    RETURN 1
   
  END SUBROUTINE SOLUTION_MAPPING_EQUATIONS_ROW_TO_SOLVER_ROWS_MAP_INITALISE
                ...
                IF(ASSOCIATED(SOLUTION_MAPPING%CREATE_VALUES_CACHE)) THEN
                  ...
                  DO matrix_idx=1,SOLUTION_MAPPING%NUMBER_OF_SOLVER_MATRICES
                    ...
                    DO WHILE(variable_type<=FIELD_NUMBER_OF_VARIABLE_TYPES.AND..NOT.MATRIX_DONE)
                      IF(EQUATIONS_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                  ...
                  DO variable_idx=variable_type+1,FIELD_NUMBER_OF_VARIABLE_TYPES
                    IF(EQUATIONS_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_idx)%NUMBER_OF_EQUATIONS_MATRICES>0) THEN
...
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_FINALISE",ERR,ERROR)    
  ...
  SUBROUTINE SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_INITALISE(EQUATIONS_SET_TO_SOLVER_MAP,ERR,ERROR,*)
    ...
    CALL ENTERS("SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_INITALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_INITALISE")
    RETURN
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_INITALISE",ERR,ERROR)    
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_INITALISE")
    RETURN 1
   
  END SUBROUTINE SOLUTION_MAPPING_EQUATIONS_SET_TO_SOLVER_MAP_INITALISE
...
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MAPS_FINALISE",ERR,ERROR)    
  ...
  SUBROUTINE SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MAPS_INITALISE(EQUATIONS_TO_SOLVER_MAP,ERR,ERROR,*)
    ...
    CALL ENTERS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MAPS_INITALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MAPS_INITALISE")
    RETURN
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MAPS_INITALISE",ERR,ERROR)    
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MAPS_INITALISE")
    RETURN 1
   
  END SUBROUTINE SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MAPS_INITALISE
...
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM_FINALISE",ERR,ERROR)    
  ...
  SUBROUTINE SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM_INITALISE(EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM,ERR,ERROR,*)
    ...
    CALL ENTERS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM_INITALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM_INITALISE")
    RETURN
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM_INITALISE",ERR,ERROR)    
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM_INITALISE")
    RETURN 1
   
  END SUBROUTINE SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_EM_INITALISE
...
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM_FINALISE",ERR,ERROR)    
  ...
  SUBROUTINE SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM_INITIALISE(EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM,ERR,ERROR,*)
    ...
    EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM%NUMBER_OF_EQUATIONS_MATRICES=0
...
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_MATRIX_MAPS_SM_INITIALISE",ERR,ERROR)    
  ...
  SUBROUTINE SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_VARIABLES_SET(SOLUTION_MAPPING,SOLVER_MATRIX,EQUATIONS_SET_INDEX, &
    ...
    CALL ENTERS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_VARIABLES_SET",ERR,ERROR,*999)
...
!!TODO: CHECK THAT THE VARIABLE TYPE IS NOT REPEATED
                        IF(VARIABLE_TYPES(variable_idx)<1.OR. &
                          ...
                          CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
                        ENDIF
                        IF(EQUATIONS_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(VARIABLE_TYPES(variable_idx))% &
                          & NUMBER_OF_EQUATIONS_MATRICES==0) THEN
                          LOCAL_ERROR="The variable type number of "// &
                            & TRIM(NUMBER_TO_VSTRING(VARIABLE_TYPES(variable_idx),"*",ERR,ERROR))// &
                            & " at position "//TRIM(NUMBER_TO_VSTRING(variable_idx,"*",ERR,ERROR))// &
    ...
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_VARIABLES_SET")
    RETURN
999 CALL ERRORS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_VARIABLES_SET",ERR,ERROR)
    CALL EXITS("SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_VARIABLES_SET")
    RETURN 1
  END SUBROUTINE SOLUTION_MAPPING_EQUATIONS_TO_SOLVER_VARIABLES_SET
  ...
  SUBROUTINE SOLUTION_MAPPING_INITIALISE(SOLUTION,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(SOLUTION)) THEN
      ...
      ELSE
        ...
        ELSE
          SOLUTION%SOLUTION_MAPPING%NUMBER_OF_SOLVER_MATRICES=0
        ENDIF
        SOLUTION%SOLUTION_MAPPING%NUMBER_OF_ROWS=0
        SOLUTION%SOLUTION_MAPPING%TOTAL_NUMBER_OF_ROWS=0
        SOLUTION%SOLUTION_MAPPING%NUMBER_OF_EQUATIONS_SETS=0
        SOLUTION%SOLUTION_MAPPING%HAVE_JACOBIAN=.FALSE.
...
999 CALL ERRORS("SOLUTION_MAPPING_SOLVER_COL_TO_EQUATIONS_MAP_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE SOLUTION_MAPPING_SOLVER_COL_TO_EQUATIONS_MAP_INITIALISE(SOLVER_COL_TO_EQUATIONS_MAP,ERR,ERROR,*)
    ...
    SOLVER_COL_TO_EQUATIONS_MAP%NUMBER_OF_EQUATIONS_MATRICES=0
...
999 CALL ERRORS("SOLUTION_MAPPING_SOLVER_COL_TO_VARIABLE_MAP_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE SOLUTION_MAPPING_SOLVER_COL_TO_VARIABLE_MAP_INITIALISE(SOLVER_COL_TO_VARIABLE_MAP,ERR,ERROR,*)
    ...
    !Argument variables
    TYPE(SOLVER_COL_TO_VARIABLE_MAP_TYPE) :: SOLVER_COL_TO_VARIABLE_MAP !<The solver column to variable map to initialise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code 
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    ...
    RETURN
999 CALL ERRORS("SOLUTION_MAPPING_SOLVER_COL_TO_VARIABLE_MAP_INITIALISE",ERR,ERROR)
    CALL EXITS("SOLUTION_MAPPING_SOLVER_COL_TO_VARIABLE_MAP_INITIALISE")
    RETURN 1
  END SUBROUTINE SOLUTION_MAPPING_SOLVER_COL_TO_VARIABLE_MAP_INITIALISE
      ...
      ELSE
        IF(ASSOCIATED(SOLUTION_MAPPING%CREATE_VALUES_CACHE)) THEN
          ...
          DO equations_set_idx=1,SOLUTION_MAPPING%NUMBER_OF_EQUATIONS_SETS
            ...
            EQUATIONS=>EQUATIONS_SET%EQUATIONS
            EQUATIONS_MAPPING=>EQUATIONS%EQUATIONS_MAPPING
            IF(EQUATIONS_MAPPING%NUMBER_OF_EQUATIONS_MATRICES>MAXIMUM_NUMBER_OF_EQUATIONS_MATRICES) &
              & MAXIMUM_NUMBER_OF_EQUATIONS_MATRICES=EQUATIONS_MAPPING%NUMBER_OF_EQUATIONS_MATRICES
          ENDDO !equations_set_idx
