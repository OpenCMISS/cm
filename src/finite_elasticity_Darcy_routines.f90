...
999 CALL ERRORS("ELASTICITY_DARCY_PROBLEM_SUBTYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE ELASTICITY_DARCY_PROBLEM_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    TYPE(CONTROL_LOOP_TYPE), POINTER :: CONTROL_LOOP,CONTROL_LOOP_ROOT,SOLID_SUB_LOOP,FLUID_SUB_LOOP
    ...
    INTEGER(INTG) :: NUMBER_LOAD_INCREMENTS
...
!---tob: \ToDo: This should be set in the example file, but there is no CMISSControlLoopSubLoopGet to access SOLID_SUB_LOOP
!               for which we want to set the max number of iterations
            NUMBER_LOAD_INCREMENTS = 2 !3
            write(*,*)'NUMBER_LOAD_INCREMENTS = ',NUMBER_LOAD_INCREMENTS
            CALL CONTROL_LOOP_MAXIMUM_ITERATIONS_SET(SOLID_SUB_LOOP,NUMBER_LOAD_INCREMENTS,ERR,ERROR,*999)
!---toe
        ...
        CASE(PROBLEM_SETUP_SOLVERS_TYPE)
          ...
          CASE(PROBLEM_SETUP_START_ACTION)
            ...
            CALL SOLVER_LIBRARY_TYPE_SET(SOLVER_MAT_PROPERTIES,SOLVER_PETSC_LIBRARY,ERR,ERROR,*999)
            !
            !Set the third solver to be a first-order dynamic solver for ALE Darcy
            CALL SOLVERS_SOLVER_GET(FLUID_SOLVERS,2,SOLVER,ERR,ERROR,*999)
...
999 CALL ERRORS("ELASTICITY_DARCY_PROBLEM_SETUP",ERR,ERROR)
  ...
  SUBROUTINE ELASTICITY_DARCY_PRE_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(CONTROL_LOOP)) THEN
      IF(CONTROL_LOOP%LOOP_TYPE==PROBLEM_CONTROL_LOAD_INCREMENT_LOOP_TYPE) THEN ! we have the control loop for solving for the finite elasticity part
