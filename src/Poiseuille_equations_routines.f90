...
! temporary input for vector-source
  ...
  PUBLIC POISEUILLE_EQUATION_EQUATIONS_SET_SETUP,POISEUILLE_EQUATION_EQUATIONS_SET_SOLUTION_METHOD_SET, &
    & POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET,POISEUILLE_EQUATION_FINITE_ELEMENT_CALCULATE,POISEUILLE_PRE_SOLVE, &
...
CONTAINS
  ...
  SUBROUTINE POISEUILLE_EQUATION_ANALYTIC_CALCULATE(EQUATIONS_SET,ERR,ERROR,*)
...
999 CALL ERRORS("POISEUILLE_EQUATION_EQUATIONS_SET_SOLUTION_METHOD_SET",ERR,ERROR)
  ...
  SUBROUTINE POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET(EQUATIONS_SET,EQUATIONS_SET_SUBTYPE,ERR,ERROR,*)
    ...
    CALL ENTERS("POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET",ERR,ERROR,*999)
    ...
    CALL EXITS("POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET")
    RETURN
999 CALL ERRORS("POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET",ERR,ERROR)
    CALL EXITS("POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET")
    RETURN 1
  END SUBROUTINE POISEUILLE_EQUATION_EQUATIONS_SET_SUBTYPE_SET
...
! !                     IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_CONSTANT_SOURCE_POISEUILLE_SUBTYPE) THEN
...
! !                       EQUATIONS_SET%ANALYTIC%ANALYTIC_FUNCTION_TYPE=EQUATIONS_SET_POISEUILLE_EQUATION_TWO_DIM_1
! !                     ELSE
! !                       LOCAL_ERROR="The equations set subtype of "// &
! !                         & TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET%SUBTYPE,"*",ERR,ERROR))// &
! !                         & " is invalid. The analytic function type of "// &
! !                         & TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET_SETUP%ANALYTIC_FUNCTION_TYPE,"*",ERR,ERROR))// &
! !                         & " requires that the equations set subtype be an exponential source Poiseuille equation."
! !                       CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
! !                     ENDIF
                  CASE DEFAULT
            ...
            IF(EQUATIONS_SET%DEPENDENT%DEPENDENT_FINISHED) THEN
              IF(ASSOCIATED(EQUATIONS_SET%ANALYTIC)) THEN
                IF(EQUATIONS_SET%ANALYTIC%ANALYTIC_FINISHED) THEN
                  CALL POISEUILLE_EQUATION_ANALYTIC_CALCULATE(EQUATIONS_SET,ERR,ERROR,*999)
                ELSE
...
999 CALL ERRORS("POISEUILLE_EQUATION_PROBLEM_SETUP",ERR,ERROR)
  ...
  SUBROUTINE POISEUILLE_EQUATION_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) FIELD_VAR_TYPE,ng,mh,mhs,mi,ms,nh,nhs,ni,ns !,k1sum,k2sum
...
999 CALL ERRORS("POISEUILLE_EQUATION_FINITE_ELEMENT_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE POISEUILLE_EQUATION_PROBLEM_SUBTYPE_SET(PROBLEM,PROBLEM_SUBTYPE,ERR,ERROR,*)
    ...
    CALL ENTERS("POISEUILLE_EQUATION_PROBLEM_SUBTYPE_SET",ERR,ERROR,*999)
    ...
    CALL EXITS("POISEUILLE_EQUATION_PROBLEM_SUBTYPE_SET")
    RETURN
999 CALL ERRORS("POISEUILLE_EQUATION_PROBLEM_SUBTYPE_SET",ERR,ERROR)
    CALL EXITS("POISEUILLE_EQUATION_PROBLEM_SUBTYPE_SET")
    RETURN 1
