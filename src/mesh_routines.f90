...
MODULE MESH_ROUTINES
  ...
  PUBLIC DECOMPOSITION_NUMBER_OF_DOMAINS_GET,DECOMPOSITION_NUMBER_OF_DOMAINS_SET, DECOMPOSITION_WORK_GROUP_SET
      ...
      IF(ASSOCIATED(MESH%TOPOLOGY)) THEN
        IF(ASSOCIATED(MESH%DECOMPOSITIONS)) THEN
          ...
          ELSE
            ...
            CALL COMPUTATIONAL_WORK_GROUP_GET_DEFAULT_WORK_GROUP(NEW_DECOMPOSITION%WORK_GROUP,ERR,ERROR,*999) !default work group
            IF(ASSOCIATED(NEW_DECOMPOSITION%WORK_GROUP)) THEN
            ELSE
              CALL FLAG_ERROR("FAILED TO GET THE DEFAULT WORK GROUP", ERR, ERROR, *999)
            ENDIF
...
!                & COMPUTATIONAL_ENVIRONMENT%MPI_COMM,ERR,ERROR,*999)
                & COMPUTATIONAL_WORK_GROUP_COMMUNICATOR_GET(DECOMPOSITION%WORK_GROUP,ERR,ERROR), ERR,ERROR,*999)
              ...
              IF(number_computational_nodes>1) THEN
                ...
                CALL MPI_ALLGATHERV(MPI_IN_PLACE,MAX_NUMBER_ELEMENTS_PER_NODE,MPI_INTEGER,DECOMPOSITION%ELEMENT_DOMAIN, &
                  & RECEIVE_COUNTS,DISPLACEMENTS,MPI_INTEGER, &
!     COMPUTATIONAL_ENVIRONMENT%MPI_COMM,MPI_IERROR)
      & COMPUTATIONAL_WORK_GROUP_COMMUNICATOR_GET(DECOMPOSITION%WORK_GROUP,ERR,ERROR),MPI_IERROR)
...
999 CALL ERRORS("DECOMPOSITION_NUMBER_OF_DOMAINS_GET",ERR,ERROR)
  ...
  SUBROUTINE DECOMPOSITION_WORK_GROUP_SET(DECOMPOSITION,WORK_GROUP,ERR,ERROR,*)
    !Argument Variables
    TYPE(DECOMPOSITION_TYPE), POINTER :: DECOMPOSITION !<A pointer to the decomposition to set the work group for.
    TYPE(COMPUTATIONAL_WORK_GROUP_TYPE),POINTER,INTENT(IN) :: WORK_GROUP!<A pointer to the work group to be set.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    ...
    CALL ENTERS("DECOMPOSITION_WORK_GROUP_SET",ERR,ERROR,*999)

    IF(ASSOCIATED(DECOMPOSITION)) THEN
      IF(DECOMPOSITION%DECOMPOSITION_FINISHED) THEN
        CALL FLAG_ERROR("Decomposition has been finished.",ERR,ERROR,*999)
      ELSE
        DECOMPOSITION%WORK_GROUP => WORK_GROUP
      ENDIF
    ELSE
      CALL FLAG_ERROR("Decomposition is not associated.",ERR,ERROR,*999)
    ENDIF

    CALL DECOMPOSITION_NUMBER_OF_DOMAINS_SET(DECOMPOSITION, &
     &DECOMPOSITION%WORK_GROUP%NUMBER_COMPUTATIONAL_NODES,ERR,ERROR,*999)
    
    CALL EXITS("DECOMPOSITION_WORK_GROUP_SET")
    RETURN
999 CALL ERRORS("DECOMPOSITION_WORK_GROUP_SET",ERR,ERROR)
    CALL EXITS("DECOMPOSITION_WORK_GROUP_SET")
    RETURN 1
  END SUBROUTINE DECOMPOSITION_WORK_GROUP_SET
  ...
  SUBROUTINE DOMAIN_MAPPINGS_DOFS_INITIALISE(DOMAIN_MAPPINGS,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DOMAIN_MAPPINGS)) THEN
      ...
      ELSE
        ...
        CALL DOMAIN_MAPPINGS_MAPPING_INITIALISE(DOMAIN_MAPPINGS%DOFS,DOMAIN_MAPPINGS%DOMAIN%DECOMPOSITION%WORK_GROUP,&
          & DOMAIN_MAPPINGS%DOMAIN%DECOMPOSITION%NUMBER_OF_DOMAINS,ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Domain mapping is not associated.",ERR,ERROR,*999)
    ENDIF
  ...
  SUBROUTINE DOMAIN_MAPPINGS_ELEMENTS_INITIALISE(DOMAIN_MAPPINGS,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DOMAIN_MAPPINGS)) THEN
      ...
      ELSE
        ...
        CALL DOMAIN_MAPPINGS_MAPPING_INITIALISE(DOMAIN_MAPPINGS%ELEMENTS,DOMAIN_MAPPINGS%DOMAIN%DECOMPOSITION%WORK_GROUP,&
          & DOMAIN_MAPPINGS%DOMAIN%DECOMPOSITION%NUMBER_OF_DOMAINS,ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Domain mapping is not associated.",ERR,ERROR,*999)
  ...
  SUBROUTINE DOMAIN_MAPPINGS_NODES_INITIALISE(DOMAIN_MAPPINGS,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DOMAIN_MAPPINGS)) THEN
      ...
      ELSE
        ...
        CALL DOMAIN_MAPPINGS_MAPPING_INITIALISE(DOMAIN_MAPPINGS%NODES,DOMAIN_MAPPINGS%DOMAIN%DECOMPOSITION%WORK_GROUP, &
          & DOMAIN_MAPPINGS%DOMAIN%DECOMPOSITION%NUMBER_OF_DOMAINS,ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Domain mapping is not associated.",ERR,ERROR,*999)
