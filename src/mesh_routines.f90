...
MODULE MESH_ROUTINES
  ...
  PUBLIC MESH_EMBEDDING_CREATE,MESH_EMBEDDING_SET_ELEMENT_XI
...
999 CALL ERRORS("DECOMPOSITION_NODE_DOMAIN_GET",ERR,ERROR)
  ...
  SUBROUTINE MESH_EMBEDDING_CREATE(EMBEDDED_MESH, PARENT_MESH, CHILD_MESH,ERR,ERROR,*)
    TYPE(MESH_EMBEDDING_TYPE), INTENT(INOUT) :: EMBEDDED_MESH
    TYPE(MESH_TYPE), POINTER, INTENT(IN) :: CHILD_MESH, PARENT_MESH
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string

    EMBEDDED_MESH%PARENT_MESH = PARENT_MESH
    EMBEDDED_MESH%CHILD_MESH  = CHILD_MESH
    ALLOCATE(EMBEDDED_MESH%ELEMENT_XI(1:PARENT_MESH%NUMBER_OF_ELEMENTS))
  END SUBROUTINE MESH_EMBEDDING_CREATE


  SUBROUTINE MESH_EMBEDDING_SET_ELEMENT_XI(EMBEDDED_MESH, ELEMENT_NUMBER, NODE_NUMBERS, XI_COORDS,ERR,ERROR,*)
    TYPE(MESH_EMBEDDING_TYPE), INTENT(INOUT) :: EMBEDDED_MESH
    INTEGER(INTG), INTENT(IN) :: ELEMENT_NUMBER !<Element number in the parent mesh
    !INTEGER(INTG), INTENT(IN) :: NUM_EMBEDDED   !<Number of child nodes embedded
    INTEGER(INTG), INTENT(IN) :: NODE_NUMBERS(:)!<Node numbers in child mesh
    REAL(DP), INTENT(IN)      :: XI_COORDS(:,:) !<Their Xi coordinates in the parent element

    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string


    IF(ELEMENT_NUMBER<1 .OR. ELEMENT_NUMBER > EMBEDDED_MESH%PARENT_MESH%NUMBER_OF_ELEMENTS) THEN
      CALL FLAG_ERROR("Element number out of range",ERR,ERROR,*999)
    ENDIF

    ALLOCATE(EMBEDDED_MESH%ELEMENT_XI(ELEMENT_NUMBER)%NODE_NUMBERS(SIZE(NODE_NUMBERS)))
    EMBEDDED_MESH%ELEMENT_XI(ELEMENT_NUMBER)%NODE_NUMBERS = NODE_NUMBERS

    ALLOCATE(EMBEDDED_MESH%ELEMENT_XI(ELEMENT_NUMBER)%XI_COORDS(SIZE(XI_COORDS,1),SIZE(XI_COORDS,2)))
    EMBEDDED_MESH%ELEMENT_XI(ELEMENT_NUMBER)%XI_COORDS = XI_COORDS

999 RETURN 1
  END SUBROUTINE MESH_EMBEDDING_SET_ELEMENT_XI
