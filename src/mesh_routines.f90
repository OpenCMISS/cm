...
MODULE MESH_ROUTINES
  ...
  PUBLIC MESHES_INITIALISE,MESHES_FINALISE,MESH_CREATE_START,MESH_CREATE_FINISH,MESH_DESTROY, &
    ...
    & MESH_TOPOLOGY_ELEMENTS_USER_NUMBER_SET
...
999 CALL ERRORS("MESH_TOPOLOGY_ELEMENTS_NUMBER_GET",ERR,ERROR)    
  ...
  SUBROUTINE MESH_TOPOLOGY_ELEMENTS_USER_NUMBER_SET(GLOBAL_NUMBER,USER_NUMBER,ELEMENTS,ERR,ERROR,*)
    ...
    CALL ENTERS("MESH_TOPOLOGY_ELEMENTS_USER_NUMBER_SET",ERR,ERROR,*999)
    ...
    CALL EXITS("MESH_TOPOLOGY_ELEMENTS_USER_NUMBER_SET")
    RETURN
999 CALL ERRORS("MESH_TOPOLOGY_ELEMENTS_USER_NUMBER_SET",ERR,ERROR)    
    CALL EXITS("MESH_TOPOLOGY_ELEMENTS_USER_NUMBER_SET")
    RETURN 1
   
  END SUBROUTINE MESH_TOPOLOGY_ELEMENTS_USER_NUMBER_SET
...
999 CALL ERRORS("MESH_TOPOLOGY_NODE_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE MESH_TOPOLOGY_NODES_CALCULATE(TOPOLOGY,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: INSERT_STATUS,NUMBER_OF_MESH_NODES,ne,nn,np  
            ...
            IF(ASSOCIATED(MESH%REGION)) THEN
              ...
              IF(ASSOCIATED(REGION%NODES)) THEN
                IF(ASSOCIATED(TOPOLOGY%NODES%NODES)) THEN
                  ...
                  CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*998)
                ELSE
                  CALL TREE_CREATE_START(TOPOLOGY%NODES%NODES_TREE,ERR,ERROR,*999)
                  CALL TREE_INSERT_TYPE_SET(TOPOLOGY%NODES%NODES_TREE,TREE_NO_DUPLICATES_ALLOWED,ERR,ERROR,*999)
                  CALL TREE_CREATE_FINISH(TOPOLOGY%NODES%NODES_TREE,ERR,ERROR,*999)
                  DO ne=1,ELEMENTS%NUMBER_OF_ELEMENTS
                    BASIS=>ELEMENTS%ELEMENTS(ne)%BASIS
                    DO nn=1,BASIS%NUMBER_OF_NODES
                      np=ELEMENTS%ELEMENTS(ne)%GLOBAL_ELEMENT_NODES(nn)
                      CALL TREE_ITEM_INSERT(TOPOLOGY%NODES%NODES_TREE,np,np,INSERT_STATUS,ERR,ERROR,*999)
                    ENDDO !nn
                  ENDDO !ne
                  CALL TREE_DETACH(TOPOLOGY%NODES%NODES_TREE,NUMBER_OF_MESH_NODES,MESH_NODES,ERR,ERROR,*999)
