            ...
            IF(ASSOCIATED(INTERFACE_REGION)) THEN
              ...
              IF(.NOT.ASSOCIATED(LAGRANGE_FIELD)) THEN
                ...
                CALL FIELD_DATA_TYPE_SET_AND_LOCK(INTERFACE_CONDITION%LAGRANGE%LAGRANGE_FIELD,FIELD_DELUDELN_VARIABLE_TYPE, &
                  & FIELD_DP_TYPE,ERR,ERROR,*999)
                !Default the number of component to the first variable of the interface dependent fields number of components
                ...
                DO component_idx=1,INTERFACE_CONDITION%LAGRANGE%NUMBER_OF_COMPONENTS
                  !/todo Temporary for removing hydrostatic pressure lagrange multiplyer from finite elasticity equation sets to simplify coupling implementation. Will be updated before code merges with trunk.
...
999 CALL ERRORS("INTERFACE_CONDITION_RESIDUAL_EVALUATE_FEM",ERR,ERROR)
  ...
  SUBROUTINE INTERFACE_CONDITION_FINITE_ELEMENT_CALCULATE(INTERFACE_CONDITION,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(BASIS_TYPE), POINTER :: INTERFACE_DEPENDENT_BASIS,INTERFACE_MATRIX_ELEMENT_BASIS,INTERFACE_GEOMETRIC_BASIS, &
    ...
    IF(ASSOCIATED(INTERFACE_CONDITION)) THEN
      ...
      IF(ASSOCIATED(INTERFACE_EQUATIONS)) THEN
        ...
        CASE(INTERFACE_CONDITION_LAGRANGE_MULTIPLIERS_METHOD,INTERFACE_CONDITION_PENALTY_METHOD)
          ...
          ENDSELECT
          !\todo make sure that the correct quadrature scheme is being used for each interface element. 
          !The quadrature scheme should depend on which interface mesh is being integrated (either M associated with region1 or N associated with region 2)since we could have different quadrature schemes in each region.
