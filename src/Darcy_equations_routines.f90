...
MODULE DARCY_EQUATIONS_ROUTINES
  ...
  PRIVATE DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD
...
!                     & 1,FIELD_NODE_BASED_INTERPOLATION,ERR,ERROR,*999)
        ...
        !-----------------------------------------------------------------
        !   e q u a t i o n s   t y p e   -   A l r e a d y   m o d i f i e d
        !-----------------------------------------------------------------
...
!   END SUBROUTINE DARCY_EQUATION_PROBLEM_STANDARD_SETUP
  ...
  SUBROUTINE DARCY_EQUATION_PRE_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*)
    ...
    IF( UPDATE_MATERIALS ) THEN
      CALL DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD(CONTROL_LOOP,SOLVER,ERR,ERROR,*999)
    ENDIF
...
999 CALL ERRORS("DARCY_EQUATION_PRE_SOLVE",ERR,ERROR)
  ...
  !>Within the Darcy pre-solve, update the material field.
  SUBROUTINE DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD(CONTROL_LOOP,SOLVER,ERR,ERROR,*)
...
!     REAL(DP) :: k_xx, k_yy, k_zz
    ...
    CALL ENTERS("DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD",ERR,ERROR,*999)
                ...
                DO EQUATIONS_SET_IDX = 1, SOLVER_MAPPING%NUMBER_OF_EQUATIONS_SETS
                  ...
                  IF(ASSOCIATED(EQUATIONS_SET)) THEN
                    ...
                    IF(ASSOCIATED(EQUATIONS)) THEN
                      ...
                      CASE(EQUATIONS_SET_QUASISTATIC_DARCY_SUBTYPE)
                        MATERIALS_FIELD=>EQUATIONS%INTERPOLATION%MATERIALS_FIELD
                        FIELD=>MATERIALS_FIELD
                        IF(ASSOCIATED(FIELD)) THEN
                          IF(FIELD%FIELD_FINISHED) THEN
                            VARIABLE_TYPE = FIELD_U_VARIABLE_TYPE
                            FIELD_VARIABLE=>FIELD%VARIABLE_TYPE_MAP(VARIABLE_TYPE)%PTR
                            IF(ASSOCIATED(FIELD_VARIABLE)) THEN
                              DO COMPONENT_NUMBER = 1, FIELD_VARIABLE%NUMBER_OF_COMPONENTS
                                SELECT CASE(FIELD_VARIABLE%COMPONENTS(COMPONENT_NUMBER)%INTERPOLATION_TYPE)
                                CASE(FIELD_CONSTANT_INTERPOLATION)
                                  VALUE = 2.0_DP + 1.0_DP * sin( 2.0_DP * PI * CURRENT_TIME / 4.0_DP )
                                  CALL FIELD_PARAMETER_SET_UPDATE_CONSTANT(FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,&
                                  ...
                                  IF(ASSOCIATED(DOMAIN)) THEN
                                    ...
                                    IF(ASSOCIATED(DOMAIN_TOPOLOGY)) THEN
                                      ...
                                      IF(ASSOCIATED(DOMAIN_NODES)) THEN
                                        DO LOCAL_NODE_NUMBER = 1, DOMAIN_NODES%TOTAL_NUMBER_OF_NODES
                                          DERIVATIVE_NUMBER = 1
                                          VALUE = 2.0_DP + 1.0_DP * sin( 2.0_DP * PI * CURRENT_TIME / 4.0_DP )
                                          !This is isotropic and homogeneous
                                          !Retrieve reference values from initial material field ONCE,
                                          !store them and subsequently apply the changes based on structure, time, etc. ???
                                          !-> Setup pre-control loop (this runs, in principle, only once)
                                          !-> copy materials_set into materials_initialise:
                                          !->   Use functionality of materials_set to set reference values
                                          CALL FIELD_PARAMETER_SET_UPDATE_LOCAL_NODE(FIELD,FIELD_U_VARIABLE_TYPE,&
                                            & FIELD_VALUES_SET_TYPE,DERIVATIVE_NUMBER,LOCAL_NODE_NUMBER,COMPONENT_NUMBER,VALUE,&
                                            & ERR,ERROR,*999)
                                          !for Hermite elements:
                                          DO DERIVATIVE_NUMBER = 2, DOMAIN_NODES%NODES(LOCAL_NODE_NUMBER)%NUMBER_OF_DERIVATIVES
                                            CALL FIELD_PARAMETER_SET_UPDATE_LOCAL_NODE(FIELD,FIELD_U_VARIABLE_TYPE,&
                                              & FIELD_VALUES_SET_TYPE,DERIVATIVE_NUMBER,LOCAL_NODE_NUMBER,COMPONENT_NUMBER,VALUE,&
                                              & ERR,ERROR,*999)
                                          ENDDO
                        ...
                        ENDIF
                      CASE DEFAULT
                        LOCAL_ERROR="Equations set subtype "//TRIM(NUMBER_TO_VSTRING(CONTROL_LOOP%PROBLEM%SUBTYPE,"*",ERR,ERROR))//&
...
! !       & DOF_CONDITION,DOF_VALUES,ERR,ERROR,*999)
    ...
    CALL EXITS("DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD")
    RETURN
999 CALL ERRORS("DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD",ERR,ERROR)
    CALL EXITS("DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD")
    RETURN 1
  END SUBROUTINE DARCY_EQUATION_PRE_SOLVE_UPDATE_MATERIALS_FIELD
      
  !   
  !================================================================================================================================
  !
 
  !>Within the Darcy pre-solve, update the boundary conditions.
...
!                               DO COMPONENT_NUMBER = 1, FIELD_VARIABLE%NUMBER_OF_COMPONENTS
                                SELECT CASE(FIELD_VARIABLE%COMPONENTS(COMPONENT_NUMBER)%INTERPOLATION_TYPE)
                                CASE(FIELD_CONSTANT_INTERPOLATION)
                                  !What shall we set here for now ??? ???
                                  LOCAL_ERROR="Can not add node for component number "// &
                                    & TRIM(NUMBER_TO_VSTRING(COMPONENT_NUMBER,"*",ERR,ERROR))//" of variable type "// &
                                    & TRIM(NUMBER_TO_VSTRING(VARIABLE_TYPE,"*",ERR,ERROR))//" of field number "// &
                                ...
                                CASE(FIELD_NODE_BASED_INTERPOLATION)                  
                                  ...
                                  IF(ASSOCIATED(DOMAIN)) THEN
                                    ...
                                    IF(ASSOCIATED(DOMAIN_TOPOLOGY)) THEN
                                      ...
                                      IF(ASSOCIATED(DOMAIN_NODES)) THEN
                                        DO LOCAL_NODE_NUMBER = 1, DOMAIN_NODES%TOTAL_NUMBER_OF_NODES
                                          !How to identify the boundary nodes ???
                                          DERIVATIVE_NUMBER = 1
                                          VALUE = 2.0_DP + 1.0_DP * sin( 2.0_DP * PI * CURRENT_TIME / 4.0_DP )
                                          CALL FIELD_PARAMETER_SET_UPDATE_LOCAL_NODE(FIELD,FIELD_U_VARIABLE_TYPE,&
                                            & FIELD_VALUES_SET_TYPE,DERIVATIVE_NUMBER,LOCAL_NODE_NUMBER,COMPONENT_NUMBER,VALUE,&
