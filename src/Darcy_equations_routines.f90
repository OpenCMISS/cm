...
!!TODO: move these and scale factor adjustment out once generalised Darcy is put in.
          ...
          DO ng=1,QUADRATURE_SCHEME%NUMBER_OF_GAUSS
            ...
            IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_ALE_DARCY_SUBTYPE .OR. &
              & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_INCOMPRESSIBLE_FINITE_ELASTICITY_DARCY_SUBTYPE) THEN
...
!---toe
            CASE(PROBLEM_ALE_DARCY_SUBTYPE,PROBLEM_PGM_DARCY_SUBTYPE,PROBLEM_STANDARD_ELASTICITY_DARCY_SUBTYPE, &
              ...
              IF(CONTROL_LOOP%LOOP_TYPE==PROBLEM_CONTROL_SIMPLE_TYPE.AND.SOLVER%GLOBAL_NUMBER==SOLVER_NUMBER_MAT_PROPERTIES) THEN
                !--- Store reference data
                IF(CONTROL_LOOP%PARENT_LOOP%TIME_LOOP%ITERATION_NUMBER==1) THEN
...
!                 CALL SOLVERS_SOLVER_GET(SOLVER%SOLVERS,SOLVER_NUMBER_DARCY,SOLVER_ALE_DARCY,ERR,ERROR,*999)
              ...
              ELSE IF(CONTROL_LOOP%LOOP_TYPE==PROBLEM_CONTROL_SIMPLE_TYPE.AND.SOLVER%GLOBAL_NUMBER==SOLVER_NUMBER_DARCY) THEN
      ...
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
          SELECT CASE(CONTROL_LOOP%PROBLEM%SUBTYPE)
            ...
            CASE(PROBLEM_ALE_DARCY_SUBTYPE,PROBLEM_PGM_DARCY_SUBTYPE,PROBLEM_STANDARD_ELASTICITY_DARCY_SUBTYPE, &
              & PROBLEM_PGM_ELASTICITY_DARCY_SUBTYPE)
              IF(CONTROL_LOOP%LOOP_TYPE==PROBLEM_CONTROL_SIMPLE_TYPE.AND.SOLVER%GLOBAL_NUMBER==SOLVER_NUMBER_DARCY) THEN
...
!     INTEGER(INTG) :: NUMBER_OF_COMPONENTS_DEPENDENT_FIELD_FINITE_ELASTICITY,NUMBER_OF_COMPONENTS_GEOMETRIC_FIELD_DARCY
    INTEGER(INTG) :: NUMBER_OF_DIMENSIONS,TOTAL_NUMBER_OF_DOFS,NDOFS_TO_PRINT,dof_number!,I
            ...
            CASE(PROBLEM_STANDARD_ELASTICITY_DARCY_SUBTYPE)
              ...
              IF(SOLVER%GLOBAL_NUMBER==SOLVER_NUMBER_DARCY) THEN  !It is called with 'SOLVER%GLOBAL_NUMBER=SOLVER_NUMBER_DARCY', otherwise it doesn't work
                ...
                IF(CONTROL_LOOP%CONTROL_LOOP_LEVEL==2) THEN
                  ! If this is a sub-loop of a time loop, then need to get the solvers for the load incremented loop
                  ! used by the solid elasticity solver
                  CALL SOLVERS_SOLVER_GET(CONTROL_LOOP%PARENT_LOOP%SUB_LOOPS(1)%PTR%SOLVERS,SOLVER_NUMBER_SOLID, &
                      & SOLVER_FINITE_ELASTICITY,ERR,ERROR,*999)
                ELSE
                  CALL SOLVERS_SOLVER_GET(SOLVER%SOLVERS,SOLVER_NUMBER_SOLID,SOLVER_FINITE_ELASTICITY,ERR,ERROR,*999)
                ENDIF
