...
999 CALL ERRORS("FIELD_IO_CREATE_DECOMPISTION",ERR,ERROR)
  ...
  SUBROUTINE FIELD_IO_FIELDS_IMPORT(NAME, METHOD, WORK_GROUP, REGION, MESH, MESH_USER_NUMBER, DECOMPOSITION, DECOMPOSITION_USER_NUMBER, &
  ...
  SUBROUTINE FIELD_IO_IMPORT_GLOBAL_MESH(NAME, WORK_GROUP,REGION, MESH, MESH_USER_NUMBER, MASTER_COMPUTATIONAL_NUMBER, &
    ...
    DO WHILE(idx_exelem<NUMBER_OF_EXELEM_FILES)
       ...
       CALL MPI_BCAST(LIST_COMP_NODAL_INDEX,number_of_node*NUMBER_OF_COMPONENTS,MPI_INTEGER,MASTER_COMPUTATIONAL_NUMBER, &
         & WORK_GROUP%COMP_ENVIRONMENT%MPI_COMM,MPI_IERROR)
       CALL MPI_ERROR_CHECK("MPI_BCAST",MPI_IERROR,ERR,ERROR,*999)
       CALL MPI_BCAST(SHAPE_INDEX,SHAPE_SIZE,MPI_INTEGER,MASTER_COMPUTATIONAL_NUMBER,WORK_GROUP%COMP_ENVIRONMENT%MPI_COMM,MPI_IERROR)
       CALL MPI_ERROR_CHECK("MPI_BCAST",MPI_IERROR,ERR,ERROR,*999)
...
999 CALL ERRORS("FIELD_IO_INFO_SET_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE FIELD_IO_NODES_EXPORT(FIELDS, FILE_NAME, METHOD, ERR,ERROR,*)
    ...
    !Get the number of computational nodes
!    computational_node_numbers=COMPUTATIONAL_NODES_NUMBER_GET(ERR,ERROR)
    computational_node_numbers=COMPUTATIONAL_NODES_NUMBER_GET(FIELDS%FIELDS%PTR%DECOMPOSITION%WORK_GROUP,ERR,ERROR)

    IF(ERR/=0) GOTO 999
    !Get my computational node number
!    my_computational_node_number=COMPUTATIONAL_NODE_NUMBER_GET(ERR,ERROR)
    my_computational_node_number=COMPUTATIONAL_NODE_NUMBER_GET(FIELDS%FIELDS%PTR%DECOMPOSITION%WORK_GROUP,ERR,ERROR)
    IF(ERR/=0) GOTO 999
    IF(METHOD=="FORTRAN") THEN
