...
!    MODULE PROCEDURE COMPUTATIONAL_WORK_GROUP_WAIT_WORKGROUP
  ...
  PRIVATE COMPUTATIONAL_LOCAL_WORK_GROUP !for further modification, keep it private
  PUBLIC COMPUTATIONAL_ENVIRONMENT, COMPUTATIONAL_WORLD_WORK_GROUP
...
999 CALL ERRORS("COMPUTATIONAL_WORK_GROUP_SUBGROUP_ADD",ERR,ERROR)
  ...
  RECURSIVE SUBROUTINE COMPUTATIONAL_WORK_GROUP_GENERATE_COMP_ENVIRONMENT(WORK_GROUP,AVAILABLE_RANK_LIST,ERR,ERROR,*)
    ...
    WORK_GROUP%COMP_ENV%MPI_COMM=MPI_COMM_NULL
    ...
    IF(MPI_GROUP_NULL==ORIGINAL_GROUP) THEN
      WRITE (*,*) MY_COMPUTATIONAL_NODE_NUMBER_IN_WORLD,",warning: CAN NOT CREATE ORIGINAL_GROUP"
    ENDIF
    ...
    I=WORK_GROUP%COMP_ENV%NUMBER_COMPUTATIONAL_NODES
    CALL MPI_GROUP_INCL(ORIGINAL_GROUP,I,AVAILABLE_RANK_LIST(1:I),NEW_GROUP,MPI_IERROR)    
    CALL MPI_BARRIER(MPI_COMM_WORLD, MPI_IERROR)
    IF(MPI_GROUP_NULL==NEW_GROUP) THEN
      WRITE (*,*) MY_COMPUTATIONAL_NODE_NUMBER_IN_WORLD,",warning: CAN NOT CREATE NEW_GROUP"
    ENDIF
    CALL MPI_ERROR_CHECK("MPI_COMM_RANK",MPI_IERROR,ERR,ERROR,*999)    
    CALL MPI_COMM_CREATE(MPI_COMM_WORLD,NEW_GROUP,WORK_GROUP%COMP_ENV%MPI_COMM,MPI_IERROR)
    CALL MPI_BARRIER(MPI_COMM_WORLD, MPI_IERROR)

    IF(MPI_COMM_NULL==WORK_GROUP%COMP_ENV%MPI_COMM) THEN
      WRITE (*,*) MY_COMPUTATIONAL_NODE_NUMBER_IN_WORLD, &
        & ",warning: CAN NOT CREATE COMMUNICATOR because this rank doesn't belong to this communicator group"
    ELSE
      !test/debug
      BCAST_VALUE=MY_COMPUTATIONAL_NODE_NUMBER_IN_WORLD
      print *, 'rank ',MY_COMPUTATIONAL_NODE_NUMBER_IN_WORLD,'bvalue before bcast:', BCAST_VALUE
      CALL MPI_BCAST(BCAST_VALUE,1,MPI_INTEGER,0,WORK_GROUP%COMP_ENV%MPI_COMM,MPI_IERROR) !BCAST IT 
      print *, 'rank ',MY_COMPUTATIONAL_NODE_NUMBER_IN_WORLD,'bvalue after bcast:', BCAST_VALUE  

      !Determine my processes rank WITHIN THIS COMMUNICATION GROUP
      CALL MPI_COMM_RANK(WORK_GROUP%COMP_ENV%MPI_COMM,RANK,MPI_IERROR) 
      CALL MPI_ERROR_CHECK("MPI_COMM_RANK",MPI_IERROR,ERR,ERROR,*999)
      WORK_GROUP%COMP_ENV%MY_COMPUTATIONAL_NODE_NUMBER=RANK

  !---------------------------------------------------------------    
      !fill the information for every computational node in this group
      ALLOCATE(WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(0:WORK_GROUP%COMP_ENV%NUMBER_COMPUTATIONAL_NODES-1))

      CALL COMPUTATIONAL_NODE_MPI_TYPE_INITIALISE(WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(RANK),ERR,ERROR,*999)
      CALL COMPUTATIONAL_NODE_INITIALISE(WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(0),RANK,ERR,ERROR,*999)
      !Now transfer all the computational node information to the other computational nodes 
      CALL MPI_ALLGATHER(WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(0),1,MPI_COMPUTATIONAL_NODE_TYPE_DATA%MPI_TYPE, &
        & WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(0),1,MPI_COMPUTATIONAL_NODE_TYPE_DATA%MPI_TYPE, &
        & WORK_GROUP%COMP_ENV%MPI_COMM,MPI_IERROR)
  ...
  !---------------------------------------------------------------
      !COPY THE WORKL ONE (WRONG)
!      DO I=1,WORK_GROUP%COMP_ENV%NUMBER_COMPUTATIONAL_NODES, 1
!        WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(I-1) = COMPUTATIONAL_ENVIRONMENT%COMPUTATIONAL_NODES(AVAILABLE_RANK_LIST(I))
!        IF(WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(I-1)%RANK == MY_COMPUTATIONAL_NODE_NUMBER) THEN
!          COMPUTATIONAL_LOCAL_WORK_GROUP => WORK_GROUP !BOOKKEEP THIS LOCAL WORK GROUP
!        ENDIF
!      END DO
  !---------------------------------------------------------------
    ...
    NEW_AVAILABLE_RANK_LIST(1:SIZE(NEW_AVAILABLE_RANK_LIST)) = AVAILABLE_RANK_LIST(1+WORK_GROUP%COMP_ENV &
      &  %NUMBER_COMPUTATIONAL_NODES:SIZE(NEW_AVAILABLE_RANK_LIST,1)+WORK_GROUP%COMP_ENV%NUMBER_COMPUTATIONAL_NODES)
...
999 CALL ERRORS("COMPUTATIONAL_WORK_GROUP_GENERATE_COMP_ENVIRONMENT",ERR,ERROR)
  ...
  SUBROUTINE COMPUTATIONAL_WORK_GROUP_CREATE_FINISH(WORLD_WORK_GROUP,ERR,ERROR,*)
    ...
    IF (WORLD_WORK_GROUP%NUMBER_SUB_WORK_GROUPS>0) THEN
      IF(ASSOCIATED(COMPUTATIONAL_LOCAL_WORK_GROUP)) THEN
         WRITE(*,*) 'rank ',COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER, ' belong to one group'
	 !temporary solution,set COMPUTATIONAL_ENVIRONMENT to be local!
	 COMPUTATIONAL_ENVIRONMENT = COMPUTATIONAL_LOCAL_WORK_GROUP%COMP_ENV
	 PETSC_COMM_WORLD=COMPUTATIONAL_LOCAL_WORK_GROUP%COMP_ENV%MPI_COMM !(re)initialise PETSc
      ELSE
         WRITE(*,*) 'rank ',COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER, & 
	  & ' does not belong to any local group, is that a bug?'
...
999 CALL ERRORS("COMPUTATIONAL_NODES_NUMBER_GET_WORLD_WORKGROUP",ERR,ERROR)
  ...
  FUNCTION COMPUTATIONAL_NODE_NUMBER_GET_WORKGROUP(WORK_GROUP, ERR,ERROR)
    ...
    IF(ASSOCIATED(WORK_GROUP)) THEN 
      IF(WORK_GROUP%COMP_ENV_FINISHED) THEN
        COMPUTATIONAL_NODE_NUMBER_GET_WORKGROUP = WORK_GROUP%COMP_ENV%MY_COMPUTATIONAL_NODE_NUMBER
      ELSE
        CALL FLAG_ERROR('computational environment of this work group has not finished', ERR, ERROR, *999)
      ENDIF
    ELSE
...
999 CALL ERRORS("COMPUTATIONAL_NODE_NUMBER_GET_WORKGROUP",ERR,ERROR)
  ...
  FUNCTION COMPUTATIONAL_NODES_NUMBER_GET_WORKGROUP(WORK_GROUP, ERR,ERROR)
    ...
    IF(ASSOCIATED(WORK_GROUP)) THEN 
      IF(WORK_GROUP%COMP_ENV_FINISHED) THEN
        COMPUTATIONAL_NODES_NUMBER_GET_WORKGROUP = WORK_GROUP%COMP_ENV%NUMBER_COMPUTATIONAL_NODES
        !double check
        IF(WORK_GROUP%COMP_ENV%NUMBER_COMPUTATIONAL_NODES /= WORK_GROUP%NUMBER_COMPUTATIONAL_NODES) THEN
