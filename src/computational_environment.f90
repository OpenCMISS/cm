...
999 CALL ERRORS("COMPUTATIONAL_WORK_GROUP_COMMUNICATOR_GET",ERR,ERROR)
  ...
  SUBROUTINE COMPUTATIONAL_WORK_GROUP_GET_LOCAL_WORK_GROUP(LOCAL_WORK_GROUP,ERR,ERROR,*)
    ...
    ELSE
      CALL FLAG_ERROR('THIS PROCESS/RANK DOES NOT BEGLONG TO ANY LOCAL WORK GROUP', ERR, ERROR, *999)
    ENDIF
...
999 CALL ERRORS("COMPUTATIONAL_WORK_GROUP_SUBGROUP_ADD",ERR,ERROR)
  ...
  RECURSIVE SUBROUTINE COMPUTATIONAL_WORK_GROUP_GENERATE_COMP_ENVIRONMENT(WORK_GROUP,AVAILABLE_RANK_LIST,ERR,ERROR,*)
    ...
    !LOCAL VARIABLES
    INTEGER(INTG) :: I,DUMMY_ERR,MPI_IERROR,RANK, ORIGINAL_GROUP, NEW_GROUP
    TYPE(VARYING_STRING) :: DUMMY_ERROR
    INTEGER(INTG), ALLOCATABLE :: NEW_AVAILABLE_RANK_LIST(:)
    INTEGER(INTG) :: MY_COMPUTATIONAL_NODE_NUMBER !<the computational node number/rank of the current process.
    ...
    DO I=1,WORK_GROUP%COMP_ENV%NUMBER_COMPUTATIONAL_NODES, 1
      ...
      IF(WORK_GROUP%COMP_ENV%COMPUTATIONAL_NODES(I)%RANK == MY_COMPUTATIONAL_NODE_NUMBER) THEN
	COMPUTATIONAL_LOCAL_WORK_GROUP => WORK_GROUP !BOOKKEEP THIS LOCAL WORK GROUP, ONE RANK ONLY BELONGS TO ONE LOCAL GROUP CURRNETLY
      ENDIF
    END DO
