          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
              ...
              DO component_idx=NUMBER_OF_DIMENSIONS+1,NUMBER_OF_MATERIALS_COMPONENTS
                CALL FIELD_COMPONENT_MESH_COMPONENT_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_STANDARD_VARIABLE_TYPE, &
                  & component_idx,EQUATIONS_SET%GEOMETRY%GEOMETRIC_FIELD%VARIABLES(FIELD_STANDARD_VARIABLE_TYPE)% &
                  & COMPONENTS(1)%MESH_COMPONENT_NUMBER,ERR,ERROR,*999) 
                CALL FIELD_COMPONENT_INTERPOLATION_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_STANDARD_VARIABLE_TYPE, &
                  & component_idx,FIELD_CONSTANT_INTERPOLATION,ERR,ERROR,*999)
...
999 CALL ERRORS("POISSON_EQUATION_EQUATIONS_SET_NONLINEAR_SOURCE_SETUP",ERR,ERROR)
  ...
  SUBROUTINE POISSON_EQUATION_PROBLEM_SETUP(PROBLEM,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    !Argument variables
    TYPE(PROBLEM_TYPE), POINTER :: PROBLEM !<A pointer to the solutions set to setup a Poisson equation on.
    INTEGER(INTG), INTENT(IN) :: SETUP_TYPE !<The setup type
    INTEGER(INTG), INTENT(IN) :: ACTION_TYPE !<The action type
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
                        ...
                        DO nj=1,GEOMETRIC_VARIABLE%NUMBER_OF_COMPONENTS
                          ...
                          DO ni=1,GEOMETRIC_BASIS%NUMBER_OF_XI
                            ...
                            PGNJ(nj)=PGNJ(nj)+ &
                              ...
                              & EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_POINT_METRICS%DXI_DX(ni,nj)
                          ENDDO !ni
                          SUM=SUM+EQUATIONS%INTERPOLATION%MATERIALS_INTERP_POINT%VALUES(nj,NO_PART_DERIV)* &
                            & PGMJ(nj)*PGNJ(nj)
                        ENDDO !nj
                        EQUATIONS_MATRIX%ELEMENT_MATRIX%MATRIX(mhs,nhs)=EQUATIONS_MATRIX%ELEMENT_MATRIX%MATRIX(mhs,nhs)+SUM*RWG
                      ENDDO !ns
                ...
                DO mh=1,DEPENDENT_VARIABLE%NUMBER_OF_COMPONENTS
                  ...
                  DO ms=1,DEPENDENT_BASIS%NUMBER_OF_ELEMENT_PARAMETERS
                    ...
                    RHS_VECTOR%ELEMENT_VECTOR%VECTOR(mhs)=RHS_VECTOR%ELEMENT_VECTOR%VECTOR(mhs)+ &
                      & EQUATIONS%INTERPOLATION%MATERIALS_INTERP_POINT%VALUES(1,NO_PART_DERIV)* &
              ...
              DO mh=1,DEPENDENT_VARIABLE%NUMBER_OF_COMPONENTS
                ...
                DO ms=1,DEPENDENT_BASIS%NUMBER_OF_ELEMENT_PARAMETERS
                  ...
                  NONLINEAR_MATRICES%ELEMENT_RESIDUAL%VECTOR(mhs)=NONLINEAR_MATRICES%ELEMENT_RESIDUAL%VECTOR(mhs)- &
                    & EQUATIONS%INTERPOLATION%MATERIALS_INTERP_POINT%VALUES(3,NO_PART_DERIV)* &
                    & QUADRATURE_SCHEME%GAUSS_BASIS_FNS(ms,NO_PART_DERIV,ng)* &
                    & EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT%VALUES(1,NO_PART_DERIV)**2
                ENDDO !ms
              ENDDO !mh
            ENDIF
          ENDDO !ng
...
999 CALL ERRORS("POISSON_EQUATION_PROBLEM_SUBTYPE_SET",ERR,ERROR)
  ...
  !>Sets up the linear source Poisson equations solution.
...
999 CALL ERRORS("POISSON_EQUATION_PROBLEM_LINEAR_SOURCE_SETUP",ERR,ERROR)
  ...
  !>Sets up the nonlinear source Poisson equations solution.
           ...
           CASE(PROBLEM_SETUP_DO_ACTION)
             ...
             IF(ASSOCIATED(EQUATIONS_SET)) THEN
               ...
               IF(EQUATIONS_SET%CLASS==EQUATIONS_SET_CLASSICAL_FIELD_CLASS.AND. &
                 ...
                 & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_QUADRATIC_SOURCE_POISSON_SUBTYPE) THEN
                 SOLUTION_MAPPING=>SOLUTION%SOLUTION_MAPPING            
                 IF(ASSOCIATED(SOLUTION_MAPPING)) THEN
                   CALL SOLUTION_MAPPING_EQUATIONS_SET_ADD(SOLUTION_MAPPING,SOLUTION%EQUATIONS_SET_TO_ADD,SOLUTION% &
                     & EQUATIONS_SET_ADDED_INDEX,ERR,ERROR,*999)
                 ...
                 ENDIF
               ELSE
                 CALL FLAG_ERROR("The equations set to add is not a linear source Poisson equations set.",ERR,ERROR,*999)
               ENDIF
             ELSE
               CALL FLAG_ERROR("Equations set to add is not associated.",ERR,ERROR,*999)
             ENDIF
