...
!          & COMPUTATIONAL_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
          & DOMAIN_MAPPING%WORK_GROUP%COMP_ENVIRONMENT%MY_COMPUTATIONAL_NODE_NUMBER) THEN
...
999 CALL ERRORS("DOMAIN_MAPPINGS_GLOBAL_TO_LOCAL_GET",ERR,ERROR)
  ...
  SUBROUTINE DOMAIN_MAPPINGS_LOCAL_FROM_GLOBAL_CALCULATE(DOMAIN_MAPPING,ERR,ERROR,*)
   ...
   !Argument variables
    ...
    IF(ASSOCIATED(DOMAIN_MAPPING)) THEN
      my_computational_node_number=COMPUTATIONAL_NODE_NUMBER_GET(DOMAIN_MAPPING%WORK_GROUP,ERR,ERROR)
      IF(ERR/=0) GOTO 999        
...
999 CALL ERRORS("DOMAIN_MAPPINGS_MAPPING_GLOBAL_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE DOMAIN_MAPPINGS_MAPPING_INITIALISE(DOMAIN_MAPPING,WORK_GROUP,NUMBER_OF_DOMAINS,ERR,ERROR,*)
!  SUBROUTINE DOMAIN_MAPPINGS_MAPPING_INITIALISE(DOMAIN_MAPPING,NUMBER_OF_DOMAINS,ERR,ERROR,*)
    ...
    IF(.NOT. ASSOCIATED(WORK_GROUP)) CALL FLAG_ERROR('WORK GROUP IS NOT ASSOCIATED',ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(DOMAIN_MAPPING)) THEN
      IF(NUMBER_OF_DOMAINS>0) THEN
        ...
        DOMAIN_MAPPING%WORK_GROUP=>WORK_GROUP !set the correct work group!
      ELSE
