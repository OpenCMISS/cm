          ...
          IF(ASSOCIATED(SOLVER)) THEN
            ...
            IF(ASSOCIATED(SOLUTION_MAPPING)) THEN
              ...
              DO matrix_idx=1,SOLVER_MATRICES%NUMBER_OF_MATRICES
                ...
                SOLVER_MATRICES%MATRICES(matrix_idx)%UPDATE_MATRIX=.TRUE.
                SOLVER_MATRICES%MATRICES(matrix_idx)%STORAGE_TYPE=SOLVER_MATRICES%CREATE_VALUES_CACHE% &
                  & MATRIX_STORAGE_TYPE(matrix_idx)
                SOLVER_MATRICES%MATRICES(matrix_idx)%NUMBER_OF_COLUMNS=SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP( &
                  & matrix_idx)%NUMBER_OF_COLUMNS
                !Allocate the distributed solver matrix
                CALL DISTRIBUTED_MATRIX_CREATE_START(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(matrix_idx)% &
                  & COLUMN_DOFS_MAPPING,SOLVER_MATRICES%MATRICES(matrix_idx)%MATRIX,ERR,ERROR,*999)
                ...
                CALL DISTRIBUTED_VECTOR_CREATE_START(SOLUTION_MAPPING%SOLVER_COL_TO_EQUATIONS_SETS_MAP(matrix_idx)% &
                  & COLUMN_DOFS_MAPPING,SOLVER_MATRICES%MATRICES(matrix_idx)%SOLVER_VECTOR,ERR,ERROR,*999)
