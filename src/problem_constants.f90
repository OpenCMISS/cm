...
MODULE PROBLEM_CONSTANTS
  ...
  INTEGER(INTG), PARAMETER :: PROBLEM_GALERKIN_PROJECTION_TYPE=9
  !Bioelectrics class
...
!!TODO: We don't really have two problem types here? Maybe a different type with nonlinear boundary conditions???
  ...
  INTEGER(INTG), PARAMETER :: PROBLEM_VECTOR_SOURCE_POISSON_SUBTYPE=3
  !  Helmholtz equation
  ...
  INTEGER(INTG), PARAMETER :: PROBLEM_NO_SOURCE_STATIC_ADVEC_DIFF_SUBTYPE=4
  INTEGER(INTG), PARAMETER :: PROBLEM_LINEAR_SOURCE_STATIC_ADVEC_DIFF_SUBTYPE=5
  INTEGER(INTG), PARAMETER :: PROBLEM_NONLINEAR_SOURCE_STATIC_ADVEC_DIFF_SUBTYPE=6
  !Bioelectric class
  !  Monodomain equation
  !  Bidomain equation
  !Modal class
  !  Galerkin projection
  INTEGER(INTG), PARAMETER :: PROBLEM_STANDARD_GALERKIN_PROJECTION_SUBTYPE=1
  INTEGER(INTG), PARAMETER :: PROBLEM_GENERALISED_GALERKIN_PROJECTION_SUBTYPE=2
  INTEGER(INTG), PARAMETER :: PROBLEM_MAT_PROPERTIES_GALERKIN_PROJECTION_SUBTYPE=3
  ...
  INTEGER(INTG), PARAMETER :: SOLVER_EQUATIONS_TIME_STEPPING=5 !<Solver equations are for time-stepping solvers \see PROBLEM_CONSTANTS_EquationTimeDependenceTypes,PROBLEM_CONSTANTS
  !>@}
