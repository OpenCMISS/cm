    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      CASE(EQUATIONS_SET_STATIC_NAVIER_STOKES_SUBTYPE, &
        ...
        CASE(EQUATIONS_SET_SETUP_INITIAL_TYPE)
          ...
          !         & "1D-0D coupling parameters",ERR,ERROR,*999)
          ...
          !     ELSE
          !       LOCAL_ERROR="User-specified fields are not yet implemented for an equations set field &
        ...
        CASE(EQUATIONS_SET_SETUP_DEPENDENT_TYPE)
          ...
          CASE(EQUATIONS_SET_BIFURCATION_NAVIER_STOKES_SUBTYPE)
            ...
            CASE(EQUATIONS_SET_SETUP_START_ACTION)
              ...
              CALL FIELD_GEOMETRIC_FIELD_SET_AND_LOCK(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD,EQUATIONS_SET%GEOMETRY% &
                & GEOMETRIC_FIELD,ERR,ERROR,*999)
              !set number of variables to 2 (1 for U and 1 for DELUDELN)
              DEPENDENT_FIELD_NUMBER_OF_VARIABLES=2
              CALL FIELD_NUMBER_OF_VARIABLES_SET_AND_LOCK(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD, &
                & DEPENDENT_FIELD_NUMBER_OF_VARIABLES,ERR,ERROR,*999)
              CALL FIELD_VARIABLE_TYPES_SET_AND_LOCK(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD,(/FIELD_U_VARIABLE_TYPE, &
...
!                    & FIELD_NODE_BASED_INTERPOLATION,ERR,ERROR,*999)
        ...
        CASE(EQUATIONS_SET_SETUP_MATERIALS_TYPE)
          ...
          CASE(EQUATIONS_SET_Coupled1D0D_NAVIER_STOKES_SUBTYPE)
            ...
            materialsFieldNavierStokesNumberOfComponents=12
                ...
                IF(EQUATIONS_MATERIALS%MATERIALS_FIELD_AUTO_CREATED) THEN
                  ...
                  DO I=1,8 !(MU,RHO,K,Bs,As,Re,Fr,St)
                    CALL FIELD_COMPONENT_INTERPOLATION_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                      & I,FIELD_CONSTANT_INTERPOLATION,ERR,ERROR,*999)
                  ENDDO
                  DO componentIdx=9,materialsFieldNavierStokesNumberOfComponents
                    CALL FIELD_COMPONENT_INTERPOLATION_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
...
999 CALL ERRORS("NAVIER_STOKES_PROBLEM_SETUP",ERR,ERROR)
  ...
  SUBROUTINE NAVIER_STOKES_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: i,en,bif_idx,dof_idx,nv1,nv3,node1,node3,mn,vn,global_element_idx,MESH_COMPONENT_NUMBER
    ...
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    INTEGER(INTG) :: xv,out,boundaryIdx
...
!    REAL(DP) :: NL_VECTOR(256) ! "N"on "L"inear vector - maximum size allocated
    ...
    REAL(DP) :: Beta(9),W(3)
    REAL(DP) :: U_DERIV(3,3),A_DERIV,Q_DERIV(3,3),Bs,As,St,Fr,Re,K,Betta,A0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_1DTRANSIENT_NAVIER_STOKES_SUBTYPE .OR. &
             & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_Coupled1D0D_NAVIER_STOKES_SUBTYPE) THEN
              Q_VALUE(1)=EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT(FIELD_U_VARIABLE_TYPE)%PTR%VALUES(1,NO_PART_DERIV)
            ...
            IF(UPDATE_NONLINEAR_RESIDUAL) THEN
              ...
              DO nodeIdx=1,numberOfElementNodes
                ...
                IF(numberOfVersions>1) THEN
                  ...
                  CALL FIELD_PARAMETER_SET_DATA_RESTORE(dependentField,FIELD_U_VARIABLE_TYPE,FIELD_PREVIOUS_VALUES_SET_TYPE, &
                    & dependentParameters,err,error,*999)

                  !Momentum Equation
                  momentum=((K*((Q_PRE(1)**2)/A_PRE(1))+ & 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        CASE(EQUATIONS_SET_OPTIMISED_NAVIER_STOKES_SUBTYPE)
          ...
          DO i=1,TOTAL_NUMBER_OF_NODES
            NUMBER_OF_VERSIONS=DEPENDENT_FIELD%DECOMPOSITION%DOMAIN(1)%PTR%TOPOLOGY%NODES%NODES(i)%DERIVATIVES(1)% &
              & NUMBER_OF_VERSIONS
...
!                  & en==DEPENDENT_FIELD%DECOMPOSITION%DOMAIN(1)%PTR%TOPOLOGY%NODES%NODES(i)%SURROUNDING_ELEMENTS(1)) THEN
          ...
          IF(boundaryNode) THEN
            ...
            Q_BIF(2)=BIF_VALUES(dof_idx+1)
            Q_BIF(3)=0.0_DP
            A_BIF(1)=BIF_VALUES(TOTAL_NUMBER_OF_DOFS+dof_idx)
            A_BIF(2)=BIF_VALUES(TOTAL_NUMBER_OF_DOFS+dof_idx+1)
            A_BIF(3)=1.0_DP
          ...
          !Bifurcation
            ...
            Q_BIF(1)=BIF_VALUES(dof_idx)
            Q_BIF(2)=BIF_VALUES(dof_idx+1)
            Q_BIF(3)=BIF_VALUES(dof_idx+2)
            A_BIF(1)=BIF_VALUES(TOTAL_NUMBER_OF_DOFS+dof_idx)
            A_BIF(2)=BIF_VALUES(TOTAL_NUMBER_OF_DOFS+dof_idx+1)
            A_BIF(3)=BIF_VALUES(TOTAL_NUMBER_OF_DOFS+dof_idx+2)
            CALL FIELD_PARAMETER_SET_DATA_RESTORE(DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,BIF_VALUES, &
              & ERR,ERROR,*999)
...
999 CALL ERRORS("NAVIER_STOKES_FINITE_ELEMENT_RESIDUAL_EVALUATE",ERR,ERROR)
  ...
  SUBROUTINE NAVIER_STOKES_FINITE_ELEMENT_JACOBIAN_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: x,en,i,bif_idx,dof_idx,nv1,nv3,node1,node3,mn,vn,boundaryIdx
    INTEGER(INTG) :: TOTAL_NUMBER_OF_ELEMENTS,TOTAL_NUMBER_OF_NODES,NUMBER_OF_VERSIONS,TOTAL_NUMBER_OF_DOFS
    ...
    LOGICAL :: UPDATE_JACOBIAN_MATRIX,boundaryNode
    LOGICAL :: GHOST_ELEMENT,USER_ELEMENT_EXISTS
...
!    REAL(DP) :: J_MATRIX(256,256)
    ...
    REAL(DP) :: U_VALUE(3),W_VALUE(3),Q_VALUE(3),A_VALUE,Q_BIF(3),A_BIF(3)
    REAL(DP) :: Beta(9),W(3)
    REAL(DP) :: U_DERIV(3,3),A_DERIV,Q_DERIV(3,3),Bs,As,St,Fr,Re,K,Betta,A0
...
!                  & en==DEPENDENT_FIELD%DECOMPOSITION%DOMAIN(1)%PTR%TOPOLOGY%NODES%NODES(i)%SURROUNDING_ELEMENTS(1)) THEN
            ...
            IF(boundaryNode) THEN
              ...
              !Current Materials Parameters at the Coupled Nodes
              CALL FIELD_PARAMETER_SET_DATA_GET(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,BIF_VALUES, &
                & ERR,ERROR,*999)
              A0_PARAM(1)=BIF_VALUES(8+en)
              A0_PARAM(2)=BIF_VALUES(8+en+1)
              ...
              !Current Q and A Values at the coupled Nodes
              CALL FIELD_PARAMETER_SET_DATA_GET(DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,BIF_VALUES, &
                & ERR,ERROR,*999)
              Q_BIF(1)=BIF_VALUES(dof_idx)
              Q_BIF(2)=BIF_VALUES(dof_idx+1)
...
999 CALL ERRORS("NAVIER_STOKES_POST_SOLVE",ERR,ERROR)
  ...
  SUBROUTINE NAVIER_STOKES_PRE_SOLVE_UPDATE_BOUNDARY_CONDITIONS(CONTROL_LOOP,SOLVER,ERR,ERROR,*)
    ...
    REAL(DP) :: X(3),MU_PARAM,RHO_PARAM,A0_PARAM(9),FLOW(3),W(2,3),Q_EX(9),A_EX(9),XI(1),areaCalculated,flowCalculated,w0d
    REAL(DP) :: Bs
    REAL(DP) :: Q_PRE(9),A_PRE(9),Beta(9),Period,As,St,Fr,Re
    ...
    REAL(DP), POINTER :: dependentParameters(:)
    REAL(DP), POINTER :: materialsParameters(:)
    REAL(DP) :: normalWave(2,3),lEigenvalue,Bn,Kr,deltaT
...
!    INTEGER(INTG) :: component_idx,node_idx,NUMBER_OF_DIMENSIONS,variable_type
    INTEGER(INTG) :: element_idx,en_idx,I,J,K,number_of_nodes_xic(3),en,TOTAL_NUMBER_OF_ELEMENTS
    INTEGER(INTG) :: bif_idx,TOTAL_NUMBER_OF_NODES,NUMBER_OF_VERSIONS,TOTAL_NUMBER_OF_DOFS
    INTEGER(INTG) :: FIELD_VAR_TYPE
    LOGICAL :: inletNode,outletNode,fixedNode,nonReflecting,solverCharacteristicFlag
                        ...
                        IF (ABS(normalWave(1,1))>0 .OR. ABS(normalWave(2,1)) >0) THEN
                                ...
                                IF(elementNodeNumber==nodeIdx) THEN
                                  ...
                                  DO versionIdx=1,numberOfVersions
                                    ...
                                    IF(elementNodeVersion==versionIdx) THEN
                                      ...
                                      materialIdx=9
                                      CALL FIELD_PARAMETER_SET_GET_ELEMENT(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                                       & FIELD_VALUES_SET_TYPE,elementNumber,materialIdx,A0_PARAM(versionIdx),err,error,*999)                
                                      materialIdx=10
                                      CALL FIELD_PARAMETER_SET_GET_ELEMENT(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                                       & FIELD_VALUES_SET_TYPE,elementNumber,materialIdx,Beta(versionIdx),err,error,*999)                
                                    ENDIF
                                  ENDDO
                                ENDIF
                              ENDDO
                            ...
                            DO versionIdx=1,numberOfVersions
                              IF(ABS(normalWave(componentIdx,versionIdx))>ZERO_TOLERANCE) THEN
                                W(componentIdx,versionIdx)=((Q_EX(versionIdx)/A_EX(versionIdx)) + &
                                 & normalWave(componentIdx,versionIdx)* &
                                 & 4.0_DP*SQRT((Fr*(Beta(versionIdx)/Bs)))*(A_EX(versionIdx)**(0.25_DP)))
                              ENDIF
                            ENDDO
                          ENDDO
                          ...
                          ELSE IF(outletNode) THEN
                            ...
                            areaCalculated = (((W(1,1)-W(2,1))/4.0_DP)**4.0_DP)* &
                             & ((Bs/(4.0_DP*Fr*Beta(versionIdx)))**2.0_DP)
                          ...
                          IF(outletNode .AND. normalWave(2,1)<ZERO_TOLERANCE .AND. nonReflecting) THEN
                            ...
                            lEigenvalue = normalWave(2,1)*(A_EX(versionIdx)**(0.25_DP)* &
                             & SQRT(Fr*Beta(versionIdx)/(Bs)))/A_EX(versionIdx)
                    ...
                    IF(OUTPUT_ITERATION_NUMBER/=0) THEN
                      IF(CONTROL_LOOP%TIME_LOOP%CURRENT_TIME<=CONTROL_LOOP%TIME_LOOP%STOP_TIME) THEN
                        ...
                        IF(EXPORT_FIELD) THEN          
                          IF(MOD(CURRENT_LOOP_ITERATION,OUTPUT_ITERATION_NUMBER)==0)  THEN   
                            CALL WRITE_STRING(GENERAL_OUTPUT_TYPE,"...",ERR,ERROR,*999)
                            CALL WRITE_STRING(GENERAL_OUTPUT_TYPE,"Now export nodes... ",ERR,ERROR,*999)
!                            CALL FLUID_MECHANICS_IO_WRITE_CMGUI(EQUATIONS_SET%REGION,EQUATIONS_SET%GLOBAL_NUMBER,FILE, &
!                              & ERR,ERROR,*999)
...
999 CALL ERRORS("NavierStokes_CalculateBoundaryFlux",err,error)
  ...
  SUBROUTINE NavierStokes_Couple1D0D(controlLoop,solver,err,error,*)
    ...
    REAL(DP) :: deltaT,Bn,Kr,lEigenvalue,beta,pTest,qTest
        ...
        DO nodeIdx=1,numberOfNodes1D
          ...
          IF(ABS(normalWave(1))>ZERO_TOLERANCE .AND. numberOfSurroundingElements==1) THEN
            ...
            CALL FIELD_PARAMETER_SET_GET_ELEMENT(materialsField,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
             & elementNumber,componentIdx,A0_PARAM,err,error,*999)                
            ! Beta
            componentIdx=10
            CALL FIELD_PARAMETER_SET_GET_ELEMENT(materialsField,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
             & elementNumber,componentIdx,beta,err,error,*999)                
            ...
            ELSE IF (iterationIdx < maxIterations) THEN
              ...
              IF (updateArea) THEN
                ...
                aBoundary  = ( (pCellML - pExternal)/beta + SQRT(A0_PARAM))**2
                versionIdx=1
                componentIdx=2
              ...
              IF(inletNode .OR. outletNode) THEN
                W(oneDComponent) = qBoundary/aBoundary+normalWave(oneDComponent)*4.0_DP*SQRT(Fr*beta/Bs)*(aboundary**0.25_DP)
                W0 = qBoundary/aBoundary+normalWave(returnComponent)*4.0_DP*SQRT(Fr*beta/Bs)*(aboundary**0.25_DP)
                W(returnComponent) = W0 + qBoundary/aBoundary + normalWave(returnComponent)*(16.0_DP*Fr/Bs)* &
