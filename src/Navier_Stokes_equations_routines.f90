              ...
              IF (ASSOCIATED(EQUATIONS_MATERIALS)) THEN
                IF (EQUATIONS_MATERIALS%MATERIALS_FIELD_AUTO_CREATED) THEN
                  ...
                  CALL FIELD_COMPONENT_MESH_COMPONENT_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE, &
                    & 1,GEOMETRIC_COMPONENT_NUMBER,ERR,ERROR,*999)
                  DO I=1,MATERIAL_FIELD_NUMBER_OF_COMPONENTS1 !(MU,RHO,alpha,Pext,LengthScale,TimeScale,MassScale,G0,Fr)
                    CALL FIELD_COMPONENT_INTERPOLATION_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                      & I,FIELD_CONSTANT_INTERPOLATION,ERR,ERROR,*999)
                  ENDDO  
                  DO I=1,MATERIAL_FIELD_NUMBER_OF_COMPONENTS2 !(A0,E,H,kp,k1,k2,k3,b1)
...
!
  ...
  SUBROUTINE NAVIER_STOKES_PROBLEM_SUBTYPE_SET(PROBLEM,PROBLEM_SUBTYPE,ERR,ERROR,*)
    ...
    IF (ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_Stree1D0DAdv_NAVIER_STOKES_SUBTYPE)
        PROBLEM%CLASS=PROBLEM_FLUID_MECHANICS_CLASS
        PROBLEM%TYPE=PROBLEM_NAVIER_STOKES_EQUATION_TYPE
        PROBLEM%SUBTYPE=PROBLEM_Stree1D0DAdv_NAVIER_STOKES_SUBTYPE
      CASE(PROBLEM_TRANSIENT_SUPG_NAVIER_STOKES_SUBTYPE)
        PROBLEM%CLASS=PROBLEM_FLUID_MECHANICS_CLASS
        PROBLEM%TYPE=PROBLEM_NAVIER_STOKES_EQUATION_TYPE
    ...
    IF (ASSOCIATED(PROBLEM)) THEN
      SELECT CASE(PROBLEM%SUBTYPE)
        ...
        CASE(PROBLEM_1dTransient_NAVIER_STOKES_SUBTYPE, &    !1D Navier-Stokes
           ...
           & PROBLEM_Stree1D0DAdv_NAVIER_STOKES_SUBTYPE)     !  with stree and advection
          ...
          CASE(PROBLEM_SETUP_CONTROL_TYPE)
            ...
            CASE(PROBLEM_SETUP_START_ACTION)
              ...
              ELSE IF (PROBLEM%SUBTYPE == PROBLEM_Stree1D0D_NAVIER_STOKES_SUBTYPE) THEN
                ...
                CALL CONTROL_LOOP_LABEL_SET(iterativeWhileLoop2,"1D Characteristic/NSE branch value convergence Loop", &
                 & ERR,ERROR,*999)
              ELSE IF (PROBLEM%SUBTYPE == PROBLEM_Stree1D0DAdv_NAVIER_STOKES_SUBTYPE) THEN
                NULLIFY(iterativeWhileLoop)
                ! The 1D-0D boundary value iterative coupling loop
                CALL CONTROL_LOOP_NUMBER_OF_SUB_LOOPS_SET(CONTROL_LOOP,2,ERR,ERROR,*999)
                CALL CONTROL_LOOP_SUB_LOOP_GET(CONTROL_LOOP,1,iterativeWhileLoop,ERR,ERROR,*999)                
    ...
    IF (ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF (ASSOCIATED(EQUATIONS)) THEN
        ...
        CASE(EQUATIONS_SET_STATIC_NAVIER_STOKES_SUBTYPE, &
          ...
          !Flux upwinding and pressure calculation
          IF (EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_1dTransient_NAVIER_STOKES_SUBTYPE    .OR. &
            & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_Coupled1D0D_NAVIER_STOKES_SUBTYPE    .OR. &
            & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_1dTransientAdv_NAVIER_STOKES_SUBTYPE .OR. &
            & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_Coupled1D0DAdv_NAVIER_STOKES_SUBTYPE) THEN
                        ...
                        DO ns=1,DEPENDENT_BASIS2%NUMBER_OF_ELEMENT_PARAMETERS
                          ...
                          IF ( EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_STATIC_SUPG_NAVIER_STOKES_SUBTYPE .OR. &
                           ...
                           IF (UPDATE_JACOBIAN_MATRIX) THEN
                             ...
                             ENDDO !ni 
                             DO ni=1,DEPENDENT_BASIS1%NUMBER_OF_XI
                                SUM=SUM+(PHINS*U_DERIV(mh,ni)*DXI_DX(ni,nh)*(PHIMS+W_SUPG)*RHO)
                             ENDDO 
                              !Calculate MATRIX  
                              ...
                              IF (nh==mh) THEN 
                                ...
                                DO x=1,DEPENDENT_BASIS1%NUMBER_OF_XI
                                  DO mi=1,DEPENDENT_BASIS2%NUMBER_OF_XI
                                    SUM=SUM+RHO*(U_VALUE(x))*DPHINS_DXI(mi)*DXI_DX(mi,x)*(PHIMS+W_SUPG)
                                  ENDDO !mi
                                ENDDO !x
                                !Calculate MATRIX
                                JACOBIAN_MATRIX%ELEMENT_JACOBIAN%MATRIX(mhs,nhs)=JACOBIAN_MATRIX%ELEMENT_JACOBIAN%MATRIX(mhs,nhs) &
...
!\todo: Introduce user-defined or default values instead for density and viscosity
     ...
     CASE(EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_POISEUILLE)
       ...
       IF (NUMBER_OF_DIMENSIONS==2 .AND. NUMBER_OF_COMPONENTS==3) THEN
         MU=MATERIALS_PARAMETERS(1)
         RHO=MATERIALS_PARAMETERS(2)
         SELECT CASE(VARIABLE_TYPE)
         CASE(FIELD_U_VARIABLE_TYPE)
           L_PARAM=ANALYTIC_PARAMETERS(1) ! channel length in x-direction
           H=ANALYTIC_PARAMETERS(2) ! channel height in y-direction
           ...
           CASE(NO_GLOBAL_DERIV)
             IF (componentNumber==1) THEN
               !calculate u
               VALUE=(3.0_DP/2.0_DP)*U_PARAM*(1.0_DP-((X(2)-H)**2)/(H**2))
             ELSE IF (componentNumber==2) THEN
               !calculate v
               VALUE=0.0_DP
             ELSE IF (componentNumber==3) THEN
               !calculate p
               VALUE=(3.0_DP*MU*U_PARAM*(X(1)-L_PARAM))/(H**2)+P_PARAM
             ELSE
               CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
             ENDIF
     ...
     CASE(EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_TAYLOR_GREEN)
       ...
       IF (NUMBER_OF_DIMENSIONS==2 .AND. NUMBER_OF_COMPONENTS==3) THEN
         MU=MATERIALS_PARAMETERS(1)
         RHO=MATERIALS_PARAMETERS(2)
         NU_PARAM=MU/RHO ! kinematic viscosity
         SELECT CASE(VARIABLE_TYPE)
         CASE(FIELD_U_VARIABLE_TYPE)
           ...
           CASE(NO_GLOBAL_DERIV)
             ...
             ELSE IF (componentNumber==3) THEN
               !calculate p
               VALUE=-1.0_DP*(U_PARAM**2)*(RHO/4.0_DP)*(COS(2.0_DP*K_PARAM*X(1))+ &
             ...
             CASE(FIELD_U_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==3) THEN
                     !calculate p
                     VALUE=2.0_DP/3.0_DP*X(1)*(3.0_DP*MU*10.0_DP**2-RHO*X(1)**2*X(2))/(10.0_DP ** 4)
                   ELSE
                     CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
                   ENDIF
                 CASE(GLOBAL_DERIV_S1)
             ...
             CASE(FIELD_U_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==3) THEN
                     !calculate p
                     VALUE=2.0_DP*MU/10.0_DP*EXP((X(1)-X(2))/10.0_DP)
                   ELSE
                     CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
                   ENDIF
                 CASE(GLOBAL_DERIV_S1)
             ...
             CASE(FIELD_U_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==3) THEN
                     !calculate p
                     VALUE=4.0_DP*MU*PI/10.0_DP*SIN(2.0_DP*PI*X(2)/10.0_DP)*COS(2.0_DP*PI*X(1)/10.0_DP)+ &
                       & 0.5_DP*RHO*COS(2.0_DP*PI*X(1)/10.0_DP)*COS(2.0_DP*PI*X(1)/10.0_DP)
                   ELSE
                     CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
                   ENDIF
                 CASE(GLOBAL_DERIV_S1)
             ...
             CASE(FIELD_DELUDELN_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==2) THEN
                     !calculate v
                     VALUE=16.0_DP*MU*PI**2/10.0_DP**2*cos(2.0_DP*PI*X(2)/ 10.0_DP)*cos(2.0_DP*PI*X(1)/10.0_DP)
                   ELSE IF (componentNumber==3) THEN
                     !calculate p
                     VALUE=0.0_DP
                   ELSE
             ...
             CASE(FIELD_U_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   IF (componentNumber==1) THEN
                     !calculate u
                     VALUE=SIN(X(1)/10.0_DP*2.0_DP*PI)*COS(X(2)/10.0_DP*2.0_DP*PI)*EXP(-2.0_DP*MU/RHO*CURRENT_TIME)
                     VALUE=SIN(X(1)/10.0_DP*PI)*COS(X(2)/10.0_DP*PI)*EXP(-2.0_DP*MU/RHO*CURRENT_TIME)
!                      VALUE=SIN(X(1))*COS(X(2))
                   ELSE IF (componentNumber==2) THEN
                     !calculate v
                     VALUE=-COS(X(1)/10.0_DP*2.0_DP*PI)*SIN(X(2)/10.0_DP*2.0_DP*PI)*EXP(-2.0_DP*MU/RHO*CURRENT_TIME)
                     VALUE=-COS(X(1)/10.0_DP*PI)*SIN(X(2)/10.0_DP*PI)*EXP(-2.0_DP*MU/RHO*CURRENT_TIME)
!                      VALUE=-COS(X(1))*SIN(X(2))
                   ELSE IF (componentNumber==3) THEN
                     !calculate p
                     VALUE=RHO/4.0_DP*(COS(2.0_DP*X(1)/10.0_DP*2.0_DP*PI)+COS(2.0_DP*X(2)/10.0_DP*2.0_DP*PI))* &
                       & EXP(-4.0_DP*MU/RHO*CURRENT_TIME)                      
                     VALUE=RHO/4.0_DP*(COS(2.0_DP*X(1)/10.0_DP*PI)+COS(2.0_DP*X(2)/10.0_DP*PI))* &
                       & EXP(-4.0_DP*MU/RHO*CURRENT_TIME)                      
!                      VALUE=RHO/4.0_DP*(COS(2.0_DP*X(1))+COS(2.0_DP*X(2)))
                   ELSE
                     CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
                   ENDIF
                 CASE(GLOBAL_DERIV_S1)
               ...
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==4) THEN
                     !calculate p
                     VALUE=2.0_DP/3.0_DP*X(1)*(6.0_DP*MU*10.0_DP**2-RHO*X(2)*X(1)**2-3.0_DP* & 
                       & RHO*X(2)* &
                       & X(3)**2-RHO*X(3)*X(1)**2-3.0_DP*RHO*X(3)*X(2)**2)/(10.0_DP**4)
                   ELSE
                     CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
                   ENDIF
                 CASE(GLOBAL_DERIV_S1)
             ...
             CASE(FIELD_U_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==4) THEN
                     !calculate p
                     VALUE=1.0_DP/10.0_DP*(2.0_DP*MU*EXP((X(1)-X(2))/10.0_DP)- & 
                       & 2.0_DP*MU*EXP((X(3)-X(1))/10.0_DP)+RHO*10.0_DP*EXP((X(1)-X(3))/10.0_DP)+ & 
                       & RHO*10.0_DP*EXP((X(2)-X(1))/10.0_DP))
                   ELSE
                     CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
                   ENDIF
                 CASE(GLOBAL_DERIV_S1)
             ...
             CASE(FIELD_DELUDELN_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==2) THEN
                     !calculate v
                     VALUE=-2.0_DP*MU*(2.0_DP*EXP(X(1)-X(2))+EXP(X(2)-X(3)))
                   ELSE IF (componentNumber==3) THEN
                     !calculate w
                     VALUE=-2.0_DP*MU*(2.0_DP*EXP(X(3)-X(1))+EXP(X(2)-X(3)))
                   ELSE IF (componentNumber==4) THEN
                     !calculate p
                     VALUE=0.0_DP
                   ELSE
             ...
             CASE(FIELD_U_VARIABLE_TYPE)
               SELECT CASE(GLOBAL_DERIV_INDEX)
                 CASE(NO_GLOBAL_DERIV)
                   ...
                   ELSE IF (componentNumber==4) THEN
                     !calculate p
                     VALUE=-COS(2.0_DP*PI*X(1)/10.0_DP)*(-12.0_DP*MU*PI*SIN(2.0_DP*PI*X(2)/10.0_DP)* & 
                       & SIN(2.0_DP*PI*X(3)/10.0_DP)-RHO*COS(2.0_DP*PI*X(1)/10.0_DP)*10.0_DP+ &
                       & 2.0_DP*RHO*COS(2.0_DP*PI*X(1)/10.0_DP)*10.0_DP*COS(2.0_DP*PI*X(3)/10.0_DP)**2- &
                       & RHO*COS(2.0_DP*PI*X(1)/10.0_DP)*10.0_DP*COS(2.0_DP*PI*X(2)/10.0_DP)**2)/10.0_DP/2.0_DP
                   ELSE
                     CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
                   ENDIF
                 CASE(GLOBAL_DERIV_S1)
              ...
              CASE(FIELD_DELUDELN_VARIABLE_TYPE)
                SELECT CASE(GLOBAL_DERIV_INDEX)
                  CASE(NO_GLOBAL_DERIV)
                    ...
                    ELSE IF (componentNumber==2) THEN
                      !calculate v
                      VALUE=36*MU*PI**2/10.0_DP**2*cos(2.0_DP*PI*X(2)/10.0_DP)*sin(2.0_DP*PI*X(3)/10.0_DP)* & 
                        & cos(2.0_DP*PI*X(1)/10.0_DP)
                    ELSE IF (componentNumber==3) THEN
                      !calculate w
                      VALUE=0.0_DP
      ...
      IF (decomposition%CALCULATE_FACES) THEN
        ...
        DO faceIdx=1,dependentBasis%NUMBER_OF_LOCAL_FACES
          ...
          DO gaussIdx=1,faceQuadratureScheme%NUMBER_OF_GAUSS
            ...
            ! See: M. E. Moghadam, Y. Bazilevs, T.-Y. Hsia, I. E. Vignon-Clementel, and A. L. Marsden, “A comparison of outlet 
            ! boundary treatments for prevention of backflow divergence with relevance to blood flow simulations,” 
            ! Comput Mech, vol. 48, no. 3, pp. 277–291, Sep. 2011.
  ...
  SUBROUTINE NavierStokes_CalculateBoundaryFlux(solver,timeIncrement,err,error,*)
    ...
    CASE(EQUATIONS_SET_TRANSIENT_SUPG_NAVIER_STOKES_MULTIDOMAIN_SUBTYPE, &
      ...
      DO elementIdx=elementsMapping%INTERNAL_START,elementsMapping%INTERNAL_FINISH
        ...
        !ENDIF !computational node check
      ENDDO !elementIdx                 
...
999 CALL ERRORS("NavierStokes_CalculateBoundaryFlux",err,error)
  ...
  SUBROUTINE NavierStokes_Couple1D0D(controlLoop,solver,err,error,*)
    ...
    REAL(DP) :: qPrevious,pPrevious,aPrevious,q1d,a1d,qError,aError,couplingTolerance,pCellML,normalWave(2)
