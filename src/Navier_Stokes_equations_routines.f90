...
999 ERRORSEXITS("NAVIER_STOKES_PROBLEM_SETUP",ERR,ERROR)
  ...
  SUBROUTINE NavierStokes_FiniteElementResidualEvaluate(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: nodeIdx,derivativeIdx,versionIdx,componentIdx,xiIdx,coordIdx
    ...
    REAL(DP) :: U_VALUE(3),W_VALUE(3),U_DERIV(3,3),Q_VALUE,A_VALUE,Q_DERIV,A_DERIV,pressure,normalWave,normal,Lref,Tref,Mref,Pext
    REAL(DP) :: TAU_SUPG,W_SUPG,U_SUPG(3),MU,RHO,Fr,A0,E,H,A0_DERIV,E_DERIV,H_DERIV,alpha,beta,G0,x1,y1,z1,x2,y2,z2,slope
            ...
            IF(UPDATE_NONLINEAR_RESIDUAL) THEN
              ...
              DO nodeIdx=1,numberOfElementNodes
                ...
                CALL Field_ParameterSetGetLocalNode(DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,2,A_VALUE,err,error,*999)
                ...
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,1,A0,err,error,*999)
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,2,E,err,error,*999)
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,3,H,err,error,*999)
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,4,kp,err,error,*999)
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,5,k1,err,error,*999)
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,6,k2,err,error,*999)
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,7,k3,err,error,*999)
                CALL Field_ParameterSetGetLocalNode(MATERIALS_FIELD,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeNumber,8,b1,err,error,*999)
                beta=kp*(A0**k1)*(E**k2)*(H**k3) !(kg/m/s2) --> (Pa)

                !!!-- P R E S S U R E    C A L C U L A T I O N --!!!
                !Pressure equation in mmHg
                pressure=(Pext+beta*((A_VALUE/A0)**b1-1.0_DP))/(Mref/(Lref*Tref**2))*0.0075_DP
                ...
                IF (numberOfVersions>1) THEN
                  ...
                  CALL Field_ParameterSetGetLocalNode(DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                    & versionIdx,derivativeIdx,nodeNumber,1,Q_VALUE,err,error,*999)         
                  CALL Field_ParameterSetGetLocalNode(DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                    & versionIdx,derivativeIdx,nodeNumber,2,A_VALUE,err,error,*999)
                  ...
                  momentum=alpha*(Qupwind**2/Aupwind-Q_VALUE**2/A_VALUE)+beta*A0/RHO* &
                    & b1/(b1+1.0_DP)*((Aupwind/A0)**(b1+1.0_DP)-(A_VALUE/A0)**(b1+1.0_DP))*normal
...
999 ERRORSEXITS("NavierStokes_FiniteElementResidualEvaluate",ERR,ERROR)
  ...
  SUBROUTINE NavierStokes_FiniteElementJacobianEvaluate(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: nodeIdx,derivativeIdx,versionIdx,firstNode,lastNode,nodeNumber
    ...
    REAL(DP) :: JGW,SUM,DXI_DX(3,3),DPHIMS_DXI(3),DPHINS_DXI(3),PHIMS,PHINS,W_SUPG,TAU_SUPG,U_SUPG(3),G0
    REAL(DP) :: U_VALUE(3),W_VALUE(3),U_DERIV(3,3),Q_VALUE,Q_DERIV,A_VALUE,A_DERIV,alpha,beta,normal,normalWave
    REAL(DP) :: MU,RHO,Fr,A0,A0_DERIV,E,E_DERIV,H,H_DERIV,mass,momentum1,momentum2,kp,k1,k2,k3,b1
          ...
          IF(EQUATIONS_SET%specification(3)==EQUATIONS_SET_TRANSIENT1D_NAVIER_STOKES_SUBTYPE .OR. &
            ...
            DO nodeIdx=1,numberOfElementNodes
              ...
              IF (numberOfVersions>1) THEN                  
                ...
                momentum1=(-2.0_DP*alpha*Q_VALUE/A_VALUE)*normal

                !Momentum Equation, d/dA
                momentum2=(alpha*(Q_VALUE/A_VALUE)**2-beta*b1/RHO*(A_VALUE/A0)**b1)*normal
...
999 ERRORSEXITS("NAVIER_STOKES_POST_SOLVE",ERR,ERROR)
  ...
  SUBROUTINE NavierStokes_PreSolveUpdateBoundaryConditions(SOLVER,ERR,ERROR,*)
    ...
    REAL(DP) :: T_COORDINATES(20,3),MU,RHO,X(3),FluidGFValue,SolidDFValue,NewLaplaceBoundaryValue,Lref,Tref,Mref
...
999 ERRORS("NavierStokes_BoundaryConditionsAnalyticCalculate",err,error)
  ...
  SUBROUTINE NAVIER_STOKES_ANALYTIC_FUNCTIONS_EVALUATE(ANALYTIC_FUNCTION_TYPE,X,TIME,VARIABLE_TYPE,GLOBAL_DERIV_INDEX, &
    ...
    REAL(DP) :: L_PARAM,H,U_PARAM,P_PARAM,MU,NU_PARAM,RHO,K_PARAM,Qo,tt,tmax
...
999 ERRORSEXITS("NavierStokes_Couple1D0D",err,error)
  ...
  SUBROUTINE NavierStokes_CoupleCharacteristics(controlLoop,solver,err,error,*)
    ...
    TYPE(VARYING_STRING) :: localError
    INTEGER(INTG) :: nodeNumber,nodeIdx,derivativeIdx,i,componentIdx,timestep
    ...
    REAL(DP) :: couplingTolerance,l2ErrorW(30),wPrevious(2,4),wNavierStokes(2,4),wCharacteristic(2,4)
    REAL(DP) :: l2ErrorQ(100),qCharacteristic(4),qNavierStokes(4),wNext(2,4),wError(2,4)
    REAL(DP) :: totalErrorWPrevious,startTime,stopTime,currentTime,timeIncrement,kp,k1,k2,k3,b1
    REAL(DP) :: l2ErrorA(100),aCharacteristic(4),aNavierStokes(4),totalErrorW,totalErrorQ,totalErrorA
    REAL(DP) :: totalErrorMass,totalErrorMomentum,RHO,normalWave,A0,E,H,beta,aNew,penaltyCoeff
    ...
    CALL FIELD_PARAMETER_SET_GET_CONSTANT(equationsSet%EQUATIONS_SET_FIELD%EQUATIONS_SET_FIELD_FIELD, &
      & FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,1,penaltyCoeff,err,error,*999)
    ...
    DO nodeIdx=1,domainNodes%NUMBER_OF_NODES
      ...
      IF (ABS(normalWave)>ZERO_TOLERANCE) THEN
        ...
        DO i=1,numberOfVersions
          DO componentIdx=1,2
            CALL Field_ParameterSetGetLocalNode(independentField,FIELD_U_VARIABLE_TYPE, &
              & FIELD_VALUES_SET_TYPE,i,derivativeIdx,nodeNumber,componentIdx,normalWave,err,error,*999)
            ...
            IF (ABS(normalWave)>ZERO_TOLERANCE) THEN
              ...
              CALL Field_ParameterSetGetLocalNode(dependentField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                & i,derivativeIdx,nodeNumber,componentIdx,wPrevious(componentIdx,i),err,error,*999)         
              ...
              CALL Field_ParameterSetGetLocalNode(dependentField,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                & i,derivativeIdx,nodeNumber,1,qNavierStokes(i),err,error,*999)         
              CALL Field_ParameterSetGetLocalNode(dependentField,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                & i,derivativeIdx,nodeNumber,2,aNavierStokes(i),err,error,*999)

              !Calculate the characteristic based on the values converged upon by the N-S solver at this iteration.
              wNavierStokes(componentIdx,i)=                   &
                & qNavierStokes(i)/aNavierStokes(i)+           &
                & normalWave*SQRT(4.0_DP*beta/RHO/b1)*         &        
                & ((aNavierStokes(i)/A0)**(b1/2.0_DP)-1.0_DP)

              IF (boundaryNode) THEN
                aNew=(2.0_DP*RHO/beta)**2.0_DP*((wNavierStokes(componentIdx,i))/8.0_DP+ &
                  & SQRT(beta/(2.0_DP*RHO))*(A0**0.25_DP))**4.0_DP
                CALL FIELD_PARAMETER_SET_UPDATE_LOCAL_NODE(dependentField,FIELD_U_VARIABLE_TYPE, &
                  & FIELD_PREVIOUS_VALUES_SET_TYPE,i,derivativeIdx,nodeNumber,2,aNew,err,error,*999)
              ENDIF
              ...
              CALL Field_ParameterSetGetLocalNode(dependentField,FIELD_U_VARIABLE_TYPE,FIELD_UPWIND_VALUES_SET_TYPE, &
                & i,derivativeIdx,nodeNumber,1,qCharacteristic(i),err,error,*999)         
              CALL Field_ParameterSetGetLocalNode(dependentField,FIELD_U_VARIABLE_TYPE,FIELD_UPWIND_VALUES_SET_TYPE, &
                & i,derivativeIdx,nodeNumber,2,aCharacteristic(i),err,error,*999)

              !Calculate the characteristic based on the upwind values
              wCharacteristic(componentIdx,i)=                   &
                & qCharacteristic(i)/aCharacteristic(i)+         &
                & normalWave*SQRT(4.0_DP*beta/RHO/b1)*           &        
            ...
            DO componentIdx=1,2
              ...
              IF (ABS(normalWave)>ZERO_TOLERANCE) THEN
                ...
                CALL FIELD_PARAMETER_SET_UPDATE_LOCAL_NODE(dependentField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & i,derivativeIdx,nodeNumber,componentIdx,wNext(componentIdx,i),err,error,*999)
              ENDIF
            ENDDO
          ENDDO
        ENDIF
...
999 ERRORSEXITS("NavierStokes_ControlLoopPostLoop",err,error)
  ...
  SUBROUTINE NavierStokes_UpdateMultiscaleBoundary(solver,err,error,*)
    ...
    REAL(DP) :: pCellml,qCellml,ABoundary,W1,W2,ACellML,normalWave(2,4),kp,k1,k2,k3,b1
    ...
    LOGICAL :: V_UP,V_DOWN,V_KIDNEY,V_FEMOR,V_LEG,V_ARM,V_HEAD
    ...
    !      IF (numberOfVersions==1 .AND. L2NORM(normalWave(:,1))>ZERO_TOLERANCE) THEN
          IF (numberOfVersions==1) THEN
            ...
            CALL Field_ParameterSetGetLocalNode(materialsField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,versionIdx, &
              & derivativeIdx,nodeIdx,3,H,err,error,*999) 
            CALL Field_ParameterSetGetLocalNode(materialsField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,versionIdx, &
              & derivativeIdx,nodeIdx,4,kp,err,error,*999)
            CALL Field_ParameterSetGetLocalNode(materialsField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,versionIdx, &
              & derivativeIdx,nodeIdx,5,k1,err,error,*999)
            CALL Field_ParameterSetGetLocalNode(materialsField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,versionIdx, &
              & derivativeIdx,nodeIdx,6,k2,err,error,*999)
            CALL Field_ParameterSetGetLocalNode(materialsField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,versionIdx, &
              & derivativeIdx,nodeIdx,7,k3,err,error,*999)
            CALL Field_ParameterSetGetLocalNode(materialsField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,versionIdx, &
              & derivativeIdx,nodeIdx,8,b1,err,error,*999)
            beta=kp*(A0**k1)*(E**k2)*(H**k3) !(kg/m/s2) --> (Pa)
            ...
            NULLIFY(fieldVariable)
            CALL FIELD_VARIABLE_GET(dependentField,FIELD_U_VARIABLE_TYPE,fieldVariable,err,error,*999)
            dependentDof=fieldVariable%COMPONENTS(2)%PARAM_TO_DOF_MAP%NODE_PARAM2DOF_MAP%NODES(nodeIdx)% &
            ...
            CASE(BOUNDARY_CONDITION_FIXED_NONREFLECTING)
              ...
              ABoundary=((2.0_DP*RHO/beta)**2.0_DP)*(((W1-W2)/8.0_DP+SQRT(beta/(2.0_DP*RHO))*(A0**0.25_DP))**4.0_DP)
              CALL FIELD_PARAMETER_SET_UPDATE_LOCAL_NODE(dependentField,FIELD_U_VARIABLE_TYPE, &
            ...
            CASE(BOUNDARY_CONDITION_FIXED_CELLML)
              ...
              !Convert pCellML --> A0D 
              ACellML=((pCellml-Pext)/beta+SQRT(A0))**2.0_DP
              !  O u t l e t
              IF (normalWave(1,1)>0.0_DP) THEN
                CALL Field_ParameterSetGetLocalNode(dependentField,FIELD_V_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
                  & versionIdx,derivativeIdx,nodeIdx,1,W1,err,error,*999)                    
            ...
            ! ------------------------------------------------------------
            ! V e n o u s   S y s t e m   B o u n d a r y
            ! ------------------------------------------------------------
