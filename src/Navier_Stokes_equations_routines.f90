...
! Get geometric values using field_parameter_set_data_get instead (TODO: add interpolation (as was below) for M1-V2-P1 type elements)
...
!                      ANALYTIC_FUNCTION_TYPE=>EQUATIONS_ANALYTIC%ANALYTIC_FUNCTION_TYPE
                      ...
                      CASE(EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_POISEUILLE)
                        !Default the analytic parameter values to 0.0
              ...
              IF(ASSOCIATED(EQUATIONS)) THEN
                ...
                IF(ASSOCIATED(EQUATIONS_SET%ANALYTIC)) THEN
                  IF(EQUATIONS_SET%ANALYTIC%ANALYTIC_FUNCTION_TYPE==EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_4 .OR. &
                    & EQUATIONS_SET%ANALYTIC%ANALYTIC_FUNCTION_TYPE==EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_5 .OR. &
                    & EQUATIONS_SET%ANALYTIC%ANALYTIC_FUNCTION_TYPE==EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_TAYLOR_GREEN.OR. &
                    & EQUATIONS_SET%ANALYTIC%ANALYTIC_FUNCTION_TYPE==EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_4 .OR. &
...
999 CALL ERRORS("NAVIER_STOKES_POST_SOLVE_OUTPUT_DATA",ERR,ERROR)
  ...
  SUBROUTINE NAVIER_STOKES_SUPG_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,W_SUPG,ERR,ERROR,*)
    ...
    REAL(DP) :: ALPHA_SUPG,UMAG_SUPG,UMAX_SUPG,H_SUPG,RE_SUPG,PE_SUPG,SUPG_TOLERANCE,TAU_SUPG,LINE_LENGTH
    INTEGER(INTG) :: NODES_PER_ELEMENT,component_idx,nl,NODE_IDX,element_node_idx,NUM_COMPONENTS,NODES_PER_COMPONENT
...
!          CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,EQUATIONS%INTERPOLATION% &
!            & GEOMETRIC_INTERP_PARAMETERS(FIELD_U_VARIABLE_TYPE)%PTR,ERR,ERROR,*999)
!          GEOMETRIC_INTERP_PARAMETERS=>EQUATIONS%INTERPOLATION%GEOMETRIC_INTERP_PARAMETERS(FIELD_U_VARIABLE_TYPE)%PTR
!          IF(ASSOCIATED(GEOMETRIC_INTERP_PARAMETERS)) THEN
!            H_SUPG=GEOMETRIC_INTERP_PARAMETERS%SUPG_H
!          CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER, &
!           & GEOMETRIC_INTERP_PARAMETERS%PTR,ERR,ERROR,*999)
!          CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER,EQUATIONS%INTERPOLATION% &
!           & DEPENDENT_INTERP_PARAMETERS(FIELD_VAR_TYPE)%PTR,ERR,ERROR,*999)
!         DEPENDENT_INTERP_POINT_METRICS=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT_METRICS(FIELD_U_VARIABLE_TYPE)%PTR
!          SUPG_W=>DEPENDENT_INTERP_POINT_METRICS%SUPG_W
!          IF(ASSOCIATED(SUPG_W)) THEN
!            !Do Nothing!!!
!          ELSE
!          Otherwise calculate here


!            Check whether SUPG_H calculated for this element (should be recalculated if mesh updated)
!             GEOMETRIC_INTERP_POINT_METRICS=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT_METRICS(FIELD_U_VARIABLE_TYPE)%PTR
!             SUPG_H=>GEOMETRIC_INTERP_POINT_METRICS%SUPG_H(ELEMENT_NUMBER)
!             IF(ASSOCIATED(SUPG_H)) THEN
!               H_SUPG=SUPG_H
             !Otherwise calculate here
!             ELSE
...
!                SUPG_H=H_SUPG
!                ALLOCATE(SUPG_H,STAT=ERR)
!              ENDIF !H_SUPG, SUPG_H calculate
                ...
                DO mh=1,(FIELD_VARIABLE%NUMBER_OF_COMPONENTS-1)
                  ...
                  DO ms=1,DEPENDENT_BASIS1%NUMBER_OF_ELEMENT_PARAMETERS
                    ...
                    DO ni=1,DEPENDENT_BASIS1%NUMBER_OF_XI
                      ...
                      END DO !mi
                      !Construct W_SUPG
                      SUM_SUPG(ms,ng)=SUM_SUPG(ms,ng)+TAU_SUPG*(U_SUPG(ng,mh)*DPHIMS_DXI(ni)*DXI_DX(ni,mh))
!                      SUM_SUPG=SUM_SUPG+(U_SUPG(ng,1)*DPHIMS_DXI(ni)*DXI_DX(ni,1)
!                        &  U_SUPG(ng,2)*DPHIMS_DXI(ni)*DXI_DX(ni,2)+ &
!                        &  U_SUPG(ng,3)*DPHIMS_DXI(ni)*DXI_DX(ni,3))
                    ENDDO !ni
                  END DO !ms
                END DO !mh
              END DO !ng
              W_SUPG(1:DEPENDENT_BASIS1%NUMBER_OF_ELEMENT_PARAMETERS,1:QUADRATURE_SCHEME%NUMBER_OF_GAUSS)= &
               & SUM_SUPG(1:DEPENDENT_BASIS1%NUMBER_OF_ELEMENT_PARAMETERS,1:QUADRATURE_SCHEME%NUMBER_OF_GAUSS)
