...
!!MERGE: move
  ...
  PUBLIC LAPLACE_EQUATION_FINITE_ELEMENT_CALCULATE,LAPLACE_EQUATION_EQUATIONS_SET_SETUP,LAPLACE_EQUATION_ANALYTIC_CALCULATE, &
    ...
    & LAPLACE_EQUATION_PROBLEM_SUBTYPE_SET,LAPLACE_EQUATION_PROBLEM_SETUP!,LAPLACE_EQUATION_FINITE_ELEMENT_CALCULATE_DNUDXI
...
999 CALL ERRORS("LAPLACE_EQUATION_ANALYTIC_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE LAPLACE_EQUATION_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) FIELD_VAR_TYPE,ng,mh,mhs,mi,ms,nh,nhs,ni,ns,nj,a,b
    REAL(DP) :: CONDUCTIVITY_MATERIAL(3,3),CONDUCTIVITY(3,3),RWG,SUM,PGMSI(3),PGNSI(3),K_VALUE(3)
...
!!TODO: Think about symmetric problems. 
            ...
            CONDUCTIVITY=0.0_DP
            DO mi=1,GEOMETRIC_VARIABLE%NUMBER_OF_COMPONENTS
              DO ni=1,GEOMETRIC_VARIABLE%NUMBER_OF_COMPONENTS
                  DO a=1,GEOMETRIC_VARIABLE%NUMBER_OF_COMPONENTS
                    DO b=1,GEOMETRIC_VARIABLE%NUMBER_OF_COMPONENTS
                      CONDUCTIVITY(mi,ni)=CONDUCTIVITY(mi,ni)+DNUDXI(mi,a)*CONDUCTIVITY_MATERIAL(a,b)*DXIDNU(b,ni)
                    ENDDO !b
                  ENDDO !a
              ENDDO !ni
            ENDDO !mi
...
999 CALL ERRORS("LAPLACE_EQUATION_EQUATIONS_SET_STANDARD_SETUP",ERR,ERROR)
  ...
  SUBROUTINE LAPLACE_EQUATION_EQUATIONS_SET_GENERALISED_SETUP(EQUATIONS_SET,EQUATIONS_SET_SETUP,ERR,ERROR,*)
    ...
    CALL ENTERS("LAPLACE_EQUATION_EQUATION_SET_GENERALISED_SETUP",ERR,ERROR,*999)
        ...
        CASE(EQUATIONS_SET_SETUP_MATERIALS_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            ...
            ELSE
              ...
              CALL FIELD_NUMBER_OF_COMPONENTS_GET(EQUATIONS_SET%GEOMETRY%GEOMETRIC_FIELD,FIELD_U_VARIABLE_TYPE, &
                & NUMBER_OF_DIMENSIONS,ERR,ERROR,*999)
              CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,NUMBER_OF_DIMENSIONS,ERR, &
                & ERROR,*999)
              SELECT CASE(EQUATIONS_SET%SOLUTION_METHOD)
              CASE(EQUATIONS_SET_FEM_SOLUTION_METHOD)
                !do nothing
...
999 CALL ERRORS("LAPLACE_EQUATION_PROBLEM_GENERALISED_SETUP",ERR,ERROR)
  ...
  SUBROUTINE LAPLACE_EQUATION_FINITE_ELEMENT_CALCULATE_DNUDXI(GEOMETRIC_INTERPOLATED_POINT,FIBRE_INTERPOLATED_POINT,&
  ...
  CALL ENTERS("LAPLACE_EQUATION_FINITE_ELEMENT_CALCULATE_DNUDXI",ERR,ERROR,*999)
...
999 CALL ERRORS("LAPLACE_EQUATION_FINITE_ELEMENT_CALCULATE_DNUDXI",ERR,ERROR)
  ...
  END SUBROUTINE LAPLACE_EQUATION_FINITE_ELEMENT_CALCULATE_DNUDXI
