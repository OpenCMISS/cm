...
CONTAINS
  ...
  SUBROUTINE LAPLACE_EQUATION_ANALYTIC_CALCULATE(FIELD,NODE_POSITION,VARIABLE_NUMBER,COMPONENT_NUMBER,NODE_NUMBER,DERIV_NUMBER,VALUE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(FIELD)) THEN
	  ...
	  CASE(FIELD_NORMAL_VARIABLE_TYPE)
	    !TODO why it is scalar value here?
	    IF(NUMBER_OF_DIMENSIONS==2) THEN 
	      IF((2*NODE_POSITION(1,NODE_NUMBER)+2*NODE_POSITION(2,NODE_NUMBER))>=0.AND. &
	        & (2*NODE_POSITION(1,NODE_NUMBER)-2*NODE_POSITION(2,NODE_NUMBER))>=0) THEN           
	        VALUE=SQRT((2*NODE_POSITION(1,NODE_NUMBER)+2*NODE_POSITION(2,NODE_NUMBER))**2 &
              & + (2*NODE_POSITION(1,NODE_NUMBER)-2*NODE_POSITION(2,NODE_NUMBER))**2)
          ELSE
            VALUE=-SQRT((2*NODE_POSITION(1,NODE_NUMBER)+2*NODE_POSITION(2,NODE_NUMBER))**2 &
              & + (2*NODE_POSITION(1,NODE_NUMBER)-2*NODE_POSITION(2,NODE_NUMBER))**2)
          ENDIF
...
999 CALL ERRORS("LAPLACE_EQUATION_ANALYTIC_CALCULATE",ERR,ERROR)
  ...
  !>Calculates the element stiffness matrices and RHS for a Laplace equation finite element equations set.
  SUBROUTINE LAPLACE_EQUATION_ANALYTIC_PARAMETER_SET_UPDATE(FIELD,NODE_POSITIONS,NODE_TYPE,ERR,ERROR,*)
          ...
          IF(ASSOCIATED(DOMAIN_NODES)) THEN
	        ...
	        DO node_idx=1,NODE_TYPE_COUNT
	          ...
	          DO dev_idx=1,DOMAIN_NODES%NODES(node_number)%NUMBER_OF_DERIVATIVES
	            CALL LAPLACE_EQUATION_ANALYTIC_CALCULATE(FIELD,NODE_POSITIONS,var_idx,comp_idx,node_number,dev_idx,VALUE,ERR,ERROR,*999)
...
999 CALL ERRORS("LAPLACE_EQUATION_EQUATIONS_SET_SUBTYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE LAPLACE_EQUATION_EQUATIONS_SET_STANDARD_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    TYPE(EQUATIONS_MATRICES_TYPE), POINTER :: EQUATIONS_MATRICES
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    INTEGER(INTG) :: dim_idx,var_idx,comp_idx,node_idx,node_number,dev_idx,NUM_OF_NODAL_DEV,NUMBER_OF_DIMENSIONS,NUMBER_OF_COMPONENTS,NUMBER_OF_NODES
    TYPE(DOMAIN_NODES_TYPE), POINTER :: DOMAIN_NODES
    TYPE(DOMAIN_MAPPING_TYPE), POINTER :: NODES_MAPPING
    REAL(DP) :: VALUE
    REAL(DP), POINTER :: GEOMETRIC_PARAMETERS(:)
    REAL(DP), POINTER :: NODE_POSITION(:,:)
        ...
        CASE(EQUATIONS_SET_SETUP_ANALYTIC_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_FINISH_ACTION)
            ...
            IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
              ...
              CALL LAPLACE_EQUATION_ANALYTIC_PARAMETER_SET_UPDATE(DEPENDENT_FIELD,NODE_POSITION,1,ERR,ERROR,*999)
              
	          CALL FIELD_PARAMETER_SET_UPDATE_START(DEPENDENT_FIELD,FIELD_ANALYTIC_SET_TYPE,ERR,ERROR,*999)
	          
	          ! Set up internal nodes
	          CALL LAPLACE_EQUATION_ANALYTIC_PARAMETER_SET_UPDATE(DEPENDENT_FIELD,NODE_POSITION,2,ERR,ERROR,*999)
              
              CALL FIELD_PARAMETER_SET_UPDATE_FINISH(DEPENDENT_FIELD,FIELD_ANALYTIC_SET_TYPE,ERR,ERROR,*999)
              DEALLOCATE(NODE_POSITION)
