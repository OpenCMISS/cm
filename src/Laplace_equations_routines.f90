...
CONTAINS
  ...
  SUBROUTINE LAPLACE_EQUATION_ANALYTIC_CALCULATE(FIELD,EQUATION_NUMBER,NODE_POSITION,VARIABLE_NUMBER,COMPONENT_NUMBER,NODE_NUMBER,DERIV_NUMBER,VALUE,ERR,ERROR,*)

    !Argument variables
    TYPE(FIELD_TYPE), POINTER :: FIELD 
    INTEGER(INTG), INTENT(IN) :: EQUATION_NUMBER 
    REAL(DP), INTENT(IN) :: NODE_POSITION(:,:)
    ...
    IF(ASSOCIATED(FIELD)) THEN
      ...
      CASE(FIELD_STANDARD_VARIABLE_TYPE)
	    IF(NUMBER_OF_DIMENSIONS==2) THEN 
	      ...
	      CASE(LAPLACE_EQUATION_TWO_DIM_1)            
	        VALUE=NODE_POSITION(1,NODE_NUMBER)**2+2*NODE_POSITION(1,NODE_NUMBER)*NODE_POSITION(2,NODE_NUMBER)-NODE_POSITION(2,NODE_NUMBER)**2
	      CASE(LAPLACE_EQUATION_TWO_DIM_2) 
	        VALUE=cos(NODE_POSITION(1,NODE_NUMBER))*cosh(NODE_POSITION(2,NODE_NUMBER))
	      CASE DEFAULT
	    ...
	    ELSE IF(NUMBER_OF_DIMENSIONS==3)THEN
          ...
          CASE(LAPLACE_EQUATION_THREE_DIM_1)            
            VALUE=NODE_POSITION(1,NODE_NUMBER)**2-2*NODE_POSITION(2,NODE_NUMBER)**2+NODE_POSITION(3,NODE_NUMBER)**2
          CASE(LAPLACE_EQUATION_THREE_DIM_2) 
            VALUE=cos(NODE_POSITION(1,NODE_NUMBER))*cosh(NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(3,NODE_NUMBER)
          CASE DEFAULT
            CALL FLAG_ERROR("The equation is not implemented.",ERR,ERROR,*999)
          END SELECT
	    ENDIF 
	  CASE(FIELD_NORMAL_VARIABLE_TYPE)
	    IF(NUMBER_OF_DIMENSIONS==2) THEN 
	      SELECT CASE(EQUATION_NUMBER)
          CASE(LAPLACE_EQUATION_TWO_DIM_1)            
            VALUE=(2*NODE_POSITION(1,NODE_NUMBER)+2*NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(1,NODE_NUMBER) &
              & + (2*NODE_POSITION(1,NODE_NUMBER)-2*NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(2,NODE_NUMBER)
          CASE(LAPLACE_EQUATION_TWO_DIM_2) 
            VALUE=-sin(NODE_POSITION(1,NODE_NUMBER))*cosh(NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(1,NODE_NUMBER) &
              & + cos(NODE_POSITION(1,NODE_NUMBER))*sinh(NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(2,NODE_NUMBER)
          CASE DEFAULT
            CALL FLAG_ERROR("The equation is not implemented.",ERR,ERROR,*999)
          END SELECT
	    ELSE IF(NUMBER_OF_DIMENSIONS==3)THEN
	      SELECT CASE(EQUATION_NUMBER)
          CASE(LAPLACE_EQUATION_THREE_DIM_1)            
            VALUE=2*NODE_POSITION(1,NODE_NUMBER)**2-4*NODE_POSITION(2,NODE_NUMBER)**2+2*NODE_POSITION(3,NODE_NUMBER)**2
          CASE(LAPLACE_EQUATION_THREE_DIM_2) 
            VALUE=-sin(NODE_POSITION(1,NODE_NUMBER))*cosh(NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(3,NODE_NUMBER)*NODE_POSITION(1,NODE_NUMBER) &
              & + cos(NODE_POSITION(1,NODE_NUMBER))*sinh(NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(3,NODE_NUMBER)*NODE_POSITION(2,NODE_NUMBER) &
              & + cos(NODE_POSITION(1,NODE_NUMBER))*cosh(NODE_POSITION(2,NODE_NUMBER))*NODE_POSITION(3,NODE_NUMBER)
          CASE DEFAULT
            CALL FLAG_ERROR("The equation is not implemented.",ERR,ERROR,*999)
          END SELECT
...
999 CALL ERRORS("LAPLACE_EQUATION_ANALYTIC_CALCULATE",ERR,ERROR)
  ...
  !>Calculates the element stiffness matrices and RHS for a Laplace equation finite element equations set.
  SUBROUTINE LAPLACE_EQUATION_ANALYTIC_PARAMETER_SET_UPDATE(EQUATIONS_SET,NODE_POSITIONS,NODE_TYPE,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_SET_TYPE), POINTER :: EQUATIONS_SET 
    REAL(DP), INTENT(IN) :: NODE_POSITIONS(:,:)
          ...
          IF(ASSOCIATED(DOMAIN_NODES)) THEN
	        ...
	        DO node_idx=1,NODE_TYPE_COUNT
	          ...
	          DO dev_idx=1,DOMAIN_NODES%NODES(node_number)%NUMBER_OF_DERIVATIVES
	            CALL LAPLACE_EQUATION_ANALYTIC_CALCULATE(FIELD,EQUATION_NUMBER,NODE_POSITIONS,var_idx,comp_idx,node_number,dev_idx,VALUE,ERR,ERROR,*999)
	            ...
	            ! Set the boundary condition for dependent field >todo for rectangular system only
	            IF (NODE_TYPE/=1.AND.var_idx==1.AND.DOMAIN_NODES%NODES(node_number)%NUMBER_OF_SURROUNDING_ELEMENTS &
	              & < 2**FIELD%REGION%COORDINATE_SYSTEM%NUMBER_OF_DIMENSIONS) &
	              & CALL EQUATIONS_SET_FIXED_CONDITIONS_SET_NODE(EQUATIONS_SET,FIELD_BOUNDARY_CONDITIONS_SET_TYPE,dev_idx,node_idx, &
...
999 CALL ERRORS("LAPLACE_EQUATION_EQUATIONS_SET_SUBTYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE LAPLACE_EQUATION_EQUATIONS_SET_STANDARD_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    TYPE(EQUATIONS_MATRICES_TYPE), POINTER :: EQUATIONS_MATRICES
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    INTEGER(INTG) :: dim_idx,node_idx,NUMBER_OF_DIMENSIONS,NUMBER_OF_NODES
        ...
        CASE(EQUATIONS_SET_SETUP_ANALYTIC_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_FINISH_ACTION)
            ...
            IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
              ...
              ! Assign the nodes position
              NUMBER_OF_NODES=DEPENDENT_FIELD%VARIABLES(1)%COMPONENTS(1)%DOMAIN%TOPOLOGY%NODES%NUMBER_OF_NODES
              NUMBER_OF_DIMENSIONS=DEPENDENT_FIELD%REGION%COORDINATE_SYSTEM%NUMBER_OF_DIMENSIONS
              ALLOCATE(NODE_POSITION(NUMBER_OF_DIMENSIONS,NUMBER_OF_NODES),STAT=ERR)
              IF (ERR/=0) CALL FLAG_ERROR("Could not allocate nodes position.",ERR,ERROR,*999)
              DO dim_idx=1,NUMBER_OF_DIMENSIONS
                DO node_idx=1,NUMBER_OF_NODES
                  NODE_POSITION(dim_idx,node_idx)=GEOMETRIC_PARAMETERS(node_idx+(dim_idx-1)*NUMBER_OF_NODES)
                ENDDO !node_idx
              ENDDO !component_idx
              
              ! Set up boundary nodes 
              CALL LAPLACE_EQUATION_ANALYTIC_PARAMETER_SET_UPDATE(EQUATIONS_SET,NODE_POSITION,1,ERR,ERROR,*999)
              
	          CALL FIELD_PARAMETER_SET_UPDATE_START(DEPENDENT_FIELD,FIELD_ANALYTIC_SET_TYPE,ERR,ERROR,*999)
	          
	          ! Set up internal nodes
	          CALL LAPLACE_EQUATION_ANALYTIC_PARAMETER_SET_UPDATE(EQUATIONS_SET,NODE_POSITION,2,ERR,ERROR,*999)
              
              CALL FIELD_PARAMETER_SET_UPDATE_FINISH(DEPENDENT_FIELD,FIELD_ANALYTIC_SET_TYPE,ERR,ERROR,*999)
              IF(ALLOCATED(NODE_POSITION)) DEALLOCATE(NODE_POSITION)
