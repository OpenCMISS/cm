...
!!MERGE: check
  ...
  SUBROUTINE LAPLACE_EQUATION_ANALYTIC_CALCULATE(FIELD,EQUATION_NUMBER,GEOMETRIC_PARAMETERS,DERIVATIVE_NUMBER,NODE_NUMBER, &
	  ...
	  ELSE IF(NUMBER_OF_DIMENSIONS==3)THEN
        ...
        CASE(FIELD_STANDARD_VARIABLE_TYPE)
          ...
          CASE(LAPLACE_EQUATION_THREE_DIM_2) 
            ...
            CASE(GLOBAL_DERIV_S1)
              VALUE=-sin(x)*cosh(y)*z*INTERPOLATED_POINT%VALUES(1,2)+cos(x)*sinh(y)*z*INTERPOLATED_POINT%VALUES(2,2)+cos(x)*cosh(y)&
                &*INTERPOLATED_POINT%VALUES(3,2)
            CASE(GLOBAL_DERIV_S2)
              VALUE=-sin(x)*cosh(y)*z*INTERPOLATED_POINT%VALUES(1,4)+cos(x)*sinh(y)*z*INTERPOLATED_POINT%VALUES(2,4)+cos(x)*cosh(y)&
                &*INTERPOLATED_POINT%VALUES(3,4)
            CASE(GLOBAL_DERIV_S1_S2)
              VALUE=-sin(x)*cosh(y)*z*INTERPOLATED_POINT%VALUES(1,6)+cos(x)*sinh(y)*z*INTERPOLATED_POINT%VALUES(2,6)+cos(x)*cosh(y)&
                &*INTERPOLATED_POINT%VALUES(3,6)
            CASE(GLOBAL_DERIV_S3)
              VALUE=-sin(x)*cosh(y)*z*INTERPOLATED_POINT%VALUES(1,7)+cos(x)*sinh(y)*z*INTERPOLATED_POINT%VALUES(2,7)+cos(x)*cosh(y)&
                &*INTERPOLATED_POINT%VALUES(3,7)
            CASE(GLOBAL_DERIV_S1_S3)
              VALUE=-sin(x)*cosh(y)*z*INTERPOLATED_POINT%VALUES(1,9)+cos(x)*sinh(y)*z*INTERPOLATED_POINT%VALUES(2,9)+cos(x)*cosh(y)&
                &*INTERPOLATED_POINT%VALUES(3,9)
            CASE(GLOBAL_DERIV_S2_S3)
              VALUE=-sin(x)*cosh(y)*z*INTERPOLATED_POINT%VALUES(1,10)+cos(x)*sinh(y)*z*INTERPOLATED_POINT%VALUES(2,10)+cos(x)&
                &*cosh(y)*INTERPOLATED_POINT%VALUES(3,10)
            CASE(GLOBAL_DERIV_S1_S2_S3)
              VALUE=-sin(x)*cosh(y)*z*INTERPOLATED_POINT%VALUES(1,11)+cos(x)*sinh(y)*z*INTERPOLATED_POINT%VALUES(2,11)+cos(x)& 
                &*cosh(y)*INTERPOLATED_POINT%VALUES(3,11)
            CASE DEFAULT
              CALL FLAG_ERROR("The derivativehas not been implemented.",ERR,ERROR,*999)
            END SELECT            
          CASE DEFAULT
...
!!MERGE: what is this doing
  ...
  SUBROUTINE LAPLACE_EQUATION_INTERPOLATED_POINT_GET(FIELD,NODE_NUMBER,COMPONENT_NUMBER,VARIABLE_NUMBER,NUMBER_OF_DIMENSIONS, &
    ...
    IF (ASSOCIATED(FIELD)) THEN
      ...
      ELEMENT_NUMBER=FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)%DOMAIN%TOPOLOGY%NODES%NODES(NODE_NUMBER)% &
          ...
          IF(ASSOCIATED(DOMAIN_NODES)) THEN
	        ...
	        DO node_idx=1,NODE_TYPE_COUNT
	          ...
	          DO dev_idx=1,DOMAIN_NODES%NODES(node_number)%NUMBER_OF_DERIVATIVES
	            CALL LAPLACE_EQUATION_ANALYTIC_CALCULATE(FIELD,EQUATION_NUMBER,GEOMETRIC_PARAMETERS,dev_idx,node_number,comp_idx, &
	            ...
	            IF (NODE_TYPE/=1.AND.var_idx==1.AND.DOMAIN_NODES%NODES(node_number)%NUMBER_OF_SURROUNDING_ELEMENTS &
	              ...
	              CALL LAPLACE_EQUATION_FIXED_CONDITIONS_SET_NODE(EQUATIONS_SET,FIELD_BOUNDARY_CONDITIONS_SET_TYPE,dev_idx,& 
	                &DOMAIN_NODES%NODES(node_number)%USER_NUMBER, &
