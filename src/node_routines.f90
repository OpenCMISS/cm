!> \file
!> $Id: node_routines.f90 27 2007-07-24 16:52:51Z cpb $
  ...
  SUBROUTINE NODE_CHECK_EXISTS(USER_NUMBER,REGION,NODE_EXISTS,GLOBAL_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(REGION)) THEN
      IF(ASSOCIATED(REGION%NODES)) THEN
        !!TODO:: Need more efficient conversion of user->global number.
        node_no=1
        DO WHILE(node_no<=REGION%NODES%NUMBER_OF_NODES.AND..NOT.NODE_EXISTS)
          IF(REGION%NODES%NODES(node_no)%USER_NUMBER==USER_NUMBER) THEN
            NODE_EXISTS=.TRUE.
            GLOBAL_NUMBER=node_no
          ELSE
            node_no=node_no+1
          ENDIF
        ENDDO
