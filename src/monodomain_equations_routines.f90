...
999 CALL ERRORS("MONODOMAIN_FINITE_ELEMENT_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE MONODOMAIN_EQUATION_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) FIELD_VAR_TYPE,ng,mh,mhs,mi,ms,nh,nhs,ni,ns,nj
    REAL(DP) :: RWG,SUM,PGMSI(3),PGNSI(3),K_VALUE(3),K_PARAM,PGMJ(3),PGNJ(3)
                      ...
                      IF(STIFFNESS_MATRIX%UPDATE_MATRIX) THEN
                        ...
                        DO nj=1,GEOMETRIC_VARIABLE%NUMBER_OF_COMPONENTS
                          ...
                          K_PARAM=EQUATIONS%INTERPOLATION%MATERIALS_INTERP_POINT(FIELD_U_VARIABLE_TYPE)%PTR% &
                            & VALUES(nj+1,NO_PART_DERIV) ! K PARAM : MATERIALS COMPONENT + 1 . axis-aligned orthotropic
...
999 CALL ERRORS("MONODOMAIN_EQUATION_FINITE_ELEMENT_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE MONODOMAIN_EQUATION_EQUATIONS_SET_SETUP(EQUATIONS_SET,EQUATIONS_SET_SETUP,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      SELECT CASE(EQUATIONS_SET%SUBTYPE)
      CASE(EQUATIONS_SET_MONODOMAIN_BUENOOROVIO_SUBTYPE)
        CALL MONODOMAIN_EQUATION_EQUATIONS_SET_BUENOOROVIO_SETUP(EQUATIONS_SET,EQUATIONS_SET_SETUP,ERR,ERROR,*999)        
      CASE DEFAULT
        LOCAL_ERROR="Equations set subtype "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET%SUBTYPE,"*",ERR,ERROR))// &
...
999 CALL ERRORS("MONODOMAIN_EQUATION_EQUATIONS_SET_SUBTYPE_SET",ERR,ERROR)
  ...
  !>Sets up the Bueno Orovio Monodomain equation.
  SUBROUTINE MONODOMAIN_EQUATION_EQUATIONS_SET_BUENOOROVIO_SETUP(EQUATIONS_SET,EQUATIONS_SET_SETUP,ERR,ERROR,*)
    ...
    CALL ENTERS("MONODOMAIN_EQUATION_EQUATION_SET_BUENOOROVIO_SETUP",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_MONODOMAIN_BUENOOROVIO_SUBTYPE) THEN
        ...
        CASE(EQUATIONS_SET_SETUP_DEPENDENT_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
            IF(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD_AUTO_CREATED) THEN
              ...
              CALL FIELD_NUMBER_OF_COMPONENTS_SET_AND_LOCK(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,4, &
                & ERR,ERROR,*999)  ! only dV/dn
              CALL FIELD_NUMBER_OF_COMPONENTS_SET_AND_LOCK(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD,FIELD_DELUDELN_VARIABLE_TYPE,4, &
                & ERR,ERROR,*999)  ! TODO: bug / different # of comp for vars not supported? need only 1 or even 0
          ...
          CASE(EQUATIONS_SET_SETUP_FINISH_ACTION)
            IF(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD_AUTO_CREATED) THEN
              ...
              CALL BUENO_OROVIO_INITIALIZE(EQUATIONS_SET%DEPENDENT%DEPENDENT_FIELD,ERR,ERROR,*999) ! initialize to y0
            ENDIF
          CASE DEFAULT
            LOCAL_ERROR="The action type of "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET_SETUP%ACTION_TYPE,"*",ERR,ERROR))// &
              & " for a setup type of "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET_SETUP%SETUP_TYPE,"*",ERR,ERROR))// &
             ...
             IF(ASSOCIATED(EQUATIONS_MATERIALS)) THEN
               IF(EQUATIONS_MATERIALS%MATERIALS_FIELD_AUTO_CREATED) THEN
                ...
                CALL FIELD_NUMBER_OF_COMPONENTS_GET(EQUATIONS_SET%GEOMETRY%GEOMETRIC_FIELD,FIELD_U_VARIABLE_TYPE, &
                  ...
                  ! + 1 for each dimension  i.e., k and a in div(k.grad(u(x)))
                  ! TODO:  fiber angles
                ...
                DO component_idx=1,NUMBER_OF_DIMENSIONS
                  ...
                  CALL FIELD_COMPONENT_MESH_COMPONENT_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                    & component_idx+1,GEOMETRIC_MESH_COMPONENT,ERR,ERROR,*999)
                  CALL FIELD_COMPONENT_INTERPOLATION_SET(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                    & component_idx+1,FIELD_CONSTANT_INTERPOLATION,ERR,ERROR,*999)
                ENDDO !component_idx
...
! ! ! Upto here
    ...
    CALL EXITS("MONODOMAIN_EQUATION_EQUATIONS_SET_BUENOOROVIO_SETUP")
    RETURN
999 CALL ERRORS("MONODOMAIN_EQUATION_EQUATIONS_SET_BUENOOROVIO_SETUP",ERR,ERROR)
    CALL EXITS("MONODOMAIN_EQUATION_EQUATIONS_SET_BUENOOROVIO_SETUP")
    RETURN 1
  END SUBROUTINE MONODOMAIN_EQUATION_EQUATIONS_SET_BUENOOROVIO_SETUP
  ...
  SUBROUTINE MONODOMAIN_EQUATION_PROBLEM_STRANG_SPLITTING_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_MONODOMAIN_BUENOOROVIO_SUBTYPE)
        CALL MONODOMAIN_EQUATION_PROBLEM_BUENOOROVIO_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*999)
      CASE DEFAULT
...
999 CALL ERRORS("MONODOMAIN_EQUATION_PROBLEM_SUBTYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE MONODOMAIN_EQUATION_PROBLEM_BUENOOROVIO_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*)
    ...
    CALL ENTERS("MONODOMAIN_EQUATION_PROBLEM_BUENOOROVIO_SETUP",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(PROBLEM%SUBTYPE==PROBLEM_MONODOMAIN_BUENOOROVIO_SUBTYPE) THEN
    ...
    CALL EXITS("MONODOMAIN_EQUATION_PROBLEM_BUENOOROVIO_SETUP")
    RETURN
999 CALL ERRORS("MONODOMAIN_EQUATION_PROBLEM_BUENOOROVIO_SETUP",ERR,ERROR)
    CALL EXITS("MONODOMAIN_EQUATION_PROBLEM_BUENOOROVIO_SETUP")
    RETURN 1
  END SUBROUTINE MONODOMAIN_EQUATION_PROBLEM_BUENOOROVIO_SETUP
...
999 CALL ERRORS("MONODOMAIN_PRE_SOLVE",ERR,ERROR)
  ...
  SUBROUTINE MONODOMAIN_POST_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*)
    ...
    TYPE(FIELD_TYPE), POINTER :: DEPENDENT_FIELD, MATERIAL_FIELD
    REAL(DP) :: TMP0, TMP1, S
    INTEGER(INTG) :: NN, I
    ...
    IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
      ...
      CASE(PROBLEM_MONODOMAIN_STRANG_SPLITTING_EQUATION_TYPE) ! CAN NOT GET EQN TYPE?! 
       ...
       NN =   DEPENDENT_FIELD%decomposition%domain(1)%ptr%topology%nodes%number_of_nodes

        CALL BUENO_OROVIO_INTEGRATE(DEPENDENT_FIELD,MATERIAL_FIELD,&
        ...
        CALL FIELD_PARAMETER_SETS_COPY(DEPENDENT_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,&  ! update previous as well, used in next solver step
        &FIELD_PREVIOUS_VALUES_SET_TYPE,1.0_DP,ERR,ERROR,*999)

        call field_parameter_set_get_node(DEPENDENT_FIELD,field_u_variable_type,field_values_set_type,1,1,1,tmp0,err,error,*999)
        call field_parameter_set_get_node(DEPENDENT_FIELD,field_u_variable_type,field_values_set_type,1,NN,1,tmp1,err,error,*999)

        WRITE(*,*) 'INTEGRATING CELL MODELS',CONTROL_LOOP%TIME_LOOP%CURRENT_TIME-CONTROL_LOOP%TIME_LOOP%TIME_INCREMENT,'->',&
        & CONTROL_LOOP%TIME_LOOP%CURRENT_TIME, &
        & 'V(1) = ', TMP0, 'V(',NN,') = ', TMP1
    
 !       DO I=1,25
 !         call field_parameter_set_get_node(DEPENDENT_FIELD,field_u_variable_type,field_values_set_type,1,I,1,tmp1,err,error,*999)
 !         WRITE(*,*) 'V(',I,')=',tmp1
 !       END DO
