           ...
           IF(ASSOCIATED(DEPENDENT_FIELD)) THEN              
              ...
              IF(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%NUMBER_OF_LINEAR_EQUATIONS_MATRICES>0) THEN                  
                ...
                IF(ASSOCIATED(LINEAR_MAPPING)) THEN
                  ...
                  ALLOCATE(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(FIELD_NUMBER_OF_VARIABLE_TYPES),STAT=ERR)
                  ...
                  DO variable_type=1,FIELD_NUMBER_OF_VARIABLE_TYPES
                    CALL EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_MATRICES_MAP_INITIALISE(LINEAR_MAPPING% &
                      & VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type),ERR,ERROR,*999)
                    LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%VARIABLE_INDEX=variable_type
                    LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%VARIABLE_TYPE=variable_type
                    LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%VARIABLE=>DEPENDENT_FIELD% &
                  ...
                  DO matrix_idx=1,LINEAR_MAPPING%NUMBER_OF_LINEAR_EQUATIONS_MATRICES
                    ...
                    LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%NUMBER_OF_LINEAR_EQUATIONS_MATRICES= &
                      & LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%NUMBER_OF_LINEAR_EQUATIONS_MATRICES+1
                  ENDDO !matrix_idx
                  IF(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%RHS_VARIABLE_TYPE/=0) LINEAR_MAPPING% &
                    & VARIABLE_TO_EQUATIONS_MATRICES_MAPS(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%RHS_VARIABLE_TYPE)% &
                  ...
                  DO variable_type=1,FIELD_NUMBER_OF_VARIABLE_TYPES
                    ...
                    IF(ASSOCIATED(DEPENDENT_VARIABLE)) THEN
                      IF(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
...
!!TODO: check if this can be removed and just allocate those variables that are actually used
                        ALLOCATE(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%DOF_TO_ROWS_MAP( &
                        ...
                        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate variable to equations matrices maps dof to rows map.", &
                          & ERR,ERROR,*999)
                        LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%DOF_TO_ROWS_MAP=0
                        LINEAR_MAPPING%NUMBER_OF_LINEAR_MATRIX_VARIABLES=LINEAR_MAPPING%NUMBER_OF_LINEAR_MATRIX_VARIABLES+1
                      ELSE IF(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                        & NUMBER_OF_LINEAR_EQUATIONS_MATRICES>0) THEN
                        ALLOCATE(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%EQUATIONS_MATRIX_NUMBERS( &
                          & LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                        ...
                        IF(ERR/=0) &
                          ...
                          & ERR,ERROR,*999)
                        ALLOCATE(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%DOF_TO_COLUMNS_MAPS( &
                          & LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                          & NUMBER_OF_LINEAR_EQUATIONS_MATRICES),STAT=ERR)
                        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate variable to equations matrices maps dof to columns map.", &
                          & ERR,ERROR,*999)                
                        LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%EQUATIONS_MATRIX_NUMBERS=0
                        LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%NUMBER_OF_LINEAR_EQUATIONS_MATRICES=0
                        DO matrix_idx=1,LINEAR_MAPPING%NUMBER_OF_LINEAR_EQUATIONS_MATRICES
                          IF(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%MATRIX_VARIABLE_TYPES(matrix_idx)==variable_type) THEN
                            LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%NUMBER_OF_LINEAR_EQUATIONS_MATRICES= &
                              & LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                              & NUMBER_OF_LINEAR_EQUATIONS_MATRICES+1
                            LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%EQUATIONS_MATRIX_NUMBERS( &
                              & LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                            ...
                            DO dof_idx=1,DEPENDENT_VARIABLE%TOTAL_NUMBER_OF_DOFS
                              ...
                              LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%DOF_TO_COLUMNS_MAPS( &
                                & LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
                        ...
                        DO dof_idx=1,DEPENDENT_VARIABLE%TOTAL_NUMBER_OF_DOFS
                          ...
                          row_idx=dof_idx
                          LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%DOF_TO_ROWS_MAP(dof_idx)=row_idx
                        ENDDO !dof_idx
                        LINEAR_MAPPING%NUMBER_OF_LINEAR_MATRIX_VARIABLES=LINEAR_MAPPING%NUMBER_OF_LINEAR_MATRIX_VARIABLES+1
                      ENDIF
                    ENDIF
                  ...
                  LINEAR_MAPPING%NUMBER_OF_LINEAR_MATRIX_VARIABLES=0
                  DO variable_type=1,FIELD_NUMBER_OF_VARIABLE_TYPES
                    IF(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%NUMBER_OF_LINEAR_EQUATIONS_MATRICES>0) THEN
                      LINEAR_MAPPING%NUMBER_OF_LINEAR_MATRIX_VARIABLES=LINEAR_MAPPING%NUMBER_OF_LINEAR_MATRIX_VARIABLES+1
                  ...
                  ALLOCATE(LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(LINEAR_MAPPING%NUMBER_OF_LINEAR_EQUATIONS_MATRICES), &
                    & STAT=ERR)
                  ...
                  DO matrix_idx=1,LINEAR_MAPPING%NUMBER_OF_LINEAR_EQUATIONS_MATRICES
                    variable_type=EQUATIONS_MAPPING%CREATE_VALUES_CACHE%MATRIX_VARIABLE_TYPES(matrix_idx)
                    DEPENDENT_VARIABLE=>DEPENDENT_FIELD%VARIABLE_TYPE_MAP(variable_type)%PTR
                    CALL EQUATIONS_MAPPING_EQUATIONS_MATRIX_TO_VARIABLE_MAP_INITIALISE(LINEAR_MAPPING% &
                      & EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx),ERR,ERROR,*999)
                    LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%MATRIX_NUMBER=matrix_idx
                    LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%VARIABLE_TYPE=variable_type
                    LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%VARIABLE=>DEPENDENT_VARIABLE
                    LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%NUMBER_OF_COLUMNS=DEPENDENT_VARIABLE% &
                      & DOMAIN_MAPPING%NUMBER_OF_GLOBAL
                    LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%MATRIX_COEFFICIENT=EQUATIONS_MAPPING% &
                      CREATE_VALUES_CACHE%MATRIX_COEFFICIENTS(matrix_idx)
                    ALLOCATE(LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%COLUMN_TO_DOF_MAP( &
                      & DEPENDENT_VARIABLE%DOMAIN_MAPPING%NUMBER_OF_GLOBAL),STAT=ERR)                  
                    LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%COLUMN_TO_DOF_MAP=0
                    DO dof_idx=1,DEPENDENT_VARIABLE%TOTAL_NUMBER_OF_DOFS
                      !1-1 mapping for now
                      column_idx=DEPENDENT_VARIABLE%DOMAIN_MAPPING%LOCAL_TO_GLOBAL_MAP(dof_idx)
                      LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%COLUMN_TO_DOF_MAP(column_idx)=dof_idx
                    ENDDO !dof_idx
                    LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%COLUMN_DOFS_MAPPING=> &
              ...
              IF(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_TYPE/=0) THEN                  
                ...
                IF(ASSOCIATED(NONLINEAR_MAPPING)) THEN
                  CALL EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE(NONLINEAR_MAPPING% &
                    & VARIABLE_TO_JACOBIAN_MAP,ERR,ERROR,*999)
                  NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%VARIABLE_TYPE=EQUATIONS_MAPPING%CREATE_VALUES_CACHE% &
                    & RESIDUAL_VARIABLE_TYPE
                  DEPENDENT_VARIABLE=>DEPENDENT_FIELD%VARIABLE_TYPE_MAP(EQUATIONS_MAPPING%CREATE_VALUES_CACHE% &
                    & RESIDUAL_VARIABLE_TYPE)%PTR
                  NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%VARIABLE=>DEPENDENT_VARIABLE
                  !Allocate and set dof to Jacobian columns map
                  ALLOCATE(NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%DOF_TO_COLUMNS_MAP(DEPENDENT_VARIABLE% &
                  ...
                  DO dof_idx=1,DEPENDENT_VARIABLE%TOTAL_NUMBER_OF_DOFS
                    !1-1 mapping for now
                    column_idx=DEPENDENT_VARIABLE%DOMAIN_MAPPING%LOCAL_TO_GLOBAL_MAP(dof_idx)
                    NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%DOF_TO_COLUMNS_MAP(dof_idx)=column_idx
                  ENDDO !dof_idx
                  !Allocate and set dof to Jacobian rows map
                  ALLOCATE(NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%DOF_TO_ROWS_MAP(DEPENDENT_VARIABLE% &
                    & TOTAL_NUMBER_OF_DOFS),STAT=ERR)
                  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate variable to Jacobian map dof to columns map.",ERR,ERROR,*999)
                  DO dof_idx=1,DEPENDENT_VARIABLE%TOTAL_NUMBER_OF_DOFS
                    !1-1 mapping for now
                    row_idx=dof_idx
                    NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%DOF_TO_ROWS_MAP(dof_idx)=row_idx
                  ENDDO !dof_idx
                  CALL EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE(NONLINEAR_MAPPING% &
                    & JACOBIAN_TO_VARIABLE_MAP,ERR,ERROR,*999)
                  NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%VARIABLE_TYPE=EQUATIONS_MAPPING%CREATE_VALUES_CACHE% &
                    & RESIDUAL_VARIABLE_TYPE
                  NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%VARIABLE=>DEPENDENT_VARIABLE
                  NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%NUMBER_OF_COLUMNS=DEPENDENT_VARIABLE%DOMAIN_MAPPING%NUMBER_OF_GLOBAL
                  NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%JACOBIAN_COEFFICIENT=EQUATIONS_MAPPING%CREATE_VALUES_CACHE% &
                    & RESIDUAL_COEFFICIENT
                  ALLOCATE(NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%EQUATIONS_COLUMN_TO_DOF_VARIABLE_MAP( &
                    & DEPENDENT_VARIABLE%DOMAIN_MAPPING%NUMBER_OF_GLOBAL),STAT=ERR)
                  NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%EQUATIONS_COLUMN_TO_DOF_VARIABLE_MAP=0
                  DO dof_idx=1,DEPENDENT_VARIABLE%TOTAL_NUMBER_OF_DOFS
                    !1-1 mapping for now
                    column_idx=DEPENDENT_VARIABLE%DOMAIN_MAPPING%LOCAL_TO_GLOBAL_MAP(dof_idx)
                    NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%EQUATIONS_COLUMN_TO_DOF_VARIABLE_MAP(column_idx)=dof_idx
                  ENDDO !dof_idx
                  NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%COLUMN_DOFS_MAPPING=>DEPENDENT_VARIABLE%DOMAIN_MAPPING
    ...
    IF(DIAGNOSTICS1) THEN
      ...
      IF(ASSOCIATED(LINEAR_MAPPING)) THEN
        ...
        DO variable_type=1,FIELD_NUMBER_OF_VARIABLE_TYPES
          ...
          IF(ASSOCIATED(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%VARIABLE)) THEN
            CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Total number of DOFs = ",LINEAR_MAPPING% &
              & VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%VARIABLE%TOTAL_NUMBER_OF_DOFS,ERR,ERROR,*999)
            CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Number of linear equations matrices = ",LINEAR_MAPPING% &
              & VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)%NUMBER_OF_LINEAR_EQUATIONS_MATRICES,ERR,ERROR,*999)
            CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
              & NUMBER_OF_LINEAR_EQUATIONS_MATRICES,4,4,LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
            ...
            DO matrix_idx=1,LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
              & NUMBER_OF_LINEAR_EQUATIONS_MATRICES
              CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"        Matrix number : ",matrix_idx,ERR,ERROR,*999)
              CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS( &
                & variable_type)%VARIABLE%TOTAL_NUMBER_OF_DOFS,5,5,LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS( &
                & variable_type)%DOF_TO_COLUMNS_MAPS(matrix_idx)%COLUMN_DOF, &
                & '("        Column numbers :",5(X,I13))','(24X,5(X,I13))',ERR,ERROR,*999) 
            ENDDO !matrix_idx
            CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
              & VARIABLE%TOTAL_NUMBER_OF_DOFS,5,5,LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type)% &
        ...
        DO matrix_idx=1,LINEAR_MAPPING%NUMBER_OF_LINEAR_EQUATIONS_MATRICES
          ...
          CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"        Variable type = ",LINEAR_MAPPING% &
            & EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%VARIABLE_TYPE,ERR,ERROR,*999)
          CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"        Number of columns = ",LINEAR_MAPPING% &
            & EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%NUMBER_OF_COLUMNS,ERR,ERROR,*999)
          CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"        Matrix coefficient = ",LINEAR_MAPPING% &
            & EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%MATRIX_COEFFICIENT,ERR,ERROR,*999)
          CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)% &
            & NUMBER_OF_COLUMNS,5,5,LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%COLUMN_TO_DOF_MAP, &
      ...
      IF(ASSOCIATED(NONLINEAR_MAPPING)) THEN
        ...
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Jacobian variable type = ",NONLINEAR_MAPPING% &
          & VARIABLE_TO_JACOBIAN_MAP%VARIABLE_TYPE,ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Total number of Jacobain DOFs = ",NONLINEAR_MAPPING% &
          & VARIABLE_TO_JACOBIAN_MAP%VARIABLE%TOTAL_NUMBER_OF_DOFS,ERR,ERROR,*999)
        CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%VARIABLE% &
          & TOTAL_NUMBER_OF_DOFS,5,5,NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%DOF_TO_COLUMNS_MAP, &
          & '("      DOF to column map :",5(X,I13))','(26X,5(X,I13))',ERR,ERROR,*999) 
        CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%VARIABLE% &
          & TOTAL_NUMBER_OF_DOFS,5,5,NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP%DOF_TO_ROWS_MAP, &
        ...
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Jacobian variable type = ",NONLINEAR_MAPPING% &
          & JACOBIAN_TO_VARIABLE_MAP%VARIABLE_TYPE,ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Number of columns = ",NONLINEAR_MAPPING% &
          & JACOBIAN_TO_VARIABLE_MAP%NUMBER_OF_COLUMNS,ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"      Jacobian coefficient = ",NONLINEAR_MAPPING% &
          & JACOBIAN_TO_VARIABLE_MAP%JACOBIAN_COEFFICIENT,ERR,ERROR,*999)
        CALL WRITE_STRING_VECTOR(DIAGNOSTIC_OUTPUT_TYPE,1,1,NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%NUMBER_OF_COLUMNS, &
          & 5,5,NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP%EQUATIONS_COLUMN_TO_DOF_VARIABLE_MAP, &
...
999 CALL ERRORS("EQUATIONS_MAPPING_DESTROY",ERR,ERROR)    
  ...
  SUBROUTINE EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_FINALISE(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_TYPE) :: EQUATIONS_JACOBIAN_TO_VARIABLE_MAP !<The equations Jacobian to variable map to finalise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables

    CALL ENTERS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_FINALISE",ERR,ERROR,*999)
    
    IF(ALLOCATED(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%EQUATIONS_COLUMN_TO_DOF_VARIABLE_MAP)) &
      & DEALLOCATE(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%EQUATIONS_COLUMN_TO_DOF_VARIABLE_MAP)
    
    CALL EXITS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_FINALISE")
    RETURN
999 CALL ERRORS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_FINALISE",ERR,ERROR)    
    CALL EXITS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_FINALISE")
    RETURN 1
   
  END SUBROUTINE EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_FINALISE
  ...
  SUBROUTINE EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_TYPE) :: EQUATIONS_JACOBIAN_TO_VARIABLE_MAP !<The equations Jacobian to variable map to initialise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables

    CALL ENTERS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE",ERR,ERROR,*999)
    
    EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%VARIABLE_TYPE=0
    NULLIFY(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%VARIABLE)
    NULLIFY(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%JACOBIAN)
    EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%NUMBER_OF_COLUMNS=0
    EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%JACOBIAN_COEFFICIENT=0
    NULLIFY(EQUATIONS_JACOBIAN_TO_VARIABLE_MAP%COLUMN_DOFS_MAPPING)    
    
    CALL EXITS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE")
    RETURN
999 CALL ERRORS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE",ERR,ERROR)    
    CALL EXITS("EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE")
    RETURN 1
   
  END SUBROUTINE EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE
...
998 CALL ERRORS("EQUATIONS_MAPPING_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_MAPPING_LINEAR_MAPPING_FINALISE(LINEAR_MAPPING,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(LINEAR_MAPPING)) THEN
      ...
      IF(ALLOCATED(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS)) THEN
        DO variable_type=1,SIZE(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS,1)
          CALL EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_MATRICES_MAP_FINALISE(LINEAR_MAPPING% &
            & VARIABLE_TO_EQUATIONS_MATRICES_MAPS(variable_type),ERR,ERROR,*999)
        ENDDO !variable_type
        DEALLOCATE(LINEAR_MAPPING%VARIABLE_TO_EQUATIONS_MATRICES_MAPS)        
      ENDIF
      IF(ALLOCATED(LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS)) THEN
        DO matrix_idx=1,SIZE(LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS,1)
          CALL EQUATIONS_MAPPING_EQUATIONS_MATRIX_TO_VARIABLE_MAP_FINALISE(LINEAR_MAPPING% &
            & EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx),ERR,ERROR,*999)
        ENDDO !matrix_idx
        DEALLOCATE(LINEAR_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS)
      ENDIF
...
999 CALL ERRORS("EQUATIONS_MAPPING_LINEAR_MATRICES_VARIABLE_TYPES_SET",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_MAPPING_NONLINEAR_MAPPING_FINALISE(NONLINEAR_MAPPING,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(NONLINEAR_MAPPING)) THEN
      CALL EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_FINALISE(NONLINEAR_MAPPING%VARIABLE_TO_JACOBIAN_MAP,ERR,ERROR,*999)
      CALL EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_FINALISE(NONLINEAR_MAPPING%JACOBIAN_TO_VARIABLE_MAP,ERR,ERROR,*999)
  ...
  SUBROUTINE EQUATIONS_MAPPING_NONLINEAR_MAPPING_INITIALISE(EQUATIONS_MAPPING,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_MAPPING)) THEN
      ...
      ELSE
        ...
        CALL EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE(EQUATIONS_MAPPING%NONLINEAR_MAPPING% &
          & VARIABLE_TO_JACOBIAN_MAP,ERR,ERROR,*999)
        CALL EQUATIONS_MAPPING_EQUATIONS_JACOBIAN_TO_VARIABLE_MAP_INITIALISE(EQUATIONS_MAPPING%NONLINEAR_MAPPING% &
          & JACOBIAN_TO_VARIABLE_MAP,ERR,ERROR,*999)
...
999 CALL ERRORS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_COLUMN_MAP_FINALISE",ERR,ERROR)    
  ...
  !>Finalises a variable to equations Jacobian map and deallocates all memory.
  SUBROUTINE EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_FINALISE(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_TYPE) :: VARIABLE_TO_EQUATIONS_JACOBIAN_MAP !<The variable to equations Jacobian map to finalise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables

    CALL ENTERS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_FINALISE",ERR,ERROR,*999)
    
    IF(ALLOCATED(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP%DOF_TO_COLUMNS_MAP)) &
      & DEALLOCATE(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP%DOF_TO_COLUMNS_MAP)
    IF(ALLOCATED(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP%DOF_TO_ROWS_MAP)) &
      & DEALLOCATE(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP%DOF_TO_ROWS_MAP)
    
    CALL EXITS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_FINALISE")
    RETURN
999 CALL ERRORS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_FINALISE",ERR,ERROR)    
    CALL EXITS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_FINALISE")
    RETURN 1
   
  END SUBROUTINE EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_FINALISE
  ...
  !>Initialises a variable to equations Jacobian map
  SUBROUTINE EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_TYPE) :: VARIABLE_TO_EQUATIONS_JACOBIAN_MAP !<The variable to equations Jacobian map to initialise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables

    CALL ENTERS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE",ERR,ERROR,*999)
    
    VARIABLE_TO_EQUATIONS_JACOBIAN_MAP%VARIABLE_TYPE=0
    NULLIFY(VARIABLE_TO_EQUATIONS_JACOBIAN_MAP%VARIABLE)
    
    CALL EXITS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE")
    RETURN
999 CALL ERRORS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE",ERR,ERROR)    
    CALL EXITS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE")
    RETURN 1
   
  END SUBROUTINE EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_JACOBIAN_MAP_INITIALISE
...
999 CALL ERRORS("EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_MATRICES_MAP_FINALISE",ERR,ERROR)    
  ...
  SUBROUTINE EQUATIONS_MAPPING_VARIABLE_TO_EQUATIONS_MATRICES_MAP_INITIALISE(VARIABLE_TO_EQUATIONS_MATRICES_MAP,ERR,ERROR,*)
    ...
    VARIABLE_TO_EQUATIONS_MATRICES_MAP%NUMBER_OF_LINEAR_EQUATIONS_MATRICES=0
