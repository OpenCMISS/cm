...
MODULE EQUATIONS_MAPPING_ROUTINES
  ...
  PUBLIC EQUATIONS_MAPPING_CREATE_FINISH,EQUATIONS_MAPPING_CREATE_START,EQUATIONS_MAPPING_DESTROY, &
    ...
    & EQUATIONS_MAPPING_RESIDUAL_JACOBIAN_TYPES_SET, &
              ...
              IF(CREATE_VALUES_CACHE%NUMBER_OF_RESIDUAL_VARIABLES/=0) THEN
                ...
                IF(ASSOCIATED(NONLINEAR_MAPPING)) THEN
                  ...
                  DO matrix_idx=1,NONLINEAR_MAPPING%NUMBER_OF_RESIDUAL_VARIABLES
                    ...
                    NONLINEAR_MAPPING%VAR_TO_JACOBIAN_MAP(matrix_idx)%JACOBIAN_CALCULATION_TYPE= &
                      & CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES(matrix_idx)
                    DEPENDENT_VARIABLE=>DEPENDENT_FIELD%VARIABLE_TYPE_MAP(CREATE_VALUES_CACHE% &
                      & RESIDUAL_VARIABLE_TYPES(matrix_idx))%PTR
...
999 CALL ERRORS("EQUATIONS_MAPPING_CREATE_START",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_MAPPING_CREATE_VALUES_CACHE_FINALISE(CREATE_VALUES_CACHE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(CREATE_VALUES_CACHE)) THEN
      ...
      IF(ALLOCATED(CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES))  &
        & DEALLOCATE(CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES)
...
! SEBK 19/08/2009 not sure about mapping here
              ...
              ENDIF
              !Allocate the residual variable types and Jacobian calculation types
              IF(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%NUMBER_OF_RESIDUAL_VARIABLES>0) THEN
                ...
                IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations mapping create values cache residual variable types.", &
                    & ERR,ERROR,*999)
                ALLOCATE(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES(EQUATIONS_MAPPING% &
                  & CREATE_VALUES_CACHE%NUMBER_OF_RESIDUAL_VARIABLES),STAT=ERR)
                IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations mapping create values cache residual variable "// &
                  & "Jacobian types.",ERR,ERROR,*999)
                ...
                IF(EQUATIONS_MAPPING%CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_TYPES(EQUATIONS_MAPPING% &
                  ...
                  & ERR,ERROR,*999)
                !Default Jacobian calculation type to finite difference type
                EQUATIONS_MAPPING%CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES= &
                  & EQUATIONS_SET_FINITE_DIFFERENCE_JACOBIAN
              ENDIF
...
998 CALL ERRORS("EQUATIONS_MAPPING_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_MAPPING_RESIDUAL_VARIABLES_NUMBER_SET(EQUATIONS_MAPPING,NUMBER_OF_VARIABLES,ERR,ERROR,*)
    ...
    INTEGER(INTG), ALLOCATABLE :: NEW_RESIDUAL_JACOBIAN_TYPES(:)
    TYPE(EQUATIONS_MAPPING_CREATE_VALUES_CACHE_TYPE), POINTER :: CREATE_VALUES_CACHE
      ...
      ELSE
        ...
        IF(ASSOCIATED(CREATE_VALUES_CACHE)) THEN
          ...
          IF(NUMBER_OF_VARIABLES/=PREVIOUS_NUMBER) THEN
            ...
            ALLOCATE(NEW_RESIDUAL_JACOBIAN_TYPES(NUMBER_OF_VARIABLES),STAT=ERR)
            IF(ERR/=0) CALL FLAG_ERROR("Could not allocate new residual variable Jacobian types.",ERR,ERROR,*999)
            ...
            NEW_RESIDUAL_JACOBIAN_TYPES=EQUATIONS_SET_FINITE_DIFFERENCE_JACOBIAN
            NEW_RESIDUAL_JACOBIAN_TYPES(1:MIN_NUMBER)=CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES(1:MIN_NUMBER)
            CALL MOVE_ALLOC(NEW_RESIDUAL_JACOBIAN_TYPES,CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES)
...
999 CALL ERRORS("EQUATIONS_MAPPING_RESIDUAL_VARIABLE_TYPES_SET",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_MAPPING_RESIDUAL_JACOBIAN_TYPES_SET(EQUATIONS_MAPPING,RESIDUAL_VARIABLE_JACOBIAN_TYPES,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_MAPPING_TYPE), POINTER :: EQUATIONS_MAPPING !<A pointer to the equations mapping to set
    INTEGER(INTG), INTENT(IN) :: RESIDUAL_VARIABLE_JACOBIAN_TYPES(:) !<RESIDUAL_VARIABLE_JACOBIAN_TYPE(variable_idx). The Jacobian calculation type for the variable_idx'th residual variable.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: variable_idx,NUMBER_OF_RESIDUAL_VARIABLES,RESIDUAL_VARIABLE_JACOBIAN_TYPE
    TYPE(EQUATIONS_MAPPING_CREATE_VALUES_CACHE_TYPE), POINTER :: CREATE_VALUES_CACHE
    TYPE(VARYING_STRING) :: LOCAL_ERROR

    CALL ENTERS("EQUATIONS_MAPPING_RESIDUAL_JACOBIAN_TYPES_SET",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(EQUATIONS_MAPPING)) THEN
      ...
      ELSE
        CREATE_VALUES_CACHE=>EQUATIONS_MAPPING%CREATE_VALUES_CACHE
        IF(ASSOCIATED(CREATE_VALUES_CACHE)) THEN
          NUMBER_OF_RESIDUAL_VARIABLES=SIZE(RESIDUAL_VARIABLE_JACOBIAN_TYPES,1)
          IF(NUMBER_OF_RESIDUAL_VARIABLES==CREATE_VALUES_CACHE%NUMBER_OF_RESIDUAL_VARIABLES) THEN
            DO variable_idx=1,NUMBER_OF_RESIDUAL_VARIABLES
              RESIDUAL_VARIABLE_JACOBIAN_TYPE=RESIDUAL_VARIABLE_JACOBIAN_TYPES(variable_idx)
              SELECT CASE(RESIDUAL_VARIABLE_JACOBIAN_TYPE)
              CASE(EQUATIONS_SET_FINITE_DIFFERENCE_JACOBIAN, &
                  & EQUATIONS_SET_ANALYTIC_JACOBIAN)
                CREATE_VALUES_CACHE%RESIDUAL_VARIABLE_JACOBIAN_TYPES(variable_idx)= &
                  & RESIDUAL_VARIABLE_JACOBIAN_TYPE
              CASE DEFAULT
                LOCAL_ERROR="Invalid Jacobian calculation type of " &
                  & //TRIM(NUMBER_TO_VSTRING(RESIDUAL_VARIABLE_JACOBIAN_TYPE,"*",ERR,ERROR))//"."
                CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
              END SELECT
            END DO
          ELSE
            LOCAL_ERROR="Invalid number of Jacobian calculation types. The number of residual variables " &
              & //TRIM(NUMBER_TO_VSTRING(NUMBER_OF_RESIDUAL_VARIABLES,"*",ERR,ERROR)) &
              & //" should be "//TRIM(NUMBER_TO_VSTRING(CREATE_VALUES_CACHE%NUMBER_OF_RESIDUAL_VARIABLES,"*",ERR,ERROR))
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          ENDIF
    ...
    CALL EXITS("EQUATIONS_MAPPING_RESIDUAL_JACOBIAN_TYPES_SET")
    RETURN
999 CALL ERRORS("EQUATIONS_MAPPING_RESIDUAL_JACOBIAN_TYPES_SET",ERR,ERROR)
    CALL EXITS("EQUATIONS_MAPPING_RESIDUAL_JACOBIAN_TYPES_SET")
    RETURN 1
  END SUBROUTINE EQUATIONS_MAPPING_RESIDUAL_JACOBIAN_TYPES_SET
