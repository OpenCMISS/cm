...
MODULE REGION_ROUTINES
  ...
  PUBLIC INTF_REGION_CREATE_START, INTF_REGION_CREATE_FINISH  ! <<>>

  PUBLIC INTF_REGION_LMHOST_SET ! <<>>
...
997 CALL ERRORS("REGION_CREATE_START",ERR,ERROR)
  ...
  SUBROUTINE INTF_REGION_CREATE_FINISH(INTF_REGION,ERR,ERROR,*)  ! <<>>
    ...
    CALL ENTERS("INTF_REGION_CREATE_FINISH",ERR,ERROR,*999)

    IF(ASSOCIATED(INTF_REGION)) THEN
      IF(INTF_REGION%REGION_FINISHED) THEN
        CALL FLAG_ERROR("Interface Region has already been finished.",ERR,ERROR,*999)
      ELSE
        IF ( INTF_REGION%INTF%LMHOST_REGION  == 0 ) CALL FLAG_ERROR("Interface host not specified.",ERR,ERROR,*999)
        INTF_REGION%REGION_FINISHED=.TRUE.
      ENDIF
    ELSE
      CALL FLAG_ERROR("Interface Region is not associated.",ERR,ERROR,*999)
    ENDIF
    
    IF(DIAGNOSTICS1) THEN
      CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"Interface Region : ",INTF_REGION%USER_NUMBER,ERR,ERROR,*999)      
      CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"  Label = ",INTF_REGION%LABEL,ERR,ERROR,*999)
      IF(ASSOCIATED(INTF_REGION%PARENT_REGION)) THEN
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"  Parent region user number = ",INTF_REGION%PARENT_REGION%USER_NUMBER, &
          & ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"  Parent region label = ",INTF_REGION%PARENT_REGION%LABEL, &
          & ERR,ERROR,*999)        
      ENDIF
    ENDIF
    
    CALL EXITS("INTF_REGION_CREATE_FINISH")
    RETURN
999 CALL ERRORS("INTF_REGION_CREATE_FINISH",ERR,ERROR)
    CALL EXITS("INTF_REGION_CREATE_FINISH")
    RETURN 1
  END SUBROUTINE INTF_REGION_CREATE_FINISH   ! <<>>


  SUBROUTINE INTF_REGION_CREATE_START(USER_NUMBER,PARENT_REGION,INTF_REGION,REGION1,REGION2,ERR,ERROR,*)   ! <<>>

    !Argument variables
    INTEGER(INTG), INTENT(IN) ::       USER_NUMBER    !<The user number of the region to create
    TYPE(REGION_TYPE), POINTER ::       PARENT_REGION    !<A pointer to the parent region
    TYPE(REGION_TYPE), POINTER ::       INTF_REGION    !<On exit, a pointer to the created region. Must not be associated on entry.
    TYPE(REGION_TYPE), POINTER ::       REGION1    !<A pointer to the parent region
    TYPE(REGION_TYPE), POINTER ::       REGION2    !<A pointer to the parent region
    INTEGER(INTG), INTENT(OUT) ::       ERR       !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) ::    ERROR       !<The error string
    !Local Variables
    ...
    CALL ENTERS("INTF_REGION_CREATE_START",ERR,ERROR,*997)
    ...
    IF(ASSOCIATED(NEW_REGION)) THEN
      ...
      CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*997)
    ELSE
      IF(ASSOCIATED(INTF_REGION))             CALL FLAG_ERROR("Region is already associated.",ERR,ERROR,*997)
      IF(.NOT.ASSOCIATED(PARENT_REGION))          CALL FLAG_ERROR("Parent region is not associated.",ERR,ERROR,*997)
      IF(PARENT_REGION%REGION_FINISHED .EQV. .FALSE.) CALL FLAG_ERROR("Parent region has not been finished.",ERR,ERROR,*997)
      IF(.NOT.ASSOCIATED(PARENT_REGION%COORDINATE_SYSTEM)) THEN
         CALL FLAG_ERROR("Parent region does not have an associated coordinate system.",ERR,ERROR,*997)
      END IF
      IF(.NOT.ASSOCIATED(REGION1))            CALL FLAG_ERROR("First region not associated.",ERR,ERROR,*997)
      IF(.NOT.REGION1%REGION_FINISHED)            CALL FLAG_ERROR("First region has not been finished.",ERR,ERROR,*997)
      IF(.NOT.ASSOCIATED(REGION2))            CALL FLAG_ERROR("Second region not associated.",ERR,ERROR,*997)
      IF(.NOT.REGION2%REGION_FINISHED)            CALL FLAG_ERROR("Second region has not been finished.",ERR,ERROR,*997)
      NULLIFY(INTF_REGION)
      !Initialise the region
      CALL REGION_INITIALISE(INTF_REGION,ERR,ERROR,*999)
      INTF_REGION%INTERFACE_REGION = .TRUE.
      !Set the user number
      INTF_REGION%USER_NUMBER=USER_NUMBER
      !Set local string name
      LOCAL_STRING="Region_"//NUMBER_TO_VSTRING(USER_NUMBER,"*",ERR,ERROR)
      INTF_REGION%LABEL=CHAR(LOCAL_STRING)
      IF(ERR/=0) GOTO 999
      !Set interface region coordinate system to that of the parent region
      INTF_REGION%COORDINATE_SYSTEM=>PARENT_REGION%COORDINATE_SYSTEM
      !Set the interface pointer to detail its connected regions
      ALLOCATE(INTF_REGION%INTF,STAT=ERR)
      IF(ERR/=0) GOTO 999
      INTF_REGION%INTF%INTERFACE_TYPE   = 0
      INTF_REGION%INTF%COUPLING_TYPE    = 0
      INTF_REGION%INTF%LMHOST_REGION    = 0
      INTF_REGION%INTF%N_COUPLED_REGIONS= 2
      ALLOCATE(INTF_REGION%INTF%COUPLED_REGIONS(INTF_REGION%INTF%N_COUPLED_REGIONS),STAT=ERR)
      IF(ERR/=0) GOTO 999
      NULLIFY(INTF_REGION%INTF%COUPLED_REGIONS(1)%PTR); INTF_REGION%INTF%COUPLED_REGIONS(1)%PTR=>REGION1
      NULLIFY(INTF_REGION%INTF%COUPLED_REGIONS(2)%PTR); INTF_REGION%INTF%COUPLED_REGIONS(2)%PTR=>REGION2
      !Adjust the parent region to include this new daughter
      NULLIFY(NEW_SUB_REGIONS)
      ALLOCATE(NEW_SUB_REGIONS(PARENT_REGION%NUMBER_OF_SUB_REGIONS+1),STAT=ERR)
      IF(ERR/=0) CALL FLAG_ERROR("Could not allocate new sub-regions.",ERR,ERROR,*999)
      DO I=1,PARENT_REGION%NUMBER_OF_SUB_REGIONS
         NEW_SUB_REGIONS(I)%PTR=>PARENT_REGION%SUB_REGIONS(I)%PTR
      ENDDO
      PARENT_REGION%NUMBER_OF_SUB_REGIONS=PARENT_REGION%NUMBER_OF_SUB_REGIONS+1
      NEW_SUB_REGIONS(PARENT_REGION%NUMBER_OF_SUB_REGIONS)%PTR=>INTF_REGION
      IF(ASSOCIATED(PARENT_REGION%SUB_REGIONS)) DEALLOCATE(PARENT_REGION%SUB_REGIONS)
      ALLOCATE(PARENT_REGION%SUB_REGIONS(PARENT_REGION%NUMBER_OF_SUB_REGIONS))
      PARENT_REGION%SUB_REGIONS=>NEW_SUB_REGIONS
      DEALLOCATE(NEW_SUB_REGIONS)
      !Set the new regions parent region to the parent region
      INTF_REGION%PARENT_REGION=>PARENT_REGION
    ENDIF

    CALL EXITS("INTF_REGION_CREATE_START")
    RETURN
999 CALL REGION_FINALISE(INTF_REGION,DUMMY_ERR,DUMMY_ERROR,*998)
998 IF(ASSOCIATED(NEW_SUB_REGIONS)) DEALLOCATE(NEW_SUB_REGIONS)
997 CALL ERRORS("INTF_REGION_CREATE_START",ERR,ERROR)
    CALL EXITS("INTF_REGION_CREATE_START")
    RETURN 1
  END SUBROUTINE INTF_REGION_CREATE_START   ! <<>>


  !>Finishes the creation of a region. \see OPENCMISS::CMISSRegionCreateFinish
  SUBROUTINE INTF_REGION_LMHOST_SET(INTF_REGION,REGION_ID,ERR,ERROR,*)    ! <<>>
    ...
    CALL ENTERS("INTF_REGION_LMHOST_SET",ERR,ERROR,*999)
    ...
    IF(INTF_REGION%REGION_FINISHED)            CALL FLAG_ERROR("Interface has already been finished.",ERR,ERROR,*999)
    IF(.NOT.ALLOCATED(INTF_REGION%INTF%COUPLED_REGIONS)) THEN
     CALL FLAG_ERROR("Interface region class is not properly associated.",ERR,ERROR,*999)
    END IF
    INTF_REGION%INTF%LMHOST_REGION=0
    DO I=1,INTF_REGION%INTF%N_COUPLED_REGIONS
       IF(INTF_REGION%INTF%COUPLED_REGIONS(I)%PTR%USER_NUMBER==REGION_ID) THEN
         INTF_REGION%INTF%LMHOST_REGION=I; EXIT
       END IF
    ENDDO
    IF(INTF_REGION%INTF%LMHOST_REGION==0) CALL FLAG_ERROR("Interface host region set unsuccessful.",ERR,ERROR,*999)
    IF(INTF_REGION%INTF%LMHOST_REGION==1) INTF_REGION%INTF%HOST_REGION=2
    IF(INTF_REGION%INTF%LMHOST_REGION==2) INTF_REGION%INTF%HOST_REGION=1

    CALL EXITS("INTF_REGION_LMHOST_SET")
    RETURN
999 CALL ERRORS("INTF_REGION_LMHOST_SET",ERR,ERROR)
    CALL EXITS("INTF_REGION_LMHOST_SET")
    RETURN 1
  END SUBROUTINE INTF_REGION_LMHOST_SET   ! <<>>
...
999 CALL ERRORS("REGION_DESTROY",ERR,ERROR)
  ...
  SUBROUTINE REGION_FINALISE(REGION,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(REGION)) THEN
      ...
      IF(ASSOCIATED(REGION%INTF)) PRINT *, 'ASSOCIATED INTF'
      IF(ASSOCIATED(REGION%INTF)) DEALLOCATE(REGION%INTF) 				!<<>>
      DEALLOCATE(REGION)
    ENDIF
