    ...
    IF(ASSOCIATED(FIELD)) THEN
      ...
      IF(ASSOCIATED(ANALYTIC_PARAMETERS)) THEN      
        ...
        IF(ASSOCIATED(DOMAIN_NODES)) THEN
          ...
          VALUE=ANALYTIC_PARAMETERS((VARIABLE_NUMBER-1)*DOMAIN_NODES%NUMBER_OF_NODES*NUMBER_OF_DERIVATIVES+(NODE_NUMBER-1)*NUMBER_OF_DERIVATIVES+DERIVATIVE_NUMBER)  
        ELSE
  ...
  SUBROUTINE ANALYTIC_ANALYSIS_NODE_NUMERICIAL_VALUE_GET(FIELD,VARIABLE_NUMBER,COMPONENT_NUMBER,NODE_NUMBER,DERIVATIVE_NUMBER,VALUE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(FIELD)) THEN
      ...
      IF(ASSOCIATED(NUMERICAL_PARAMETERS)) THEN      
        ...
        IF(ASSOCIATED(DOMAIN_NODES)) THEN
          NUMBER_OF_DERIVATIVES=DOMAIN_NODES%NODES(node_number)%NUMBER_OF_DERIVATIVES
          VALUE=NUMERICAL_PARAMETERS((VARIABLE_NUMBER-1)*DOMAIN_NODES%NUMBER_OF_NODES*NUMBER_OF_DERIVATIVES+(NODE_NUMBER-1)*NUMBER_OF_DERIVATIVES+DERIVATIVE_NUMBER)  
        ELSE
          CALL FLAG_ERROR("Domain nodes are not associated",ERR,ERROR,*999)
        ENDIF
      ELSE
