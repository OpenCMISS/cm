            ...
            IF(ASSOCIATED(DECOMPOSITION_TOPOLOGY)) THEN
              ...
              DO var_idx=1,FIELD%NUMBER_OF_VARIABLES
                ...
                IF(ASSOCIATED(FIELD_VARIABLE)) THEN
                  ...
                  ALLOCATE(INTEGRAL_ERRORS(10,FIELD_VARIABLE%NUMBER_OF_COMPONENTS),STAT=ERR)
                  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate integral errors.",ERR,ERROR,*999)
                  ALLOCATE(GHOST_INTEGRAL_ERRORS(10,FIELD_VARIABLE%NUMBER_OF_COMPONENTS),STAT=ERR)
...
999 IF(ALLOCATED(INTEGRAL_ERRORS)) DEALLOCATE(INTEGRAL_ERRORS)
  ...
  SUBROUTINE ANALYTIC_ANALYSIS_INTEGRAL_ERRORS(FIELD_VARIABLE,INTEGRAL_ERRORS,GHOST_INTEGRAL_ERRORS,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(FIELD_VARIABLE)) THEN
      IF(SIZE(INTEGRAL_ERRORS,1)>=10.AND.SIZE(INTEGRAL_ERRORS,2)>=FIELD_VARIABLE%NUMBER_OF_COMPONENTS) THEN
        IF(SIZE(GHOST_INTEGRAL_ERRORS,1)>=10.AND.SIZE(GHOST_INTEGRAL_ERRORS,2)>=FIELD_VARIABLE%NUMBER_OF_COMPONENTS) THEN
                  ...
                  DO element_idx=1,DOMAIN_ELEMENTS1%NUMBER_OF_ELEMENTS
                    ...
                    DO gauss_idx=1,QUADRATURE_SCHEME%NUMBER_OF_GAUSS
                      ...
                      DO component_idx=1,FIELD_VARIABLE%NUMBER_OF_COMPONENTS
                        ...
                        INTEGRAL_ERRORS(5,component_idx)=INTEGRAL_ERRORS(5,component_idx) &
                          & +ANALYTIC_ANALYSIS_ABSOLUTE_ERROR(ANALYTIC_INT,NUMERICAL_INT)*RWG
                        INTEGRAL_ERRORS(6,component_idx)=INTEGRAL_ERRORS(6,component_idx) &
                          & +ANALYTIC_ANALYSIS_ABSOLUTE_ERROR(ANALYTIC_INT,NUMERICAL_INT)**2*RWG
                        INTEGRAL_ERRORS(7,component_idx)=INTEGRAL_ERRORS(7,component_idx) &
                          & +ANALYTIC_ANALYSIS_RELATIVE_ERROR(ANALYTIC_INT,NUMERICAL_INT)*RWG
                        INTEGRAL_ERRORS(8,component_idx)=INTEGRAL_ERRORS(8,component_idx) &
                          & +ANALYTIC_ANALYSIS_RELATIVE_ERROR(ANALYTIC_INT,NUMERICAL_INT)**2*RWG
                        INTEGRAL_ERRORS(9,component_idx)=INTEGRAL_ERRORS(9,component_idx) &
                          & +ANALYTIC_ANALYSIS_PERCENTAGE_ERROR(ANALYTIC_INT,NUMERICAL_INT)*RWG
                        INTEGRAL_ERRORS(10,component_idx)=INTEGRAL_ERRORS(10,component_idx) &
                          & +ANALYTIC_ANALYSIS_PERCENTAGE_ERROR(ANALYTIC_INT,NUMERICAL_INT)**2*RWG
                      ENDDO !component_idx
                    ENDDO !gauss_idx
                  ENDDO !element_idx
                    ...
                    DO gauss_idx=1,QUADRATURE_SCHEME%NUMBER_OF_GAUSS
                      ...
                      DO component_idx=1,FIELD_VARIABLE%NUMBER_OF_COMPONENTS
                        ...
                        GHOST_INTEGRAL_ERRORS(6,component_idx)=GHOST_INTEGRAL_ERRORS(6,component_idx)+ &
                          & ANALYTIC_ANALYSIS_ABSOLUTE_ERROR(ANALYTIC_INT,NUMERICAL_INT)**2*RWG
                        GHOST_INTEGRAL_ERRORS(7,component_idx)=GHOST_INTEGRAL_ERRORS(7,component_idx)+ &
                          & ANALYTIC_ANALYSIS_RELATIVE_ERROR(ANALYTIC_INT,NUMERICAL_INT)*RWG
                        GHOST_INTEGRAL_ERRORS(8,component_idx)=GHOST_INTEGRAL_ERRORS(8,component_idx)+ &
                          & ANALYTIC_ANALYSIS_RELATIVE_ERROR(ANALYTIC_INT,NUMERICAL_INT)**2*RWG
                        GHOST_INTEGRAL_ERRORS(9,component_idx)=GHOST_INTEGRAL_ERRORS(9,component_idx)+ &
                          & ANALYTIC_ANALYSIS_PERCENTAGE_ERROR(ANALYTIC_INT,NUMERICAL_INT)*RWG
                        GHOST_INTEGRAL_ERRORS(10,component_idx)=GHOST_INTEGRAL_ERRORS(10,component_idx)+ &
                          & ANALYTIC_ANALYSIS_PERCENTAGE_ERROR(ANALYTIC_INT,NUMERICAL_INT)**2*RWG
                      ENDDO !component_idx
                    ENDDO !gauss_idx
                  ENDDO !element_idx
...
999 CALL ERRORS("ANALYTIC_ANALYSIS_INTEGRAL_NUMERICAL_VALUE_GET",ERR,ERROR)
  ...
  SUBROUTINE ANALYTIC_ANALYSIS_INTEGRAL_PERCENTAGE_ERROR_GET(FIELD,VARIABLE_TYPE,COMPONENT_NUMBER,INTEGRAL_ERROR, &
    ...
    IF(ASSOCIATED(FIELD)) THEN
      FIELD_VARIABLE=>FIELD%VARIABLE_TYPE_MAP(VARIABLE_TYPE)%PTR
      CALL ANALYTIC_ANALYSIS_INTEGRAL_ERRORS(FIELD_VARIABLE,INTEGRAL_ERRORS,GHOST_INTEGRAL_ERRORS,ERR,ERROR,*999)
      INTEGRAL_ERROR(1)=INTEGRAL_ERRORS(9,COMPONENT_NUMBER)
      INTEGRAL_ERROR(2)=INTEGRAL_ERRORS(10,COMPONENT_NUMBER)
      GHOST_INTEGRAL_ERROR(1)=GHOST_INTEGRAL_ERRORS(9,COMPONENT_NUMBER)
      GHOST_INTEGRAL_ERROR(2)=GHOST_INTEGRAL_ERRORS(10,COMPONENT_NUMBER)
    ELSE
      CALL FLAG_ERROR("Field is not associated",ERR,ERROR,*999)
    ENDIF 
...
999 CALL ERRORS("ANALYTIC_ANALYSIS_INTEGRAL_PERCENTAGE_ERROR_GET",ERR,ERROR)
  ...
  SUBROUTINE ANALYTIC_ANALYSIS_INTEGRAL_RELATIVE_ERROR_GET(FIELD,VARIABLE_TYPE,COMPONENT_NUMBER,INTEGRAL_ERROR, &
    ...
    IF(ASSOCIATED(FIELD)) THEN
      FIELD_VARIABLE=>FIELD%VARIABLE_TYPE_MAP(VARIABLE_TYPE)%PTR
      CALL ANALYTIC_ANALYSIS_INTEGRAL_ERRORS(FIELD_VARIABLE,INTEGRAL_ERRORS,GHOST_INTEGRAL_ERRORS,ERR,ERROR,*999)
      INTEGRAL_ERROR(1)=INTEGRAL_ERRORS(7,COMPONENT_NUMBER)
      INTEGRAL_ERROR(2)=INTEGRAL_ERRORS(8,COMPONENT_NUMBER)
      GHOST_INTEGRAL_ERROR(1)=GHOST_INTEGRAL_ERRORS(7,COMPONENT_NUMBER)
      GHOST_INTEGRAL_ERROR(2)=GHOST_INTEGRAL_ERRORS(8,COMPONENT_NUMBER)
    ELSE
      CALL FLAG_ERROR("Field is not associated",ERR,ERROR,*999)
    ENDIF 
