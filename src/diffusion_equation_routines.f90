...
MODULE DIFFUSION_EQUATION_ROUTINES
  ...
  PUBLIC DIFFUSION_EQUATION_EQUATIONS_SET_SETUP,DIFFUSION_EQUATION_EQUATIONS_SET_SUBTYPE_SET, &
    & DIFFUSION_EQUATION_FINITE_ELEMENT_CALCULATE,DIFFUSION_EQUATION_FINITE_ELEMENT_JACOBIAN_EVALUATE, &
    & DIFFUSION_EQUATION_FINITE_ELEMENT_RESIDUAL_EVALUATE,DIFFUSION_EQUATION_PROBLEM_SUBTYPE_SET, &
...
CONTAINS
  ...
  SUBROUTINE DIFFUSION_EQUATION_EQUATIONS_SET_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      CASE(EQUATIONS_SET_LINEAR_DIFFUSION_SUBTYPE)
        CALL DIFFUSION_EQUATION_EQUATIONS_SET_LINEAR_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*999)
      CASE(EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE)
        CALL DIFFUSION_EQUATION_EQUATIONS_SET_NONLINEAR_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*999)
      CASE DEFAULT
  ...
  SUBROUTINE DIFFUSION_EQUATION_EQUATIONS_SET_SUBTYPE_SET(EQUATIONS_SET,EQUATIONS_SET_SUBTYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      CASE(EQUATIONS_SET_LINEAR_DIFFUSION_SUBTYPE)
        ...
        EQUATIONS_SET%SUBTYPE=EQUATIONS_SET_LINEAR_DIFFUSION_SUBTYPE
        CALL DIFFUSION_EQUATION_EQUATIONS_SET_LINEAR_SETUP(EQUATIONS_SET,EQUATIONS_SET_SETUP_INITIAL_TYPE, &
          & EQUATIONS_SET_SETUP_START_ACTION,ERR,ERROR,*999)
      CASE(EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE)
        EQUATIONS_SET%CLASS=EQUATIONS_SET_CLASSICAL_FIELD_CLASS
        EQUATIONS_SET%TYPE=EQUATIONS_SET_DIFFUSION_EQUATION_TYPE
        EQUATIONS_SET%SUBTYPE=EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE
        CALL DIFFUSION_EQUATION_EQUATIONS_SET_NONLINEAR_SETUP(EQUATIONS_SET,EQUATIONS_SET_SETUP_INITIAL_TYPE, &
          & EQUATIONS_SET_SETUP_START_ACTION,ERR,ERROR,*999)
      CASE DEFAULT
...
999 CALL ERRORS("DIFFUSION_EQUATION_EQUATIONS_SET_SUBTYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE DIFFUSION_EQUATION_EQUATIONS_SET_LINEAR_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_LINEAR_DIFFUSION_SUBTYPE) THEN
...
999 CALL ERRORS("DIFFUSION_EQUATION_EQUATIONS_SET_LINEAR_SETUP",ERR,ERROR)
  ...
  !>Sets up the nonlinear diffusion equation.
  SUBROUTINE DIFFUSION_EQUATION_EQUATIONS_SET_NONLINEAR_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    CALL ENTERS("DIFFUSION_EQUATION_EQUATION_SET_NONLINEAR_SETUP",ERR,ERROR,*999)

    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE) THEN
        SELECT CASE(SETUP_TYPE)
        CASE(EQUATIONS_SET_SETUP_INITIAL_TYPE)
          SELECT CASE(ACTION_TYPE)
          CASE(EQUATIONS_SET_SETUP_START_ACTION)
  ...
  SUBROUTINE DIFFUSION_EQUATION_PROBLEM_SETUP(PROBLEM,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_NONLINEAR_DIFFUSION_SUBTYPE)
        CALL DIFFUSION_EQUATION_PROBLEM_NONLINEAR_SETUP(PROBLEM,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*999)
      CASE DEFAULT
...
!!TODO: CHANGE ELEMENT CALCULATE TO WORK OF ns ???
        ...
        CASE(EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE)
          CALL FLAG_ERROR("Can not calculate finite element stiffness matrices for a nonlinear diffusion equation.",ERR,ERROR,*999)
        CASE DEFAULT
          LOCAL_ERROR="Equations set subtype "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET%SUBTYPE,"*",ERR,ERROR))// &
            & " is not valid for a diffusion equation type of a classical field equations set class."
          CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
...
999 CALL ERRORS("DIFFUSION_EQUATION_FINITE_ELEMENT_CALCULATE",ERR,ERROR)
  ...
  !>Evaluates the Jacobian element stiffness matrices and RHS for a diffusion equation finite element equations set.
  SUBROUTINE DIFFUSION_EQUATION_FINITE_ELEMENT_JACOBIAN_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        SELECT CASE(EQUATIONS_SET%SUBTYPE)
        CASE(EQUATIONS_SET_LINEAR_DIFFUSION_SUBTYPE)
          CALL FLAG_ERROR("Can not evaluate a Jacobian for a linear diffusion equation.",ERR,ERROR,*999)
        CASE(EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE)
          EQUATIONS_MATRICES=>EQUATIONS%EQUATIONS_MATRICES
  ...
  SUBROUTINE DIFFUSION_EQUATION_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        SELECT CASE(EQUATIONS_SET%SUBTYPE)
        CASE(EQUATIONS_SET_LINEAR_DIFFUSION_SUBTYPE)
          CALL FLAG_ERROR("Can not evaluate a residual for a linear diffusion equation.",ERR,ERROR,*999)
        CASE(EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE)                 
          !Store all these in equations matrices/somewhere else?????
          DEPENDENT_FIELD=>EQUATIONS%INTERPOLATION%DEPENDENT_FIELD
          GEOMETRIC_FIELD=>EQUATIONS%INTERPOLATION%GEOMETRIC_FIELD
  ...
  SUBROUTINE DIFFUSION_EQUATION_PROBLEM_SUBTYPE_SET(PROBLEM,PROBLEM_SUBTYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_LINEAR_DIFFUSION_SUBTYPE)        
        ...
        PROBLEM%SUBTYPE=PROBLEM_LINEAR_DIFFUSION_SUBTYPE     
        CALL DIFFUSION_EQUATION_PROBLEM_LINEAR_SETUP(PROBLEM,PROBLEM_SETUP_INITIAL_TYPE,PROBLEM_SETUP_START_ACTION, &
          & ERR,ERROR,*999)
      CASE(PROBLEM_NONLINEAR_DIFFUSION_SUBTYPE)        
        PROBLEM%CLASS=PROBLEM_CLASSICAL_FIELD_CLASS
        PROBLEM%TYPE=PROBLEM_DIFFUSION_EQUATION_TYPE
        PROBLEM%SUBTYPE=PROBLEM_NONLINEAR_DIFFUSION_SUBTYPE     
        CALL DIFFUSION_EQUATION_PROBLEM_NONLINEAR_SETUP(PROBLEM,PROBLEM_SETUP_INITIAL_TYPE,PROBLEM_SETUP_START_ACTION, &
          & ERR,ERROR,*999)
      CASE DEFAULT
...
999 CALL ERRORS("DIFFUSION_EQUATION_PROBLEM_SUBTYPE_SET",ERR,ERROR)
  ...
  !>Sets up the linear diffusion equations solution.
  SUBROUTINE DIFFUSION_EQUATION_PROBLEM_LINEAR_SETUP(PROBLEM,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(PROBLEM%SUBTYPE==PROBLEM_LINEAR_DIFFUSION_SUBTYPE) THEN
        SELECT CASE(SETUP_TYPE)
           ...
           CASE(PROBLEM_SETUP_DO_ACTION)
             ...
             IF(ASSOCIATED(EQUATIONS_SET)) THEN
               ...
               IF(EQUATIONS_SET%CLASS==EQUATIONS_SET_CLASSICAL_FIELD_CLASS.AND. &
                 ...
                 & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_LINEAR_DIFFUSION_SUBTYPE) THEN
                 SOLUTION_MAPPING=>SOLUTION%SOLUTION_MAPPING            
                 IF(ASSOCIATED(SOLUTION_MAPPING)) THEN
                 ...
                 ENDIF
               ELSE
                 CALL FLAG_ERROR("The equations set to add is not a classical field linear diffusion equations set.",ERR,ERROR,*999)
               ENDIF
             ELSE
...
999 CALL ERRORS("DIFFUSION_EQUATION_PROBLEM_LINEAR_SETUP",ERR,ERROR)
  ...
  !>Sets up the nonlinear diffusion equation solution.
  SUBROUTINE DIFFUSION_EQUATION_PROBLEM_NONLINEAR_SETUP(PROBLEM,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(PROBLEM%SUBTYPE==PROBLEM_NONLINEAR_DIFFUSION_SUBTYPE) THEN
        SELECT CASE(SETUP_TYPE)
        CASE(PROBLEM_SETUP_INITIAL_TYPE)
          SELECT CASE(ACTION_TYPE)
          CASE(PROBLEM_SETUP_START_ACTION)
...
!!TODO:
          ...
          IF(ASSOCIATED(SOLUTION)) THEN
           ...
           CASE(PROBLEM_SETUP_DO_ACTION)
             ...
             IF(ASSOCIATED(EQUATIONS_SET)) THEN
               ...
               IF(EQUATIONS_SET%CLASS==EQUATIONS_SET_CLASSICAL_FIELD_CLASS.AND. &
                 & EQUATIONS_SET%TYPE==EQUATIONS_SET_DIFFUSION_EQUATION_TYPE.AND. &
                 & EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_NONLINEAR_DIFFUSION_SUBTYPE) THEN
                 SOLUTION_MAPPING=>SOLUTION%SOLUTION_MAPPING            
                 IF(ASSOCIATED(SOLUTION_MAPPING)) THEN
                   CALL SOLUTION_MAPPING_EQUATIONS_SET_ADD(SOLUTION_MAPPING,SOLUTION%EQUATIONS_SET_TO_ADD,SOLUTION% &
                     & EQUATIONS_SET_ADDED_INDEX,ERR,ERROR,*999)
                 ...
                 ENDIF
               ELSE
                 CALL FLAG_ERROR("The equations set to add is not a nonlinear diffusion equations set.",ERR,ERROR,*999)
               ENDIF
             ELSE
               CALL FLAG_ERROR("Equations set to add is not associated.",ERR,ERROR,*999)
             ENDIF
      ...
      ELSE
        LOCAL_ERROR="The problem subtype of "//TRIM(NUMBER_TO_VSTRING(PROBLEM%SUBTYPE,"*",ERR,ERROR))// &
          & " does not equal a nonlinear diffusion equation subtype."
        CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem is not associated.",ERR,ERROR,*999)
...
999 CALL ERRORS("DIFFUSION_EQUATION_PROBLEM_NONLINEAR_SETUP",ERR,ERROR)
    CALL EXITS("DIFFUSION_EQUATION_PROBLEM_NONLINEAR_SETUP")
    RETURN 1
  END SUBROUTINE POISSON_EQUATION_PROBLEM_NONLINEAR_SETUP
