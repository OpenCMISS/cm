...
MODULE FIELDML_OUTPUT_ROUTINES
  ...
  TYPE CONNECTIVITY_INFO_TYPE
    INTEGER(C_INT) :: CONNECTIVITY_HANDLE !<The basis connectivity evaluator handle.
    INTEGER(C_INT) :: LAYOUT_HANDLE !<The local node layout.
  END TYPE CONNECTIVITY_INFO_TYPE

  TYPE BASIS_INFO_TYPE
    TYPE(BASIS_TYPE), POINTER :: BASIS !<The OpenCMISS basis.
    INTEGER(C_INT) :: CONNECTIVITY_HANDLE !<The basis connectivity evaluator handle.
    INTEGER(C_INT) :: REFERENCE_HANDLE !<The reference evaluator representing the basis.
    INTEGER(C_INT) :: LAYOUT_HANDLE !<The local node layout.
  ...
  PUBLIC :: FIELDML_OUTPUT_WRITE, FIELDML_OUTPUT_ADD_FIELD, FIELDML_OUTPUT_INITIALISE_INFO, &
    & FIELDML_OUTPUT_ADD_FIELD_COMPONENTS, FIELDML_OUTPUT_CREATE_ENSEMBLE_TYPE, FIELDML_OUTPUT_CREATE_CONTINUOUS_TYPE
...
CONTAINS
  ...
  SUBROUTINE FIELDML_OUTPUT_GET_TP_BASIS_EVALUATOR( FML_HANDLE, XI_INTERPOLATIONS, COLLAPSE_INFO, EVALUATOR_HANDLE, &
    ...
    INTEGER(C_INT), INTENT(IN) :: XI_INTERPOLATIONS(:) !<The per-xi interpolations used by the basis.
    INTEGER(C_INT), INTENT(IN) :: COLLAPSE_INFO(:) !<The basis collapse info.
    INTEGER(C_INT), INTENT(OUT) :: EVALUATOR_HANDLE !<The evaluator handle for the basis.
    INTEGER(C_INT), INTENT(OUT) :: PARAMETERS_HANDLE !<The basis parameters argument evaluator.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.

    !Locals
    INTEGER(C_INT) :: XI_COUNT, FIRST_INTERPOLATION, I, IMPORT_INDEX
...
999 CALL ERRORS( "FIELDML_OUTPUT_GET_TP_BASIS_EVALUATOR", ERR, ERROR )
  ...
  FUNCTION FIELDML_OUTPUT_FIND_LAYOUT( CONNECTIVITY_INFO, LAYOUT_HANDLE )
    !Argument variables
    TYPE(CONNECTIVITY_INFO_TYPE), INTENT(IN) :: CONNECTIVITY_INFO(:) !<The connectivity info array to search.
    INTEGER(C_INT), INTENT(IN) :: LAYOUT_HANDLE !<The local node layout handle to search for.
  ...
  FUNCTION FIELDML_OUTPUT_FIND_BASIS( BASIS_INFO, BASIS )
    !Argument variables
  ...
  SUBROUTINE FIELDML_OUTPUT_GET_SIMPLE_LAYOUT_NAME( FML_HANDLE, LAYOUT_HANDLE, NAME, ERR, ERROR, * )
    !Argument variables
    INTEGER(C_INT), INTENT(IN) :: FML_HANDLE !<The FieldML session handle
    INTEGER(C_INT), INTENT(IN) :: LAYOUT_HANDLE !<The local node layout.
...
999 CALL ERRORS( "FIELDML_OUTPUT_GET_SIMPLE_BASIS_NAME", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_CREATE_BASIS_REFERENCE( FIELDML_INFO, BASE_NAME, BASIS_INFO, ERR, ERROR, * )
    ...
    !Locals
    INTEGER(C_INT) :: BASIS_TYPE, XI_COUNT, INTERPOLATION_PARAMETERS_HANDLE, HANDLE, EVALUATOR_HANDLE
    INTEGER(C_INT) :: VARIABLE_HANDLE, AGGREGATE_HANDLE, INDEX_EVALUATOR_HANDLE, FML_ERR
    ...
    IF( BASIS_TYPE == BASIS_LAGRANGE_HERMITE_TP_TYPE ) THEN
      ...
      INDEX_EVALUATOR_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, BASIS_INFO%LAYOUT_HANDLE, .TRUE. )
      ...
      VARIABLE_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, HANDLE, .FALSE. )
      ...
      CALL FIELDML_UTIL_GET_XI_TYPE( FIELDML_INFO%FML_HANDLE, XI_COUNT, .TRUE., HANDLE, ERR, ERROR, *999 )
      VARIABLE_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, HANDLE, .TRUE. )
      ...
      VARIABLE_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, INTERPOLATION_PARAMETERS_HANDLE, .TRUE. )
      FML_ERR = Fieldml_SetBind( FIELDML_INFO%FML_HANDLE, BASIS_INFO%REFERENCE_HANDLE, VARIABLE_HANDLE, &
        & AGGREGATE_HANDLE )
...
999 CALL ERRORS( "FIELDML_OUTPUT_CREATE_BASIS_REFERENCE", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_CREATE_LAYOUT_PARAMETERS( FIELDML_INFO, LAYOUT_HANDLE, COMPONENT_NAME, &
    ...
    INTEGER(C_INT) :: INDEX_HANDLE, FML_ERR
    ...
    INDEX_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, LAYOUT_HANDLE, .TRUE. )
...
999 CALL ERRORS( "FIELDML_OUTPUT_CREATE_LAYOUT_PARAMETERS", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_MESH_COMPONENT( FIELDML_INFO, BASE_NAME, COMPONENT_NUMBER, MESH_ELEMENTS, ERR, ERROR, * )
    ...
    !Locals
    INTEGER(C_INT) :: LAYOUT_HANDLE, CONNECTIVITY_HANDLE, ELEMENT_COUNT, DEFAULT_HANDLE, TEMPLATE_HANDLE, TYPE_HANDLE
    INTEGER(INTG) :: CONNECTIVITY_COUNT, BASIS_COUNT, I, J, LAYOUT_NODE_COUNT, IDX
    INTEGER(C_INT), ALLOCATABLE, TARGET :: IBUFFER(:)
    TYPE(BASIS_TYPE), POINTER :: BASIS
    INTEGER(C_INT) :: WRITER, SOURCE_HANDLE, WRITE_COUNT, FML_ERR, RESOURCE_HANDLE
    ...
    CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Cannot get element count for mesh "//BASE_NAME//".", FIELDML_INFO, ERR, ERROR, *999 )
    ...
    CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Cannot get node dofs FieldML type.", FIELDML_INFO, ERR, ERROR, *999 )
    ...
    DO I = 1, ELEMENT_COUNT
      ...
      CALL FIELDML_UTIL_GET_CONNECTIVITY_ENSEMBLE( FIELDML_INFO%FML_HANDLE, BASIS, LAYOUT_HANDLE, ERR, ERROR, *999 )
      
      IDX = -1
      IF( CONNECTIVITY_COUNT > 0 ) THEN
        IDX = FIELDML_OUTPUT_FIND_LAYOUT( CONNECTIVITY_INFO, LAYOUT_HANDLE )
      ENDIF
      ...
      IF( BASIS_COUNT == 0 ) THEN
        IDX = -1
      ELSE
        IDX = FIELDML_OUTPUT_FIND_BASIS( BASIS_INFO, BASIS )
      ENDIF
      IF( IDX == -1 ) THEN
        IF( BASIS_COUNT == 0 ) THEN
    ...
    DO I = 1, CONNECTIVITY_COUNT
      ...
      CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Cannot close connectivity data writer.", FIELDML_INFO, ERR, ERROR, *999 )
    ENDDO
    ...
    CALL LIST_ITEM_SET_C_INT( FIELDML_INFO%COMPONENT_HANDLES, COMPONENT_NUMBER, TEMPLATE_HANDLE, ERR, ERROR, *999 )
...
999 CALL ERRORS( "FIELDML_OUTPUT_ADD_MESH_COMPONENT", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELD_NODE_DOFS( FIELDML_INFO, BASE_NAME, TYPE_HANDLE, FIELD, FIELD_COMPONENT_NUMBERS, &
    ...
    TYPE(FIELDML_INFO_TYPE), INTENT(IN) :: FIELDML_INFO !<The FieldML parsing state.
    TYPE(VARYING_STRING), INTENT(IN) :: BASE_NAME !<The root name of the basis evaluator.
    INTEGER(C_INT), INTENT(IN) :: TYPE_HANDLE !<The FieldML type handle for the field.
    ...
    INTEGER(C_INT), INTENT(INOUT) :: NODE_DOFS_HANDLE !<The handle of the nodal dofs parameter evaluator.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.

    !Locals
    TYPE(MESH_TYPE), POINTER :: MESH
    INTEGER(C_INT) :: TYPE_COMPONENT_HANDLE, REAL_1D_HANDLE, NODE_COUNT, INDEX_HANDLE, RESOURCE_HANDLE, SOURCE_HANDLE
    ...
    INTEGER(C_INT) :: WRITER, WRITE_COUNT, FML_ERR
    ...
    CALL FIELDML_UTIL_GET_GENERIC_TYPE( FIELDML_INFO%FML_HANDLE, 1, REAL_1D_HANDLE, .TRUE., ERR, ERROR, *999 )
    ...
    IF( TYPE_COMPONENT_HANDLE /= FML_INVALID_HANDLE ) THEN
      TYPE_COMPONENT_HANDLE = FIELDML_UTIL_IMPORT_HANDLE( FIELDML_INFO%FML_HANDLE, TYPE_COMPONENT_HANDLE )
      INDEX_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, TYPE_COMPONENT_HANDLE, .TRUE. )
...
999 CALL ERRORS( "FIELDML_OUTPUT_ADD_FIELD_NODE_DOFS", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELD_ELEMENT_DOFS( FIELDML_INFO, BASE_NAME, TYPE_HANDLE, FIELD, FIELD_COMPONENT_NUMBERS, &
    ...
    INTEGER(INTG), INTENT(IN) :: FIELD_COMPONENT_NUMBERS(:) !<The field component numbers for which dof evaluators are to be created.
    INTEGER(INTG), INTENT(IN) :: VARIABLE_TYPE !<The OpenCMISS variable type to generate dofs for.
    INTEGER(C_INT), INTENT(INOUT) :: ELEMENT_DOFS_HANDLE !<The handle of the element dofs parameter evaluator.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.

    !Locals
    INTEGER(C_INT) :: TYPE_COMPONENT_HANDLE, REAL_1D_HANDLE, ELEMENT_COUNT, INDEX_HANDLE, RESOURCE_HANDLE, SOURCE_HANDLE
    ...
    IF( TYPE_COMPONENT_HANDLE /= FML_INVALID_HANDLE ) THEN
      ...
      CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Cannot add component index for element dofs parameter set "//BASE_NAME//&
        & ".dofs.element.", FIELDML_INFO, ERR, ERROR, *999 )
    ENDIF
...
999 CALL ERRORS( "FIELDML_OUTPUT_ADD_FIELD_ELEMENT_DOFS", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELDConstantDofs( FIELDML_INFO, BASE_NAME, TYPE_HANDLE, FIELD, FIELD_COMPONENT_NUMBERS, &
    ...
    INTEGER(INTG), INTENT(IN) :: FIELD_COMPONENT_NUMBERS(:) !<The field component numbers for which dof evaluators are to be created.
    INTEGER(INTG), INTENT(IN) :: VARIABLE_TYPE !<The OpenCMISS variable type to generate dofs for.
    INTEGER(C_INT), INTENT(INOUT) :: CONSTANT_DOFS_HANDLE !<The handle of the constant dofs parameter evaluator.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.

    !Locals
    INTEGER(C_INT) :: DOFTYPE_HANDLE, TYPE_TYPE, COMPONENT_TYPE, DATA_TYPE, INDEX_HANDLE, RESOURCE_HANDLE, SOURCE_HANDLE
    INTEGER(INTG) :: COMPONENT_COUNT, I, J, INTERPOLATION_TYPE
    INTEGER(INTG), ALLOCATABLE :: MESH_COMPONENT_NUMBERS(:)
    INTEGER(C_INT) :: WRITER, WRITE_COUNT, FML_ERR
    REAL(C_DOUBLE), ALLOCATABLE, TARGET :: DBUFFER(:)
    ...
    IF( TYPE_TYPE == FHT_ENSEMBLE_TYPE ) THEN
      ...
      IS_REAL = .FALSE.
    ELSE
      CALL FIELDML_UTIL_GET_GENERIC_TYPE( FIELDML_INFO%FML_HANDLE, 1, DOFTYPE_HANDLE, .TRUE., ERR, ERROR, *999 )
      COMPONENT_COUNT = Fieldml_GetTypeComponentCount( FIELDML_INFO%FML_HANDLE, TYPE_HANDLE )
    ...
    IF( COMPONENT_TYPE /= FML_INVALID_HANDLE ) THEN
      COMPONENT_TYPE = FIELDML_UTIL_IMPORT_HANDLE( FIELDML_INFO%FML_HANDLE, COMPONENT_TYPE )
      INDEX_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, COMPONENT_TYPE, .TRUE. )
...
999 CALL ERRORS( "FIELDML_OUTPUT_ADD_FIELDConstantDofs", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_INITIALISE_INFO( MESH, LOCATION, BASE_NAME, FIELDML_INFO, ERR, ERROR, * )
    ...
    INTEGER(C_INT) :: REAL_1D_HANDLE, XI_COMPONENT_HANDLE, FML_ERR
    ...
    CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Cannot create mesh type "//BASE_NAME//".mesh", FIELDML_INFO, ERR, ERROR, *999 )
...
999 CALL ERRORS( "FIELDML_OUTPUT_INITIALISE_INFO", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELD_COMPONENTS( FIELDML_INFO, TYPE_HANDLE, BASE_NAME, FIELD, FIELD_COMPONENT_NUMBERS, &
    ...
    !Locals
    TYPE(MESH_TYPE), POINTER :: MESH
    INTEGER(C_INT) :: FIELD_HANDLE, COMPONENT_HANDLE, NODAL_DOFS_HANDLE, ELEMENT_DOFS_HANDLE, CONSTANT_DOFS_HANDLE, INDEX_HANDLE
    INTEGER(INTG) :: COMPONENT_COUNT, I, MESH_COMPONENT_NUMBER, INTERPOLATION_TYPE, FML_ERR
    INTEGER(C_INT), ALLOCATABLE, TARGET :: COMPONENT_EVALUATORS(:)
    ...
    DO I = 1, COMPONENT_COUNT
      ...
      IF( INTERPOLATION_TYPE == FIELD_NODE_BASED_INTERPOLATION ) THEN
        ...
        CALL LIST_ITEM_GET_C_INT( FIELDML_INFO%COMPONENT_HANDLES, MESH_COMPONENT_NUMBER, COMPONENT_EVALUATORS( I ), &
          & ERR, ERROR, *999 )
    ...
    IF( COMPONENT_HANDLE /= FML_INVALID_HANDLE ) THEN
      ...
      CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Cannot create field aggregate evaluator "//BASE_NAME, &
        & FIELDML_INFO, ERR, ERROR, *999 )
      INDEX_HANDLE = FIELDML_UTIL_GET_TYPE_ARGUMENT_HANDLE( FIELDML_INFO, COMPONENT_HANDLE, .TRUE. )
      FML_ERR = Fieldml_SetIndexEvaluator( FIELDML_INFO%FML_HANDLE, FIELD_HANDLE, 1, INDEX_HANDLE )
      CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Cannot set index evaluator for aggregate evaluator "//BASE_NAME//".", FIELDML_INFO, &
        & ERR, ERROR, *999 )
...
999 CALL ERRORS( "FIELDML_OUTPUT_ADD_FIELD_NO_TYPE", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_ADD_FIELD_WITH_TYPE( FIELDML_INFO, BASE_NAME, FIELD, VARIABLE_TYPE, TYPE_HANDLE, ERR, ERROR, * )
    ...
    CALL FIELD_NUMBER_OF_COMPONENTS_GET( FIELD, FIELD_U_VARIABLE_TYPE, COMPONENT_COUNT, ERR, ERROR, *999 )
...
999 CALL ERRORS( "FIELDML_OUTPUT_ADD_FIELD_WITH_TYPE", ERR, ERROR )
  ...
  SUBROUTINE FIELDML_OUTPUT_CREATE_ENSEMBLE_TYPE( FIELDML_INFO, TYPE_NAME, ELEMENT_COUNT, TYPE_HANDLE, ERR, ERROR, * )
    !Argument variables
    TYPE(FIELDML_INFO_TYPE), INTENT(IN) :: FIELDML_INFO !<The FieldML parsing state.
    TYPE(VARYING_STRING), INTENT(IN) :: TYPE_NAME !<The name of the ensemble type.
    INTEGER(INTG), INTENT(IN) :: ELEMENT_COUNT !<The number of members in the ensemble.
    INTEGER(INTG), INTENT(OUT) :: TYPE_HANDLE !<The FieldML type handle for the ensemble.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.
    ...
    CALL ENTERS( "FIELDML_OUTPUT_CREATE_ENSEMBLE_TYPE", ERR, ERROR, *999 )

    TYPE_HANDLE = Fieldml_CreateEnsembleType( FIELDML_INFO%FML_HANDLE, cchar(TYPE_NAME) )
    CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Error creating ensemble type "//TYPE_NAME//".", FIELDML_INFO, ERR, ERROR, *999 )
    
    FML_ERR = Fieldml_SetEnsembleMembersRange( FIELDML_INFO%FML_HANDLE, TYPE_HANDLE, 1, ELEMENT_COUNT, 1 )
    CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Error setting ensemble type bounds for "//TYPE_NAME//".", &
      & FIELDML_INFO, ERR, ERROR, *999 )

    CALL EXITS( "FIELDML_OUTPUT_CREATE_ENSEMBLE_TYPE" )
    RETURN
999 CALL ERRORS( "FIELDML_OUTPUT_CREATE_ENSEMBLE_TYPE", ERR, ERROR )
    CALL EXITS( "FIELDML_OUTPUT_CREATE_ENSEMBLE_TYPE" )
    RETURN 1

  END SUBROUTINE FIELDML_OUTPUT_CREATE_ENSEMBLE_TYPE
  ...
  SUBROUTINE FIELDML_OUTPUT_CREATE_CONTINUOUS_TYPE( FIELDML_INFO, TYPE_NAME, COMPONENT_COUNT, TYPE_HANDLE, ERR, ERROR, * )
    !Argument variables
    TYPE(FIELDML_INFO_TYPE), INTENT(IN) :: FIELDML_INFO !<The FieldML parsing state.
    TYPE(VARYING_STRING), INTENT(IN) :: TYPE_NAME !<The name of the continuous type.
    INTEGER(INTG), INTENT(IN) :: COMPONENT_COUNT !<The number of components in the continuous type.
    INTEGER(INTG), INTENT(OUT) :: TYPE_HANDLE !<The FieldML type handle for the continuous type.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code.
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.

    !Local variables
    INTEGER(INTG) :: COMPONENT_HANDLE

    CALL ENTERS( "FIELDML_OUTPUT_CREATE_CONTINUOUS_TYPE", ERR, ERROR, *999 )

    COMPONENT_HANDLE = FML_INVALID_HANDLE

    TYPE_HANDLE = Fieldml_CreateContinuousType( FIELDML_INFO%FML_HANDLE, cchar(TYPE_NAME) )
    CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Error creating continuous type "//TYPE_NAME//".", FIELDML_INFO, ERR, ERROR, *999 )

    IF( COMPONENT_COUNT > 1 ) THEN
      COMPONENT_HANDLE = Fieldml_CreateContinuousTypeComponents( FIELDML_INFO%FML_HANDLE, TYPE_HANDLE, &
        & cchar(TYPE_NAME//".component"), COMPONENT_COUNT )
      CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Error creating component type for "//TYPE_NAME//".", &
        & FIELDML_INFO, ERR, ERROR, *999 )
    ENDIF

    CALL EXITS( "FIELDML_OUTPUT_CREATE_CONTINUOUS_TYPE" )
    RETURN
999 CALL ERRORS( "FIELDML_OUTPUT_CREATE_CONTINUOUS_TYPE", ERR, ERROR )
    CALL EXITS( "FIELDML_OUTPUT_CREATE_CONTINUOUS_TYPE" )
    RETURN 1

  END SUBROUTINE FIELDML_OUTPUT_CREATE_CONTINUOUS_TYPE
  ...
  SUBROUTINE FIELDML_OUTPUT_WRITE( FIELDML_INFO, FILENAME, ERR, ERROR, * )
    ...
    CALL FIELDML_UTIL_CHECK_FIELDML_ERROR( "Error writing fieldml file "//FILENAME//".", FIELDML_INFO, ERR, ERROR, *999 )
