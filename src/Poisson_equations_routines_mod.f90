            ...
            CASE(EQUATIONS_SET_FEM_SOLUTION_METHOD)
              ...
              IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_EXTRACELLULAR_BIDOMAIN_POISSON_SUBTYPE) THEN              
                ...
                CALL EQUATIONS_MAPPING_LINEAR_MATRICES_VARIABLE_TYPES_SET(EQUATIONS_MAPPING, &
                  & (/FIELD_U_VARIABLE_TYPE,FIELD_U_VARIABLE_TYPE/),ERR,ERROR,*999)
              ELSE
          ...
          DO ng=1,QUADRATURE_SCHEME%NUMBER_OF_GAUSS
            ...
            IF(SOURCE_VECTOR%UPDATE_VECTOR) THEN
              ...
              DO nh=1,FIELD_VARIABLE%NUMBER_OF_COMPONENTS
                DO ns=1,DEPENDENT_BASIS%NUMBER_OF_ELEMENT_PARAMETERS
                  nhs=nhs+1
                  !RHS_VECTOR%ELEMENT_VECTOR%VECTOR(nhs)=-V_m(nhs)
                  RHS_VECTOR%ELEMENT_VECTOR%VECTOR(nhs)=-1.0_DP
                ENDDO !ns
              ENDDO !nh
              
              ! matrix-vector product Ax=b: b_i = sum_k a_ik * x_k
              DO I=1,mhs
                SOURCE_VECTOR%ELEMENT_VECTOR%VECTOR(I)=0.0_DP
                DO K=1,nhs
                  SOURCE_VECTOR%ELEMENT_VECTOR%VECTOR(I)=SOURCE_VECTOR%ELEMENT_VECTOR%VECTOR(I)+ &
                    & RHS_MATRIX%ELEMENT_MATRIX%MATRIX(I,K)*RHS_VECTOR%ELEMENT_VECTOR%VECTOR(K)
                ENDDO !K
              ENDDO !I
