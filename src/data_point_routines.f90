...
999 CALL ERRORS("DATA_POINT_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE DATA_POINTS_CREATE_FINISH(DATA_POINTS,ERR,ERROR,*)
    ...
    IF(DIAGNOSTICS1) THEN !<TODO Still Diagnostics 1??
      ...
      DO np=1,DATA_POINTS%NUMBER_OF_DATA_POINTS
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"  Data Points = ",np,ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"    Global number    = ",DATA_POINTS%DATA_POINTS(np)%GLOBAL_NUMBER, &
          & ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"    User number      = ",DATA_POINTS%DATA_POINTS(np)%USER_NUMBER, &
          & ERR,ERROR,*999)
        CALL WRITE_STRING_VALUE(DIAGNOSTIC_OUTPUT_TYPE,"    Label            = ",DATA_POINTS%DATA_POINTS(np)%LABEL, &
          & ERR,ERROR,*999)
      ENDDO !np
...
999 CALL ERRORS("DATA_POINTS_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE DATA_POINTS_CREATE_START(REGION,NUMBER_OF_DATA_POINTS,DATA_POINTS,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: DUMMY_ERR,INSERT_STATUS,np,coord_idx
    ...
    IF(ASSOCIATED(REGION)) THEN
      ...
      ELSE
        ...
        ELSE
          ...
          DO np=1,REGION%DATA_POINTS%NUMBER_OF_DATA_POINTS
            REGION%DATA_POINTS%DATA_POINTS(np)%GLOBAL_NUMBER=np
            REGION%DATA_POINTS%DATA_POINTS(np)%USER_NUMBER=np
            REGION%DATA_POINTS%DATA_POINTS(np)%LABEL=""
            ! initialise data points values to 0.0 and weights to 1.0
            ALLOCATE(REGION%DATA_POINTS%DATA_POINTS(np)%VALUES(REGION%COORDINATE_SYSTEM%NUMBER_OF_DIMENSIONS),STAT=ERR)
            IF(ERR/=0) CALL FLAG_ERROR("Could not allocate data points data points values("//TRIM(NUMBER_TO_VSTRING &
              & (np,"*",ERR,ERROR))//").",ERR,ERROR,*999)
            ALLOCATE(REGION%DATA_POINTS%DATA_POINTS(np)%WEIGHTS(REGION%COORDINATE_SYSTEM%NUMBER_OF_DIMENSIONS),STAT=ERR)
            IF(ERR/=0) CALL FLAG_ERROR("Could not allocate data points data points weights("//TRIM(NUMBER_TO_VSTRING &
              & (np,"*",ERR,ERROR))//").",ERR,ERROR,*999)              
            DO coord_idx=1,SIZE(REGION%DATA_POINTS%DATA_POINTS(np)%VALUES,1)
              REGION%DATA_POINTS%DATA_POINTS(np)%VALUES(coord_idx)=0.0_DP
              REGION%DATA_POINTS%DATA_POINTS(np)%WEIGHTS(coord_idx)=1.0_DP
            ENDDO
            CALL TREE_ITEM_INSERT(REGION%DATA_POINTS%DATA_POINTS_TREE,np,np,INSERT_STATUS,ERR,ERROR,*999)
          ENDDO !np
          !Return the pointer        
  ...
  SUBROUTINE DATA_POINTS_FINALISE(DATA_POINTS,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DATA_POINTS)) THEN
      IF(ALLOCATED(DATA_POINTS%DATA_POINTS)) THEN
        DO np=1,SIZE(DATA_POINTS%DATA_POINTS,1)
          CALL DATA_POINT_FINALISE(DATA_POINTS%DATA_POINTS(np),ERR,ERROR,*999)
        ENDDO !np
