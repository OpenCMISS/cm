...
999 CALL ERRORS("DATA_POINTS_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE DATA_POINTS_CREATE_START_GENERIC(DATA_POINTS,NUMBER_OF_DATA_POINTS,NUMBER_OF_DIMENSIONS, &
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: INSERT_STATUS,data_point_idx,coord_idx,data_projection_idx
    ...
    IF(ASSOCIATED(DATA_POINTS)) THEN
      ...
      IF(ALLOCATED(DATA_POINTS%DATA_PROJECTIONS)) THEN
        DO data_projection_idx=1,SIZE(DATA_POINTS%DATA_PROJECTIONS,1)
          !CALL DATA_PROJECTION_FINALISE(DATA_POINTS%DATA_PROJECTIONS(data_projection_idx),ERR,ERROR,*999)
...
999 CALL ERRORS("DATA_POINTS_VALUES_GET",ERR,ERROR)    
  ...
  SUBROUTINE DATA_POINTS_VALUES_SET(DATA_POINTS,GLOBAL_NUMBER,VALUES,ERR,ERROR,*)
    ...
    !Local Variables
    TYPE(VARYING_STRING) :: LOCAL_ERROR
    REAL(DP) :: Y(3) !<The global number to set
    !REAL(DP) :: X(2) !<The global number to set
