...
MODULE EQUATIONS_SET_ROUTINES
  ...
  PUBLIC EQUATIONS_SET_FIXED_CONDITIONS_CREATE_START,EQUATIONS_SET_FIXED_CONDITIONS_CREATE_FINISH, &
    & EQUATIONS_SET_FIXED_CONDITIONS_DESTROY,EQUATIONS_SET_FIXED_CONDITIONS_SET_DOF
...
998 CALL ERRORS("EQUATIONS_SET_ANALYTIC_INITIALISE",ERR,ERROR)
  ...
  !>Assembles the equations stiffness matrix and rhs for a linear static equations using the finite element method.
  SUBROUTINE EQUATIONS_ASSEMBLE_LINEAR_STATIC_FEM(EQUATIONS_SET,ERR,ERROR,*)
    ...
    CALL ENTERS("EQUATIONS_ASSEMBLE_LINEAR_STATIC_FEM",ERR,ERROR,*999)
...
!!TODO: sort out combining boundary and ghost list
    ...
    CALL EXITS("EQUATIONS_ASSEMBLE_LINEAR_STATIC_FEM")
    RETURN
999 CALL ERRORS("EQUATIONS_ASSEMBLE_LINEAR_STATIC_FEM",ERR,ERROR)
    CALL EXITS("EQUATIONS_ASSEMBLE_LINEAR_STATIC_FEM")
    RETURN 1
  END SUBROUTINE EQUATIONS_ASSEMBLE_LINEAR_STATIC_FEM
        ...
        IF(EQUATIONS%OUTPUT_TYPE>=EQUATIONS_SET_ELEMENT_MATRIX_OUTPUT) THEN
          ...
          IF(ASSOCIATED(EQUATIONS_MATRICES)) THEN
            ...
            DO matrix_idx=1,EQUATIONS_MATRICES%NUMBER_OF_MATRICES
              ...
              CALL WRITE_STRING_VALUE(GENERAL_OUTPUT_TYPE,"  Update matrix = ",EQUATIONS_MATRICES%MATRICES(matrix_idx)% &
                & UPDATE_MATRIX,ERR,ERROR,*999)
              ELEMENT_MATRIX=>EQUATIONS_MATRICES%MATRICES(matrix_idx)%ELEMENT_MATRIX
...
999 CALL ERRORS("EQUATIONS_SET_FINITE_ELEMENT_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_INTERPOLATION_FINALISE(EQUATIONS_INTERPOLATION,ERR,ERROR,*)
    ...
    ELSE
      CALL FLAG_ERROR("Equations interpolation is not associated.",ERR,ERROR,*999)
    ENDIF
      ...
      ELSE
        IF(ASSOCIATED(EQUATIONS_SET%EQUATIONS)) THEN
          ...
          CASE(EQUATIONS_SET_FULL_MATRICES)
            EQUATIONS_SET%EQUATIONS%SPARSITY_TYPE=EQUATIONS_SET_FULL_MATRICES
          CASE DEFAULT
