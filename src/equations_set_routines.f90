!> \file
!> $Id: equations_set_routines.f90 28 2007-07-27 08:35:14Z cpb $
...
MODULE EQUATIONS_SET_ROUTINES
  ...
  PUBLIC EQUATIONS_SET_ANALYTIC_CREATE_START,EQUATIONS_SET_ANALYTIC_CREATE_FINISH,EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET
...
998 CALL ERRORS("EQUATIONS_SET_ANALYTIC_CREATE_START",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET(EQUATIONS_SET,EQUATION_NUMBER,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_SET_TYPE), POINTER :: EQUATIONS_SET !<A pointer to the equations set to set the number
    INTEGER(INTG), INTENT(IN) :: EQUATION_NUMBER !<The equation number to set 
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
 
    CALL ENTERS("EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET",ERR,ERROR,*999)

    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(EQUATIONS_SET%ANALYTIC%ANALYTIC_FINISHED) THEN
        CALL FLAG_ERROR("Equations set analytic has been finished",ERR,ERROR,*999)
      ELSE
        EQUATIONS_SET%ANALYTIC%EQUATION_NUMBER=EQUATION_NUMBER
      ENDIF
    ELSE
      CALL FLAG_ERROR("Equations set is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET")
    RETURN
999 CALL ERRORS("EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET",ERR,ERROR)
    CALL EXITS("EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET")
    RETURN 1
  END SUBROUTINE EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET
          ...
          CASE(EQUATIONS_SET_LINEAR)
            ...
            CASE(EQUATIONS_SET_STATIC)
              ...
              CASE(EQUATIONS_SET_FEM_SOLUTION_METHOD)
                CALL EQUATIONS_SET_ASSEMBLE_LINEAR_STATIC_FEM(EQUATIONS_SET,ERR,ERROR,*999)
              ...
              END SELECT
            CASE(EQUATIONS_SET_DYNAMIC,EQUATIONS_SET_QUASISTATIC)
              CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
            CASE DEFAULT
              ...
              CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
            END SELECT
          CASE(EQUATIONS_SET_NONLINEAR,EQUATIONS_SET_NONLINEAR_BCS)
            CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
          CASE DEFAULT
...
999 CALL ERRORS("EQUATIONS_SET_ASSEMBLE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_SET_ASSEMBLE_LINEAR_STATIC_FEM(EQUATIONS_SET,ERR,ERROR,*)
    ...
    CALL ENTERS("EQUATIONS_SET_ASSEMBLE_LINEAR_STATIC_FEM",ERR,ERROR,*999)
...
!!TODO: sort out combining boundary and ghost list
    ...
    CALL EXITS("EQUATIONS_SET_ASSEMBLE_LINEAR_STATIC_FEM")
    RETURN
999 CALL ERRORS("EQUATIONS_SET_ASSEMBLE_LINEAR_STATIC_FEM",ERR,ERROR)
    CALL EXITS("EQUATIONS_SET_ASSEMBLE_LINEAR_STATIC_FEM")
    RETURN 1
  ...
  SUBROUTINE EQUATIONS_SET_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      CASE(EQUATIONS_SET_CLASSICAL_FIELD_CLASS)
        CALL CLASSICAL_FIELD_EQUATIONS_SET_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*999)
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        IF(EQUATIONS%OUTPUT_TYPE>=EQUATIONS_SET_ELEMENT_MATRIX_OUTPUT) THEN
          ...
          IF(ASSOCIATED(EQUATIONS_MATRICES)) THEN
            ...
            CALL WRITE_STRING_VALUE(GENERAL_OUTPUT_TYPE,"Number of element matrices = ",EQUATIONS_MATRICES%NUMBER_OF_MATRICES, &
              & ERR,ERROR,*999)
            DO matrix_idx=1,EQUATIONS_MATRICES%NUMBER_OF_MATRICES
              ...
              CALL WRITE_STRING_VALUE(GENERAL_OUTPUT_TYPE,"  Update matrix = ",EQUATIONS_MATRICES%MATRICES(matrix_idx)%PTR% &
                & UPDATE_MATRIX,ERR,ERROR,*999)
              ELEMENT_MATRIX=>EQUATIONS_MATRICES%MATRICES(matrix_idx)%PTR%ELEMENT_MATRIX
            ...
            CALL WRITE_STRING_VALUE(GENERAL_OUTPUT_TYPE,"  Update vector = ",EQUATIONS_MATRICES%UPDATE_VECTOR,ERR,ERROR,*999)
            ELEMENT_VECTOR=>EQUATIONS_MATRICES%ELEMENT_VECTOR
...
999 CALL ERRORS("EQUATIONS_SET_FINITE_ELEMENT_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_INTERPOLATION_FINALISE(EQUATIONS_INTERPOLATION,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_INTERPOLATION)) THEN
      ...
      CALL FIELD_INTERPOLATION_PARAMETERS_FINALISE(EQUATIONS_INTERPOLATION%MATERIAL_INTERP_PARAMETERS,ERR,ERROR,*999)
      ...
      CALL FIELD_INTERPOLATED_POINT_FINALISE(EQUATIONS_INTERPOLATION%MATERIAL_INTERP_POINT,ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(EQUATIONS)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS_SET)) THEN
        ...
        ELSE
          ...
          NULLIFY(EQUATIONS%INTERPOLATION%MATERIAL_INTERP_PARAMETERS)
          ...
          NULLIFY(EQUATIONS%INTERPOLATION%MATERIAL_INTERP_POINT)
          ...
          IF(ASSOCIATED(EQUATIONS_SET%MATERIALS)) THEN
            EQUATIONS%INTERPOLATION%MATERIAL_FIELD=>EQUATIONS_SET%MATERIALS%MATERIAL_FIELD
          ELSE
            NULLIFY(EQUATIONS%INTERPOLATION%MATERIAL_FIELD)
          ENDIF
          ...
          ENDIF
          IF(ASSOCIATED(EQUATIONS%INTERPOLATION%MATERIAL_FIELD)) THEN
            CALL FIELD_INTERPOLATION_PARAMETERS_INITIALISE(EQUATIONS%INTERPOLATION%MATERIAL_FIELD, &
              & FIELD_STANDARD_VARIABLE_TYPE,EQUATIONS%INTERPOLATION%MATERIAL_INTERP_PARAMETERS,ERR,ERROR,*999)
            CALL FIELD_INTERPOLATED_POINT_INITIALISE(EQUATIONS%INTERPOLATION%MATERIAL_INTERP_PARAMETERS,  &
              & EQUATIONS%INTERPOLATION%MATERIAL_INTERP_POINT,ERR,ERROR,*999)
          ENDIF
  ...
  SUBROUTINE EQUATIONS_SET_MATERIALS_COMPONENT_INTERPOLATION_SET(EQUATIONS_SET,COMPONENT_NUMBER,INTERPOLATION_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(ASSOCIATED(EQUATIONS_SET%MATERIALS)) THEN
        ...
        ELSE
          CALL FIELD_COMPONENT_INTERPOLATION_SET(EQUATIONS_SET%MATERIALS%MATERIAL_FIELD,FIELD_STANDARD_VARIABLE_TYPE, &
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(ASSOCIATED(EQUATIONS_SET%MATERIALS)) THEN
        IF(EQUATIONS_SET%MATERIALS%MATERIALS_FINISHED) THEN
          CALL FLAG_ERROR("Equations set materials has been finished",ERR,ERROR,*999)
        ELSE
          CALL FIELD_COMPONENT_MESH_COMPONENT_SET(EQUATIONS_SET%MATERIALS%MATERIAL_FIELD,FIELD_STANDARD_VARIABLE_TYPE, &
...
999 CALL ERRORS("EQUATIONS_SET_MATERIALS_DESTROY",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_SET_MATERIALS_FINALISE(EQUATIONS_SET_MATERIALS,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET_MATERIALS)) THEN
      IF(ASSOCIATED(EQUATIONS_SET_MATERIALS%MATERIAL_FIELD))  &
        & CALL FIELD_DESTROY(EQUATIONS_SET_MATERIALS%MATERIAL_FIELD,ERR,ERROR,*999)
  ...
  SUBROUTINE EQUATIONS_SET_MATERIALS_INITIALISE(EQUATIONS_SET,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      ELSE
        ...
        NULLIFY(EQUATIONS_SET%MATERIALS%MATERIAL_FIELD)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Equations set is not associated",ERR,ERROR,*998)
    ENDIF
  ...
  SUBROUTINE EQUATIONS_SET_MATERIALS_SCALING_SET(EQUATIONS_SET,SCALING_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      IF(ASSOCIATED(EQUATIONS_SET%MATERIALS)) THEN
        ...
        ELSE
          CALL FIELD_SCALING_TYPE_SET(EQUATIONS_SET%MATERIALS%MATERIAL_FIELD,SCALING_TYPE,ERR,ERROR,*999)
        ENDIF
      ELSE
        CALL FLAG_ERROR("Equations set materials is not associated.",ERR,ERROR,*999)
      ENDIF
