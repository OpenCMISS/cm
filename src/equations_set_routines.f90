...
MODULE EQUATIONS_SET_ROUTINES
  ...
  PUBLIC EQUATIONS_SET_EQUATIONS_CREATE_START,EQUATIONS_SET_EQUATIONS_CREATE_FINISH, EQUATIONS_SET_EQUATIONS_LUMPING_SET, &
...
999 CALL ERRORS("EQUATIONS_INTERPOLATION_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_SET_INITIALISE(EQUATIONS_SET,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      EQUATIONS_SET%TIME_DEPENDENCE=0
...
999 CALL ERRORS("EQUATIONS_SET_DEPENDENT_SCALING_SET",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_SET_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      CASE(EQUATIONS_SET_BIOELECTRICS_CLASS)
        CALL BIOELECTRIC_EQUATIONS_SET_SETUP(EQUATIONS_SET,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*999)
      CASE(EQUATIONS_SET_MODAL_CLASS)
        CALL FLAG_ERROR("Not implemented.",ERR,ERROR,*999)
      CASE DEFAULT
        LOCAL_ERROR="Equations set class "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET%CLASS,"*",ERR,ERROR))//" is not valid."
...
999 CALL ERRORS("EQUATIONS_SET_EQUATIONS_FINALISE",ERR,ERROR)
  ...
  !>Sets/changes the matrix lumping for the equations set.
  SUBROUTINE EQUATIONS_SET_EQUATIONS_LUMPING_SET(EQUATIONS_SET,LUMPING_TYPE,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_SET_TYPE), POINTER :: EQUATIONS_SET !<A pointer to the equations to set the lumping for
    INTEGER(INTG), INTENT(IN) :: LUMPING_TYPE !<The lumping type to set \see EQUATIONS_SET_CONSTANTS_LumpingTypes,EQUATIONS_SET_CONSTANTS
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    TYPE(EQUATIONS_TYPE), POINTER :: EQUATIONS
    ...
    CALL ENTERS("EQUATIONS_SET_EQUATIONS_LUMPING_SET",ERR,ERROR,*999)
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        ...
        ELSE
          IF(EQUATIONS_SET%TIME_DEPENDENCE==EQUATIONS_SET_FIRST_ORDER_DYNAMIC.OR. &
            ...
            CASE(EQUATIONS_SET_UNLUMPED_MATRICES)
              EQUATIONS%LUMPING_TYPE=EQUATIONS_SET_UNLUMPED_MATRICES
            CASE(EQUATIONS_SET_LUMPED_MATRICES)
              EQUATIONS%LUMPING_TYPE=EQUATIONS_SET_LUMPED_MATRICES
            CASE DEFAULT
            ...
            END SELECT
          ELSE
            LOCAL_ERROR="Invalid equations set time dependence. The equations set time dependence of "// &
              & TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET%TIME_DEPENDENCE,"*",ERR,ERROR))// &
              & " does not correspond to a dynamic equation set. You can only set lumping for a dynamic equations set."
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          ENDIF
        ENDIF
      ELSE
    ...
    CALL EXITS("EQUATIONS_SET_EQUATIONS_LUMPING_SET")
    RETURN
999 CALL ERRORS("EQUATIONS_SET_EQUATIONS_LUMPING_SET",ERR,ERROR)
    CALL EXITS("EQUATIONS_SET_EQUATIONS_LUMPING_SET")
    RETURN 1
  END SUBROUTINE EQUATIONS_SET_EQUATIONS_LUMPING_SET
  ...
  SUBROUTINE EQUATIONS_SET_EQUATIONS_INITIALISE(EQUATIONS_SET,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      ELSE
        ...
        EQUATIONS_SET%EQUATIONS%LUMPING_TYPE=EQUATIONS_SET_UNLUMPED_MATRICES
