...
CONTAINS
  ...
  SUBROUTINE EQUATIONS_JACOBIAN_FINALISE(EQUATIONS_JACOBIAN,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_JACOBIAN)) THEN
      CALL DISTRIBUTED_MATRIX_DESTROY(EQUATIONS_JACOBIAN%JACOBIAN,ERR,ERROR,*999)
...
999 CALL ERRORS("EQUATIONS_JACOBIAN_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_JACOBIAN_INITIALISE(EQUATIONS_MATRICES,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_MATRICES_TYPE), POINTER :: EQUATIONS_MATRICES !<A pointer to the equations matrices to initialise the equations Jacobian for
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    ...
    IF(ASSOCIATED(EQUATIONS_MATRICES)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS_MAPPING)) THEN
        IF(ASSOCIATED(EQUATIONS_MATRICES%JACOBIAN)) THEN
          CALL FLAG_ERROR("Equations Jacobian is already associated.",ERR,ERROR,*998)
        ELSE
          ALLOCATE(EQUATIONS_MATRICES%JACOBIAN,STAT=ERR)
          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations Jacobian.",ERR,ERROR,*999)
          EQUATIONS_JACOBIAN=>EQUATIONS_MATRICES%JACOBIAN
          EQUATIONS_JACOBIAN%EQUATIONS_MATRICES=>EQUATIONS_MATRICES
          EQUATIONS_JACOBIAN%STORAGE_TYPE=MATRIX_BLOCK_STORAGE_TYPE
          EQUATIONS_JACOBIAN%STRUCTURE_TYPE=EQUATIONS_MATRIX_NO_STRUCTURE
          EQUATIONS_JACOBIAN%UPDATE_JACOBIAN=.TRUE.
          !EQUATIONS_MATRIX%NUMBER_OF_COLUMNS=EQUATIONS_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(MATRIX_NUMBER)%NUMBER_OF_COLUMNS
          !EQUATIONS_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(MATRIX_NUMBER)%EQUATIONS_MATRIX=>EQUATIONS_MATRIX
          NULLIFY(EQUATIONS_JACOBIAN%JACOBIAN)
          CALL EQUATIONS_MATRICES_ELEMENT_MATRIX_INITIALISE(EQUATIONS_JACOBIAN%ELEMENT_JACOBIAN,ERR,ERROR,*999)
          CALL EQUATIONS_MATRICES_ELEMENT_VECTOR_INITIALISE(EQUATIONS_JACOBIAN%ELEMENT_RESIDUAL,ERR,ERROR,*999)
        ENDIF
      ELSE
    ...
    RETURN
999 CALL EQUATIONS_JACOBIAN_FINALISE(EQUATIONS_MATRICES%JACOBIAN,DUMMY_ERR,DUMMY_ERROR,*998)
        ...
        IF(ASSOCIATED(EQUATIONS_MAPPING)) THEN
          ...
          IF(ASSOCIATED(ROW_DOMAIN_MAP)) THEN
            DO matrix_idx=1,EQUATIONS_MATRICES%NUMBER_OF_MATRICES
              EQUATIONS_MATRIX=>EQUATIONS_MATRICES%MATRICES(matrix_idx)%PTR
              IF(ASSOCIATED(EQUATIONS_MATRIX)) THEN
                COLUMN_DOMAIN_MAP=>EQUATIONS_MAPPING%EQUATIONS_MATRIX_TO_VARIABLE_MAPS(matrix_idx)%COLUMN_DOFS_MAPPING
                IF(ASSOCIATED(COLUMN_DOMAIN_MAP)) THEN
                  ...
                  IF(EQUATIONS_MATRIX%STORAGE_TYPE/=MATRIX_BLOCK_STORAGE_TYPE) THEN
                    ...
                    CALL DISTRIBUTED_MATRIX_STORAGE_LOCATIONS_SET(EQUATIONS_MATRIX%MATRIX,ROW_INDICES,COLUMN_INDICES,ERR,ERROR,*999)
            ...
            CALL DISTRIBUTED_VECTOR_CREATE_START(ROW_DOMAIN_MAP,EQUATIONS_MATRICES%VECTOR,ERR,ERROR,*999)
  ...
  SUBROUTINE EQUATIONS_MATRIX_INITIALISE(EQUATIONS_MATRICES,MATRIX_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(EQUATIONS_MATRICES)) THEN
      IF(MATRIX_NUMBER>0.AND.MATRIX_NUMBER<=EQUATIONS_MATRICES%NUMBER_OF_MATRICES) THEN
        EQUATIONS_MAPPING=>EQUATIONS_MATRICES%EQUATIONS_MAPPING
        IF(ASSOCIATED(EQUATIONS_MAPPING)) THEN
          IF(ASSOCIATED(EQUATIONS_MATRICES%MATRICES(MATRIX_NUMBER)%PTR)) THEN
            CALL FLAG_ERROR("Equations matrix is already associated.",ERR,ERROR,*998)
          ELSE
            ALLOCATE(EQUATIONS_MATRICES%MATRICES(MATRIX_NUMBER)%PTR,STAT=ERR)
            IF(ERR/=0) CALL FLAG_ERROR("Could not allocate equations matrix.",ERR,ERROR,*999)
            EQUATIONS_MATRIX=>EQUATIONS_MATRICES%MATRICES(MATRIX_NUMBER)%PTR
            EQUATIONS_MATRIX%MATRIX_NUMBER=MATRIX_NUMBER
            EQUATIONS_MATRIX%EQUATIONS_MATRICES=>EQUATIONS_MATRICES
    ...
    RETURN
999 CALL EQUATIONS_MATRIX_FINALISE(EQUATIONS_MATRICES%MATRICES(MATRIX_NUMBER)%PTR,DUMMY_ERR,DUMMY_ERROR,*998)
    ...
    IF(ASSOCIATED(EQUATIONS_MATRICES)) THEN
      IF(EQUATIONS_MATRICES%EQUATIONS_MATRICES_FINISHED) THEN
        ...
        DO matrix_idx=1,EQUATIONS_MATRICES%NUMBER_OF_MATRICES
          ...
          ENDIF
        ENDDO !matrix_idx
        IF(ASSOCIATED(EQUATIONS_MATRICES%VECTOR)) THEN
    ...
    IF(ASSOCIATED(EQUATIONS_MATRICES)) THEN
      IF(ALLOCATED(EQUATIONS_MATRICES%MATRICES)) THEN
        DO matrix_idx=1,SIZE(EQUATIONS_MATRICES%MATRICES,1)
          CALL EQUATIONS_MATRIX_FINALISE(EQUATIONS_MATRICES%MATRICES(matrix_idx)%PTR,ERR,ERROR,*999)
        ENDDO !matrix_idx
        DEALLOCATE(EQUATIONS_MATRICES%MATRICES)
      ENDIF
      CALL EQUATIONS_JACOBIAN_FINALISE(EQUATIONS_MATRICES%JACOBIAN,ERR,ERROR,*999)
      !CALL DISTRIBUTED_VECTOR_DESTROY(EQUATIONS_MATRICES%RESIDUAL,ERR,ERROR,*999)   !??? kmith
      CALL EQUATIONS_MATRICES_ELEMENT_VECTOR_FINALISE(EQUATIONS_MATRICES%ELEMENT_RESIDUAL,ERR,ERROR,*999)
        ...
        IF(ASSOCIATED(EQUATIONS_MAPPING)) THEN
          IF(EQUATIONS_MAPPING%EQUATIONS_MAPPING_FINISHED) THEN
            ...
            DO matrix_idx=1,EQUATIONS_MAPPING%NUMBER_OF_EQUATIONS_MATRICES
              NULLIFY(EQUATIONS%EQUATIONS_MATRICES%MATRICES(matrix_idx)%PTR)
              CALL EQUATIONS_MATRIX_INITIALISE(EQUATIONS%EQUATIONS_MATRICES,matrix_idx,ERR,ERROR,*999)
            ENDDO !matrix_idx
            NULLIFY(EQUATIONS%EQUATIONS_MATRICES%JACOBIAN)
            EQUATIONS%EQUATIONS_MATRICES%UPDATE_RESIDUAL=.TRUE.
            NULLIFY(EQUATIONS%EQUATIONS_MATRICES%RESIDUAL)
            CALL EQUATIONS_MATRICES_ELEMENT_VECTOR_INITIALISE(EQUATIONS%EQUATIONS_MATRICES%ELEMENT_RESIDUAL,ERR,ERROR,*999)
