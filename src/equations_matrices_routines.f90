...
MODULE EQUATIONS_MATRICES_ROUTINES
 ...
 !>@}
  ...
  PUBLIC EQUATIONS_MATRICES_LINEAR_LUMPING_TYPE_SET,EQUATIONS_MATRICES_LINEAR_STORAGE_TYPE_SET, &
    & EQUATIONS_MATRICES_LINEAR_STRUCTURE_TYPE_SET,EQUATIONS_MATRICES_NONLINEAR_STORAGE_TYPE_SET, &
    & EQUATIONS_MATRICES_NONLINEAR_STRUCTURE_TYPE_SET
...
998 CALL ERRORS("EQUATIONS_MATRICES_SOURCE_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE EQUATIONS_MATRICES_LINEAR_LUMPING_TYPE_SET(EQUATIONS_MATRICES,LUMPING_TYPE,ERR,ERROR,*)

    !Argument variables
    TYPE(EQUATIONS_MATRICES_TYPE), POINTER :: EQUATIONS_MATRICES !<A pointer to the eqautions matrices
    INTEGER(INTG), INTENT(IN) :: LUMPING_TYPE(:) !<LUMPING_TYPE(matrix_idx). The lumping type for the matrix_idx'th equations matrix
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: matrix_idx
    ...
    CALL ENTERS("EQUATIONS_MATRICES_LINEAR_LUMPING_TYPE_SET",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(EQUATIONS_MATRICES)) THEN
      ...
      ELSE
        LINEAR_MATRICES=>EQUATIONS_MATRICES%LINEAR_MATRICES
        IF(ASSOCIATED(LINEAR_MATRICES)) THEN
          IF(SIZE(LUMPING_TYPE,1)==LINEAR_MATRICES%NUMBER_OF_LINEAR_MATRICES) THEN
            DO matrix_idx=1,LINEAR_MATRICES%NUMBER_OF_LINEAR_MATRICES
              EQUATIONS_MATRIX=>LINEAR_MATRICES%MATRICES(matrix_idx)%PTR
              IF(ASSOCIATED(EQUATIONS_MATRIX)) THEN
    ...
    CALL EXITS("EQUATIONS_MATRICES_LINEAR_LUMPING_TYPE_SET")
    RETURN
999 CALL ERRORS("EQUATIONS_MATRICES_LINEAR_LUMPING_TYPE_SET",ERR,ERROR)
    CALL EXITS("EQUATIONS_MATRICES_LINEAR_LUMPING_TYPE_SET")
    RETURN 1
  END SUBROUTINE EQUATIONS_MATRICES_LINEAR_LUMPING_TYPE_SET
