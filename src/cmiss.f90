...
999 RETURN
  ...
  SUBROUTINE CMISS_HANDLE_ERROR(ERR,ERROR)
    ...
    TYPE(VARYING_STRING), INTENT(INOUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: MPI_IERROR, ORIGINAL_ERROR
    ...
    CASE(CMISS_OUTPUT_ERROR)
      CALL WRITE_ERROR(ERR,ERROR,*999)
    CASE(CMISS_TRAP_ERROR)
      ORIGINAL_ERROR=ERR !Calling WRITE_ERROR resets ERR to zero, but we want to exit with a non-zero status
      CALL WRITE_ERROR(ERR,ERROR,*999)
      CALL MPI_ABORT(MPI_COMM_WORLD,ORIGINAL_ERROR,MPI_IERROR)
      STOP
    CASE DEFAULT
      !Do nothing
    END SELECT
