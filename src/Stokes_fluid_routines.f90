...
!, STOKES_FLUID_FINITE_ELEMENT_CALCULATE
...
!!MERGE: check
  ...
  !>Calculates the element stiffness matrices and RHS for a Stokes fluid finite element equations set. \todo for regular mesh only
  SUBROUTINE STOKES_FLUID_ANALYTIC_CALCULATE(FIELD,ANALYTIC_FUNCTION,GEOMETRIC_PARAMETERS,DERIVATIVE_NUMBER,NODE_NUMBER, &
    ...
    CALL ENTERS("STOKES_FLUID_ANALYTIC_CALCULATE",ERR,ERROR,*999)
...
! ! !           CASE(EQUATIONS_SET_STOKES_FLUID_TWO_DIM_1)
...
! ! !             END SELECT
! ! !           CASE(EQUATIONS_SET_STOKES_FLUID_TWO_DIM_2)
! ! !             !TODO Analytical calculation for regular mesh only, need to implement generic analytical calculation,
! ! !             !i.e. du/ds1=(du/dxi1)(dxi1/ds1)=(du/dx)*(dx/dxi1)*(dxi1/ds1),du/ds2=(du/dxi2)(dxi2/ds2)=(du/dx)*(dx/dxi2)*(dxi2/ds2)
! ! !             SELECT CASE(DERIVATIVE_NUMBER)
! ! !             CASE(NO_GLOBAL_DERIV)
...
! ! !         CASE(FIELD_U_VARIABLE_TYPE)
! ! !           SELECT CASE(ANALYTIC_FUNCTION)
! ! !           CASE(EQUATIONS_SET_STOKES_FLUID_THREE_DIM_1)
! ! !             !TODO Analytical calculation for regular mesh only, need to implement generic analytical calculation            
! ! !             SELECT CASE(DERIVATIVE_NUMBER)
! ! !             CASE(NO_GLOBAL_DERIV)
! ! !               VALUE=x**2-2*y**2+z**2
...
! ! !               CALL FLAG_ERROR("The derivativehas not been implemented.",ERR,ERROR,*999)
! ! !             END SELECT
! ! !           CASE(EQUATIONS_SET_STOKES_FLUID_THREE_DIM_2) 
! ! !             !TODO Analytical calculation for regular mesh only, need to implement generic analytical calculation 
! ! !             SELECT CASE(DERIVATIVE_NUMBER)
! ! !             CASE(NO_GLOBAL_DERIV)
...
! ! !     ENDIF
! ! !     
    CALL EXITS("STOKES_FLUID_ANALYTIC_CALCULATE")
    RETURN
999 CALL ERRORS("STOKES_FLUID_ANALYTIC_CALCULATE",ERR,ERROR)
    CALL EXITS("STOKES_FLUID_ANALYTIC_CALCULATE")
    RETURN 1
  END SUBROUTINE STOKES_FLUID_ANALYTIC_CALCULATE
...
!!MERGE: what is this
  ...
  SUBROUTINE STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE(EQUATIONS_SET,GEOMETRIC_PARAMETERS,NODE_TYPE,ERR,ERROR,*)
    ...
    CALL ENTERS("STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE",ERR,ERROR,*999)
          ...
          IF(ASSOCIATED(DOMAIN_NODES)) THEN
            ...
            DO node_idx=NODES_MAPPING%INTERNAL_START,NODES_MAPPING%BOUNDARY_FINISH
              ...
              DO dev_idx=1,DOMAIN_NODES%NODES(node_number)%NUMBER_OF_DERIVATIVES
                CALL STOKES_FLUID_ANALYTIC_CALCULATE(FIELD,ANALYTIC_FUNCTION,GEOMETRIC_PARAMETERS,dev_idx,node_number, &
    ...
    CALL EXITS("STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE")
    RETURN
999 CALL ERRORS("STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE",ERR,ERROR)
    CALL EXITS("STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE")
    RETURN 1
  END SUBROUTINE STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE
...
! ! !                 EQUATIONS_SET%ANALYTIC%ANALYTIC_FUNCTION=EQUATIONS_SET_STOKES_FLUID_THREE_DIM_1
...
! ! !               CALL STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE(EQUATIONS_SET,GEOMETRIC_PARAMETERS,DOMAIN_LOCAL_BOUNDARY, &
! ! !                 & ERR,ERROR,*999)
! ! !               
! ! !               CALL FIELD_PARAMETER_SET_UPDATE_START(DEPENDENT_FIELD,FIELD_ANALYTIC_VALUES_SET_TYPE,ERR,ERROR,*999)
! ! !               
! ! !               ! Set up internal nodes
! ! !               CALL STOKES_FLUID_ANALYTIC_PARAMETER_SET_UPDATE(EQUATIONS_SET,GEOMETRIC_PARAMETERS,DOMAIN_LOCAL_INTERNAL, &
! ! !                 & ERR,ERROR,*999)
! ! !               
