      ...
      ELSE
        IF(SOLVER%SOLVERTYPE==SOLVER_LINEAR_TYPE) THEN
          IF(ASSOCIATED(SOLVER%LINEAR_SOLVER)) THEN
            IF(SOLVER%LINEAR_SOLVER%LINEAR_SOLVER_TYPE==SOLVER_LINEAR_DIRECT_SOLVE) THEN
        ...
        IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
          ...
          IF(ASSOCIATED(SOLUTION_MAPPING)) THEN
            IF(SOLUTION_MAPPING%NUMBER_OF_SOLVER_MATRICES==1) THEN
              ...
              SOLVER%LINEAR_SOLVER%LINEAR_SOLVER_TYPE=SOLVER_LINEAR_ITERATIVE_SOLVE
  ...
  SUBROUTINE SOLVER_LINEAR_ITERATIVE_ABSOLUTE_TOLERANCE_SET(SOLVER,ABSOLUTE_TOLERANCE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(SOLVER)) THEN
      ...
      ELSE
        IF(SOLVER%SOLVERTYPE==SOLVER_LINEAR_TYPE) THEN
          IF(ASSOCIATED(SOLVER%LINEAR_SOLVER)) THEN
            IF(SOLVER%LINEAR_SOLVER%LINEAR_SOLVER_TYPE==SOLVER_LINEAR_ITERATIVE_SOLVE) THEN
...
999 CALL ERRORS("SOLVER_SOLVE",ERR,ERROR)    
  ...
  SUBROUTINE SOLVER_SOLVE_EIGENPROBLEM(EIGENPROBLEM_SOLVER,ERR,ERROR,*)
    ...
    CALL ENTERS("SOLVER_SOLVE_EIGENPROBLEM",ERR,ERROR,*999)
    ...
    CALL EXITS("SOLVER_SOLVE_EIGENPROBLEM")
    RETURN
999 CALL ERRORS("SOLVER_SOLVE_EIGENPROBLEM",ERR,ERROR)    
    CALL EXITS("SOLVER_SOLVE_EIGENPROBLEM")
    RETURN 1
   
  END SUBROUTINE SOLVER_SOLVE_EIGENPROBLEM
