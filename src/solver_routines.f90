...
CONTAINS
  ...
  SUBROUTINE SOLVER_CREATE_FINISH(PROBLEM_SOLVER,ERR,ERROR,*)
    ...
    RETURN
999 IF(ASSOCIATED(PROBLEM_SOLVER)) THEN
      IF(ASSOCIATED(PROBLEM_SOLVER%PROBLEM_SOLUTION)) &
        & CALL SOLVER_FINALISE(PROBLEM_SOLVER%PROBLEM_SOLUTION,DUMMY_ERR,DUMMY_ERROR,*998)
    ENDIF
...
999 CALL ERRORS("SOLVER_LINEAR_ITERATIVE_DIVERGENCE_TOLERANCE_SET",ERR,ERROR)    
  ...
  SUBROUTINE SOLVER_LINEAR_ITERATIVE_FINALISE(LINEAR_ITERATIVE_SOLVER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(LINEAR_ITERATIVE_SOLVER)) THEN
      CALL PETSC_KSPDESTROY(LINEAR_ITERATIVE_SOLVER%KSP,ERR,ERROR,*999)
        ...
        IF(PROBLEM_SOLVER%SOLVER_TYPE==SOLVER_LINEAR_TYPE) THEN
          IF(ASSOCIATED(PROBLEM_SOLVER%LINEAR_SOLVER)) THEN
            IF(LINEAR_SOLVER_TYPE/=PROBLEM_SOLVER%LINEAR_SOLVER%LINEAR_SOLVER_TYPE) THEN
              ...
              CASE(SOLVER_LINEAR_DIRECT_SOLVE)
                CALL SOLVER_LINEAR_DIRECT_FINALISE(PROBLEM_SOLVER%LINEAR_SOLVER%DIRECT_SOLVER,ERR,ERROR,*999)
              CASE(SOLVER_LINEAR_ITERATIVE_SOLVE)
                CALL SOLVER_LINEAR_ITERATIVE_FINALISE(PROBLEM_SOLVER%LINEAR_SOLVER%ITERATIVE_SOLVER,ERR,ERROR,*999)
              CASE DEFAULT
...
999 IF(ASSOCIATED(PROBLEM_SOLVER)) THEN
      IF(ASSOCIATED(PROBLEM_SOLVER%LINEAR_SOLVER)) THEN
        SELECT CASE(LINEAR_SOLVER_TYPE)
        CASE(SOLVER_LINEAR_DIRECT_SOLVE)
          CALL SOLVER_LINEAR_DIRECT_FINALISE(PROBLEM_SOLVER%LINEAR_SOLVER%DIRECT_SOLVER,DUMMY_ERR,DUMMY_ERROR,*998)
        CASE(SOLVER_LINEAR_ITERATIVE_SOLVE)
          CALL SOLVER_LINEAR_ITERATIVE_FINALISE(PROBLEM_SOLVER%LINEAR_SOLVER%ITERATIVE_SOLVER,DUMMY_ERR,DUMMY_ERROR,*998)
        END SELECT
      ENDIF
    ENDIF
    ...
    IF(ASSOCIATED(PROBLEM_SOLVER)) THEN
      ...
      ELSE
        IF(SOLVER_TYPE/=PROBLEM_SOLVER%SOLVER_TYPE) THEN
          ...
          SELECT CASE(PROBLEM_SOLVER%SOLVER_TYPE)
          CASE(SOLVER_LINEAR_TYPE)
            CALL SOLVER_LINEAR_FINALISE(PROBLEM_SOLVER%LINEAR_SOLVER,ERR,ERROR,*999)
          CASE(SOLVER_NONLINEAR_TYPE)
            CALL SOLVER_NONLINEAR_FINALISE(PROBLEM_SOLVER%NONLINEAR_SOLVER,ERR,ERROR,*999)
          CASE(SOLVER_TIME_INTEGRATION_TYPE)
            CALL SOLVER_TIME_INTEGRATION_FINALISE(PROBLEM_SOLVER%TIME_INTEGRATION_SOLVER,ERR,ERROR,*999)
          CASE DEFAULT
            LOCAL_ERROR="The solver type of "//TRIM(NUMBER_TO_VSTRING(PROBLEM_SOLVER%SOLVER_TYPE,"*",ERR,ERROR))//" is invalid"
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          END SELECT          
...
999 IF(ASSOCIATED(PROBLEM_SOLVER)) THEN
      SELECT CASE(SOLVER_TYPE)
      CASE(SOLVER_LINEAR_TYPE)
        CALL SOLVER_LINEAR_FINALISE(PROBLEM_SOLVER%LINEAR_SOLVER,DUMMY_ERR,DUMMY_ERROR,*998)
      CASE(SOLVER_NONLINEAR_TYPE)
        CALL SOLVER_NONLINEAR_FINALISE(PROBLEM_SOLVER%NONLINEAR_SOLVER,DUMMY_ERR,DUMMY_ERROR,*998)
      CASE(SOLVER_TIME_INTEGRATION_TYPE)
        CALL SOLVER_TIME_INTEGRATION_FINALISE(PROBLEM_SOLVER%TIME_INTEGRATION_SOLVER,DUMMY_ERR,DUMMY_ERROR,*998)
      END SELECT
    ENDIF
