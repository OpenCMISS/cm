...
!             & CALL FLAG_ERROR("Only not use a direct solver with a linked PETSC nonlinear Newton solver.",ERR,ERROR,*999)
          ...
          CASE(SOLVER_DIRECT_LU)
            ...
            CASE(SOLVER_MUMPS_LIBRARY)
              ...
              CALL PETSC_KSPCREATE(SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD &
               & %DECOMPOSITION%WORK_GROUP%COMP_ENVIRONMENT%MPI_COMM,LINEAR_DIRECT_SOLVER%KSP,ERR,ERROR,*999)
        ...
        IF(ASSOCIATED(SOLVER)) THEN
          ...
          CASE(SOLVER_PETSC_LIBRARY)
            ...
            ELSE
              ...
              CALL PETSC_KSPCREATE(SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD% &
               & DECOMPOSITION%WORK_GROUP%COMP_ENVIRONMENT%MPI_COMM,LINEAR_ITERATIVE_SOLVER%KSP,ERR,ERROR,*999)
            ENDIF
...
!                  CALL PETSC_SNESCREATE(COMPUTATIONAL_ENVIRONMENT%MPI_COMM,LINESEARCH_SOLVER%SNES,ERR,ERROR,*999)
                  CALL PETSC_SNESCREATE(SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD%DECOMPOSITION%WORK_GROUP% &
                    & COMP_ENVIRONMENT%MPI_COMM,LINESEARCH_SOLVER%SNES,ERR,ERROR,*999)
...
!                  CALL PETSC_SNESCREATE(COMPUTATIONAL_ENVIRONMENT%MPI_COMM,TRUSTREGION_SOLVER%SNES,ERR,ERROR,*999)
                  CALL PETSC_SNESCREATE(SOLVER_EQUATIONS%SOLVER_MAPPING%EQUATIONS_SETS(1)%PTR%DEPENDENT%DEPENDENT_FIELD% &
                    & DECOMPOSITION%WORK_GROUP%COMP_ENVIRONMENT%MPI_COMM,TRUSTREGION_SOLVER%SNES,ERR,ERROR,*999)
