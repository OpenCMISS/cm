...
999 CALL ERRORS("PROBLEM_CONTROL_LOOP_GET_1",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SETUP(PROBLEM,PROBLEM_SETUP_INFO,ERR,ERROR,*)
    ...
    !PRINT *,"PROBLEM_SETUP class=",PROBLEM%CLASS,PROBLEM_CLASSICAL_FIELD_CLASS,PROBLEM_FMM_CLASS
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_BIOELECTRICS_CLASS)
        IF(PROBLEM%TYPE == PROBLEM_MONODOMAIN_STRANG_SPLITTING_EQUATION_TYPE) THEN
          CALL MONODOMAIN_EQUATION_PROBLEM_SETUP(PROBLEM,PROBLEM_SETUP_INFO,ERR,ERROR,*999)
        ELSE
      ...
      CASE(PROBLEM_FMM_CLASS)
        CALL FMM_PROBLEM_SETUP(PROBLEM,PROBLEM_SETUP_INFO,ERR,ERROR,*999)
      CASE DEFAULT
      ...
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
          SELECT CASE(CONTROL_LOOP%PROBLEM%CLASS)
            CASE(PROBLEM_ELASTICITY_CLASS)
              IF(CONTROL_LOOP%PROBLEM%TYPE == PROBLEM_FINITE_ELASTICITY_TYPE) THEN
                CALL FINITE_ELASTICITY_PRE_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*999)
              ENDIF
            CASE(PROBLEM_BIOELECTRICS_CLASS)
              IF(CONTROL_LOOP%PROBLEM%TYPE == PROBLEM_MONODOMAIN_STRANG_SPLITTING_EQUATION_TYPE) THEN
         ...
         !    do nothing???
            ...
            CASE(PROBLEM_FMM_CLASS)
              CALL FMM_PRE_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*999)
            CASE DEFAULT
      ...
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
         ...
         !    do nothing???
            ...
            CASE(PROBLEM_FMM_CLASS)
              CALL FMM_POST_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*999)
            CASE DEFAULT
              LOCAL_ERROR="Problem class "//TRIM(NUMBER_TO_VSTRING(CONTROL_LOOP%PROBLEM%CLASS,"*",ERR,ERROR))//" &
              & is not valid."
              CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
      ...
      IF(SOLVER_EQUATIONS%SOLVER_EQUATIONS_FINISHED) THEN
        ...
        CASE(SOLVER_EQUATIONS_FIRST_ORDER_DYNAMIC,SOLVER_EQUATIONS_SECOND_ORDER_DYNAMIC)
          ...
          CASE(SOLVER_EQUATIONS_STATEITERATION)
            CALL PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE(SOLVER_EQUATIONS,ERR,ERROR,*999)
          CASE DEFAULT
            LOCAL_ERROR="The solver equations linearity of "//TRIM(NUMBER_TO_VSTRING(SOLVER_EQUATIONS%LINEARITY,"*",ERR,ERROR))// &
              & " is invalid."
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
...
999 CALL ERRORS("PROBLEM_SOLVER_EQUATIONS_DYNAMIC_NONLINEAR_SOLVE",ERR,ERROR)
  ...
  !>Solves dynamic nonlinear solver equations.
  SUBROUTINE PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE(SOLVER_EQUATIONS,ERR,ERROR,*)
   ...
   !Argument variables
    ...
    CALL ENTERS("PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE",ERR,ERROR,*999)
    PRINT *,"PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE"
...
!                 CALL EQUATIONS_SET_ASSEMBLE(EQUATIONS_SET,ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE")
    RETURN
999 CALL ERRORS("PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE",ERR,ERROR)
    CALL EXITS("PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE")
    RETURN 1
  END SUBROUTINE PROBLEM_SOLVER_EQUATIONS_DYNAMIC_STATEITERATION_SOLVE
  ...
  SUBROUTINE PROBLEM_SPECIFICATION_SET(PROBLEM,PROBLEM_CLASS,PROBLEM_EQUATION_TYPE,PROBLEM_SUBTYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      ELSE
        ...
        CASE(PROBLEM_BIOELECTRICS_CLASS)
          IF(PROBLEM_EQUATION_TYPE == PROBLEM_MONODOMAIN_STRANG_SPLITTING_EQUATION_TYPE) THEN
        ...
        CASE(PROBLEM_FMM_CLASS)
          CALL FMM_PROBLEM_CLASS_TYPE_SET(PROBLEM,PROBLEM_EQUATION_TYPE,PROBLEM_SUBTYPE,ERR,ERROR,*999)
        CASE DEFAULT
          LOCAL_ERROR="Problem class "//TRIM(NUMBER_TO_VSTRING(PROBLEM_CLASS,"*",ERR,ERROR))//" is not valid."
          CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
        END SELECT
