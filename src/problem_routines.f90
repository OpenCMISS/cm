      ...
      IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
        ...
        IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
          ...
          IF(ASSOCIATED(GLOBAL_MATRICES)) THEN
            ...
            CALL PROBLEM_GLOBAL_MATRICES_VALUES_INITIALISE(GLOBAL_MATRICES,0.0_DP,ERR,ERROR,*999)
            !Assemble the elements
            !Allocate the element matrices 
            CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_INITIALISE(GLOBAL_MATRICES,ERR,ERROR,*999)
            ...
            DO element_idx=1,ELEMENTS_MAPPING%NUMBER_OF_INTERNAL
              ...
              CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_CALCULATE(GLOBAL_MATRICES,ne,ERR,ERROR,*999)
              CALL PROBLEM_FINITE_ELEMENT_CALCULATE(PROBLEM,ne,ERR,ERROR,*999)
              CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_ADD(GLOBAL_MATRICES,ERR,ERROR,*999)
            ENDDO !element_idx                  
            !Output timing information if required
            IF(PROBLEM_SOLUTION%OUTPUT_TYPE>=PROBLEM_SOLUTION_TIMING_OUTPUT) THEN
...
!!TODO: sort out combining boundary and ghost list
            ...
            CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE(GLOBAL_MATRICES,ERR,ERROR,*999)
            ...
            IF(PROBLEM_SOLUTION%OUTPUT_TYPE>=PROBLEM_SOLUTION_GLOBAL_MATRIX_OUTPUT) THEN
              CALL PROBLEM_GLOBAL_MATRICES_OUTPUT(GENERAL_OUTPUT_TYPE,GLOBAL_MATRICES,ERR,ERROR,*999)
            ENDIF
            !Output timing information if required
            IF(PROBLEM_SOLUTION%OUTPUT_TYPE>=PROBLEM_SOLUTION_TIMING_OUTPUT) THEN
...
999 CALL ERRORS("PROBLEM_FINITE_ELEMENT_CALCUALTE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_FINALISE(DOF_TO_GLOBAL_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(DOF_TO_GLOBAL_MAPPING_TYPE) :: DOF_TO_GLOBAL_MAP !<The DOF to global map to finalise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables

    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_FINALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_FINALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_INITIALISE(DOF_TO_GLOBAL_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(DOF_TO_GLOBAL_MAPPING_TYPE) :: DOF_TO_GLOBAL_MAP !<The DOF to global map to initalise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables

    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_INITIALISE",ERR,ERROR,*999)

    DOF_TO_GLOBAL_MAP%ROW_MAPPING=0
    DOF_TO_GLOBAL_MAP%NUMBER_OF_MATRICES=0
      
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_INITIALISE
  ...
  !>Finalises the DOF to global matrices map for a problem and deallocates all memory
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_FINALISE(GLOBAL_MATRICES,ERR,ERROR,*)

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRICES_TYPE), POINTER :: GLOBAL_MATRICES !<The pointer to the global matrices
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: ny
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_FINALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(GLOBAL_MATRICES)) THEN
      IF(ALLOCATED(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING)) THEN
        ...
        IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
          DO ny=1,DEPENDENT_FIELD%MAPPINGS%DOMAIN_MAPPING%NUMBER_OF_LOCAL
            CALL PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_FINALISE(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(ny),ERR,ERROR,*999)
          ENDDO !ny
        ENDIF
        DEALLOCATE(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Global matrices is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_FINALISE_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_FINALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_INITIALISE(GLOBAL_MATRICES,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_INITIALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(GLOBAL_MATRICES)) THEN
      IF(ALLOCATED(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING)) THEN
        CALL FLAG_ERROR("DOF to global mapping is already associated for the global matrices",ERR,ERROR,*999)
      ELSE
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_INITIALISE
...
999 CALL ERRORS("PROBLEM_LAPLACE_EQUATION_SUBTYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_STANDARD_LAPLACE_SETUP(PROBLEM,SETUP_TYPE,ACTION_TYPE,ERR,ERROR,*)
    ...
    INTEGER(INTG), POINTER :: ROW_LOCATIONS(:),COLUMN_LOCATIONS(:)
            ...
            CASE(PROBLEM_SOLUTION_SETUP_TYPE)
              ...
              CASE(PROBLEM_FINISH_ACTION)
                ...
                CASE(PROBLEM_FEM_SOLUTION_METHOD)
                  ...
                  CALL PROBLEM_GLOBAL_MATRICES_CREATE_START(PROBLEM%SOLUTION,GLOBAL_MATRICES,ERR,ERROR,*999)
                  CALL PROBLEM_GLOBAL_MATRICES_NUMBER_SET(GLOBAL_MATRICES,1,ERR,ERROR,*999)
...
999 CALL ERRORS("PROBLEM_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_FINALISE(ELEMENT_MATRIX,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_FINALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_FINALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_INITIALISE(ELEMENT_MATRIX,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_INITIALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_INITIALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_FINALISE(ELEMENT_VECTOR,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_FINALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_FINALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_INITIALISE(ELEMENT_VECTOR,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_INITIALISE",ERR,ERROR,*999)

    ELEMENT_VECTOR%NUMBER_OF_ROWS=0
    ELEMENT_VECTOR%MAX_NUMBER_OF_ROWS=0
       
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_INITIALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_ADD(GLOBAL_MATRICES,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_ADD",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_ADD")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_ADD",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_ADD")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_ADD
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_CALCULATE(GLOBAL_MATRICES,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_CALCULATE",ERR,ERROR,*999)
...
!!TODO :: CHANGE TO LOOP OVER ELEMENT PARAMETERS (ns)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_CALCULATE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_CALCULATE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_CALCULATE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_CALCULATE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE(GLOBAL_MATRICES,ERR,ERROR,*)

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRICES_TYPE), POINTER :: GLOBAL_MATRICES !<The global matrices for which to finalise the elements
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: matrix_idx
    
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_INITIALISE(GLOBAL_MATRICES,ERR,ERROR,*)

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRICES_TYPE), POINTER :: GLOBAL_MATRICES !The global matrices to initialise the element information for
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_ELEMENT_INITIALISE",ERR,ERROR,*998)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_INITIALISE")
    RETURN
999 CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_FINALISE(GLOBAL_MATRICES,DUMMY_ERR,DUMMY_ERROR,*998)
998 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_ELEMENT_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_ELEMENT_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_ELEMENT_INITIALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRIX_FINALISE(GLOBAL_MATRIX,ERR,ERROR,*)

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRIX_TYPE):: GLOBAL_MATRIX !<The global matrix to finalise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: ny
    
    CALL ENTERS("PROBLEM_GLOBAL_MATRIX_FINALISE",ERR,ERROR,*999)

    IF(ALLOCATED(GLOBAL_MATRIX%DOF_ROW_MAP)) DEALLOCATE(GLOBAL_MATRIX%DOF_ROW_MAP)
    IF(ALLOCATED(GLOBAL_MATRIX%DOF_COLUMN_MAP)) DEALLOCATE(GLOBAL_MATRIX%DOF_COLUMN_MAP)
    IF(ALLOCATED(GLOBAL_MATRIX%SOLVER_ROW_MAP)) THEN
      DO ny=1,GLOBAL_MATRIX%NUMBER_OF_ROWS
        CALL PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE(GLOBAL_MATRIX%SOLVER_ROW_MAP(ny),ERR,ERROR,*999)        
      ENDDO !ny
      DEALLOCATE(GLOBAL_MATRIX%SOLVER_ROW_MAP)
    ENDIF
    IF(ALLOCATED(GLOBAL_MATRIX%SOLVER_COLUMN_MAP)) THEN
      DO ny=1,GLOBAL_MATRIX%NUMBER_OF_COLUMNS
        CALL PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE(GLOBAL_MATRIX%SOLVER_COLUMN_MAP(ny),ERR,ERROR,*999)
      ENDDO !ny
      DEALLOCATE(GLOBAL_MATRIX%SOLVER_COLUMN_MAP)
    ENDIF
    IF(ASSOCIATED(GLOBAL_MATRIX%MATRIX)) CALL DISTRIBUTED_MATRIX_DESTROY(GLOBAL_MATRIX%MATRIX,ERR,ERROR,*999)
    CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_FINALISE(GLOBAL_MATRIX%ELEMENT_MATRIX,ERR,ERROR,*999)
    
    CALL EXITS("PROBLEM_GLOBAL_MATRIX_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRIX_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRIX_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRIX_FINALISE

  !
  !================================================================================================================================
  !

  SUBROUTINE PROBLEM_GLOBAL_MATRIX_INITIALISE(GLOBAL_MATRIX,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_GLOBAL_MATRIX_INITIALISE
    !###  Description:
    !###    Initialise the global matrix.

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRIX_TYPE) :: GLOBAL_MATRIX
    INTEGER(INTG), INTENT(OUT) :: ERR
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR
    !Local Variables

    CALL ENTERS("PROBLEM_GLOBAL_MATRIX_INITIALISE",ERR,ERROR,*999)
    ...
    CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_MATRIX_INITIALISE(GLOBAL_MATRIX%ELEMENT_MATRIX,ERR,ERROR,*999)
       
    CALL EXITS("PROBLEM_GLOBAL_MATRIX_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRIX_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRIX_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRIX_INITIALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_FINISH(GLOBAL_MATRICES,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.    
    !Local Variables
    INTEGER(INTG) :: DUMMY_ERR,matrix_idx,global_ny,field_ny,NUMBER_OF_NON_ZEROS,row_ny,column_ny,variable_idx,variable_type, &
      & dof_type,dof_idx
    INTEGER(INTG) :: MATRIX_VARIABLES(0:FIELD_NUMBER_OF_VARIABLE_TYPES,FIELD_NUMBER_OF_VARIABLE_TYPES)=0
    INTEGER(INTG), POINTER :: ROW_LOCATIONS(:),COLUMN_LOCATIONS(:)
    LOGICAL :: FOUND
    TYPE(DOMAIN_MAPPING_TYPE), POINTER :: DEPENDENT_DOMAIN_MAP,DEPENDENT_VARIABLE_DOMAIN_MAP
    TYPE(FIELD_TYPE), POINTER :: DEPENDENT_FIELD
    TYPE(FIELD_DOF_TO_PARAM_MAP_TYPE), POINTER :: DEPENDENT_DOF_MAP
    TYPE(PROBLEM_SOLUTION_TYPE), POINTER :: PROBLEM_SOLUTION
    TYPE(VARYING_STRING) :: DUMMY_ERROR,LOCAL_ERROR

    NULLIFY(ROW_LOCATIONS)
    NULLIFY(COLUMN_LOCATIONS)

    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_CREATE_FINISH",ERR,ERROR,*998)
        ...
        IF(ASSOCIATED(GLOBAL_MATRICES%CREATE_VALUES_CACHE)) THEN
          ...
          IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
            IF(ASSOCIATED(PROBLEM_SOLUTION%PROBLEM)) THEN
              DEPENDENT_FIELD=>PROBLEM_SOLUTION%PROBLEM%DEPENDENT%DEPENDENT_FIELD
              IF(ASSOCIATED(DEPENDENT_FIELD)) THEN                
                !Check all the field variables are included in the problem and calculate the number of matrices each variable
                !is mapped to
                DO variable_idx=1,DEPENDENT_FIELD%NUMBER_OF_VARIABLES
                  variable_type=DEPENDENT_FIELD%VARIABLES(variable_idx)%VARIABLE_TYPE
                  MATRIX_VARIABLES(0,variable_type)=0
                  FOUND=.FALSE.
                  IF(GLOBAL_MATRICES%RHS_VARIABLE_TYPE/=variable_type) THEN
                    DO matrix_idx=1,GLOBAL_MATRICES%NUMBER_OF_MATRICES
                      IF(GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_VARIABLE_TYPES(matrix_idx)==variable_type) THEN
                        FOUND=.TRUE.
                        MATRIX_VARIABLES(0,variable_type)=MATRIX_VARIABLES(0,variable_type)+1
                        MATRIX_VARIABLES(MATRIX_VARIABLES(0,variable_type),variable_type)=matrix_idx
                      ENDIF
                    ENDDO !matrix_idx
                  ELSE
                    FOUND=.TRUE.
                  ENDIF
                  IF(.NOT.FOUND) THEN
                    LOCAL_ERROR="Variable type "//TRIM(NUMBER_TO_VSTRING(variable_type,"*",ERR,ERROR))// &
                      & " of the dependent field is not mapped to any global matrix or the global rhs vector"
                    CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
                  ENDIF
                ENDDO !variable_idx
                !Calculate DOF to global mapping
                DEPENDENT_DOF_MAP=>DEPENDENT_FIELD%MAPPINGS%DOF_TO_PARAM_MAP
                DEPENDENT_DOMAIN_MAP=>DEPENDENT_FIELD%MAPPINGS%DOMAIN_MAPPING
                !Calculate the row and column maps for the dof to matrix mapping
                ALLOCATE(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(DEPENDENT_DOMAIN_MAP%NUMBER_OF_LOCAL),STAT=ERR)
                IF(ERR/=0) CALL FLAG_ERROR("Could not allocate dof to global map",ERR,ERROR,*999)                
                DO field_ny=1,DEPENDENT_DOMAIN_MAP%NUMBER_OF_LOCAL
                  CALL PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAP_INITIALISE(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny), &
                    & ERR,ERROR,*999)
                  dof_type=DEPENDENT_DOF_MAP%DOF_TYPE(1,field_ny)
                  dof_idx=DEPENDENT_DOF_MAP%DOF_TYPE(2,field_ny)
                  SELECT CASE(dof_type)
                  CASE(FIELD_CONSTANT_DOF_TYPE)
                    variable_type=DEPENDENT_DOF_MAP%CONSTANT_DOF2PARAM_MAP(2,dof_idx)
                  CASE(FIELD_ELEMENT_DOF_TYPE)
                    variable_type=DEPENDENT_DOF_MAP%ELEMENT_DOF2PARAM_MAP(3,dof_idx)
                  CASE(FIELD_NODE_DOF_TYPE)
                    variable_type=DEPENDENT_DOF_MAP%NODE_DOF2PARAM_MAP(4,dof_idx)
                  CASE(FIELD_POINT_DOF_TYPE)
                    variable_type=DEPENDENT_DOF_MAP%POINT_DOF2PARAM_MAP(3,dof_idx)
                  CASE DEFAULT
                    LOCAL_ERROR="The field dof type of "//TRIM(NUMBER_TO_VSTRING(dof_type,"*",ERR,ERROR))//" is invalid"
                    CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
                  END SELECT
                  IF(GLOBAL_MATRICES%RHS_VARIABLE_TYPE==variable_type) THEN
                    !RHS variable ny
                    GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny)%ROW_MAPPING=field_ny
                  ELSE
                    !Matrix variable ny
                    GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny)%ROW_MAPPING=field_ny
                    GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny)%NUMBER_OF_MATRICES=MATRIX_VARIABLES(0,variable_type)
                    ALLOCATE(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny)%MATRIX_MAPPINGS( &
                      & MATRIX_VARIABLES(0,variable_type)),STAT=ERR)
                    IF(ERR/=0) CALL FLAG_ERROR("Could not allocate global matrices dof to global map matrix mappings", &
                      & ERR,ERROR,*999)
                    ALLOCATE(GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny)%COLUMN_MAPPINGS( &
                      & MATRIX_VARIABLES(0,variable_type)),STAT=ERR)
                    IF(ERR/=0) CALL FLAG_ERROR("Could not allocate global matrices dof to global map column mappings", &
                      & ERR,ERROR,*999)
                    DO matrix_idx=1,MATRIX_VARIABLES(0,variable_type)
                      global_ny=DEPENDENT_FIELD%VARIABLES(variable_type)%DOMAIN_MAPPING%LOCAL_TO_GLOBAL_MAP(field_ny)
                      GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny)%MATRIX_MAPPINGS(matrix_idx)= &
                        & MATRIX_VARIABLES(matrix_idx,variable_type)
                      GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING(field_ny)%COLUMN_MAPPINGS(matrix_idx)=global_ny
                    ENDDO !matrix_idx
                  ENDIF
                ENDDO !field_ny
                ...
                DO matrix_idx=1,GLOBAL_MATRICES%NUMBER_OF_MATRICES
                  ...
                  CALL PROBLEM_GLOBAL_MATRIX_INITIALISE(GLOBAL_MATRICES%MATRICES(matrix_idx),ERR,ERROR,*999)
                  !Set up the matrix parameters
                  GLOBAL_MATRICES%MATRICES(matrix_idx)%MATRIX_NUMBER=matrix_idx
                  GLOBAL_MATRICES%MATRICES(matrix_idx)%VARIABLE_TYPE= &
                  ...
                  ALLOCATE(GLOBAL_MATRICES%MATRICES(matrix_idx)%DOF_ROW_MAP(GLOBAL_MATRICES%MATRICES(matrix_idx)% &
                    & NUMBER_OF_ROWS),STAT=ERR)
                  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate global matrix dof row map",ERR,ERROR,*999)
                  ALLOCATE(GLOBAL_MATRICES%MATRICES(matrix_idx)%DOF_COLUMN_MAP(GLOBAL_MATRICES%MATRICES(matrix_idx)% &
                    & NUMBER_OF_COLUMNS),STAT=ERR)
                  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate global matrix dof column map",ERR,ERROR,*999)
                  !Set up 1-1 mapping between row and column numbers and the field dof number
                  DO row_ny=1,GLOBAL_MATRICES%MATRICES(matrix_idx)%NUMBER_OF_ROWS
                    GLOBAL_MATRICES%MATRICES(matrix_idx)%DOF_ROW_MAP(row_ny)=row_ny
                  ENDDO !row_ny
                  DO column_ny=1,GLOBAL_MATRICES%MATRICES(matrix_idx)%NUMBER_OF_COLUMNS
                    GLOBAL_MATRICES%MATRICES(matrix_idx)%DOF_COLUMN_MAP(column_ny)=column_ny
                  ENDDO !column_ny
                  ...
                  IF(GLOBAL_MATRICES%MATRICES(matrix_idx)%STORAGE_TYPE/=MATRIX_BLOCK_STORAGE_TYPE) THEN
                    CALL PROBLEM_GLOBAL_MATRIX_STRUCTURE_CALCULATE(GLOBAL_MATRICES,matrix_idx,NUMBER_OF_NON_ZEROS,ROW_LOCATIONS, &
                      & COLUMN_LOCATIONS,ERR,ERROR,*999)
                    CALL DISTRIBUTED_MATRIX_STORAGE_LOCATIONS_SET(GLOBAL_MATRICES%MATRICES(matrix_idx)%MATRIX,ROW_LOCATIONS, &
                      & COLUMN_LOCATIONS,ERR,ERROR,*999)
                  ENDIF
                  CALL DISTRIBUTED_MATRIX_CREATE_FINISH(GLOBAL_MATRICES%MATRICES(matrix_idx)%MATRIX,ERR,ERROR,*999)
                  IF(ASSOCIATED(ROW_LOCATIONS)) DEALLOCATE(ROW_LOCATIONS)
                  IF(ASSOCIATED(COLUMN_LOCATIONS)) DEALLOCATE(COLUMN_LOCATIONS)
                ENDDO !matrix_idx
                ...
                CALL PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE(GLOBAL_MATRICES,ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_CREATE_FINISH")
    RETURN
999 IF(ASSOCIATED(ROW_LOCATIONS)) DEALLOCATE(ROW_LOCATIONS)
    IF(ASSOCIATED(COLUMN_LOCATIONS)) DEALLOCATE(COLUMN_LOCATIONS)
    PROBLEM_SOLUTION=>GLOBAL_MATRICES%PROBLEM_SOLUTION
    CALL PROBLEM_GLOBAL_MATRICES_FINALISE(PROBLEM_SOLUTION,DUMMY_ERR,DUMMY_ERROR,*998)
998 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_CREATE_FINISH",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_CREATE_FINISH")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_FINISH
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_START(PROBLEM_SOLUTION,GLOBAL_MATRICES,ERR,ERROR,*)

    !Argument variables
    TYPE(PROBLEM_SOLUTION_TYPE), POINTER :: PROBLEM_SOLUTION !<The pointer to the problem solution
    TYPE(PROBLEM_GLOBAL_MATRICES_TYPE), POINTER :: GLOBAL_MATRICES !<On return, a pointer to the global matrices being created.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string.    
    !Local Variables
    ...
    RETURN
999 CALL PROBLEM_GLOBAL_MATRICES_FINALISE(PROBLEM_SOLUTION,DUMMY_ERR,DUMMY_ERROR,*998)
998 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_CREATE_START",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_CREATE_START")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_START
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE(GLOBAL_MATRICES,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(GLOBAL_MATRICES)) THEN
      IF(ASSOCIATED(GLOBAL_MATRICES%CREATE_VALUES_CACHE)) THEN
        IF(ALLOCATED(GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_VARIABLE_TYPES)) &
          & DEALLOCATE(GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_VARIABLE_TYPES)
        IF(ALLOCATED(GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_STORAGE_TYPE)) &
          & DEALLOCATE(GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_STORAGE_TYPE)
        IF(ALLOCATED(GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_STRUCTURE_TYPE)) &
          & DEALLOCATE(GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_STRUCTURE_TYPE)
        DEALLOCATE(GLOBAL_MATRICES%CREATE_VALUES_CACHE)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Global matrices is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_INITIALISE(GLOBAL_MATRICES,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_INITIALISE",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(GLOBAL_MATRICES)) THEN
      ...
      ELSE
        ...
        GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_STRUCTURE_TYPE(1)=PROBLEM_GLOBAL_MATRIX_NO_STRUCTURE
        DO matrix_idx=2,GLOBAL_MATRICES%NUMBER_OF_MATRICES
          ...
          GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_STRUCTURE_TYPE(matrix_idx)=PROBLEM_GLOBAL_MATRIX_NO_STRUCTURE
        ENDDO !matrix_idx
      ENDIF
    ELSE
      CALL FLAG_ERROR("Global matrices is not associated",ERR,ERROR,*998)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_INITIALISE")
    RETURN
999 CALL PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE(GLOBAL_MATRICES,DUMMY_ERR,DUMMY_ERROR,*998)
998 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_INITIALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_NUMBER_SET(GLOBAL_MATRICES,NUMBER_OF_GLOBAL_MATRICES,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_NUMBER_SET",ERR,ERROR,*999)
      ...
      ELSE
        IF(ASSOCIATED(GLOBAL_MATRICES%CREATE_VALUES_CACHE)) THEN
          ...
          IF(NUMBER_OF_GLOBAL_MATRICES/=GLOBAL_MATRICES%NUMBER_OF_MATRICES) THEN
            ...
            DEALLOCATE(OLD_MATRIX_STORAGE_TYPE)
            DEALLOCATE(OLD_MATRIX_STRUCTURE_TYPE)
          ENDIF
        ELSE
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_NUMBER_SET")
    RETURN
999 IF(ALLOCATED(OLD_MATRIX_VARIABLE_TYPES)) DEALLOCATE(OLD_MATRIX_VARIABLE_TYPES)
    ...
    CALL ERRORS("PROBLEM_GLOBAL_MATRICES_NUMBER_SET",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_NUMBER_SET")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_NUMBER_SET
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_OUTPUT(ID,GLOBAL_MATRICES,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_OUTPUT",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_OUTPUT")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_OUTPUT",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_OUTPUT")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_OUTPUT
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_STORAGE_TYPE_SET(GLOBAL_MATRICES,STORAGE_TYPE,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_STORAGE_TYPE_SET",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_STORAGE_TYPE_SET")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_STORAGE_TYPE_SET",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_STORAGE_TYPE_SET")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_STORAGE_TYPE_SET
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_STRUCTURE_TYPE_SET(GLOBAL_MATRICES,STRUCTURE_TYPE,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_STRUCTURE_TYPE_SET",ERR,ERROR,*999)
          ...
          IF(SIZE(STRUCTURE_TYPE,1)==GLOBAL_MATRICES%NUMBER_OF_MATRICES) THEN
            DO matrix_idx=1,GLOBAL_MATRICES%NUMBER_OF_MATRICES
              ...
              CASE(PROBLEM_GLOBAL_MATRIX_FEM_STRUCTURE)
                GLOBAL_MATRICES%CREATE_VALUES_CACHE%MATRIX_STRUCTURE_TYPE(matrix_idx)=PROBLEM_GLOBAL_MATRIX_FEM_STRUCTURE
              CASE DEFAULT
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_STRUCTURE_TYPE_SET")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_STRUCTURE_TYPE_SET",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_STRUCTURE_TYPE_SET")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_STRUCTURE_TYPE_SET
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_RHS_VARIABLE_TYPE_SET(GLOBAL_MATRICES,RHS_VARIABLE_TYPE,ERR,ERROR,*)

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRICES_TYPE), POINTER :: GLOBAL_MATRICES !<A pointer to the global matrices to set
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_RHS_VARIABLE_TYPE_SET",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_RHS_VARIABLE_TYPE_SET")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_RHS_VARIABLE_TYPE_SET",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_RHS_VARIABLE_TYPE_SET")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_RHS_VARIABLE_TYPE_SET
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_VARIABLE_TYPES_SET(GLOBAL_MATRICES,MATRIX_VARIABLE_TYPES,ERR,ERROR,*)

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRICES_TYPE), POINTER :: GLOBAL_MATRICES !<A pointer to the global matrices
    INTEGER(INTG), INTENT(IN) :: MATRIX_VARIABLE_TYPES(:)
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: matrix_idx
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_VARIABLE_TYPES_SET",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_VARIABLE_TYPES_SET")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_VARIABLE_TYPES_SET",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_VARIABLE_TYPES_SET")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_VARIABLE_TYPES_SET

  !
  !================================================================================================================================
  !

  SUBROUTINE PROBLEM_GLOBAL_MATRICES_FINALISE(PROBLEM_SOLUTION,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_GLOBAL_MATRICES_FINALISE
    !###  Description:
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_FINALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      IF(ASSOCIATED(PROBLEM_SOLUTION%GLOBAL_MATRICES)) THEN
        IF(ALLOCATED(PROBLEM_SOLUTION%GLOBAL_MATRICES%MATRIX_TYPE_MAP)) &
          & DEALLOCATE(PROBLEM_SOLUTION%GLOBAL_MATRICES%MATRIX_TYPE_MAP)
        IF(ALLOCATED(PROBLEM_SOLUTION%GLOBAL_MATRICES%MATRICES)) THEN
          DO matrix_idx=1,PROBLEM_SOLUTION%GLOBAL_MATRICES%NUMBER_OF_MATRICES
            CALL PROBLEM_GLOBAL_MATRIX_FINALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES%MATRICES(matrix_idx),ERR,ERROR,*999)
          ENDDO !matrix_idx
          DEALLOCATE(PROBLEM_SOLUTION%GLOBAL_MATRICES%MATRICES)
        ENDIF
        IF(ALLOCATED(PROBLEM_SOLUTION%GLOBAL_MATRICES%DOF_TO_GLOBAL_MAPPING)) &
          & CALL PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_FINALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES,ERR,ERROR,*999)
        IF(ALLOCATED(PROBLEM_SOLUTION%GLOBAL_MATRICES%GLOBAL_ROW_TO_DOF_MAP)) &
          & DEALLOCATE(PROBLEM_SOLUTION%GLOBAL_MATRICES%GLOBAL_ROW_TO_DOF_MAP)
        IF(ASSOCIATED(PROBLEM_SOLUTION%GLOBAL_MATRICES%VECTOR)) &
          & CALL DISTRIBUTED_VECTOR_DESTROY(PROBLEM_SOLUTION%GLOBAL_MATRICES%VECTOR,ERR,ERROR,*999)
        CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_FINALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES%ELEMENT_VECTOR,ERR,ERROR,*999)
        IF(ASSOCIATED(PROBLEM_SOLUTION%GLOBAL_MATRICES%CREATE_VALUES_CACHE)) &
          & CALL PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_FINALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES,ERR,ERROR,*999)
        DEALLOCATE(PROBLEM_SOLUTION%GLOBAL_MATRICES)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem solution is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_FINALISE

  !
  !================================================================================================================================
  !

  SUBROUTINE PROBLEM_GLOBAL_MATRICES_INITIALISE(PROBLEM_SOLUTION,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_GLOBAL_MATRICES_INITIALISE
    !###  Description:
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_INITIALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      IF(ASSOCIATED(PROBLEM_SOLUTION%GLOBAL_MATRICES)) THEN
        CALL FLAG_ERROR("Global matrices is already associated for this problem solution",ERR,ERROR,*999)
      ELSE
        ...
        ALLOCATE(PROBLEM_SOLUTION%GLOBAL_MATRICES%MATRIX_TYPE_MAP(FIELD_NUMBER_OF_VARIABLE_TYPES),STAT=ERR)
        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate global matrices matrix type map",ERR,ERROR,*999)
        DO matrix_idx=1,FIELD_NUMBER_OF_VARIABLE_TYPES
          NULLIFY(PROBLEM_SOLUTION%GLOBAL_MATRICES%MATRIX_TYPE_MAP(matrix_idx)%PTR)
        ENDDO !matrix_idx
        PROBLEM_SOLUTION%GLOBAL_MATRICES%UPDATE_VECTOR=.TRUE.
        CALL PROBLEM_GLOBAL_MATRICES_DOF_TO_GLOBAL_MAPPING_INITIALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES,ERR,ERROR,*999)
        NULLIFY(PROBLEM_SOLUTION%GLOBAL_MATRICES%VECTOR)
        CALL PROBLEM_GLOBAL_MATRICES_ELEMENT_VECTOR_INITIALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES%ELEMENT_VECTOR,ERR,ERROR,*999)
        NULLIFY(PROBLEM_SOLUTION%GLOBAL_MATRICES%CREATE_VALUES_CACHE)
        CALL PROBLEM_GLOBAL_MATRICES_CREATE_VALUES_CACHE_INITIALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES,ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem solution is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_INITIALISE
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_VALUES_INITIALISE(GLOBAL_MATRICES,VALUE,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRICES_VALUES_INITIALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_VALUES_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_VALUES_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRICES_VALUES_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_MATRICES_VALUES_INITIALISE

  !
  !================================================================================================================================
  !

  !>Finalise a global to solution matrix variable map and deallocate all memory
  SUBROUTINE PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE(GLOBAL_TO_SOLVER_MAP,ERR,ERROR,*)

    !Argument variables
    TYPE(GLOBAL_TO_SOLVER_MAP_TYPE):: GLOBAL_TO_SOLVER_MAP !<The global to solver map to finalise
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    
    CALL ENTERS("PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_TO_SOLVER_MAP_FINALISE

  !
  !================================================================================================================================
  !

  !>Initialise the global to solver matrix variable map.
  SUBROUTINE PROBLEM_GLOBAL_TO_SOLVER_MAP_INITIALISE(GLOBAL_TO_SOLVER_MAP,ERR,ERROR,*)
    ...
    CALL ENTERS("PROBLEM_GLOBAL_TO_SOLVER_MAP_INITIALISE",ERR,ERROR,*999)

    GLOBAL_TO_SOLVER_MAP%NUMBER_OF_SOLUTION_DOFS=0
       
    CALL EXITS("PROBLEM_GLOBAL_TO_SOLVER_MAP_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_TO_SOLVER_MAP_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_TO_SOLVER_MAP_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_GLOBAL_TO_SOLVER_MAP_INITIALISE
  ...
  SUBROUTINE PROBLEM_INITIALISE(PROBLEM,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(OUT) :: ERR !<The errror code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
...
999 CALL ERRORS("PROBLEM_SOLVE_LINEAR_STATIC_FEM",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SOLVE_SETUP_LINEAR_STATIC_FEM(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      !CALL PROBLEM_GLOBAL_MATRICES_INITIALISE(PROBLEM%SOLUTION,ERR,ERROR,*999)
      !GLOBAL_MATRICES=>PROBLEM%SOLUTION%GLOBAL_MATRICES
      !DEPENDENT_FIELD=>PROBLEM%DEPENDENT%DEPENDENT_FIELD
      !DEPENDENT_DOFS_MAPPING=>DEPENDENT_FIELD%MAPPINGS%DOMAIN_MAPPING
      !IF(PROBLEM%SOLUTION%GLOBAL_MATRICES%USE_STIFFNESS) THEN
      !  CALL DISTRIBUTED_MATRIX_CREATE_START(DEPENDENT_DOFS_MAPPING,GLOBAL_MATRICES%STIFFNESS,ERR,ERROR,*999)
      !  IF(GLOBAL_MATRICES%SPARSE_GLOBAL_MATRICES) THEN
      !    CALL DISTRIBUTED_MATRIX_STORAGE_TYPE_SET(GLOBAL_MATRICES%STIFFNESS,MATRIX_COMPRESSED_ROW_STORAGE_TYPE, &
      !      & ERR,ERROR,*999)
      !    CALL PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM(PROBLEM,ROW_LOCATIONS,COLUMN_LOCATIONS,ERR,ERROR,*999)
      !    CALL DISTRIBUTED_MATRIX_STORAGE_LOCATIONS_SET(GLOBAL_MATRICES%STIFFNESS,ROW_LOCAxTIONS,COLUMN_LOCATIONS,ERR,ERROR,*999)
      !    IF(ASSOCIATED(ROW_LOCATIONS)) DEALLOCATE(ROW_LOCATIONS)
      !    IF(ASSOCIATED(COLUMN_LOCATIONS)) DEALLOCATE(COLUMN_LOCATIONS)
      !  ENDIF
      !  CALL DISTRIBUTED_MATRIX_CREATE_FINISH(GLOBAL_MATRICES%STIFFNESS,ERR,ERROR,*999)
      !ENDIF
    ELSE
      CALL FLAG_ERROR("Problem is not associated",ERR,ERROR,*999)
    ENDIF
...
999 CALL ERRORS("PROBLEM_SOLVE_SETUP_LINEAR_STATIC_FEM",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRIX_STRUCTURE_CALCULATE(GLOBAL_MATRICES,matrix_idx,NUMBER_OF_NON_ZEROS,ROW_LOCATIONS, &
    & COLUMN_LOCATIONS,ERR,ERROR,*)
    ...
    INTEGER(INTG), POINTER :: ROW_LOCATIONS(:) !<On return a pointer to row location indicies. The calling routine is responsible for deallocation.
    INTEGER(INTG), POINTER :: COLUMN_LOCATIONS(:) !<On return a pointer to the column location indices. The calling routine is responsible for deallocation.
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) ::  INITIAL_SIZE,global_ny,local_ny,nc,nh,nk,np,nyy
    ...
    CALL ENTERS("PROBLEM_GLOBAL_MATRIX_STRUCTURE_CALCULATE",ERR,ERROR,*999)

    NUMBER_OF_NON_ZEROS=0
    IF(.NOT.ASSOCIATED(ROW_LOCATIONS)) THEN
      IF(.NOT.ASSOCIATED(COLUMN_LOCATIONS)) THEN
        IF(ASSOCIATED(GLOBAL_MATRICES)) THEN
            ...
            CASE(PROBLEM_GLOBAL_MATRIX_FEM_STRUCTURE)
              ...
              DO local_ny=1,DEPENDENT_DOFS_DOMAIN_MAPPING%TOTAL_NUMBER_OF_LOCAL
                IF(DEPENDENT_DOFS_PARAM_MAPPING%DOF_TYPE(1,local_ny)==FIELD_NODE_DOF_TYPE) THEN
                  ...
                  nk=DEPENDENT_DOFS_PARAM_MAPPING%NODE_DOF2PARAM_MAP(1,nyy)
                  np=DEPENDENT_DOFS_PARAM_MAPPING%NODE_DOF2PARAM_MAP(2,nyy)
                  nh=DEPENDENT_DOFS_PARAM_MAPPING%NODE_DOF2PARAM_MAP(3,nyy)
                  nc=DEPENDENT_DOFS_PARAM_MAPPING%NODE_DOF2PARAM_MAP(4,nyy)
                  !Loop over all elements containing the node
                ELSE
        ...
        ENDIF
      ELSE
        CALL FLAG_ERROR("Column locations is already associated",ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Row locations is already associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_GLOBAL_MATRIX_STRUCTURE_CALCULATE")
    RETURN
999 CALL ERRORS("PROBLEM_GLOBAL_MATRIX_STRUCTURE_CALCULATE",ERR,ERROR)
    CALL EXITS("PROBLEM_GLOBAL_MATRIX_STRUCTURE_CALCULATE")
    RETURN 1
...
999 CALL ERRORS("PROBLEMS_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEMS_INITIALISE(REGION,ERR,ERROR,*)
    ...
    !#### Subroutine: PROBLEMS_INITIALISE
    !###  Description:
