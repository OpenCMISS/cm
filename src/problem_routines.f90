...
MODULE PROBLEM_ROUTINES
  ...
  INTEGER(INTG), PARAMETER :: PROBLEM_LINEAR=1 !<The problem is linear. \see PROBLEM_ROUTINES_LinearityTypes,PROBLEM_ROUTINES
  ...
  PUBLIC PROBLEM_SOLUTION_CREATE_START,PROBLEM_SOLUTION_CREATE_FINISH
...
999 CALL ERRORS("PROBLEM_ANALYTIC_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_ASSEMBLE_LINEAR_STATIC_FEM(PROBLEM,ERR,ERROR,*)
    ...
    !#### Subroutine: PROBLEM_ASSEMBLE_LINEAR_STATIC_FEM
    !###  Description:
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      GEOMETRIC_FIELD=>PROBLEM%GEOMETRY%GEOMETRIC_FIELD
      FIBRE_FIELD=>PROBLEM%GEOMETRY%FIBRE_FIELD
      DEPENDENT_FIELD=>PROBLEM%DEPENDENT%DEPENDENT_FIELD
      MATERIAL_FIELD=>PROBLEM%MATERIALS%MATERIAL_FIELD
      SOURCE_FIELD=>PROBLEM%SOURCE%SOURCE_FIELD
      !DEPENDENT_ELEMENTS_MAPPING=>DEPENDENT_FIELD%DECOMPOSITION%DOMAIN(DEPENDENT_FIELD%DECOMPOSITION%MESH_COMPONENT_NUMBER)% &
      !  & MAPPINGS_ELEMENTS
      ...
      !IF(GLOBAL_MATRICES%UPDATE_MATRIX.OR.GLOBAL_MATRICES%UPDATE_VECTOR) THEN
      !  IF(GLOBAL_MATRICES%UPDATE_MATRIX) THEN
      !    CALL DISTRIBUTED_MATRIX_ALL_VALUES_SET(GLOBAL_MATRICES%STIFFNESS,0.0_DP,ERR,ERROR,*999)
      !  ENDIF
      !  IF(GLOBAL_MATRICES%UPDATE_VECTOR) THEN
      !    CALL DISTRIBUTED_VECTOR_ALL_VALUES_SET(GLOBAL_MATRICES%RHS,0.0_DP,ERR,ERROR,*999)
      !  ENDIF

        
        !CALL FIELD_INTERPOLATION_PARAMETERS_INITIALISE(GEOMETRIC_FIELD,FIELD_STANDARD_VARIABLE_TYPE,GEOMETRIC_INTERP_PARAMETERS, &
        !  & ERR,ERROR,*999)
        !CALL FIELD_INTERPOLATED_POINT_INITIALISE(GEOMETRIC_INTERP_PARAMETERS,GEOMETRIC_INTERP_POINT,ERR,ERROR,*999)
        !CALL FIELD_INTERPOLATION_PARAMETERS_INITIALISE(DEPENDENT_FIELD,FIELD_STANDARD_VARIABLE_TYPE,DEPENDENT_INTERP_PARAMETERS, &
        !  & ERR,ERROR,*999)
        !CALL FIELD_INTERPOLATED_POINT_INITIALISE(DEPENDENT_INTERP_PARAMETERS,DEPENDENT_INTERP_POINT,ERR,ERROR,*999)
        !IF(ASSOCIATED(MATERIAL_FIELD)) THEN
        !  CALL FIELD_INTERPOLATION_PARAMETERS_INITIALISE(MATERIAL_FIELD,FIELD_STANDARD_VARIABLE_TYPE,MATERIAL_INTERP_PARAMETERS, &
        !    & ERR,ERROR,*999)
        !  CALL FIELD_INTERPOLATED_POINT_INITIALISE(MATERIAL_INTERP_PARAMETERS,MATERIAL_INTERP_POINT,ERR,ERROR,*999)
        !ELSE
        !  NULLIFY(MATERIAL_INTERP_PARAMETERS)
        !  NULLIFY(MATERIAL_INTERP_POINT)
        !ENDIF
        !IF(ASSOCIATED(SOURCE_FIELD)) THEN
        !  CALL FIELD_INTERPOLATION_PARAMETERS_INITIALISE(SOURCE_FIELD,FIELD_STANDARD_VARIABLE_TYPE,SOURCE_INTERP_PARAMETERS, &
        !    & ERR,ERROR,*999)
        !  CALL FIELD_INTERPOLATED_POINT_INITIALISE(SOURCE_INTERP_PARAMETERS,SOURCE_INTERP_POINT,ERR,ERROR,*999)
        !ELSE
        !  NULLIFY(SOURCE_INTERP_PARAMETERS)
        !  NULLIFY(SOURCE_INTERP_POINT)
        !ENDIF
        
        !CALL FEM_ELEMENT_MATRICES_INITIALISE(PROBLEM,ELEMENT_MATRICES,ERR,ERROR,*999)
        !Loop over the elements
        DO ne=1,DEPENDENT_FIELD%DECOMPOSITION%TOPOLOGY%ELEMENTS%TOTAL_NUMBER_OF_ELEMENTS
          
        ENDDO !element_idx
        !CALL FEM_ELEMENT_MATRICES_FINALISE(ELEMENT_MATRICES,ERR,ERROR,*999)
        
        !CALL FIELD_INTERPOLATED_POINT_FINALISE(SOURCE_INTERP_POINT,ERR,ERROR,*999)
        !CALL FIELD_INTERPOLATED_POINT_FINALISE(MATERIAL_INTERP_POINT,ERR,ERROR,*999)
        !CALL FIELD_INTERPOLATED_POINT_FINALISE(DEPENDENT_INTERP_POINT,ERR,ERROR,*999)
        !CALL FIELD_INTERPOLATED_POINT_FINALISE(GEOMETRIC_INTERP_POINT,ERR,ERROR,*999)
        !CALL FIELD_INTERPOALTION_PARAMETERS_FINALISE(SOURCE_INTERP_PARAMETERS,ERR,ERROR,*999)
        !CALL FIELD_INTERPOALTION_PARAMETERS_FINALISE(MATERIAL_INTERP_PARAMETERS,ERR,ERROR,*999)
        !CALL FIELD_INTERPOALTION_PARAMETERS_FINALISE(DEPENDENT_INTERP_PARAMETERS,ERR,ERROR,*999)
        !CALL FIELD_INTERPOALTION_PARAMETERS_FINALISE(GEOMETRIC_INTERP_PARAMETERS,ERR,ERROR,*999)
      ...
      !Calculate the global right hand side.
    ELSE
      CALL FLAG_ERROR("Problem is not associated",ERR,ERROR,*999)
    ENDIF
...
999 CALL ERRORS("PROBLEM_CLASSICAL_FIELD_CLASS_TYPE_SET",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_ELEMENT_DATA_FINALISE(PROBLEM_SOLUTION,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_ELEMENT_DATA_FINALISE
    !###  Description:
    !###    Finalise the element data for a problem solution.
    ...
    CALL ENTERS("PROBLEM_ELEMENT_DATA_FINALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      IF(ASSOCIATED(PROBLEM_SOLUTION%ELEMENT_DATA)) THEN
        IF(ALLOCATED(PROBLEM_SOLUTION%ELEMENT_DATA%GLOBAL_MATRIX_INDICES)) &
          & DEALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA%GLOBAL_MATRIX_INDICES)
        IF(ALLOCATED(PROBLEM_SOLUTION%ELEMENT_DATA%STIFFNESS)) DEALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA%STIFFNESS)
        IF(ALLOCATED(PROBLEM_SOLUTION%ELEMENT_DATA%FLUX)) DEALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA%FLUX)
        IF(ALLOCATED(PROBLEM_SOLUTION%ELEMENT_DATA%DAMPING)) DEALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA%DAMPING)
        IF(ALLOCATED(PROBLEM_SOLUTION%ELEMENT_DATA%MASS)) DEALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA%MASS)
        IF(ALLOCATED(PROBLEM_SOLUTION%ELEMENT_DATA%RHS)) DEALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA%RHS)
        CALL FIELD_INTERPOLATION_PARAMETERS_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%GEOMETRIC_INTERP_PARAMETERS,ERR,ERROR,*999)
        CALL FIELD_INTERPOLATION_PARAMETERS_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%DEPENDENT_INTERP_PARAMETERS,ERR,ERROR,*999)
        CALL FIELD_INTERPOLATION_PARAMETERS_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%MATERIAL_INTERP_PARAMETERS,ERR,ERROR,*999)
        CALL FIELD_INTERPOLATION_PARAMETERS_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%SOURCE_INTERP_PARAMETERS,ERR,ERROR,*999)
        CALL FIELD_INTERPOLATED_POINT_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%GEOMETRIC_INTERP_POINT,ERR,ERROR,*999)
        CALL FIELD_INTERPOLATED_POINT_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%DEPENDENT_INTERP_POINT,ERR,ERROR,*999)
        CALL FIELD_INTERPOLATED_POINT_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%MATERIAL_INTERP_POINT,ERR,ERROR,*999)
        CALL FIELD_INTERPOLATED_POINT_FINALISE(PROBLEM_SOLUTION%ELEMENT_DATA%SOURCE_INTERP_POINT,ERR,ERROR,*999)
        DEALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem solution is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_ELEMENT_DATA_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_ELEMENT_DATA_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_ELEMENT_DATA_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_ELEMENT_DATA_FINALISE

  !
  !================================================================================================================================
  !

  SUBROUTINE PROBLEM_ELEMENT_DATA_INITIALISE(PROBLEM_SOLUTION,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_ELEMENT_DATA_INITIALISE
    !###  Description:
    !###    Initialises the element data for a problem solution.
    ...
    CALL ENTERS("PROBLEM_ELEMENT_DATA_INITIALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      IF(ASSOCIATED(PROBLEM_SOLUTION%ELEMENT_DATA)) THEN
        CALL FLAG_ERROR("Element data is already associated for this problem solution",ERR,ERROR,*999)
      ELSE
        ALLOCATE(PROBLEM_SOLUTION%ELEMENT_DATA,STAT=ERR)
        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate problem solution element data",ERR,ERROR,*999)
        PROBLEM_SOLUTION%ELEMENT_DATA%PROBLEM_SOLUTION=>PROBLEM_SOLUTION
        PROBLEM_SOLUTION%ELEMENT_DATA%NUMBER_ELEMENT_MATRIX_ROWS=0
        PROBLEM_SOLUTION%ELEMENT_DATA%NUMBER_ELEMENT_MATRIX_COLUMNS=0
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%GEOMETRIC_INTERP_PARAMETERS)
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%DEPENDENT_INTERP_PARAMETERS)
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%MATERIAL_INTERP_PARAMETERS)
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%SOURCE_INTERP_PARAMETERS)
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%GEOMETRIC_INTERP_POINT)
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%DEPENDENT_INTERP_POINT)
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%MATERIAL_INTERP_POINT)
        NULLIFY(PROBLEM_SOLUTION%ELEMENT_DATA%SOURCE_INTERP_POINT)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem solution is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_ELEMENT_DATA_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_ELEMENT_DATA_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_ELEMENT_DATA_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_ELEMENT_DATA_INITIALISE
        ...
        IF(PROBLEM%TYPE==PROBLEM_LAPLACE_EQUATION_TYPE) THEN
          IF(PROBLEM%SUBTYPE==PROBLEM_STANDARD_LAPLACE_SUBTYPE) THEN
            ...
            CASE(PROBLEM_ANALYTIC_SETUP_TYPE)
              SELECT CASE(ACTION_TYPE)
              CASE(PROBLEM_START_ACTION)
                IF(ASSOCIATED(PROBLEM%DEPENDENT)) THEN
...
999 CALL ERRORS("PROBLEM_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRIX_FINALISE(GLOBAL_MATRIX,ERR,ERROR,*)
    ...
    !#### Subroutine: PROBLEM_GLOBAL_MATRIX_FINALISE
    !###  Description:
    !###    Finalise the global matrix and deallocate all memory.

    !Argument variables
    TYPE(PROBLEM_GLOBAL_MATRICES_TYPE), POINTER :: GLOBAL_MATRIX
    INTEGER(INTG), INTENT(OUT) :: ERR
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR
    !Local Variables
    ...
    !IF(ASSOCIATED(GLOBAL_MATRIX%DOF_TO_SOLUTION_MAP)) &
    !  & CALL PROBLEM_DOF_TO_SOLUTION_MAP_FINALISE(GLOBAL_MATRIX%DOF_TO_SOLUTION_MAP,ERR,ERROR,*999)
    !IF(ASSOCIATED(GLOBAL_MATRIX%MATRIX)) CALL DISTRIBUTED_MATRIX_DESTORY(GLOBAL_MATRIX%MATRIX,ERR,ERROR,*999)
...
999 CALL ERRORS("PROBLEM_GLOBAL_MATRIX_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRIX_INITIALISE(GLOBAL_MATRIX,ERR,ERROR,*)
    ...
    !GLOBAL_MATRIX%GLOBAL_MATRIX_NUMBER=0
    !GLOBAL_MATRIX%GLOBAL_MATRIX_TYPE=0
    !GLOBAL_MATRIX%UPDATE_MATRIX=.FALSE.
    !NULLIFY(GLOBAL_MATRIX%DOF_TO_SOLUTION_MAP)
    !NULLIFY(GLOBAL_MATRIX%MATRXI)
...
999 CALL ERRORS("PROBLEM_GLOBAL_MATRIX_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_FINALISE(PROBLEM_SOLUTION,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      ...
      !      CALL PROBLEM_GLOBAL_MATRIX_FINALISE(PROBLEM_SOLUTION%GLOBAL_MATRICES,matrix_idx,ERR,ERROR,*999)
      !    ENDDO !matrix_idx
  ...
  SUBROUTINE PROBLEM_GLOBAL_MATRICES_INITIALISE(PROBLEM_SOLUTION,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      ...
      ELSE
        ...
        PROBLEM_SOLUTION%GLOBAL_MATRICES%SPARSE_GLOBAL_MATRICES=.FALSE.
        PROBLEM_SOLUTION%GLOBAL_MATRICES%NUMBER_OF_MATRICES=0
        NULLIFY(PROBLEM_SOLUTION%GLOBAL_MATRICES%RHS)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem solution is not associated",ERR,ERROR,*999)
    ENDIF
...
999 CALL ERRORS("PROBLEM_GLOBAL_MATRICES_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_INITIALISE(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      NULLIFY(PROBLEM%GEOMETRY)
      NULLIFY(PROBLEM%MATERIALS)
      NULLIFY(PROBLEM%SOURCE)
      NULLIFY(PROBLEM%DEPENDENT)
...
999 CALL ERRORS("PROBLEM_FIXED_CONDITIONS_SET_DOF1",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GEOMETRY_FINALISE(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(ASSOCIATED(PROBLEM%GEOMETRY)) THEN
        DEALLOCATE(PROBLEM%GEOMETRY)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem is not associated",ERR,ERROR,*999)
    ENDIF
...
999 CALL ERRORS("PROBLEM_GEOMETRY_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_GEOMETRY_INITIALISE(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(ASSOCIATED(PROBLEM%GEOMETRY)) THEN
        CALL FLAG_ERROR("Geometry is already associated for this problem",ERR,ERROR,*999)
      ELSE
        ALLOCATE(PROBLEM%GEOMETRY,STAT=ERR)
        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate problem geometry",ERR,ERROR,*999)
...
999 CALL ERRORS("PROBLEM_GEOMETRY_INITIALISE",ERR,ERROR)
 ...
 !
  ...
  SUBROUTINE PROBLEM_MAPPINGS_CALCULATE(PROBLEM_SOLUTION,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_MAPPINGS_CALCULATE
    !###  Description:
    !###    Calculate the global to solution mappings for a problem solution.
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR
    !Local Variables
    INTEGER(INTG) :: global_ny,my_computational_node_number,NUMBER_OF_GLOBAL_SOLUTION_DOFS
    TYPE(DOMAIN_MAPPING_TYPE), POINTER :: DEPENDENT_DOF_MAPPINGS
    TYPE(DOMAIN_MAPPING_TYPE), POINTER :: SOLUTION_DOF_MAPPINGS
    TYPE(FIELD_TYPE), POINTER :: DEPENDENT_FIELD
    TYPE(PROBLEM_DEPENDENT_TYPE), POINTER :: DEPENDENT
    TYPE(PROBLEM_FIXED_CONDITIONS_TYPE), POINTER :: FIXED_CONDITIONS
    TYPE(PROBLEM_MAPPINGS_TYPE), POINTER :: PROBLEM_MAPPINGS

    CALL ENTERS("PROBLEM_MAPPINGS_CALCUALTE",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      PROBLEM_MAPPINGS=>PROBLEM_SOLUTION%MAPPINGS
      IF(ASSOCIATED(PROBLEM_MAPPINGS)) THEN
        DEPENDENT=>PROBLEM_SOLUTION%PROBLEM%DEPENDENT
        FIXED_CONDITIONS=>PROBLEM_SOLUTION%PROBLEM%FIXED_CONDITIONS
        DEPENDENT_FIELD=>DEPENDENT%DEPENDENT_FIELD
        DEPENDENT_DOF_MAPPINGS=>DEPENDENT_FIELD%MAPPINGS%DOMAIN_MAPPING
        my_computational_node_number=COMPUTATIONAL_NODE_NUMBER_GET(ERR,ERROR)
        IF(ERR/=0) GOTO 999
        NUMBER_OF_GLOBAL_SOLUTION_DOFS=COUNT(FIXED_CONDITIONS%GLOBAL_BOUNDARY_CONDITIONS==PROBLEM_NOT_FIXED)
        ALLOCATE(PROBLEM_MAPPINGS%DOMAIN_MAPPING,STAT=ERR)
        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate problem domain mappings",ERR,ERROR,*999)
        SOLUTION_DOF_MAPPINGS=>PROBLEM_MAPPINGS%DOMAIN_MAPPING
        ALLOCATE(SOLUTION_DOF_MAPPINGS%GLOBAL_TO_LOCAL_MAP(NUMBER_OF_GLOBAL_SOLUTION_DOFS),STAT=ERR)
        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate problem domain mappings global to local map",ERR,ERROR,*999)
        SOLUTION_DOF_MAPPINGS%NUMBER_OF_GLOBAL=NUMBER_OF_GLOBAL_SOLUTION_DOFS
        DO global_ny=1,DEPENDENT_DOF_MAPPINGS%NUMBER_OF_GLOBAL
        ENDDO!
      ELSE
        CALL FLAG_ERROR("Problem mappings is not associated",ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_MAPPINGS_CALCULATE")
    RETURN
999 CALL ERRORS("PROBLEM_MAPPINGS_CALCULATE",ERR,ERROR)
    CALL EXITS("PROBLEM_MAPPINGS_CALCULATE")
    RETURN 1
  END SUBROUTINE PROBLEM_MAPPINGS_CALCULATE

   !
  !================================================================================================================================
  !

  SUBROUTINE PROBLEM_MAPPINGS_FINALISE(PROBLEM_SOLUTION,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_MAPPINGS_FINALISE
    !###  Description:
    !###    Finalise the mappings for a problem solution.
    ...
    CALL ENTERS("PROBLEM_MAPPINGS_FINALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      IF(ASSOCIATED(PROBLEM_SOLUTION%MAPPINGS)) THEN
        DEALLOCATE(PROBLEM_SOLUTION%MAPPINGS)
      ENDIF
    ENDIF
       
    CALL EXITS("PROBLEM_MAPPINGS_FINALISE")
    RETURN
999 CALL ERRORS("PROBLEM_MAPPINGS_FINALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_MAPPINGS_FINALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_MAPPINGS_FINALISE

  !
  !================================================================================================================================
  !

  SUBROUTINE PROBLEM_MAPPINGS_INITIALISE(PROBLEM_SOLUTION,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_MAPPINGS_INITIALISE
    !###  Description:
    !###    Initialises the mappings for a problem solution.
    ...
    CALL ENTERS("PROBLEM_MAPPINGS_INITIALISE",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM_SOLUTION)) THEN
      IF(ASSOCIATED(PROBLEM_SOLUTION%MAPPINGS)) THEN
        CALL FLAG_ERROR("Mappings is already associated for this problem solution",ERR,ERROR,*999)
      ELSE
        ALLOCATE(PROBLEM_SOLUTION%MAPPINGS,STAT=ERR)
        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate problem solution mappings",ERR,ERROR,*999)
        PROBLEM_SOLUTION%MAPPINGS%PROBLEM_SOLUTION=>PROBLEM_SOLUTION
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem solution is not associated",ERR,ERROR,*999)
    ENDIF
       
    CALL EXITS("PROBLEM_MAPPINGS_INITIALISE")
    RETURN
999 CALL ERRORS("PROBLEM_MAPPINGS_INITIALISE",ERR,ERROR)
    CALL EXITS("PROBLEM_MAPPINGS_INITIALISE")
    RETURN 1
  END SUBROUTINE PROBLEM_MAPPINGS_INITIALISE
...
999 CALL ERRORS("PROBLEM_DEPENDENT_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_DEPENDENT_CREATE_START(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(ASSOCIATED(PROBLEM%DEPENDENT)) THEN
        CALL FLAG_ERROR("The problem dependent is already associated",ERR,ERROR,*999)        
      ELSE
        !Initialise the problem solution
  ...
  SUBROUTINE PROBLEM_DEPENDENT_DEPENDENT_FIELD_GET(PROBLEM,DEPENDENT_FIELD,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(ASSOCIATED(PROBLEM%DEPENDENT)) THEN
        ...
        ENDIF
      ELSE
        CALL FLAG_ERROR("Dependent is not associated for this problem",ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem is not associated",ERR,ERROR,*999)
    ENDIF
...
999 CALL ERRORS("PROBLEM_DEPENDENT_DEPENDENT_FIELD_GET",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_DEPENDENT_FINALISE(PROBLEM,ERR,ERROR,*)
    ...
    !#### Subroutine: PROBLEM_DEPENDENT_FINALISE
    !###  Description:
    !###    Finalise the dependent variables for a problem.
...
999 CALL ERRORS("PROBLEM_DEPENDENT_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_DEPENDENT_INITIALISE(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(ASSOCIATED(PROBLEM%DEPENDENT)) THEN
        CALL FLAG_ERROR("Dependent is already associated for this problem",ERR,ERROR,*999)
      ELSE
        ALLOCATE(PROBLEM%DEPENDENT,STAT=ERR)
        IF(ERR/=0) CALL FLAG_ERROR("Could not allocate problem dependent",ERR,ERROR,*999)
...
999 CALL ERRORS("PROBLEM_DEPENDENT_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SOLVER_MATRICES_FINALISE(PROBLEM_SOLUTION,ERR,ERROR,*)
    ...
    !#### Subroutine: PROBLEM_SOLVER_MATRICES_FINALISE
    !###  Description:
    !###    Finalise the solver matrices for the problem solution and deallocates all memory.
...
999 CALL ERRORS("PROBLEM_SOLVER_MATRICES_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SOLVER_MATRICES_INITALISE(PROBLEM_SOLUTION,ERR,ERROR,*)
    ...
    !#### Subroutine: PROBLEM_SOLVER_MATRICES_INITALISE
    !###  Description:
    !###    Initialise the solver matrices for the problem solution.
...
999 CALL ERRORS("PROBLEM_SOLVE_LINEAR_STATIC_FEM",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SOLVE_SETUP_LINEAR_STATIC_FEM(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      !      & ERR,ERROR,*999)
      !    CALL PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM(PROBLEM,ROW_LOCATIONS,COLUMN_LOCATIONS,ERR,ERROR,*999)
      !    CALL DISTRIBUTED_MATRIX_STORAGE_LOCATIONS_SET(GLOBAL_MATRICES%STIFFNESS,ROW_LOCATIONS,COLUMN_LOCATIONS,ERR,ERROR,*999)
...
999 CALL ERRORS("PROBLEM_SOLVE_SETUP_LINEAR_STATIC_FEM",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM(PROBLEM,FIELD_VARIABLE_NUMBER,ROW_LOCATIONS,COLUMN_LOCATIONS,ERR,ERROR,*)

    !#### Subroutine: PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM
    !###  Description:
    !###    Calculates the storage locations (sparsity) for finite element problems.
    
    !Argument variables
    TYPE(PROBLEM_TYPE), POINTER :: PROBLEM
    INTEGER(INTG), INTENT(IN) :: FIELD_VARIABLE_NUMBER
    INTEGER(INTG), POINTER :: ROW_LOCATIONS(:)
    INTEGER(INTG), POINTER :: COLUMN_LOCATIONS(:)
    INTEGER(INTG), INTENT(OUT) :: ERR
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR
    !Local Variables
    ...
    CALL ENTERS("PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM",ERR,ERROR,*999)

    IF(ASSOCIATED(PROBLEM)) THEN
      IF(FIELD_VARIABLE_NUMBER>0.AND.FIELD_VARIABLE_NUMBER<=FIELD_NUMBER_OF_VARIABLE_TYPES) THEN
        DEPENDENT_FIELD=>PROBLEM%DEPENDENT%DEPENDENT_FIELD
        FIELD_VARIABLE=>DEPENDENT_FIELD%VARIABLE_TYPE_MAP(FIELD_VARIABLE_NUMBER)%PTR
        DEPENDENT_DOFS_DOMAIN_MAPPING=>DEPENDENT_FIELD%MAPPINGS%DOMAIN_MAPPING
        ...
        INITIAL_SIZE=FIELD_VARIABLE%MAX_NUMBER_OF_INTERPOLATION_PARAMETERS*FIELD_VARIABLE%NUMBER_OF_COMPONENTS
        DO local_ny=1,DEPENDENT_DOFS_DOMAIN_MAPPING%TOTAL_NUMBER_OF_LOCAL
          ...
          CALL LIST_INITIAL_SIZE_SET(COLUMN_INDICES_LISTS(local_ny)%PTR,INITIAL_SIZE,ERR,ERROR,*999)
          CALL LIST_CREATE_FINISH(COLUMN_INDICES_LISTS(local_ny)%PTR,ERR,ERROR,*999)
          global_ny=DEPENDENT_DOFS_DOMAIN_MAPPING%LOCAL_TO_GLOBAL_MAP(local_ny)
          IF(DEPENDENT_DOFS_DOMAIN_MAPPING%GLOBAL_TO_LOCAL_MAP(global_ny)%LOCAL_TYPE(1)/=DOMAIN_LOCAL_GHOST) THEN
        ...
        ENDDO !local_ny
      ELSE
        LOCAL_ERROR="The field variable type of "//TRIM(NUMBER_TO_VSTRING(FIELD_VARIABLE_NUMBER,"*",ERR,ERROR))// &
          & " is invalid. It must be >0 and <="//TRIM(NUMBER_TO_VSTRING(FIELD_NUMBER_OF_VARIABLE_TYPES,"*",ERR,ERROR))
        CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
      ENDIF
    ELSE
      CALL FLAG_ERROR("Problem is not associated",ERR,ERROR,*999)
    ...
    CALL EXITS("PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM")
    RETURN
999 CALL ERRORS("PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM",ERR,ERROR)
    CALL EXITS("PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM")
    RETURN 1
  END SUBROUTINE PROBLEM_STORAGE_LOCATIONS_CALCULATE_FEM
  ...
  SUBROUTINE PROBLEM_SOLUTION_CREATE_FINISH(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      IF(ASSOCIATED(SOLUTION)) THEN
        ...
        ELSE
          CALL PROBLEM_MAPPINGS_INITIALISE(SOLUTION,ERR,ERROR,*999)
          CALL PROBLEM_MAPPINGS_CALCULATE(SOLUTION,ERR,ERROR,*999)
...
999 CALL ERRORS("PROBLEM_SOLUTION_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SOLUTION_CREATE_START(PROBLEM,ERR,ERROR,*)
    ...
    !#### Subroutine: PROBLEM_SOLUTION_CREATE_START
    !###  Description:
    !###    Start the creation of a solution for a problem.
...
999 CALL ERRORS("PROBLEM_SOLUTION_CREATE_START",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SOLUTION_FINALISE(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(ASSOCIATED(PROBLEM%SOLUTION)) THEN
        IF(ASSOCIATED(PROBLEM%SOLUTION%ELEMENT_DATA)) CALL PROBLEM_ELEMENT_DATA_FINALISE(PROBLEM%SOLUTION,ERR,ERROR,*999)
  ...
  SUBROUTINE PROBLEM_SOLUTION_INITIALISE(PROBLEM,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      ELSE
        ...
        NULLIFY(PROBLEM%SOLUTION%ELEMENT_DATA)
