...
MODULE FIELD_ROUTINES
  ...
  PUBLIC FIELD_CREATE_FINISH,FIELD_CREATE_START,FIELD_DESTROY,FIELDS_FINALISE,FIELDS_INITIALISE,FIELD_NUMBER_OF_COMPONENTS_SET, &
    & FIELD_NUMBER_OF_VARIABLES_SET
  
  PUBLIC FIELD_COMPONENT_MESH_COMPONENT_SET,FIELD_COMPONENT_INTERPOLATION_SET,FIELD_COMPONENT_VALUES_INITIALISE
  
  PUBLIC FIELD_DEPENDENT_TYPE_SET,FIELD_GEOMETRIC_FIELD_SET,FIELD_MESH_DECOMPOSITION_SET,FIELD_NEXT_NUMBER_FIND, &
  ...
  PUBLIC FIELD_ADD_FIELD_COMPONENT_TO_ANOTHER_FIELD_COMPONENT,FIELD_PARAMETER_SET_ADD_CONSTANT,FIELD_PARAMETER_SET_ADD_DOF, &
    & FIELD_PARAMETER_SET_ADD_ELEMENT,FIELD_PARAMETER_SET_ADD_NODE,FIELD_PARAMETER_SET_CREATE,FIELD_PARAMETER_SET_DATA_GET, &
    & FIELD_PARAMETER_SET_DATA_RESTORE,FIELD_PARAMETER_SET_UPDATE_FINISH,FIELD_PARAMETER_SET_UPDATE_START, &
    & FIELD_PARAMETER_SET_UPDATE_CONSTANT,FIELD_PARAMETER_SET_UPDATE_DOF,FIELD_PARAMETER_SET_UPDATE_ELEMENT, &
    & FIELD_PARAMETER_SET_UPDATE_NODE,FIELD_PARAMETER_SET_VECTOR_GET
...
CONTAINS
  ...
  SUBROUTINE FIELD_COMPONENT_INTERPOLATION_GET(FIELD,FIELD_VARIABLE_NUMBER,FIELD_COMPONENT_NUMBER,INTERPOLATION_TYPE,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(OUT) :: INTERPOLATION_TYPE !<On return, the interpolation type to get \see FIELD_ROUTINES_VariableTypes,FIELD_ROUTINES
...
999 CALL ERRORS("FIELD_COMPONENT_INTERPOLATION_SET_PTR",ERR,ERROR)
  ...
  SUBROUTINE FIELD_COMPONENT_MESH_COMPONENT_GET(FIELD,VARIABLE_TYPE,COMPONENT_NUMBER,MESH_COMPONENT,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(IN) :: VARIABLE_TYPE !<The field variable type to get the field variable component for
...
999 CALL ERRORS("FIELD_COMPONENT_MESH_COMPONENT_SET_PTR",ERR,ERROR)
  ...
  SUBROUTINE FIELD_COMPONENT_VALUES_INITIALISE(FIELD,FIELD_VAR_TYPE,FIELD_COMPONENT_NUMBER,FIELD_SET_TYPE,VALUE,ERR,ERROR,*)
    
    !Argument variables
    TYPE(FIELD_TYPE), POINTER :: FIELD !<A pointer to the field to initialise the values for 
    INTEGER(INTG), INTENT(IN) :: FIELD_VAR_TYPE !<The field variable type to intiialise
    INTEGER(INTG), INTENT(IN) :: FIELD_COMPONENT_NUMBER !<The field component number to initialise
    INTEGER(INTG), INTENT(IN) :: FIELD_SET_TYPE !<The field parameter set identifier to initialise
    ...
    IF(ASSOCIATED(FIELD)) THEN
      IF(FIELD%FIELD_FINISHED) THEN
        ...
        IF(FIELD_VAR_TYPE>0.AND.FIELD_VAR_TYPE<=FIELD_NUMBER_OF_VARIABLE_TYPES) THEN
          FIELD_VARIABLE=>FIELD%VARIABLE_TYPE_MAP(FIELD_VAR_TYPE)%PTR
          IF(ASSOCIATED(FIELD_VARIABLE)) THEN
            !Check the component number
            IF(FIELD_COMPONENT_NUMBER>0.AND.FIELD_COMPONENT_NUMBER<=FIELD_VARIABLE%NUMBER_OF_COMPONENTS) THEN
              !Check the from set type input
              IF(FIELD_SET_TYPE>0.AND.FIELD_SET_TYPE<=FIELD_NUMBER_OF_SET_TYPES) THEN
                FIELD_PARAMETER_SET=>FIELD%PARAMETER_SETS%SET_TYPE(FIELD_SET_TYPE)%PTR
                IF(ASSOCIATED(FIELD_PARAMETER_SET)) THEN
                  ...
                  SELECT CASE(FIELD_VARIABLE%COMPONENTS(FIELD_COMPONENT_NUMBER)%INTERPOLATION_TYPE)
                  CASE(FIELD_CONSTANT_INTERPOLATION)
                    field_dof=FIELD_VARIABLE%COMPONENTS(FIELD_COMPONENT_NUMBER)%PARAM_TO_DOF_MAP%CONSTANT_PARAM2DOF_MAP(0)
                    FIELD_PARAMETERS(field_dof)=VALUE
                  CASE(FIELD_ELEMENT_BASED_INTERPOLATION)
                    COMPONENT_DOMAIN=>FIELD_VARIABLE%COMPONENTS(FIELD_COMPONENT_NUMBER)%DOMAIN
                      ...
                      IF(ASSOCIATED(DOMAIN_TOPOLOGY)) THEN
                        ...
                        IF(ASSOCIATED(DOMAIN_ELEMENTS)) THEN
                          DO element_idx=1,DOMAIN_ELEMENTS%TOTAL_NUMBER_OF_ELEMENTS
                            field_dof=FIELD_VARIABLE%COMPONENTS(FIELD_COMPONENT_NUMBER)%PARAM_TO_DOF_MAP% &
                              & ELEMENT_PARAM2DOF_MAP(element_idx,0)
                            FIELD_PARAMETERS(field_dof)=VALUE
                          ENDDO !element_idx
                        ELSE
                        ...
                        IF(ASSOCIATED(DOMAIN_NODES)) THEN
                          DO node_idx=1,DOMAIN_NODES%TOTAL_NUMBER_OF_NODES
                            DO derivative_idx=1,DOMAIN_NODES%NODES(node_idx)%NUMBER_OF_DERIVATIVES
                              field_dof=FIELD_VARIABLE%COMPONENTS(FIELD_COMPONENT_NUMBER)%PARAM_TO_DOF_MAP% &
                                & NODE_PARAM2DOF_MAP(derivative_idx,node_idx,0)
                  ...
                  CASE DEFAULT
                    LOCAL_ERROR="The interpolation type of "//TRIM(NUMBER_TO_VSTRING(FIELD_VARIABLE% &
                      & COMPONENTS(FIELD_COMPONENT_NUMBER)%INTERPOLATION_TYPE,"*",ERR,ERROR))// &
                      & " is invalid for field component number "// &
                      & TRIM(NUMBER_TO_VSTRING(FIELD_COMPONENT_NUMBER,"*",ERR,ERROR))// &
                      & " of field variable type "//TRIM(NUMBER_TO_VSTRING(FIELD_VAR_TYPE,"*",ERR,ERROR))// &
                ...
                ELSE
                  LOCAL_ERROR="The field parameter set type of "//TRIM(NUMBER_TO_VSTRING(FIELD_SET_TYPE,"*",ERR,ERROR))// &
                    & " has not be created on field number "//TRIM(NUMBER_TO_VSTRING(FIELD%USER_NUMBER,"*",ERR,ERROR))//"."
                  CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
                ENDIF
              ELSE
                LOCAL_ERROR="The field to set type of "//TRIM(NUMBER_TO_VSTRING(FIELD_SET_TYPE,"*",ERR,ERROR))// &                 
            ...
            ELSE
              LOCAL_ERROR="The field variable component number of "// &
                & TRIM(NUMBER_TO_VSTRING(FIELD_COMPONENT_NUMBER,"*",ERR,ERROR))//" is invalid for a field variable type of "//&
                & TRIM(NUMBER_TO_VSTRING(FIELD_VAR_TYPE,"*",ERR,ERROR))//" on field number "// &
                & TRIM(NUMBER_TO_VSTRING(FIELD%USER_NUMBER,"*",ERR,ERROR))//". The number of components must be > 1 and <= "// &
                & TRIM(NUMBER_TO_VSTRING(FIELD_VARIABLE%NUMBER_OF_COMPONENTS,"*",ERR,ERROR))//"."
              CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
            ENDIF
          ELSE
            LOCAL_ERROR="The field variable type of "//TRIM(NUMBER_TO_VSTRING(FIELD_VAR_TYPE,"*",ERR,ERROR))// &
...
999 CALL ERRORS("FIELD_VARIABLE_COMPONENT_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE FIELD_VARIABLE_COMPONENT_INITIALISE(FIELD,VARIABLE_NUMBER,COMPONENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: COMP_NUMBER,DUMMY_ERR,ne
            ...
            IF(COMPONENT_NUMBER>=1.AND.COMPONENT_NUMBER<=FIELD%VARIABLES(VARIABLE_NUMBER)%NUMBER_OF_COMPONENTS) THEN
              ...
              IF(ASSOCIATED(DECOMPOSITION)) THEN
                ...
                IF(ASSOCIATED(MESH)) THEN
                  COMP_NUMBER=FIELD%CREATE_VALUES_CACHE%MESH_COMPONENT_NUMBER(COMPONENT_NUMBER,VARIABLE_NUMBER)
                  IF(COMP_NUMBER>0.AND.COMP_NUMBER<=MESH%NUMBER_OF_COMPONENTS) THEN
                    FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)%MESH_COMPONENT_NUMBER=COMP_NUMBER
                    FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)%DOMAIN=> &
                      & DECOMPOSITION%DOMAIN(COMP_NUMBER)%PTR
    ...
    RETURN
999 CALL FIELD_VARIABLE_COMPONENT_FINALISE(FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER),DUMMY_ERR,DUMMY_ERROR,*998)
...
999 CALL ERRORS("FIELD_NEXT_NUMBER_FIND",ERR,ERROR)
  ...
  SUBROUTINE FIELD_NUMBER_OF_COMPONENTS_GET(FIELD,VARIABLE_TYPE,NUMBER_OF_COMPONENTS,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(IN) :: VARIABLE_TYPE !<The field variable type
...
999 IF(ALLOCATED(OLD_VARIABLE_TYPES)) DEALLOCATE(OLD_VARIABLE_TYPES)
  ...
  SUBROUTINE FIELD_PARAMETER_SETS_ADD_DP(FIELD,ALPHA,FIELD_FROM_SET_TYPE,FIELD_TO_SET_TYPE,ERR,ERROR,*)
              ...
              IF(SIZE(ALPHA,1)==SIZE(FIELD_FROM_SET_TYPE,1)) THEN
                DO parameter_set_idx=1,SIZE(FIELD_FROM_SET_TYPE,1)
                  IF(FIELD_FROM_SET_TYPE(parameter_set_idx)>0.AND. &
                    ...
                    FIELD_FROM_PARAMETER_SET=>FIELD%PARAMETER_SETS%SET_TYPE(FIELD_TO_SET_TYPE)%PTR
                    IF(ASSOCIATED(FIELD_TO_PARAMETER_SET)) THEN
                ...
                !Add the field dofs
                DO dof_idx=1,FIELD_DOMAIN_MAPPING%TOTAL_NUMBER_OF_LOCAL
                  VALUE=0.0_DP
                  DO parameter_set_idx=1,SIZE(FIELD_FROM_SET_TYPE,1)
...
999 CALL ERRORS("FIELD_PARAMETER_SETS_ADD_DP",ERR,ERROR)
 ...
 !
  ...
  !>Adds the alpha times the parameter set values from one parameter set type to another parameter set type \todo make this call distributed vector add???
  SUBROUTINE FIELD_PARAMETER_SETS_ADD_DP1(FIELD,ALPHA,FIELD_FROM_SET_TYPE,FIELD_TO_SET_TYPE,ERR,ERROR,*)
    ...
    CALL FIELD_PARAMETER_SETS_ADD_DP(FIELD,(/ALPHA/),(/FIELD_FROM_SET_TYPE/),FIELD_TO_SET_TYPE,ERR,ERROR,*999)
...
999 CALL ERRORS("FIELD_PARAMETER_SETS_ADD_DP1",ERR,ERROR)
  ...
  SUBROUTINE FIELD_PARAMETER_SETS_COPY(FIELD,FIELD_FROM_SET_TYPE,FIELD_TO_SET_TYPE,ERR,ERROR,*)
...
999 CALL ERRORS("FIELD_PARAMETER_SETS_COPY",ERR,ERROR)
  ...
  !>Adds parameters of a component of a variable to paramters of a compoenet of another field
  SUBROUTINE FIELD_ADD_FIELD_COMPONENT_TO_ANOTHER_FIELD_COMPONENT(FIELD,FIELD_VARIABLE_NUMBER,FIELD_COMPONENT_NUMBER, &
    & FIELD_TO,FIELD_TO_VARIABLE_NUMBER,FIELD_TO_COMPONENT_NUMBER,ERR,ERROR,*)

    !Argument variables
    TYPE(FIELD_TYPE), POINTER :: FIELD,FIELD_TO !<Pointers to the field and to-field 
    INTEGER(INTG), INTENT(IN) :: FIELD_VARIABLE_NUMBER,FIELD_TO_VARIABLE_NUMBER !<Variable numbers of the above fields 
    INTEGER(INTG), INTENT(IN) :: FIELD_COMPONENT_NUMBER,FIELD_TO_COMPONENT_NUMBER !<Component numbers in the above variables
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: TOTAL_COMPONENT_DOFS,TOTAL_TO_COMPONENT_DOFS,dof_idx  
    REAL(DP), POINTER :: FIELD_PARAMETERS(:)
    REAL(DP) :: VALUE
    TYPE(VARYING_STRING) :: LOCAL_ERROR

  
    CALL ENTERS("FIELD_ADD_FIELD_COMPONENT_TO_ANOTHER_FIELD_COMPONENT",ERR,ERROR,*999)
    
    IF(ASSOCIATED(FIELD)) THEN	
      IF(FIELD_VARIABLE_NUMBER<=FIELD%NUMBER_OF_VARIABLES) THEN
        IF(FIELD_COMPONENT_NUMBER<=FIELD%VARIABLES(FIELD_VARIABLE_NUMBER)%NUMBER_OF_COMPONENTS) THEN  
	  TOTAL_COMPONENT_DOFS=FIELD%VARIABLES(FIELD_VARIABLE_NUMBER)%COMPONENTS(FIELD_COMPONENT_NUMBER)% &
	    & PARAM_TO_DOF_MAP%NUMBER_OF_NODE_PARAMETERS	    
          IF(ASSOCIATED(FIELD_TO)) THEN
            IF(FIELD_TO_VARIABLE_NUMBER<=FIELD_TO%NUMBER_OF_VARIABLES) THEN	    
	      IF(FIELD_TO_COMPONENT_NUMBER<=FIELD_TO%VARIABLES(FIELD_TO_VARIABLE_NUMBER)%NUMBER_OF_COMPONENTS) THEN	  
	        TOTAL_TO_COMPONENT_DOFS=FIELD_TO%VARIABLES(FIELD_TO_VARIABLE_NUMBER)%COMPONENTS(FIELD_TO_COMPONENT_NUMBER)% &
	          & PARAM_TO_DOF_MAP%NUMBER_OF_NODE_PARAMETERS	    		
		IF(TOTAL_COMPONENT_DOFS==TOTAL_TO_COMPONENT_DOFS) THEN
                  CALL FIELD_PARAMETER_SET_DATA_GET(FIELD,FIELD_VALUES_SET_TYPE,FIELD_PARAMETERS,ERR,ERROR,*999)
		  DO dof_idx=(FIELD_COMPONENT_NUMBER-1)*TOTAL_COMPONENT_DOFS+1, &
		    & (FIELD_COMPONENT_NUMBER-1)*TOTAL_COMPONENT_DOFS+TOTAL_COMPONENT_DOFS
		    VALUE=FIELD_PARAMETERS(dof_idx)
		    CALL FIELD_PARAMETER_SET_ADD_DOF(FIELD_TO,FIELD_VALUES_SET_TYPE,dof_idx,VALUE,ERR,ERROR,*999)
		  ENDDO
		  NULLIFY(FIELD_PARAMETERS)				
	        ELSE
                  LOCAL_ERROR="Number of component parameters is not equal"
		  CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)	      
                ENDIF  	                  
	      ELSE
                LOCAL_ERROR="Component number of FIELD_TO is not valid"
		CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)             
	      ENDIF	
	    ELSE
              LOCAL_ERROR="Field variable number of FIELD_TO is not valid"
              CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)	  	  	    
	    ENDIF      	  
	  ELSE	  
            LOCAL_ERROR="FIELD_TO is not associated"
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)	  	  
	  ENDIF
        ELSE
          LOCAL_ERROR="Component number of the specified field number of FIELD is not valid"
          CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)	  	  
	ENDIF
      ELSE
        LOCAL_ERROR="Field number of FIELD is not valid"
	CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)      
      ENDIF
    ELSE
      LOCAL_ERROR="FIELD is not associated"
      CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
    ENDIF


    CALL EXITS("FIELD_ADD_FIELD_COMPONENT_TO_ANOTHER_FIELD_COMPONENT")
    RETURN
999 CALL ERRORS("FIELD_ADD_FIELD_COMPONENT_TO_ANOTHER_FIELD_COMPONENT",ERR,ERROR)
    CALL EXITS("FIELD_ADD_FIELD_COMPONENT_TO_ANOTHER_FIELD_COMPONENT")
    RETURN 1
  END SUBROUTINE FIELD_ADD_FIELD_COMPONENT_TO_ANOTHER_FIELD_COMPONENT
  ...
  SUBROUTINE FIELD_PARAMETER_SET_ADD_CONSTANT(FIELD,FIELD_SET_TYPE,COMPONENT_NUMBER,VARIABLE_NUMBER,VALUE,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(IN) :: VARIABLE_NUMBER !<The field variable to add
    ...
    CALL ENTERS("FIELD_PARAMETER_SET_ADD_CONSTANT",ERR,ERROR,*999)
    ...
    CALL EXITS("FIELD_PARAMETER_SET_ADD_CONSTANT")
    RETURN
999 CALL ERRORS("FIELD_PARAMETER_SET_ADD_CONSTANT",ERR,ERROR)
    CALL EXITS("FIELD_PARAMETER_SET_ADD_CONSTANT")
    RETURN 1
  END SUBROUTINE FIELD_PARAMETER_SET_ADD_CONSTANT

  !
  !================================================================================================================================
  !

  !>Adds the given value to the given parameter set for a particular dof of the field.
  SUBROUTINE FIELD_PARAMETER_SET_ADD_DOF(FIELD,FIELD_SET_TYPE,DOF_NUMBER,VALUE,ERR,ERROR,*)
    ...
    CALL ENTERS("FIELD_PARAMETER_SET_ADD_DOF",ERR,ERROR,*999)
...
!!TODO: Allow to specify a global number and then have it all update accordingly???
    ...
    CALL EXITS("FIELD_PARAMETER_SET_ADD_DOF")
    RETURN
999 CALL ERRORS("FIELD_PARAMETER_SET_ADD_DOF",ERR,ERROR)
    CALL EXITS("FIELD_PARAMETER_SET_ADD_DOF")
    RETURN 1
  END SUBROUTINE FIELD_PARAMETER_SET_ADD_DOF

  !
  !================================================================================================================================
  !

  !>Adds the given value to the given parameter set for a particular element of the field variable component.
  SUBROUTINE FIELD_PARAMETER_SET_ADD_ELEMENT(FIELD,FIELD_SET_TYPE,ELEMENT_NUMBER,COMPONENT_NUMBER,VARIABLE_NUMBER, &
    & VALUE,ERR,ERROR,*)

    !Argument variables
    TYPE(FIELD_TYPE), POINTER :: FIELD !<A pointer to the field to add
    INTEGER(INTG), INTENT(IN) :: FIELD_SET_TYPE !<The field parameter set identifier
    INTEGER(INTG), INTENT(IN) :: ELEMENT_NUMBER !<The element number to add
    ...
    CALL ENTERS("FIELD_PARAMETER_SET_ADD_ELEMENT",ERR,ERROR,*999)
    ...
    CALL EXITS("FIELD_PARAMETER_SET_ADD_ELEMENT")
    RETURN
999 CALL ERRORS("FIELD_PARAMETER_SET_ADD_ELEMENT",ERR,ERROR)
    CALL EXITS("FIELD_PARAMETER_SET_ADD_ELEMENT")
    RETURN 1
  END SUBROUTINE FIELD_PARAMETER_SET_ADD_ELEMENT

  !
  !================================================================================================================================
  !

  !>Adds the given value to the given parameter set for a particular node and derivative of the field variable component.
  SUBROUTINE FIELD_PARAMETER_SET_ADD_NODE(FIELD,FIELD_SET_TYPE,DERIVATIVE_NUMBER,NODE_NUMBER,COMPONENT_NUMBER,VARIABLE_NUMBER, &
    & VALUE,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(IN) :: NODE_NUMBER !<The node number to add
    INTEGER(INTG), INTENT(IN) :: COMPONENT_NUMBER !<The field variable component number to add
    INTEGER(INTG), INTENT(IN) :: VARIABLE_NUMBER !<The field variable to add
    REAL(DP), INTENT(IN) :: VALUE !<The value to add
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    ...
    CALL ENTERS("FIELD_PARAMETER_SET_ADD_NODE",ERR,ERROR,*999)
    ...
    CALL EXITS("FIELD_PARAMETER_SET_ADD_NODE")
    RETURN
999 CALL ERRORS("FIELD_PARAMETER_SET_ADD_NODE",ERR,ERROR)
    CALL EXITS("FIELD_PARAMETER_SET_ADD_NODE")
    RETURN 1
  END SUBROUTINE FIELD_PARAMETER_SET_ADD_NODE
    ...
    IF(ASSOCIATED(FIELD)) THEN
      ...
      IF(FIELD_SET_TYPE>0.AND.FIELD_SET_TYPE<FIELD_NUMBER_OF_SET_TYPES) THEN
        ...
        ELSE
          ...
          ALLOCATE(FIELD%PARAMETER_SETS%PARAMETER_SETS(FIELD%PARAMETER_SETS%NUMBER_OF_PARAMETER_SETS+1),STAT=ERR)
          IF(ERR/=0) CALL FLAG_ERROR("Could not allocat field parameter sets parameter sets.",ERR,ERROR,*999)
          DO parameter_set_idx=1,FIELD%PARAMETER_SETS%NUMBER_OF_PARAMETER_SETS+1
            FIELD%PARAMETER_SETS%PARAMETER_SETS(parameter_set_idx)%PTR=>NEW_PARAMETER_SETS(parameter_set_idx)%PTR
          ENDDO !parameter_set_idx
...
999 CALL ERRORS("FIELD_PARAMETER_SET_FINALISE",ERR,ERROR)
  ...
  !>Returns a pointer to the specified field parameter set array. The pointer must be restored with a call to FIELD_PARAMETER_SET_DATA_RESTORE call. Note: the values can be used for read operations but a FIELD_PARAMETER_SET_UPDATE call must be used to change any values.
  SUBROUTINE FIELD_PARAMETER_SET_DATA_GET(FIELD,FIELD_SET_TYPE,PARAMETERS,ERR,ERROR,*)
    ...
    CALL ENTERS("FIELD_PARAMETER_SET_DATA_GET",ERR,ERROR,*999)
    ...
    CALL EXITS("FIELD_PARAMETER_SET_DATA_GET")
    RETURN
999 CALL ERRORS("FIELD_PARAMETER_SET_DATA_GET",ERR,ERROR)
    CALL EXITS("FIELD_PARAMETER_SET_DATA_GET")
    RETURN 1
  END SUBROUTINE FIELD_PARAMETER_SET_DATA_GET

  !
  !================================================================================================================================
  !

  !>Restores the specified field parameter set array that was obtained with FIELD_PARAMETER_SET_DATA_GET.
  SUBROUTINE FIELD_PARAMETER_SET_DATA_RESTORE(FIELD,FIELD_SET_TYPE,PARAMETERS,ERR,ERROR,*)
    ...
    CALL ENTERS("FIELD_PARAMETER_SET_DATA_RESTORE",ERR,ERROR,*999)
    ...
    CALL EXITS("FIELD_PARAMETER_SET_DATA_RESTORE")
    RETURN
999 CALL ERRORS("FIELD_PARAMETER_SET_DATA_RESTORE",ERR,ERROR)
    CALL EXITS("FIELD_PARAMETER_SET_DATA_RESTORE")
    RETURN 1
  END SUBROUTINE FIELD_PARAMETER_SET_DATA_RESTORE
...
999 CALL ERRORS("FIELD_PARAMETER_SET_UPDATE_START",ERR,ERROR)
  ...
  SUBROUTINE FIELD_PARAMETER_SET_VECTOR_GET(FIELD,FIELD_SET_TYPE,DISTRIBUTED_VECTOR,ERR,ERROR,*)
...
999 IF(ASSOCIATED(MESH_COMPONENTS)) DEALLOCATE(MESH_COMPONENTS)
  ...
  SUBROUTINE FIELD_SCALING_TYPE_GET(FIELD,SCALING_TYPE,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(OUT) :: SCALING_TYPE !<On return, the scaling type for the specified fieldto get \see FIELD_ROUTINES_ScalingTypes,FIELD_ROUTINES
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
...
999 CALL ERRORS("FIELD_VARIABLE_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE FIELD_VARIABLE_INITIALISE(FIELD,VARIABLE_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(FIELD)) THEN
      IF(ASSOCIATED(FIELD%CREATE_VALUES_CACHE)) THEN
        IF(VARIABLE_NUMBER>=1.AND.VARIABLE_NUMBER<=FIELD%NUMBER_OF_VARIABLES) THEN
          ...
          IF(FIELD%CREATE_VALUES_CACHE%NUMBER_OF_COMPONENTS>0) THEN
    ...
    RETURN
999 CALL FIELD_VARIABLE_FINALISE(FIELD%VARIABLES(VARIABLE_NUMBER),DUMMY_ERR,DUMMY_ERROR,*998)
