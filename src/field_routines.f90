...
MODULE FIELD_ROUTINES
  ...
  PUBLIC FIELD_CREATE_FINISH,FIELD_CREATE_START,FIELD_DESTROY,FIELDS_FINALISE,FIELDS_INITIALISE, &
    ...
    & FIELD_INTERPOLATED_POINT_FINALISE,FIELD_INTERPOLATION_PARAMETERS_FINALISE,FIELD_MESH_DECOMPOSITION_SET, &
    & FIELD_NEXT_NUMBER_FIND,FIELD_NUMBER_OF_COMPONENTS_SET,FIELD_NUMBER_OF_VARIABLES_SET,FIELD_PARAMETER_SET_GET, &
    & FIELD_PARAMETER_SET_CREATE,FIELD_PARAMETER_SET_UPDATE_FINISH, &
...
999 CALL ERRORS("FIELD_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE FIELD_CREATE_START(USER_NUMBER,REGION,FIELD,ERR,ERROR,*)
    ...
    !#### Subroutine: FIELD_CREATE_START
    !###  Description:
    !###    Starts the creation of a field defined by a user number in specified region.
...
999 CALL ERRORS("FIELD_MAPPINGS_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE FIELD_MAPPINGS_CALCULATE(FIELD,ERR,ERROR,*)
    ...
    TYPE(DOMAIN_TYPE), POINTER :: DOMAIN
    TYPE(DOMAIN_MAPPING_TYPE), POINTER :: ELEMENTS_MAPPING,DOFS_MAPPING,FIELD_DOFS_MAPPING
      ...
      DO variable_idx=1,FIELD%NUMBER_OF_VARIABLES
        DO component_idx=1,FIELD%VARIABLES(variable_idx)%NUMBER_OF_COMPONENTS
          ...
          CASE(FIELD_CONSTANT_INTERPOLATION)
            ...
            !Allocate and set up global to local domain map
            ...
            FIELD_COMPONENT%PARAM_TO_DOF_MAP%CONSTANT_PARAM2DOF_MAP=local_ny
            !Adjust the offsets
          ...
          CASE(FIELD_ELEMENT_BASED_INTERPOLATION)
            ...
            DO ne=1,ELEMENTS_MAPPING%TOTAL_NUMBER_OF_LOCAL
              ...
              !Handle local dofs
              FIELD_COMPONENT%PARAM_TO_DOF_MAP%ELEMENT_PARAM2DOF_MAP(ne)=local_ny              
            ENDDO !ne
            !Adjust the offsets            
          ...
          CASE(FIELD_NODE_BASED_INTERPOLATION)
            ...
            DO ny=1,DOFS_MAPPING%TOTAL_NUMBER_OF_LOCAL
              ...
              !Handle local dofs
              FIELD_COMPONENT%PARAM_TO_DOF_MAP%NODE_PARAM2DOF_MAP(nk,np)=local_ny              
            ENDDO !ny
            !Adjust the offsets            
...
!!TODO: Adjust delta calculation for polar coordinate discontinuities
                    ...
                    DO nk=1,DOMAIN_NODES%NODES(np)%NUMBER_OF_DERIVATIVES
                      ...
                      IF(nu==PART_DERIV_S1.OR.nu==PART_DERIV_S2.OR.nu==PART_DERIV_S3) THEN
                        ...
                        DO component_idx2=1,FIELD_VARIABLE%NUMBER_OF_COMPONENTS
                          ny=FIELD_VARIABLE%COMPONENTS(component_idx2)%PARAM_TO_DOF_MAP%NODE_PARAM2DOF_MAP(nk,np)
    ...
    RETURN
999 CALL ERRORS("FIELD_GEOMETRICS_PARAMETERS_UPDATE_FROM_INITIAL_MESH",ERR,ERROR)
    CALL EXITS("FIELD_GEOMETRICS_PARAMETERS_UPDATE_FROM_INITIAL_MESH")
    RETURN 1   
            ...
            IF(VARIABLE_NUMBER>0.AND.VARIABLE_NUMBER<=FIELD%NUMBER_OF_VARIABLES) THEN
              IF(COMPONENT_NUMBER>=1.AND.COMPONENT_NUMBER<=FIELD%VARIABLES(VARIABLE_NUMBER)%NUMBER_OF_COMPONENTS) THEN
                IF(FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)%PARAM_TO_DOF_MAP% &
                  ...
                  ny=FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)%PARAM_TO_DOF_MAP%CONSTANT_PARAM2DOF_MAP
              ...
              IF(COMPONENT_NUMBER>=1.AND.COMPONENT_NUMBER<=FIELD%VARIABLES(VARIABLE_NUMBER)%NUMBER_OF_COMPONENTS) THEN
                IF(NODE_NUMBER>0.AND.NODE_NUMBER<=FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)% &
                  ...
                  IF(DERIVATIVE_NUMBER>0.AND.DERIVATIVE_NUMBER<=FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)% &
                    ...
                    ny=FIELD%VARIABLES(VARIABLE_NUMBER)%COMPONENTS(COMPONENT_NUMBER)%PARAM_TO_DOF_MAP% &
                      & NODE_PARAM2DOF_MAP(DERIVATIVE_NUMBER,NODE_NUMBER)
                    CALL DISTRIBUTED_VECTOR_VALUES_SET(PARAMETER_SET%PARAMETERS,ny,VALUE,ERR,ERROR,*999)
                  ELSE
...
999 CALL ERRORS("FIELD_SCALINGS_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE FIELD_SCALINGS_INITIALISE(FIELD,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(FIELD)) THEN
      ...
      CALL LIST_DETACH(MESH_COMPONENTS_LIST,NUMBER_OF_MESH_COMPONENTS,MESH_COMPONENTS,ERR,ERROR,*999)
      CALL LIST_DESTROY(MESH_COMPONENTS_LIST,ERR,ERROR,*999)
...
999 CALL ERRORS("FIELD_USER_NUMBER_FIND",ERR,ERROR)
  ...
  SUBROUTINE FIELD_VARIABLE_FINALISE(FIELD_VARIABLE,ERR,ERROR,*)
    ...
    !#### Subroutine: FIELD_VARIABLE_FINALISE
    !###  Description:
