...
999 CALL ERRORS("EQUATIONS_SET_BOUNDARY_CONDITIONS_GET",ERR,ERROR)
  ...
  SUBROUTINE BOUNDARY_CONDITIONS_DIRICHLET_INITIALISE(BOUNDARY_CONDITIONS_VARIABLE,ERR,ERROR,*)
    ...
    !Argument variables
    TYPE(BOUNDARY_CONDITIONS_VARIABLE_TYPE), POINTER :: BOUNDARY_CONDITIONS_VARIABLE !<A pointer to the boundary conditions to initialise a variable t ype for
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    ...
    IF(ASSOCIATED(BOUNDARY_CONDITIONS_VARIABLE)) THEN
      ...
      ELSE
        ...
        ALLOCATE(BOUNDARY_CONDITIONS_DIRICHLET%LINEAR_SPARSITY_INDICES(BOUNDARY_CONDITIONS_VARIABLE%BOUNDARY_CONDITIONS% &
          & EQUATIONS_SET%EQUATIONS%EQUATIONS_MATRICES%LINEAR_MATRICES%NUMBER_OF_LINEAR_MATRICES),STAT=ERR) !CORRECT??!?? need more error handling??!?!?! Equations mapping
