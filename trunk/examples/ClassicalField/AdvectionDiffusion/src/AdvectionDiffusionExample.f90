...
!> Main program
PROGRAM ADVECTIONDIFFUSIONEXAMPLE
...
! 
  ...
  IF(EXPORT_FIELD) THEN
    ...
    CALL CMISSFieldIONodesExport(Fields,"AdvectionDiffusion","FORTRAN",Err)
    CALL CMISSFieldIOElementsExport(Fields,"AdvectionDiffusion","FORTRAN",Err)
...
!   TYPE(FIELD_TYPE), POINTER :: GEOMETRIC_FIELD,DEPENDENT_FIELD,MATERIALS_FIELD,INDEPENDENT_FIELD
...
! 
!   FILE="AdvectionDiffusionExample"
!   METHOD="FORTRAN"
!   IMPORT_FIELD=.FALSE.
...
!   !Set the equations set to be a standard Laplace problem
!   CALL EQUATIONS_SET_SPECIFICATION_SET(EQUATIONS_SET,EQUATIONS_SET_CLASSICAL_FIELD_CLASS, & 
!     & EQUATIONS_SET_ADVECTION_DIFFUSION_EQUATION_TYPE,EQUATIONS_SET_NO_SOURCE_ADVECTION_DIFFUSION_SUBTYPE,ERR,ERROR,*999)
!   !Finish creating the equations set
...
! 
!   !Create the equations set independent field variables - this stores the advective velocity field
!   NULLIFY(INDEPENDENT_FIELD)  
!   CALL EQUATIONS_SET_INDEPENDENT_CREATE_START(EQUATIONS_SET,4,INDEPENDENT_FIELD,ERR,ERROR,*999)  
...
!   CALL PROBLEM_SPECIFICATION_SET(PROBLEM,PROBLEM_CLASSICAL_FIELD_CLASS,PROBLEM_ADVECTION_DIFFUSION_EQUATION_TYPE, &
!     & PROBLEM_NO_SOURCE_ADVECTION_DIFFUSION_SUBTYPE,ERR,ERROR,*999)
...
END PROGRAM ADVECTIONDIFFUSIONEXAMPLE
