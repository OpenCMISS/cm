...
!> \example LinearElasticity/3DLagrangeBasisAnisotropicFibreField/src/LinearElasticityExample.f90
!! Example program to solve a 3D linear elasticity problem with anisotropic fibre field using openCMISS calls.
!<
...
!###    Index for a partial derivative.
  ...
  TYPE(FIELD_TYPE), POINTER :: GEOMETRIC_FIELD,DEPENDENT_FIELD,MATERIAL_FIELD,FIBRE_FIELD
  ...
  REAL(DP) :: l,w,h
  REAL(DP),ALLOCATABLE :: MATE_PARA_E(:),MATE_PARA_v(:),FIBRE_ANGLES_DEG(:)
  ...
  XI_INTERPOLATION(1,:) = &
    & (/BASIS_LINEAR_LAGRANGE_INTERPOLATION,BASIS_LINEAR_LAGRANGE_INTERPOLATION,BASIS_LINEAR_LAGRANGE_INTERPOLATION/)
  XI_INTERPOLATION(2,:) = &
    & (/BASIS_LINEAR_LAGRANGE_INTERPOLATION,BASIS_LINEAR_LAGRANGE_INTERPOLATION,BASIS_LINEAR_LAGRANGE_INTERPOLATION/)
  XI_INTERPOLATION(3,:) = &
    & (/BASIS_LINEAR_LAGRANGE_INTERPOLATION,BASIS_LINEAR_LAGRANGE_INTERPOLATION,BASIS_LINEAR_LAGRANGE_INTERPOLATION/)
  ...
  w = 100.0_DP
  h = 100.0_DP
  ELEMENTS(1)%MESH_COMP(1)%DERIV(1)%NODE_COORDINATES = (/0.0_DP,l,0.0_DP,l,0.0_DP,l,0.0_DP,l/)
  ELEMENTS(1)%MESH_COMP(2)%DERIV(1)%NODE_COORDINATES = (/0.0_DP,0.0_DP,w,w,0.0_DP,0.0_DP,w,w/)
  ELEMENTS(1)%MESH_COMP(3)%DERIV(1)%NODE_COORDINATES = (/0.0_DP,0.0_DP,0.0_DP,0.0_DP,h,h,h,h/)
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  DO xi=1,NUMBER_OF_XI
    MESH_COMP_IDX = xi
    FIELD_COMP_IDX = xi
  ...
  NUMBER_OF_FIELD_COMPS = NUMBER_OF_XI
  NULLIFY(FIBRE_FIELD)
  CALL FIELD_CREATE_START(FIELD_USER_NUMBER,REGION,FIBRE_FIELD,ERR,ERROR,*999)
  ...
  CALL FIELD_NUMBER_OF_VARIABLES_SET(FIBRE_FIELD,NUMBER_OF_FIELD_VARIABLES,ERR,ERROR,*999)
  CALL FIELD_NUMBER_OF_COMPONENTS_SET(FIBRE_FIELD,FIELD_U_VARIABLE_TYPE,NUMBER_OF_FIELD_COMPS,ERR,ERROR,*999)
  DO xi=1,NUMBER_OF_XI
    MESH_COMP_IDX = xi
    FIELD_COMP_IDX = xi
    CALL FIELD_COMPONENT_MESH_COMPONENT_SET(FIBRE_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_COMP_IDX,MESH_COMP_IDX,ERR,ERROR,*999)
  ENDDO !xi
  CALL FIELD_CREATE_FINISH(FIBRE_FIELD,ERR,ERROR,*999)
  ALLOCATE(FIBRE_ANGLES_DEG(NUMBER_OF_FIELD_COMPS),STAT=ERR)
  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate FIBRE_ANGLES_DEG",ERR,ERROR,*999)
  !-=MODIFIY-HERE-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  FIBRE_ANGLES_DEG = (/0.0_DP,0.0_DP,0.0_DP/)
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  DO FIELD_COMP_IDX=1,NUMBER_OF_FIELD_COMPS
    CALL FIELD_COMPONENT_VALUES_INITIALISE(FIBRE_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
      & FIELD_COMP_IDX,(FIBRE_ANGLES_DEG(FIELD_COMP_IDX)*PI)/180.0_DP,ERR,ERROR,*999)
  ...
  ALLOCATE(MATE_PARA_E(NUMBER_OF_FIELD_COMPS/2),STAT=ERR)
  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate MATE_PARA",ERR,ERROR,*999)
  ALLOCATE(MATE_PARA_v(NUMBER_OF_FIELD_COMPS/2),STAT=ERR)
  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate MATE_PARA",ERR,ERROR,*999)
  MATE_PARA_E = (/30E6_DP,30E6_DP,30E6_DP/)
  MATE_PARA_v = (/0.25_DP,0.25_DP,0.25_DP/)
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  DO xi=1,NUMBER_OF_XI
    MESH_COMP_IDX = xi
    DO FIELD_COMP_IDX=1,NUMBER_OF_FIELD_COMPS/2
      CALL FIELD_COMPONENT_VALUES_INITIALISE(MATERIAL_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
        & FIELD_COMP_IDX,MATE_PARA_E(FIELD_COMP_IDX),ERR,ERROR,*999)
      CALL FIELD_COMPONENT_VALUES_INITIALISE(MATERIAL_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE, &
        & FIELD_COMP_IDX+NUMBER_OF_XI,MATE_PARA_v(FIELD_COMP_IDX),ERR,ERROR,*999)
    ENDDO !FIELD_COMP_IDX
  ENDDO !xi
  ...
  CALL EQUATIONS_SET_SPECIFICATION_SET(EQUATIONS_SET,EQUATIONS_SET_ELASTICITY_CLASS,EQUATIONS_SET_LINEAR_ELASTICITY_TYPE, &
        EQUATIONS_SET_THREE_DIMENSIONAL_SUBTYPE,ERR,ERROR,*999)
  ...
  FIELD_USER_NUMBER = 4 !Material Field
  ...
  FORCE_BC%MESH_COMP(1)%DERIV(1)%NODE_COORDINATES(:)=400.0_DP
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  DO xi=1,NUMBER_OF_XI
    MESH_COMP_IDX=xi
    FIELD_COMP_IDX=xi
